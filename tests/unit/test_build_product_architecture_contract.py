...
def _split_router_root(tmp_path: Path) -> Path:
    (tmp_path / "api").mkdir(parents=True)
    (tmp_path / "api" / "product.py").write_text("# product facade\n", encoding="utf-8")
    (tmp_path / "api" / "product_architecture.py").write_text(
        '@router.get("/architecture")\n@router.get("/architecture-validation")\n',
        encoding="utf-8",
    )
    (tmp_path / "api" / "product_service_contracts.py").write_text(
        '@router.get("/service-boundary")\n@router.get("/api-contract")\n',
        encoding="utf-8",
    )
    (tmp_path / "api" / "cameo.py").write_text("# cameo api\n", encoding="utf-8")
...
def test_product_architecture_contract_accepts_split_product_route_modules(tmp_path: Path) -> None:
    payload = mod.build_product_architecture_contract(
        product_capability_packet=_packet(
            {
                "status": "product_capability_surface_contract_ready",
        ...
        ligand_cleanup_work_order_packet=_packet({"status": "cleanup_work_order_ready"}),
        ligand_cleanup_preflight_packet=_packet(
            {"status": "ligand_heavy_cleanup_execution_preflight_ready", "blocker_count": 0}
        ),
        casp17_transition_packet=_packet({"status": "casp17_transition_surface_contract_ready", "surface_ready": True}),
    ...
    assert summary["product_service_boundary_ready"] is True
    assert summary["product_api_contract_ready"] is True
    structure_row = next(row for row in payload["rows"] if row["lane_id"] == "structure_analysis_product_surface")
    service_row = next(row for row in payload["rows"] if row["lane_id"] == "product_service_boundary_contract")
    api_row = next(row for row in payload["rows"] if row["lane_id"] == "product_api_contract")
    assert structure_row["status"] == "ready"
    assert "api/product_architecture.py" in structure_row["artifact_path"]
    assert service_row["status"] == "ready"
    assert "api/product_service_contracts.py" in service_row["artifact_path"]
    assert api_row["status"] == "ready"
    assert "api/product_service_contracts.py" in api_row["artifact_path"]
