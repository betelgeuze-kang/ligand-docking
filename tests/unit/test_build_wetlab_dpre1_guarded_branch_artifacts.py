...
def _watch_action_payload() -> dict:
    return {
        "summary": {
            ...
            "next_required_step": "Pause auto-advance for DprE1; it hit 3 consecutive auto-holds.",
        }
    }


def test_build_dpre1_guarded_operator_packet_uses_review_branch_when_only_standard_bridge_is_available() -> None:
    bridge_payload = {
        "summary": {
    ...
    assert summary["status"] == "wetlab_dpre1_guarded_operator_packet_ready"
    ...
    assert summary["decision_case"] == "dpre1_guarded_review_required"
...
def test_build_dpre1_guarded_operator_packet_prefers_exploratory_gate51_focus_over_standard_bridge() -> None:
    bridge_payload = {
        "summary": {
            "status": "wetlab_broad_screen_throughput_bridge_ready",
            "target_id": "DprE1",
    ...
    assert summary["decision_case"] == "dpre1_guarded_gate51_review_candidate"
    assert summary["action"] == "pause_default_lane_and_review_gate51_retry"


def test_build_dpre1_guarded_branch_summary_switches_to_tuned_branch_when_tuned_bridge_is_selected() -> None:
    bridge_payload = {
        "summary": {
            "status": "wetlab_broad_screen_throughput_bridge_ready",
            "target_id": "DprE1",
    ...
    summary = payload["summary"]
    assert summary["status"] == "wetlab_dpre1_guarded_branch_summary_ready"
