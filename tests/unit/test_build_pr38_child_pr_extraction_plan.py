...
def test_child_pr_extraction_plan_orders_source_of_truth_reconciliation_last(tmp_path: Path) -> None:
    ...
    assert summary["child_pr_count"] == 5
    assert summary["ready_child_pr_count"] == 5
    assert summary["source_of_truth_sequence"] == 5
    assert summary["source_of_truth_depends_on_slice_count"] == 4
    assert summary["source_of_truth_registry_reconciles_last"] is True
    assert summary["hunk_split_review_required_count"] == 2
    assert summary["hunk_split_review_required_child_pr_count"] == 2
    assert summary["external_state_mutated"] is False
    rows = {row["slice_id"]: row for row in payload["rows"]}
    assert rows["f2g_f2h_preflight"]["sequence"] == 1
...
def test_child_pr_extraction_plan_blocks_if_split_packet_is_not_ready(tmp_path: Path) -> None:
    ...
    assert payload["summary"]["not_ready_child_pr_count"] == 5
