...
def test_product_ai_report_ux_contract_ready_from_local_evidence(tmp_path: Path) -> None:
    ...
    viewer_app.write_text(
        ...
        "'interaction_rationale','uncertainty_narrative','scope_claim_limit',"
    ...
    assert summary["ready_section_count"] == 9
    ...
    assert summary["customer_report_ready_block_count"] == 6
    assert summary["customer_report_required_block_count"] == 6
    ...
    assert "ready_block_count=6" in delivery["observed"]
...
def test_product_ai_report_ux_contract_cli_writes_outputs(tmp_path: Path) -> None:
    ...
    assert json.loads(out_json.read_text(encoding="utf-8"))["summary"]["section_count"] == 9
