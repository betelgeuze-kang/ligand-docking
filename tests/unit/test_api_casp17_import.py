...
def test_api_app_imports_with_casp17_router() -> None:
    ...
    assert transition["large_cleanup_known_payload_size_gb"] == 406.131
    assert transition["cleanup_execution_approval_gate_status"] == "blocked_cleanup_execution_operator_approval_gate"
    assert transition["cleanup_execution_awaiting_operator_approval_row_count"] == 5
    assert transition["cleanup_execution_authorized_reclaim_size_gb"] == 0.0
    assert transition["cleanup_execution_total_reclaim_size_gb"] == 49.216
    assert transition["cleanup_execution_operator_approval_csv_present"] is False
    ...
    assert transition["cleanup_postcheck_row_count"] == 7
    assert transition["cleanup_postcheck_blocked_row_count"] == 0
    assert transition["cleanup_completion_gate_status"] == "blocked_cleanup_completion_gate"
    assert transition["cleanup_completion_complete"] is False
    assert transition["cleanup_completion_blocked_stage_count"] == 4
    assert transition["cleanup_completion_approval_ready"] is False
    assert transition["cleanup_completion_transition_cleanup_complete"] is False
    assert transition["cleanup_completion_ligand_heavy_cleanup_complete"] is False
    assert transition["cleanup_completion_protected_policy_resolved"] is False
    assert transition["release_allowed"] is False
    assert transition["cleanup_objective_ready"] is False
