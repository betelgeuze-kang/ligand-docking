...
def _attestation(**overrides: object) -> dict[str, object]:
    ...
    return build_signed_reference_minimization_validation_review_attestation(**values)  # type: ignore[arg-type]


def _verify(
    source: object,
    *,
    anchor: MinimizationScientificReviewerTrustAnchor | None = None,
...
def test_review_contract_is_frozen_dependency_bound_and_result_free() -> None:
    first = reference_minimization_validation_review_contract_document()
    second = reference_minimization_validation_review_contract_document()
    ...
    assert (
        require_reference_minimization_validation_review_contract_document(first)
        == first
    )


def test_review_contract_rejects_tamper() -> None:
    document = reference_minimization_validation_review_contract_document()
    ...
    with pytest.raises(
        ...
        require_reference_minimization_validation_review_contract_document(tampered)
...
def test_signed_review_attestation_verifies_exact_scope_and_stays_non_authorizing() -> (
    ...
    assert attestation["scientific_minimization_validation_recommended"] is False
...
def test_signed_review_attestation_rejects_duplicate_json_or_signature_fields() -> None:
    ...
    with pytest.raises(
        ReferenceMinimizationValidationReviewError, match="duplicate JSON key"
    ):
        _verify(duplicate)
    ...
    with pytest.raises(
        ReferenceMinimizationValidationReviewError, match="signature fields"
    ):
        _verify(attestation)
...
def test_signed_review_attestation_rejects_unsigned_tamper(
    ...
    with pytest.raises(ReferenceMinimizationValidationReviewError, match=message):
        _verify(tampered)


def test_signed_review_attestation_rejects_untrusted_or_mismatched_keys() -> None:
    attestation = _attestation()
    with pytest.raises(ReferenceMinimizationValidationReviewError, match="not trusted"):
        verify_signed_reference_minimization_validation_review_attestation(
            attestation,
...
def test_signed_review_attestation_rejects_incomplete_or_reordered_scope() -> None:
    ...
    with pytest.raises(
        ReferenceMinimizationValidationReviewError, match="check coverage"
    ):
        _verify(missing)
    ...
    with pytest.raises(ReferenceMinimizationValidationReviewError, match="limitations"):
        _verify(reordered)


def test_signed_review_attestation_enforces_freshness_window() -> None:
    with pytest.raises(
        ReferenceMinimizationValidationReviewError, match="not yet valid"
    ):
        _verify(_attestation(), checked_at=REVIEWED_AT - timedelta(seconds=1))
    with pytest.raises(ReferenceMinimizationValidationReviewError, match="expired"):
    ...
    with pytest.raises(
        ReferenceMinimizationValidationReviewError, match="exceeds the frozen maximum"
    ):
        _verify(overlong)

    inverted = _attestation(expires_at=REVIEWED_AT)
    with pytest.raises(
        ReferenceMinimizationValidationReviewError, match="expiry must follow"
    ):
...
def test_review_trust_anchor_redacts_key_from_repr_and_rejects_short_key() -> None:
    ...
    with pytest.raises(
        ReferenceMinimizationValidationReviewError, match="bytes or text"
    ):
        MinimizationScientificReviewerTrustAnchor(
            reviewer_identity_sha256=REVIEWER_IDENTITY,
