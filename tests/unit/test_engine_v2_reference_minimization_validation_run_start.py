...
import betelgeuze_engine_v2.physics.reference_minimization_validation_run_start as module
    ...
    environment_rows = tuple(
        sorted(
            (
                ...
                ("TZ", "UTC"),
                ("PYTHONHASHSEED", "123"),
                (REFERENCE_MINIMIZATION_VALIDATION_APPLICATION_SEED_ENV, "456"),
            )
        )
    )
...
def _network_attestation(
    ...
    *,
    ...
    return build_signed_reference_minimization_validation_network_isolation_attestation(
...
def _create(
    ...
    **overrides: object,
    ...
    return create_reference_minimization_validation_execution_environment_receipt(  # type: ignore[arg-type]
        **values
    )


def test_run_start_contract_is_frozen_and_current_decision_is_closed() -> None:
    first = reference_minimization_validation_run_start_contract_document()
    second = reference_minimization_validation_run_start_contract_document()
...
def test_run_start_contract_rejects_tamper() -> None:
    ...
    with pytest.raises(
        ...
        require_reference_minimization_validation_run_start_contract_document(tampered)
...
def test_network_attestation_rejects_tamper_crosswire_expiry_and_revocation(
    ...
    with pytest.raises(
        ...
        _network_attestation(
            receipt,
            output_root,
            expires_at=NETWORK_OBSERVED_AT + timedelta(minutes=5, seconds=1),
        )
    ...
    with pytest.raises(
        ...
        verify_signed_reference_minimization_validation_network_isolation_attestation(
            attestation,
            trusted_operator_keys={OPERATOR_KEY_ID: object()},  # type: ignore[dict-item]
            checked_at=NOW,
            expected_authorization=expected_authorization,
            expected_artifact_output_root_identity_sha256=(
                reference_minimization_validation_artifact_output_root_identity_sha256(output_root)
...
def test_valid_chain_persists_environment_receipt_without_opening_execution(
    ...
    assert payload["minimization_validated"] is False
...
def test_bootstrap_source_manifest_bytes_are_canonical_and_live_bound(
    ...
    attribute = module.REFERENCE_MINIMIZATION_VALIDATION_BOOTSTRAP_STATE_ATTRIBUTE
    ...
    def install_state(manifest_bytes: bytes) -> None:
        monkeypatch.setattr(
            ...
            attribute,
            (
                module.REFERENCE_MINIMIZATION_VALIDATION_CONTROLLED_INNER_STATE,
...
def test_runtime_mismatch_fails_before_environment_receipt(
    ...
    with pytest.raises(ReferenceMinimizationValidationRunStartError, match=message):
        _create(
            reservation_root,
            output_root,
            review=review,
...
def test_module_has_no_clock_override_runner_evaluator_writer_or_delete_api() -> None:
    ...
    assert "minimize_reference_force_field" not in source
    assert "evaluate_independent_minimization_oracle" not in source
    assert "materialize_frozen_cpu_minimization_validation_case" not in source
