...
GO_NO_GO = {"summary": {"status": "tcruzi_krs1_go_no_go_card_ready"}}
SUCCESSOR = {"summary": {"target_id": "LRRK2"}}


def test_build_tcruzi_krs1_run_record_defaults_to_blocked_when_dpre1_gate_is_closed() -> None:
    payload = mod.build_payload(
        LAUNCH,
        {"summary": {"status": "tcruzi_krs1_result_review_ready", "tcruzi_krs1_gate_open": False}},
        SUCCESSOR,
        GO_NO_GO,
    )
    summary = payload["summary"]

    assert summary["status"] == "tcruzi_krs1_run_record_ready"
    ...
    assert summary["successor_gate_state"] == "blocked_until_tcruzi_krs1_result_ready_or_explicit_hold"


def test_build_tcruzi_krs1_run_record_opens_lrrk2_after_result_ready() -> None:
    payload = mod.build_payload(
        LAUNCH,
        {
            "summary": {
                "status": "tcruzi_krs1_result_review_ready",
                "tcruzi_krs1_gate_open": True,
                "tcruzi_krs1_review_state": "ready_to_capture_tcruzi_krs1_result_review",
            }
        },
        SUCCESSOR,
        GO_NO_GO,
...
def test_build_tcruzi_krs1_run_record_rejects_advanced_state_when_gate_is_closed() -> None:
