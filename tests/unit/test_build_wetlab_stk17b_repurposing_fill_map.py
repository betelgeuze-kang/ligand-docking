...
from tools import build_wetlab_stk17b_repurposing_fill_map as mod


def test_build_wetlab_stk17b_repurposing_fill_map() -> None:
    ...
    assert summary["status"] == "wetlab_stk17b_repurposing_fill_map_ready"
    assert summary["target_count"] == 1
    assert summary["row_count"] == 3
    assert rows[0]["compound_name"] == "PFE-PKIS 43 (4)"
    assert rows[0]["first_contact_use_mode"] == "benchmark_control"
    assert rows[1]["compound_name"] == "PFE-PKIS 14 (5)"
    assert rows[1]["first_contact_use_mode"] == "benchmark_control"
    assert rows[2]["compound_name"] == "PFE-PKIS 9 (6)"
