...
from tools import build_wetlab_next3_novelty_fill_map as mod
from tools import build_wetlab_next3_repurposing_fill_map as rep_mod
...
def test_build_wetlab_next3_novelty_fill_map() -> None:
    ...
    repurposing_fill = rep_mod.build_payload(fill_queue, queue)
    payload = mod.build_payload(repurposing_fill)
    summary = payload["summary"]
    rows = payload["rows"]

    assert summary["status"] == "wetlab_next3_novelty_fill_map_ready"
    assert summary["next3_target_count"] == 3
    assert summary["row_count"] == 9
    assert any(row["target_id"] == "Cruzain" and row["novelty_compound_name"].startswith("ML217") for row in rows)
    assert any(row["target_id"] == "SARS-CoV-2 PLpro" and row["novelty_compound_name"] == "PF-07957472" for row in rows)
    assert any(row["target_id"] == "ALK2" and row["novelty_compound_name"] == "M4K2009" for row in rows)
