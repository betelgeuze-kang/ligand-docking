...
def test_release_source_of_truth_tracks_customer_report_ux_artifacts() -> None:
    ...
    assert (
        "refine_tier_public_benchmark_statistical_support_coordinate_fetch_plan_semantic_ready"
        in status_ids
    )
    assert (
    ...
    assert "product_scope_breadth_evidence_operator_staging_apply" in artifact_ids
    ...
    assert (
        mod.REFINE_TIER_PUBLIC_BENCHMARK_STATISTICAL_SUPPORT_COORDINATE_FETCH_PLAN_COMMAND
        in mod.RELEASE_REFRESH_COMMANDS
    )
    assert (
    ...
    assert "production_ai_registry_promotion_operator_staging_apply_blocked_semantic_ready" in status_ids
    ...
    assert phase_ab_status_spec["required_status"] == "blocked_gpcr_commercial_phase_ab_closure_claim_locked"
    assert "phase_a_claim_closure_ready" not in phase_ab_status_spec["required_true_fields"]
    ...
    assert ai_md_bundle_status_spec["required_int_exact_fields"]["release_claim_ready"] == 0
    assert ai_md_bundle_status_spec["required_text_exact_fields"][
        "release_claim_blocked_reason"
    ] == "product_ci_runtime_gate_not_ready"
    ...
    assert "r4_ready_for_review_row_count=17" in bottleneck_status_spec[
        "required_text_exact_fields"
    ]["engine_refinement_claim_evidence_priority_packet_top_next_operator_step"]
    assert "required_input_artifacts=34/34/0" in bottleneck_status_spec[
        "required_text_exact_fields"
    ]["engine_refinement_claim_evidence_priority_packet_top_next_operator_step"]
    assert "fetch_required_row_count=0" in bottleneck_status_spec[
        "required_text_exact_fields"
    ]["engine_refinement_claim_evidence_priority_packet_top_next_operator_step"]
    assert "local_coordinate_present_targets=17" in bottleneck_status_spec[
        "required_text_exact_fields"
    ]["engine_refinement_claim_evidence_priority_packet_top_next_operator_step"]
    assert "local_coordinate_missing_targets=0" in bottleneck_status_spec[
        "required_text_exact_fields"
    ]["engine_refinement_claim_evidence_priority_packet_top_next_operator_step"]
    assert "planned_metric_source_payload_count=51" in bottleneck_status_spec[
        "required_text_exact_fields"
    ]["engine_refinement_claim_evidence_priority_packet_top_next_operator_step"]
    assert "fill and review the 51 DockQ/lDDT-PLI/internal DeltaG metric source payloads" in bottleneck_status_spec[
        "required_text_exact_fields"
    ]["engine_refinement_claim_evidence_priority_packet_top_next_operator_step"]
    assert "before any R9 claim receipt or canonical intake promotion" in bottleneck_status_spec[
        "required_text_exact_fields"
    ]["engine_refinement_claim_evidence_priority_packet_top_next_operator_step"]
    assert bottleneck_status_spec["required_text_exact_fields"][
        "product_scope_breadth_evidence_priority_source_json"
    ...
    scope_staging_apply_status_spec = next(
        spec
        for spec in mod.DEFAULT_STATUS_SPECS
        if spec["artifact_id"]
        == "product_scope_breadth_evidence_operator_staging_apply_blocked_semantic_ready"
    )
    assert scope_staging_apply_status_spec["required_int_exact_fields"][
        "candidate_blocked_row_count"
    ] == 6
    assert scope_staging_apply_status_spec["required_int_exact_fields"][
        "field_worksheet_pending_field_count"
    ] == 36
    assert scope_staging_apply_status_spec["required_text_exact_fields"][
        "candidate_first_blocked_scope_blocker_id"
    ] == "direct_binding_evidence_missing"
    assert scope_staging_apply_status_spec["required_text_exact_fields"][
    ...
    assert statistical_candidate_queue_status_spec["required_int_exact_fields"][
        "seed_csv_source_row_count"
    ] == 16
    assert statistical_candidate_queue_status_spec["required_int_exact_fields"][
        "local_pose_inventory_pose_row_count"
    ] == 22492
    assert statistical_candidate_queue_status_spec["required_int_exact_fields"][
        "local_pose_inventory_ligand_reference_row_count"
    ] == 285
    assert statistical_candidate_queue_status_spec["required_int_exact_fields"][
        "candidate_source_eligible_row_count"
    ] == 17
    assert statistical_candidate_queue_status_spec["required_int_exact_fields"][
        "candidate_source_excluded_existing_target_row_count"
    ] == 8
    assert statistical_candidate_queue_status_spec["required_int_exact_fields"][
        "candidate_source_distinct_target_count"
    ] == 17
    assert statistical_candidate_queue_status_spec["required_int_exact_fields"][
        "candidate_fill_recovery_candidate_count"
    ] == 17
    assert statistical_candidate_queue_status_spec["required_int_exact_fields"][
        "candidate_fill_recovery_payload_write_allowed"
    ] == 0
    assert statistical_candidate_queue_status_spec["required_int_exact_fields"][
        "candidate_fill_recovery_claim_promotion_allowed"
    ] == 0
    assert statistical_candidate_queue_status_spec["required_text_exact_fields"][
        "candidate_source_mode"
    ] == "candidate_fill_recovery"
    assert statistical_candidate_queue_status_spec["required_text_exact_fields"][
        "next_required_step"
    ] == (
    ...
    statistical_coordinate_fetch_status_spec = next(
        spec
        for spec in mod.DEFAULT_STATUS_SPECS
        if spec["artifact_id"]
        == "refine_tier_public_benchmark_statistical_support_coordinate_fetch_plan_semantic_ready"
    )
    assert statistical_coordinate_fetch_status_spec["required_status"] == (
        "refine_tier_public_benchmark_statistical_support_coordinate_fetch_plan_ready"
    )
    assert "coordinate_fetch_plan_ready" in statistical_coordinate_fetch_status_spec[
        "required_true_fields"
    ]
    assert statistical_coordinate_fetch_status_spec["required_int_exact_fields"][
        "coordinate_fetch_row_count"
    ] == 17
    assert statistical_coordinate_fetch_status_spec["required_int_exact_fields"][
        "coordinate_fetch_required_row_count"
    ] == 0
    assert statistical_coordinate_fetch_status_spec["required_int_exact_fields"][
        "coordinate_fetch_primary_url_row_count"
    ] == 17
    assert statistical_coordinate_fetch_status_spec["required_int_exact_fields"][
        "coordinate_fetch_ready_for_validation_row_count"
    ] == 17
    assert statistical_coordinate_fetch_status_spec["required_text_exact_fields"][
        "next_required_step"
    ] == (
    ...
    statistical_coordinate_fetch_apply_status_spec = next(
        spec
        for spec in mod.DEFAULT_STATUS_SPECS
        if spec["artifact_id"]
        == "refine_tier_public_benchmark_statistical_support_coordinate_fetch_apply_blocked_semantic_ready"
    )
    assert statistical_coordinate_fetch_apply_status_spec["required_status"] == (
    ...
    assert statistical_coordinate_fetch_apply_status_spec["required_int_exact_fields"][
        "coordinate_fetch_apply_preflight_pass_row_count"
    ] == 17
    assert statistical_coordinate_fetch_apply_status_spec["required_int_exact_fields"][
    ...
    assert statistical_metric_materialization_readiness_status_spec["required_status"] == (
        "refine_tier_public_benchmark_statistical_support_metric_materialization_readiness_ready"
    )
    assert "metric_materialization_readiness_ready" in (
        statistical_metric_materialization_readiness_status_spec["required_true_fields"]
    )
    assert statistical_metric_materialization_readiness_status_spec["required_int_exact_fields"][
    ...
    assert "required_input_artifacts=34/34/0" in engine_field_worksheet_status_spec[
        "required_text_exact_fields"
    ]["next_required_step"]
    assert "local_coordinate_present_targets=17" in engine_field_worksheet_status_spec[
        "required_text_exact_fields"
    ]["next_required_step"]
    assert "local_coordinate_missing_targets=0" in engine_field_worksheet_status_spec[
        "required_text_exact_fields"
    ]["next_required_step"]
    assert "operator_review_surface_ready_count=51" in engine_field_worksheet_status_spec[
        "required_text_exact_fields"
    ]["next_required_step"]
    assert "receipt_manual_field_pending_count=510" in engine_field_worksheet_status_spec[
        "required_text_exact_fields"
    ]["next_required_step"]
    assert (
    ...
    assert engine_staging_apply_status_spec["required_int_exact_fields"][
        "field_worksheet_public_benchmark_statistical_support_metric_source_payload_operator_receipt_coordinate_validation_pass_payload_row_count"
    ] == 51
    assert engine_staging_apply_status_spec["required_int_exact_fields"][
    ...
    assert "operator_review_surface_ready_count=51" in engine_staging_apply_status_spec[
        "required_text_exact_fields"
    ]["next_required_step"]
    assert "receipt_manual_field_pending_count=510" in engine_staging_apply_status_spec[
        "required_text_exact_fields"
    ]["next_required_step"]
    ...
    assert goal_audit_status_spec["required_int_exact_fields"]["release_blocker_fail_count"] == 2
    assert goal_audit_status_spec["required_text_exact_fields"]["primary_release_blocker_requirement_id"] == (
        "R8_full_scope_claim_closure"
    )
    assert goal_audit_status_spec["required_text_exact_fields"]["primary_release_blocker_tier"] == (
        "full_commercial_scope"
    )
    ...
    assert action_board_status_spec["required_int_exact_fields"]["product_goal_release_blocker_fail_count"] == 2
    ...
    registry_staging_apply_spec = next(
        spec
        for spec in mod.DEFAULT_ARTIFACT_SPECS
        if spec["artifact_id"] == "production_ai_registry_promotion_operator_staging_apply"
    )
    assert registry_staging_apply_spec["builder_command"] == (
        "python3 tools/build_production_ai_registry_promotion_operator_staging_apply.py"
    )
    assert "runs/production_ai_registry_promotion_operator_field_worksheet_current.json" in (
        registry_staging_apply_spec["depends_on"]
    )
    assert "config/production_ai_registry_promotion_operator_receipt_current.csv" in (
        registry_staging_apply_spec["depends_on"]
    )
    registry_staging_apply_status_spec = next(
        spec
        for spec in mod.DEFAULT_STATUS_SPECS
        if spec["artifact_id"]
        == "production_ai_registry_promotion_operator_staging_apply_blocked_semantic_ready"
    )
    assert registry_staging_apply_status_spec["required_int_exact_fields"][
        "candidate_blocked_row_count"
    ] == 1
    assert registry_staging_apply_status_spec["required_int_exact_fields"][
        "field_worksheet_pending_field_count"
    ] == 13
    assert registry_staging_apply_status_spec["required_text_exact_fields"][
        "candidate_first_blocked_artifact_id"
    ...
    scope_staging_apply_spec = next(
        spec
        for spec in mod.DEFAULT_ARTIFACT_SPECS
        if spec["artifact_id"] == "product_scope_breadth_evidence_operator_staging_apply"
    )
    assert scope_staging_apply_spec["builder_command"] == (
        "python3 tools/build_product_scope_breadth_evidence_operator_staging_apply.py"
    )
    assert "config/product_scope_breadth_evidence_receipt_current.csv" in scope_staging_apply_spec[
        "depends_on"
    ]
    assert "runs/product_scope_breadth_evidence_operator_field_worksheet_current.json" in (
        scope_staging_apply_spec["depends_on"]
    )
    ...
    assert "runs/product_scope_breadth_evidence_operator_staging_apply_current.json" in intake_kit_spec[
        "depends_on"
    ]
    ...
    assert "runs/product_scope_breadth_evidence_operator_staging_apply_current.json" in (
    ...
    assert "runs/self_hosted_license_distribution_audit_current.json" not in third_party_review_spec["depends_on"]
    ...
    statistical_coordinate_fetch_spec = next(
        spec
        for spec in mod.DEFAULT_ARTIFACT_SPECS
        if spec["artifact_id"] == "refine_tier_public_benchmark_statistical_support_coordinate_fetch_plan"
    )
    assert statistical_coordinate_fetch_spec["builder_command"] == (
        mod.REFINE_TIER_PUBLIC_BENCHMARK_STATISTICAL_SUPPORT_COORDINATE_FETCH_PLAN_COMMAND
    )
    assert "tools/product/build_refine_tier_public_benchmark_statistical_support_coordinate_fetch_plan.py" in (
        statistical_coordinate_fetch_spec["depends_on"]
    )
    assert "tools/build_refine_tier_public_benchmark_statistical_support_coordinate_fetch_plan.py" in (
        statistical_coordinate_fetch_spec["depends_on"]
    )
    assert "runs/refine_tier_public_benchmark_statistical_support_coordinate_intake_current.json" in (
        statistical_coordinate_fetch_spec["depends_on"]
    )
    statistical_coordinate_fetch_apply_spec = next(
        spec
        for spec in mod.DEFAULT_ARTIFACT_SPECS
        if spec["artifact_id"] == "refine_tier_public_benchmark_statistical_support_coordinate_fetch_apply"
    )
    assert statistical_coordinate_fetch_apply_spec["builder_command"] == (
        mod.REFINE_TIER_PUBLIC_BENCHMARK_STATISTICAL_SUPPORT_COORDINATE_FETCH_APPLY_COMMAND
    )
    assert "tools/product/apply_refine_tier_public_benchmark_statistical_support_coordinate_fetch_plan.py" in (
        statistical_coordinate_fetch_apply_spec["depends_on"]
    )
    assert "tools/apply_refine_tier_public_benchmark_statistical_support_coordinate_fetch_plan.py" in (
        statistical_coordinate_fetch_apply_spec["depends_on"]
    )
    assert "tools/product/fetch_public_benchmark_native_structure.py" in (
        statistical_coordinate_fetch_apply_spec["depends_on"]
    )
    ...
    assert "tools/build_refine_tier_public_benchmark_statistical_support_metric_materialization_readiness.py" in (
        statistical_metric_materialization_readiness_spec["depends_on"]
    )
    assert "runs/refine_tier_public_benchmark_statistical_support_candidate_queue_current.json" in (
        statistical_metric_materialization_readiness_spec["depends_on"]
    ...
    assert mod.RELEASE_REFRESH_COMMANDS.index("python3 tools/build_residual_force_derivation_validation.py") < (
        _last_refresh_index("python3 tools/build_residual_force_gpu_worker_return_receipt.py")
    )
    assert mod.RELEASE_REFRESH_COMMANDS.index("python3 tools/build_residual_force_derivation_validation.py") < (
        mod.RELEASE_REFRESH_COMMANDS.index("python3 tools/build_residual_energy_force_label_validation.py")
    )
    assert _last_refresh_index("python3 tools/build_residual_force_gpu_worker_return_receipt.py") < (
        mod.RELEASE_REFRESH_COMMANDS.index("python3 tools/build_residual_energy_force_label_validation.py")
    )
    assert mod.RELEASE_REFRESH_COMMANDS.index("python3 tools/build_residual_energy_force_label_validation.py") < (
        mod.RELEASE_REFRESH_COMMANDS.index("python3 tools/build_residual_energy_force_label_evidence_work_order.py")
    )
    assert mod.RELEASE_REFRESH_COMMANDS.index("python3 tools/build_residual_energy_force_label_evidence_work_order.py") < (
        mod.RELEASE_REFRESH_COMMANDS.index("python3 tools/build_residual_production_training_data_contract.py")
    )
    assert mod.RELEASE_REFRESH_COMMANDS.index("python3 tools/build_residual_production_training_data_contract.py") < (
    ...
    assert len(ai_md_kpi_indices) == 3
    assert len(ai_md_bundle_indices) == 3
    ...
    assert (
        ...
        < ai_md_kpi_indices[1]
        < ai_md_bundle_indices[1]
        < ai_md_kpi_indices[2]
        < ai_md_bundle_indices[2]
    )
    assert ai_md_bundle_indices[2] < (
    ...
    assert _last_refresh_index("python3 deploy/product_release_bundle.py") < (
        _last_refresh_index("python3 tools/build_product_rollout_execution_readiness.py")
    )
    assert _last_refresh_index("python3 tools/build_product_rollout_execution_readiness.py") < (
        _last_refresh_index("python3 tools/product/build_product_launch_r4_preflight.py")
    )
    assert _last_refresh_index("python3 tools/product/build_product_launch_r4_preflight.py") < (
        _last_refresh_index("python3 tools/build_product_rollout_execution_smoke_receipt.py")
    )
    assert _last_refresh_index("python3 tools/build_product_rollout_execution_smoke_receipt.py") < (
        _last_refresh_index("python3 tools/build_deploy_ops_legal_gap_closure.py")
    )
    assert _last_refresh_index("python3 tools/build_deploy_ops_legal_gap_closure.py") < (
        _last_refresh_index("python3 tools/build_master_gap_closure_rollup.py")
    )
    assert _last_refresh_index("python3 tools/build_master_gap_closure_rollup.py") < (
...
def test_release_source_of_truth_tracks_capability_surface_builder_sources() -> None:
    ...
    assert "runs/gpcr_hard_decoy_claim_unlock_audit_current.json" not in depends_on
...
def test_release_source_of_truth_tracks_product_operator_cockpit() -> None:
    ...
    hbond_spec = next(
        spec
        for spec in mod.DEFAULT_ARTIFACT_SPECS
        if spec["artifact_id"] == "hbond_backmap_report"
    )
    ledger_spec = next(
        spec
        for spec in mod.DEFAULT_ARTIFACT_SPECS
    ...
    score_template_receipt_spec = next(
        spec
        for spec in mod.DEFAULT_ARTIFACT_SPECS
    ...
    spec = next(
        spec
        for spec in mod.DEFAULT_ARTIFACT_SPECS
        if spec["artifact_id"] == "product_operator_cockpit"
    )
    depends_on = spec["depends_on"]
    ...
    assert "tools/build_product_execution_work_order.py" in developer_preview_spec["depends_on"]
    assert "tools/build_product_pose_sampling_readiness.py" in developer_preview_spec["depends_on"]
    assert "tools/product/build_backmapping_scoring_batch_smoke_benchmark.py" in developer_preview_spec["depends_on"]
    assert (
        "tools/product/build_developer_preview_large_model_oom_guard_receipt.py"
        in developer_preview_spec["depends_on"]
    )
    ...
    assert score_template_receipt_spec["artifact_path"] == (
        "runs/public_benchmark_vina_gnina_score_template_receipt_current.json"
    )
    assert score_template_receipt_spec["builder_command"] == (
        mod.PUBLIC_BENCHMARK_VINA_GNINA_SCORE_TEMPLATE_RECEIPT_COMMAND
    )
    for receipt_source in (
    ...
    assert "api/main.py" in depends_on
    assert "api/product.py" in depends_on
    assert "api/product_operator_cockpit.py" in depends_on
    ...
    assert _last_refresh_index(mod.HBOND_BACKMAP_REPORT_COMMAND) < _last_refresh_index(
        mod.PRODUCT_OPERATOR_COCKPIT_COMMAND
    )
    assert _last_refresh_index("python3 tools/build_goal_operator_action_board.py") < _last_refresh_index(
        mod.PRODUCT_OPERATOR_COCKPIT_COMMAND
    )
...
def test_release_source_of_truth_tracks_pocketmd_lite_topk_refinement_chain() -> None:
    ...
    expected = {
        ...
        "pocketmd_lite_bounded_metric_collector": (
            "runs/pocketmd_lite_bounded_metric_collector_current.json",
            mod.POCKETMD_LITE_BOUNDED_METRIC_COLLECTOR_COMMAND,
            [
        ...
        "pocketmd_lite_candidate_metric_fill_preview_report": (
            ...
            mod.POCKETMD_LITE_CANDIDATE_METRIC_FILL_PREVIEW_REPORT_COMMAND,
            [
        ...
        "pocketmd_lite_canonical_report_review_packet": (
            ...
            mod.POCKETMD_LITE_CANONICAL_REPORT_REVIEW_PACKET_COMMAND,
            [
    ...
    assert order[mod.POCKETMD_LITE_BOUNDED_METRIC_COLLECTOR_COMMAND] < order[
        mod.POCKETMD_LITE_METRIC_COLLECTION_PROBE_COMMAND
    ]
    ...
    assert order[mod.POCKETMD_LITE_CANDIDATE_METRIC_FILL_PREVIEW_REPORT_COMMAND] < order[
        mod.POCKETMD_LITE_CANONICAL_REPORT_REVIEW_PACKET_COMMAND
    ]
    assert order[mod.POCKETMD_LITE_CANONICAL_REPORT_REVIEW_PACKET_COMMAND] < order[
        mod.POCKETMD_LITE_TOPK_REFINEMENT_AUDIT_COMMAND
    ]
    ...
    assert "runs/pocketmd_lite_canonical_report_review_packet_current.json" in (
        capability_spec["depends_on"]
    )
...
def test_release_source_of_truth_tracks_gpcr_hard_decoy_phase3_closure_chain() -> None:
    ...
    claim_unlock_depends_on = by_id["gpcr_hard_decoy_claim_unlock_audit"]["depends_on"]
    assert "runs/gpcr_broad_claim_scope_readiness_current.json" not in claim_unlock_depends_on
    assert "runs/gpcr_active_scorer_promotion_decision_packet_current.json" not in claim_unlock_depends_on
    assert mod.GPCR_A1_ACCURACY_REPAIR_QUEUE_COMMAND not in mod.RELEASE_REFRESH_COMMANDS
    ...
    assert order["python3 tools/build_gpcr_commercial_phase_ab_closure_chain.py"] < order[
        mod.GPCR_A1_INDEPENDENT_REPEAT_PACKET_COMMAND
    ]
    ...
    assert order["python3 tools/build_gpcr_broad_claim_scope_readiness.py"] < order[
        mod.GPCR_HARD_DECOY_CLAIM_UNLOCK_AUDIT_COMMAND
    ]
