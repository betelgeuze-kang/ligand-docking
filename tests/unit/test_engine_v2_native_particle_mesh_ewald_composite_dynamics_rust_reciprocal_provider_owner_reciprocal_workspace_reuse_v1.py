...
from tools import (
    verify_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_owner_reciprocal_workspace_reuse_v1
    as verifier,
)
...
def test_exact_profile_manifest_and_contracts() -> None:
    ...
    assert result["source_count"] == 314
...
def test_exact_anchors_and_delta() -> None:
    assert verifier.PREDECESSOR["pull_request"] == 463
    assert verifier.PREDECESSOR["reviewed_head"] == "375e29d8fe43ca588366139fdb5ea0c1a4016b0f"
    assert verifier.PREDECESSOR["merge_commit"] == "a011c42d2c9902fb4e2ae2bf3f5ab4cffc86737c"
    assert verifier.PREDECESSOR["merge_tree"] == "84cdf4c64f8b41f99380700f0cdb090a852a3c03"
...
def test_workflow_static_trigger_closure_and_bodies() -> None:
    assert len(verifier.REQUIRED_TRIGGER_PATHS) == 132
    assert len(set(verifier.REQUIRED_TRIGGER_PATHS)) == 132
    ...
    assert "refs/pull/463/head" in workflow
...
def test_owner_workspace_source_hashes_mirrors_and_contract() -> None:
    verifier.require_rust_reciprocal_provider_owner_reciprocal_workspace_reuse_contract(ROOT)
...
def test_predecessor_workflow_executes_exact_frozen_merge() -> None:
    ...
    for token in (
        "Materialize exact PR 463 evidence and reviewed head",
        "Verify exact frozen PR 463 evidence",
...
def test_predecessor_unit_skip_is_exact_and_frozen() -> None:
    ...
    assert "OWNER_RECIPROCAL_WORKSPACE_REUSE_EVIDENCE_PRESENT" in transformed
    assert "exact frozen PR 463 object" in transformed
