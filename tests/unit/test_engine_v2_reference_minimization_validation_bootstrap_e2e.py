...
def test_verified_request_runs_environment_runner_and_writer_in_order(
    ...
    assert {
        field for field, value in response.items() if value is True
    } == {"failure_inclusive_result_receipt_written"}
    for field in (
...
def test_environment_and_observation_do_not_complete_a_failed_result_write(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[str] = []
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (
            calls.append("environment")
            or SimpleNamespace(receipt_sha256="5" * 64)
        ),
    )
    monkeypatch.setattr(
        physics,
        ...
        lambda *args, **kwargs: (
            calls.append("runner")
            or SimpleNamespace(to_dict=lambda: {"observation": "bounded"})
        ),
    )

    def fail_result_write(*args: object, **kwargs: object) -> object:
        del args, kwargs
        calls.append("writer")
        raise RuntimeError("result receipt was not persisted")
