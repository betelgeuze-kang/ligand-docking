...
import pytest
import tools.verify_engine_v2_native_fixed64_cpu_profile_v5 as verifier
...
from tools.verify_engine_v2_native_fixed64_cpu_profile_v5 import (
    ...
    NativeFixed64CPUProfileV5Error,
...
_PROBE_SOURCE = (
    _ROOT / "rust/betelgeuze-runtime/src/bin/betelgeuze-fixed64-cpu-probe-v5.rs"
)
_TRANSITIVE_SOURCES = {
...
def test_canonical_native_fixed64_cpu_profile_v5_is_frozen() -> None:
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            "docking",
            b"betelgeuze.engine_v2_native_fixed64_complete_pipeline/2.0.0",
            b"betelgeuze.engine_v2_native_fixed64_complete_pipeline/2.0.1",
        ),
        (
            "native_pipeline",
            b"betelgeuze.engine_v2_native_fixed64_complete_pipeline/2.0.0",
        ...
        (
            "native_pipeline",
            b"committed.post_admitted_count = post_admitted_count;",
            b"committed.post_admitted_count = 0;",
        ),
        (
            "probe",
            b"Fixed64CpuProbeConfigV5::qualification_profile()",
            b"Fixed64CpuProbeConfigV5::unit_test()",
        ),
        (
            "qualification",
            b"pub const FIXED64_CPU_V5_LIVE_ACTIVATION_ADMITTED: bool = false;",
            b"pub const FIXED64_CPU_V5_LIVE_ACTIVATION_ADMITTED: bool = true;",
        ),
        (
            "qualification",
            b"pub const fn fixed64_cpu_v5_live_activation_admitted() -> bool {\n"
            b"    FIXED64_CPU_V5_LIVE_ACTIVATION_ADMITTED\n"
            b"}",
            b"pub const fn fixed64_cpu_v5_live_activation_admitted() -> bool {\n"
            b"    true\n}",
        ),
...
def test_profile_v5_rejects_compiled_gate_drift(
    ...
    with pytest.raises(NativeFixed64CPUProfileV5Error, match="compiled|entry point"):
        require_compiled_profile_binding(
            profile,
            qualification,
            docking,
...
def test_profile_v5_rejects_measurement_moved_before_activation_guard() -> None:
    profile = require_profile_document(_PROFILE.read_bytes())
    probe = _PROBE_SOURCE.read_bytes()
    measurement_call = b"run_native_fixed64_cpu_probe_v5(config)"
    activation_guard = b"if !fixed64_cpu_v5_live_activation_admitted()"
    ...
    changed["rust/betelgeuze-runtime/src/bin/betelgeuze-fixed64-cpu-probe-v5.rs"] = (
        probe
    )
    ...
    with pytest.raises(NativeFixed64CPUProfileV5Error, match="entry point|precede"):
        require_compiled_profile_binding(
            profile,
            _QUALIFICATION_SOURCE.read_bytes(),
            _DOCKING_SOURCE.read_bytes(),
...
def test_profile_v5_requires_runtime_observation_of_release_activation_constant() -> (
    None
):
    profile = require_profile_document(_PROFILE.read_bytes())
    changed = dict(_TRANSITIVE_SOURCES)
    path = "rust/betelgeuze-runtime/tests/fixed64_cpu_probe_v5_activation.rs"
    old = b"std::hint::black_box(FIXED64_CPU_V5_LIVE_ACTIVATION_ADMITTED)"
    new = b"std::convert::identity(FIXED64_CPU_V5_LIVE_ACTIVATION_ADMITTED)"
...
def test_profile_v5_rejects_transitive_kernel_source_drift(target: str) -> None:
    profile = require_profile_document(_PROFILE.read_bytes())
    changed = dict(_TRANSITIVE_SOURCES)
    drift = b"\n// semantic drift\n"
...
def test_profile_v5_rejects_transitive_source_path_omission() -> None:
    profile = require_profile_document(_PROFILE.read_bytes())
    changed = dict(_TRANSITIVE_SOURCES)
...
def test_profile_v5_rejects_cross_wired_qualification_source() -> None:
    profile = require_profile_document(_PROFILE.read_bytes())
...
def test_profile_v5_rejects_abi_probe_source_drift() -> None:
    profile = require_profile_document(_PROFILE.read_bytes())
    changed = dict(_TRANSITIVE_SOURCES)
...
def test_profile_v5_rejects_vendor_source_drift() -> None:
    profile = require_profile_document(_PROFILE.read_bytes())
    changed = dict(_TRANSITIVE_SOURCES)
...
def test_profile_v5_rejects_vendor_equality_manifest_drift() -> None:
    profile = require_profile_document(_PROFILE.read_bytes())
    changed = dict(_TRANSITIVE_SOURCES)
...
def test_profile_v5_rejects_cfg_shadowed_vendor_declaration() -> None:
    profile = require_profile_document(_PROFILE.read_bytes())
    changed = dict(_TRANSITIVE_SOURCES)
    build_path = "rust/betelgeuze-sys/build.rs"
...
def test_profile_v5_rejects_vendor_shadow_header() -> None:
    expected = tuple(
        path.as_posix() for path in NATIVE_VENDOR_COPY_SOURCE_RELATIVE_PATHS
    )
...
def test_profile_v5_rejects_symlinked_vendor_root(tmp_path: Path) -> None:
...
def test_profile_v5_rejects_symlinked_vendor_parent(tmp_path: Path) -> None:
...
def test_profile_v5_rejects_runtime_module_drift(target: str) -> None:
    profile = require_profile_document(_PROFILE.read_bytes())
    changed = dict(_TRANSITIVE_SOURCES)
...
def test_profile_v5_rejects_embedded_fixture_drift(target: str) -> None:
    profile = require_profile_document(_PROFILE.read_bytes())
    changed = dict(_TRANSITIVE_SOURCES)
...
def test_profile_v5_rejects_unbound_rust_source() -> None:
    with pytest.raises(
        NativeFixed64CPUProfileV5Error,
...
def test_profile_v5_discovers_exact_rust_source_tree() -> None:
    assert discover_rust_compiled_source_tree_paths(_ROOT) == tuple(
        sorted(_RUST_SOURCE_TREE_PATHS)
    )


def test_profile_v5_discovers_exact_rust_embedded_inputs() -> None:
...
def test_profile_v5_scans_only_present_nonempty_rust_source_roots() -> None:
...
def test_profile_v5_discovers_exact_cargo_target_sources() -> None:
    assert (
...
def test_profile_v5_rejects_package_root_probe_target() -> None:
    changed = tuple(
        "rust/betelgeuze-runtime/probe.rs"
        if path == "rust/betelgeuze-runtime/src/bin/betelgeuze-fixed64-cpu-probe-v5.rs"
        else path
...
def test_profile_v5_rejects_unbound_local_dependency_root() -> None:
    changed = tuple(
        (
            *binding[:4],
...
def test_profile_v5_rejects_local_dependency_compile_option_drift(
    binding_index: int,
    replacement: object,
) -> None:
    changed = []
...
def test_profile_v5_rejects_external_local_dependency_from_cargo_metadata(
    tmp_path: Path,
) -> None:
...
def test_profile_v5_rejects_local_dependency_feature_from_cargo_metadata(
    tmp_path: Path,
) -> None:
    copied_root = tmp_path / "copied-repository"
    shutil.copytree(
...
def test_profile_v5_rejects_cargo_patch_table_before_metadata(
    tmp_path: Path,
) -> None:
    copied_root = tmp_path / "copied-repository"
    shutil.copytree(
...
def test_profile_v5_rejects_cargo_root_assignment_before_metadata(
    tmp_path: Path,
    root_assignment: str,
) -> None:
    copied_root = tmp_path / "copied-repository"
...
def test_profile_v5_rejects_rust_path_attribute_outside_inventory(
    tmp_path: Path,
) -> None:
    copied_root = tmp_path / "copied-repository"
    shutil.copytree(
...
def test_profile_v5_rejects_unbound_rust_include_input(
    tmp_path: Path,
    macro: str,
    expected_error: str,
) -> None:
    ...
    with pytest.raises(NativeFixed64CPUProfileV5Error, match=expected_error):
        discover_rust_compiled_source_tree_paths(copied_root)
...
def test_profile_v5_rejects_comment_separated_rust_include_input(
    tmp_path: Path,
    macro_source: str,
) -> None:
    copied_root = tmp_path / "copied-repository"
...
def test_profile_v5_rejects_include_between_rust_character_literals(
    tmp_path: Path,
    character_type: str,
...
def test_profile_v5_ignores_include_tokens_inside_comments_and_raw_strings(
    tmp_path: Path,
) -> None:
    copied_root = tmp_path / "copied-repository"
    shutil.copytree(
...
def test_profile_v5_requires_embedded_target_in_transitive_manifest(monkeypatch) -> None:
...
def test_profile_v5_rejects_symlinked_rust_source_subdirectory(
    tmp_path: Path,
) -> None:
    for relative in RUST_COMPILED_SOURCE_TREE_ROOT_RELATIVE_PATHS:
...
def test_profile_v5_discovers_exact_rust_package_build_scripts() -> None:
    assert discover_rust_package_build_script_paths(_ROOT) == (
        "rust/betelgeuze-sys/build.rs",
    )


def test_profile_v5_rejects_unbound_default_rust_build_script(
    tmp_path: Path,
) -> None:
...
def test_profile_v5_rejects_symlinked_compiler_input(tmp_path: Path) -> None:
...
def test_profile_v5_rejects_symlinked_compiler_input_parent(
    tmp_path: Path,
) -> None:
...
def test_profile_v5_rejects_build_script_inventory_drift() -> None:
    with pytest.raises(
        NativeFixed64CPUProfileV5Error,
        match="build-script set contains an unbound",
    ):
...
def test_profile_v5_canonical_repository_has_no_cargo_configuration() -> None:
    require_repository_cargo_configuration_absent(_ROOT)


def test_profile_v5_release_non_test_activation_check_is_in_native_ci() -> None:
    ...
    assert source.count("--test fixed64_cpu_probe_v5_activation") == 1
    assert (
...
def test_profile_v5_native_workflow_change_triggers_focused_suite() -> None:
...
def test_profile_v5_root_compiler_overrides_trigger_and_reach_ci_checkout(
    relative: Path,
) -> None:
...
def test_profile_v5_rejects_repository_cargo_configuration(
    tmp_path: Path,
    relative: Path,
) -> None:
...
def test_profile_v5_rejects_symlinked_cargo_configuration_parent(
    tmp_path: Path,
) -> None:
...
def test_profile_v5_canonical_repository_has_no_rust_toolchain_override() -> None:
...
def test_profile_v5_rejects_repository_rust_toolchain_override(
    tmp_path: Path,
    relative: Path,
) -> None:
    path = tmp_path / relative
...
def test_profile_v5_rejects_authority_or_numeric_drift(mutate) -> None:
    profile = json.loads(_PROFILE.read_text(encoding="ascii"))
    changed = deepcopy(profile)
    mutate(changed)

    with pytest.raises(NativeFixed64CPUProfileV5Error):
        require_profile_document(_canonical(changed))


def test_profile_v5_rejects_noncanonical_or_duplicate_json() -> None:
    profile = json.loads(_PROFILE.read_text(encoding="ascii"))
    compact = json.dumps(profile, sort_keys=True, separators=(",", ":")).encode("ascii")
    with pytest.raises(NativeFixed64CPUProfileV5Error, match="serialization"):
    ...
    with pytest.raises(NativeFixed64CPUProfileV5Error, match="duplicate"):
        require_profile_document(duplicate)


def test_profile_v5_cli_reports_non_consuming_authority_false() -> None:
