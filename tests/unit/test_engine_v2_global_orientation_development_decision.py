...
def _replace_case(
    ...
    **changes: object,
    ...
    return tuple(
        replace(row, **changes) if row.case_id == case_id else row for row in rows
    )
...
def test_one_new_recovery_does_not_meet_breadth_criterion() -> None:
    ...
    assert (
        ...
        "previously_uncovered_cases" not in decision.go_criteria
    )
...
def test_incomplete_evidence_fails_an_invariant() -> None:
    ...
    assert "complete_source_and_observation_rederivation" in decision.invariant_failures
