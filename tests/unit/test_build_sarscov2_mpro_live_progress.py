...
from tools import build_sarscov2_mpro_live_progress as mod
from tools.wetlab_target_render_utils import load_json


def test_build_sarscov2_mpro_live_progress_defaults_to_not_started() -> None:
...
def test_build_sarscov2_mpro_live_progress_marks_running() -> None:
    ...
    assert summary["active_stage_label"] == "fluorogenic_primary_assay"
    assert summary["current_stage"] == "fluorogenic_primary_assay"
    assert summary["started_at"] == "2026-03-29T09:15:00"
    assert summary["updated_at"] == "2026-03-29T10:30:00"
