...
def test_cif_lexer_counts_crlf_as_one_physical_line_separator(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(mmcif_syntax, "_MAX_CIF_LINE_COUNT", 3)
    block = parse_cif_block("data_x\r\n_entry.id\r\ndemo")
    assert block.name == "x"
    assert block.scalar_values["_entry.id"].value == "demo"


def test_cif_lexer_reports_invalid_character_line_for_cr_only_input() -> None:
    with pytest.raises(CifSyntaxError) as exc_info:
        tokenize_cif("data_x\r_entry.id demo\r_entry.note café")
