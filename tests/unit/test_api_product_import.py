...
from tests.route_compat import route_contexts, route_paths
...
def test_api_product_router_is_registered_when_fastapi_is_available() -> None:
    ...
    app_routes = route_contexts(main.app)
    paths = route_paths(main.app)
    architecture_router_paths = route_paths(product_architecture.router)
    benchmark_router_paths = route_paths(product_benchmark.router)
    capability_router_paths = route_paths(product_capabilities.router)
    docking_router_paths = route_paths(product_docking.router)
    service_contract_router_paths = route_paths(product_service_contracts.router)
    operational_router_paths = route_paths(product_operational.router)
    release_ops_router_paths = route_paths(product_release_ops.router)
    ai_surface_router_paths = route_paths(product_ai_surface.router)
    cameo_runner_router_paths = route_paths(product_cameo_runner.router)
    license_router_paths = route_paths(product_license.router)
    production_ai_router_paths = route_paths(product_production_ai.router)
    scope_router_paths = route_paths(product_scope.router)
    commercial_readiness_router_paths = route_paths(product_commercial_readiness.router)
    evidence_goal_router_paths = route_paths(product_evidence_goal.router)
    hbond_backmap_router_paths = route_paths(product_hbond_backmap.router)
    ...
    assert "/product/gpcr-hard-decoy-suite-report" in route_paths(
        product_gpcr_hard_decoy.router
    )
    ...
    assert sum(1 for route in app_routes if route.path == "/product/architecture") == 1
    assert sum(1 for route in app_routes if route.path == "/product/architecture-validation") == 1
    assert sum(1 for route in app_routes if route.path == "/product/capabilities") == 1
    assert sum(1 for route in app_routes if route.path == "/product/hbond-backmap-report") == 1
    assert sum(1 for route in app_routes if route.path == "/product/gpcr-hard-decoy-suite-report") == 1
    assert sum(1 for route in app_routes if route.path == "/product/service-boundary") == 1
    assert sum(1 for route in app_routes if route.path == "/product/api-contract") == 1
    assert sum(1 for route in app_routes if route.path == "/product/operational-quality") == 1
    assert sum(1 for route in app_routes if route.path == "/product/security-deployment-contract") == 1
    assert sum(1 for route in app_routes if route.path == "/product/operations") == 1
    assert sum(1 for route in app_routes if route.path == "/product/commercial-independence") == 1
    assert sum(1 for route in app_routes if route.path == "/product/release-readiness") == 1
    assert sum(1 for route in app_routes if route.path == "/product/job-orchestration-contract") == 1
    assert sum(1 for route in app_routes if route.path == "/product/external-metrics") == 1
    assert sum(1 for route in app_routes if route.path == "/product/public-benchmark") == 1
    assert sum(1 for route in app_routes if route.path == "/product/trajectory-sla-contract") == 1
    assert sum(1 for route in app_routes if route.path == "/product/rollout-execution-smoke-receipt") == 1
    assert sum(1 for route in app_routes if route.path == "/product/ai-decision-graph") == 1
    assert sum(1 for route in app_routes if route.path == "/product/pose-sampling-readiness") == 1
    assert sum(1 for route in app_routes if route.path == "/product/ai-report-ux") == 1
    assert sum(1 for route in app_routes if route.path == "/product/residual-model-registry") == 1
    assert sum(1 for route in app_routes if route.path == "/product/cameo-live-validation") == 1
    assert sum(1 for route in app_routes if route.path == "/product/cameo-official-result-fetch-preflight") == 1
    assert (
        sum(
            1
    ...
    assert sum(1 for route in app_routes if route.path == "/product/license-decision") == 1
    assert sum(1 for route in app_routes if route.path == "/product/license-options") == 1
    assert sum(1 for route in app_routes if route.path == "/product/license-file-work-order") == 1
    assert (
        sum(1 for route in app_routes if route.path == "/product/self-hosted-license-distribution-audit")
        == 1
    )
    assert sum(1 for route in app_routes if route.path == "/product/production-ai-checkpoint-readiness") == 1
    assert (
        sum(1 for route in app_routes if route.path == "/product/production-ai-gpu-worker-dispatch-manifest")
        == 1
    )
    assert (
        sum(1 for route in app_routes if route.path == "/product/production-ai-gpu-worker-dispatch-bundle")
        == 1
    )
    assert (
        sum(1 for route in app_routes if route.path == "/product/production-ai-gpu-worker-execution-runbook")
        == 1
    )
    assert sum(1 for route in app_routes if route.path == "/product/production-ai-gpu-return-intake") == 1
    assert sum(1 for route in app_routes if route.path == "/product/production-ai-promotion-workbench") == 1
    assert (
        sum(
            1
            for route in app_routes
    ...
    assert (
        sum(1 for route in app_routes if route.path == "/product/production-ai-registry-promotion-priority")
        == 1
    )
    assert sum(1 for route in app_routes if route.path == "/product/scope-breadth-contract") == 1
    assert sum(1 for route in app_routes if route.path == "/product/scope-claim-guard") == 1
    assert sum(1 for route in app_routes if route.path == "/product/scope-evidence-priority") == 1
    assert sum(1 for route in app_routes if route.path == "/product/scope-evidence-intake-readiness") == 1
    assert sum(1 for route in app_routes if route.path == "/product/transporter-manual-review-intake") == 1
    assert sum(1 for route in app_routes if route.path == "/product/pxr-exact-review-intake") == 1
    assert sum(1 for route in app_routes if route.path == "/product/aqp1-operator-validation-candidate") == 1
    assert (
        sum(1 for route in app_routes if route.path == "/product/aqp1-direct-binding-procurement-packet")
        == 1
    )
    assert sum(1 for route in app_routes if route.path == "/product/commercial-readiness-operator-packet") == 1
    assert (
        sum(1 for route in app_routes if route.path == "/product/commercial-readiness-operator-packet-freshness")
        == 1
    )
    assert sum(1 for route in app_routes if route.path == "/product/commercial-readiness-execution-ladder") == 1
    assert sum(1 for route in app_routes if route.path == "/product/commercial-readiness-handoff-bundle") == 1
    assert sum(1 for route in app_routes if route.path == "/product/scope-breadth-evidence-receipt") == 1
    assert sum(1 for route in app_routes if route.path == "/product/engine-refinement-claim-evidence-receipt") == 1
    assert (
        sum(1 for route in app_routes if route.path == "/product/engine-refinement-claim-evidence-priority")
        == 1
    )
    assert (
        sum(1 for route in app_routes if route.path == "/product/full-commercial-blocker-evidence-matrix")
        == 1
    )
    assert sum(1 for route in app_routes if route.path == "/product/goal-completion-audit") == 1
    ...
    assert operations["commercial_independent_product_claim_allowed"] is True
    ...
    assert commercial["restricted_commercial_scope_claim_ready"] is True
    assert commercial["commercial_claim_scope_tier"] == "restricted_family_local_product"
    ...
    assert release["goal_release_blocker_count"] == 3
    ...
    assert checkpoint["fail_check_count"] == 1
    assert checkpoint["failed_check_ids"] == ["registry_customer_facing_promotion_allowed"]
    ...
    assert checkpoint["force_gpu_worker_post_return_promotion_ladder_current_next_stage_validation_command"] == (
        "python3 tools/build_residual_model_registry.py && python3 tools/build_product_production_ai_checkpoint_readiness.py"
    )
    ...
    assert checkpoint["production_inference_actionable_blocker_observed"] == (
        "default_residual_mode=shadow;production_promotion_allowed=false;customer_facing_mutation_flags=false"
    )
    ...
    assert checkpoint["production_inference_actionable_blocker_next_action"] == (
        "Complete the guarded production AI registry promotion operator receipt while keeping customer-facing mutation disabled until approval."
    )
    ...
    assert checkpoint["production_inference_actionable_operator_completion_diagnostic_completion_rule"] == (
        "production_promotion_allowed=true requires guarded residual mode, trained_model_checkpoint_count>0, and explicit customer-facing mutation approval."
    )
    ...
    assert len(checkpoint["force_gpu_worker_post_run_validation_commands"]) == 18
    ...
    assert checkpoint["checkpoint_closure_blockers"] == [
    ...
    assert handoff_bundle["status"] == "product_commercial_readiness_handoff_bundle_ready"
    assert handoff_bundle["handoff_bundle_ready"] is True
    assert handoff_bundle["goal_complete"] is False
    assert handoff_bundle["artifact_count"] == 3
    assert handoff_bundle["ready_artifact_count"] == 3
    assert handoff_bundle["blocked_artifact_count"] == 0
    assert handoff_bundle["operator_packet_ready"] is True
    ...
    assert handoff_bundle["ladder_parallelizable_action_count"] == 2
    assert handoff_bundle["first_parallelizable_action_id"] == "transporter_next_slot_exact_evidence"
    ...
    assert handoff_bundle["production_ai_registry_promotion_operator_completion_packet_ready"] is True
    ...
    assert engine_priority["top_required_input"] == (
    ...
    assert (
        ...
        in engine_priority["top_verification_command"]
    )
    ...
    assert completion["product_ai_architecture_gap_status"] == (
    ...
    assert completion["product_ai_architecture_closed_gap_count"] == 4
    assert completion["product_ai_architecture_open_gap_count"] == 3
    assert completion["product_ai_architecture_gap_blocker_matrix_count"] == 1
    ...
    assert completion["production_ai_gpu_return_fail_check_count"] == 5
