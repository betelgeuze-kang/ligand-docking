...
def _results() -> dict[str, dict[str, object]]:
    ...
    for case_id in evidence.EXPECTED_CASE_IDS:
        ...
        results[case_id] = {
            "engine_v2_diagnostics": {"candidates": candidates},
        }
    return results
...
@pytest.mark.parametrize("drift", ("schema", "pair_product", "non_target_value"))
...
def test_reviewed_evidence_identities_are_pinned() -> None:
    assert len(evidence.EXPECTED_SOURCE_COMMIT_SHA1) == 40
    assert all(
        character in "0123456789abcdef"
        for character in evidence.EXPECTED_SOURCE_COMMIT_SHA1
    )
    for value in (
...
def _synthetic_bundle(
    ...
    report: dict[str, object] = {
        ...
        "source_commit_sha1": evidence.EXPECTED_SOURCE_COMMIT_SHA1,
        "decision": "fixture",
    }
