...
from tools import run_wetlab_dpre1_exploratory_retry as mod


def test_run_wetlab_dpre1_exploratory_retry_refreshes_lane_before_launch(monkeypatch, tmp_path) -> None:
    ...
    assert captured["target_id"] == "DprE1"
    assert captured["shard_id"] == "04_of_20"
...
def test_run_wetlab_dpre1_exploratory_retry_can_force_preflight_only(monkeypatch, tmp_path) -> None:
    lane_json = tmp_path / "lane.json"
    lane_json.write_text("{}", encoding="utf-8")
    (tmp_path / "hold.json").write_text(json.dumps({"summary": {}, "rows": []}), encoding="utf-8")
    (tmp_path / "queue.json").write_text(json.dumps({"summary": {}, "rows": []}), encoding="utf-8")
