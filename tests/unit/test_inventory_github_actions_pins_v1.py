...
SPEC = importlib.util.spec_from_file_location(
    "actions_inventory",
    ROOT / "tools/inventory_github_actions_pins_v1.py",
)
...
""",
    ...
    report = ACTIONS.inspect(tmp_path)
...
def test_exact_sha_remote_action_is_pinned(tmp_path: Path) -> None:
    workflows = tmp_path / ".github/workflows"
    workflows.mkdir(parents=True)
...
""",
    ...
    report = ACTIONS.inspect(tmp_path)
    action = report["workflows"][0]["actions"][0]
    assert action["kind"] == "remote"
    assert action["exact_sha_pinned"] is True
    assert report["mutable_remote_total"] == 0
