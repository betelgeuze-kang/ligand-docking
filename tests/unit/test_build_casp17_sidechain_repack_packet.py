...
def test_build_casp17_sidechain_repack_packet_reduces_soft_clashes(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [
            "python3",
            str(ROOT / "tools/build_casp17_sidechain_repack_packet.py"),
            "--target-ids",
            "T9999",
            "--source-dir",
            str(source_dir),
    ...
    assert payload["summary"]["sidechain_repack_status"] == "pass"
    assert payload["summary"]["selection_mode"] == "sequential_coordinate_aware_greedy"
    assert row["sidechain_repack_status"] == "pass"
    ...
    assert "REMARK CASP17 SIDECHAIN_REPACK" in text
