...
def test_minimization_trajectory_comparison_public_exports_and_gate_are_closed() -> None:
    from betelgeuze_engine_v2 import physics
    from betelgeuze_engine_v2.physics.reference_minimization_validation_trajectory_comparison import (
        __all__ as trajectory_comparison_exports,
        reference_minimization_validation_trajectory_comparison_contract_decision,
    )

    assert set(trajectory_comparison_exports) <= set(physics.__all__)
    decision = reference_minimization_validation_trajectory_comparison_contract_decision()
    assert decision["trajectory_comparison_contract_frozen"] is True
    assert decision["production_comparison_receipt_present"] is False
    assert decision["two_cpu_host_reproduction_complete"] is False
    assert decision["external_implementation_comparison_complete"] is False
    assert decision["s0_accepted"] is False
    assert decision["s1_admission"] is False
...
def test_production_evidence_and_process_identity_public_exports_are_closed() -> None:
    ...
    from betelgeuze_engine_v2.physics.validation_production_reservation_witness_quorum_non_equivocation import (
        ...
        __all__ as reservation_witness_quorum_exports,
    ...
    custody_extension_contract = validation_production_review_authorization_custody_extension_contract_document()
    custody_extension_decision = validation_production_review_authorization_custody_extension_decision()
    reservation_contract = validation_production_reservation_custody_extension_contract_document()
    reservation_decision = validation_production_reservation_custody_extension_decision()
    registry_proof_contract = validation_production_reservation_registry_proof_contract_document()
    registry_proof_decision = validation_production_reservation_registry_proof_decision()
    head_receipt_contract = validation_production_reservation_authenticated_head_receipt_contract_document()
    head_receipt_decision = validation_production_reservation_authenticated_head_receipt_decision()
    later_head_contract = validation_production_reservation_later_head_consistency_contract_document()
    ...
    assert custody_contract["custody_event"]["custody_successor_uniqueness_enforced"] is False
    ...
    assert custody_extension_contract["custody_extension_event"]["implemented_sequences"] == [3, 4]
    assert custody_extension_contract["custody_extension_event"]["eligible_for_atomic_execution_reservation"] is False
    assert custody_extension_decision["pre_execution_review_carrier_implemented"] is True
    ...
    assert custody_extension_decision["production_validation_execution_authorized"] is False
    assert custody_extension_decision["production_validation_results_collected"] is False
    assert custody_extension_decision["claim_safe"] is False
    assert (
        reservation_contract["contract_sha256"]
        == FROZEN_VALIDATION_PRODUCTION_RESERVATION_CUSTODY_EXTENSION_CONTRACT_SHA256
    )
    assert reservation_contract["atomic_commit"]["external_serializable_commit_independently_verified"] is False
    ...
    assert registry_proof_contract["purpose"]["external_registry_backend_implemented_by_package"] is False
    assert (
        runtime_integrity_contract["bound_contracts"]["production_reservation_registry_proof_contract_sha256"]
        == VALIDATION_RUNTIME_INTEGRITY_BOUND_RESERVATION_REGISTRY_PROOF_CONTRACT_SHA256
        == FROZEN_VALIDATION_PRODUCTION_RESERVATION_REGISTRY_PROOF_CONTRACT_SHA256
    )
    assert registry_proof_decision["verifier_implemented"] is True
    assert registry_proof_decision["external_registry_transaction_proof_present"] is False
    assert registry_proof_decision["external_serializable_registry_commit_verified"] is False
    ...
    assert head_receipt_contract["purpose"]["strict_post_receipt_status_descendant_reverification_required"] is True
    assert (
    ...
    assert head_receipt_decision["authenticated_external_head_status_receipt_verified"] is False
    ...
    assert later_head_contract["purpose"]["global_non_equivocation_supported"] is False
    assert (
        runtime_integrity_contract["bound_contracts"]["production_reservation_later_head_consistency_contract_sha256"]
        == VALIDATION_RUNTIME_INTEGRITY_BOUND_RESERVATION_LATER_HEAD_CONSISTENCY_CONTRACT_SHA256
        == FROZEN_VALIDATION_PRODUCTION_RESERVATION_LATER_HEAD_CONSISTENCY_CONTRACT_SHA256
    )
    ...
    assert witness_quorum_contract["contract_sha256"] == (
        FROZEN_VALIDATION_PRODUCTION_RESERVATION_WITNESS_QUORUM_CONTRACT_SHA256
    )
    assert witness_quorum_contract["purpose"]["verifier_only"] is True
    assert witness_quorum_contract["purpose"]["individual_certificate_proves_non_equivocation"] is False
    assert (
        runtime_integrity_contract["bound_contracts"]["production_reservation_witness_quorum_contract_sha256"]
        == VALIDATION_RUNTIME_INTEGRITY_BOUND_RESERVATION_WITNESS_QUORUM_CONTRACT_SHA256
        == FROZEN_VALIDATION_PRODUCTION_RESERVATION_WITNESS_QUORUM_CONTRACT_SHA256
    )
    assert witness_quorum_decision["verifier_implemented"] is True
    assert witness_quorum_decision["external_witness_quorum_proof_present"] is False
...
def test_capability_yaml_matches_executable_v2_schema_v4_snapshot() -> None:
    ...
    assert "custody_stages_after_status_snapshot_not_implemented" not in custody_foundation["blockers"]
    for blocker in (
    ...
    assert "external_custody_successor_uniqueness_not_provisioned" in custody_foundation["blockers"]
    ...
    assert model_policy["current_state"] == ("bounded_complete_atom_site_model_set_and_single_model_1_execution_policy")
    ...
    assert "model_selection_ensemble_and_trajectory_semantics_not_interpreted" in model_policy["blockers"]

    assembly_policy = rows[MMCIF_BIOLOGICAL_ASSEMBLY_POLICY_CAPABILITY_ID]
    assert assembly_policy["current_state"] == ("bounded_source_declared_biological_assembly_preparation_admission")
    assert assembly_policy["internal_reference_execution_enabled"] is True
    assert "source_declared_biological_assembly_expansion_not_supported" in (assembly_policy["blockers"])
    assert "operation_matrix_vector_and_composition_not_interpreted" in (assembly_policy["blockers"])
    assert "absence_does_not_prove_asymmetric_unit_is_biological_assembly" in (assembly_policy["blockers"])

    missing_policy = rows[MMCIF_MISSING_ATOM_RESIDUE_POLICY_CAPABILITY_ID]
    assert missing_policy["current_state"] == ("bounded_source_declared_observation_gap_preparation_admission")
    assert missing_policy["internal_reference_execution_enabled"] is True
    assert "source_declared_unobserved_residue_repair_not_supported" in (missing_policy["blockers"])
    assert "source_declared_zero_occupancy_atom_repair_not_supported" in (missing_policy["blockers"])
    ...
    assert modified_residue["current_state"] == ("bounded_source_declared_modified_polymer_residue_identity")
    ...
    assert "parent_component_chemistry_not_interpreted" in (modified_residue["blockers"])
    assert "modified_residue_preparation_not_supported" in (modified_residue["blockers"])
    ...
    assert component_declarations["current_state"] == ("bounded_component_atom_and_bond_source_declarations")
    ...
    assert component_roles["current_state"] == ("bounded_water_monoatomic_metal_and_nonmetal_ion_composition_roles")
    ...
    assert "monoatomic_nonmetal_ion_preparation_not_supported" in component_roles["blockers"]

    struct_conn = rows[MMCIF_STRUCT_CONN_DECLARATIONS_CAPABILITY_ID]
    assert struct_conn["current_state"] == ("bounded_nonpoly_struct_conn_identity_declarations")
    ...
    assert atom_site["current_state"] == ("bounded_nonpoly_atom_site_observation_identity_join")
    ...
    assert coordinate_values["current_state"] == ("bounded_nonpoly_finite_binary64_coordinate_values")
    ...
    assert "occupancy_b_factor_and_formal_charge_not_interpreted" in coordinate_values["blockers"]

    scalar_values = rows[MMCIF_NONPOLY_ATOM_SITE_SCALAR_VALUES_CAPABILITY_ID]
    assert scalar_values["current_state"] == ("bounded_nonpoly_atom_site_scalar_value_semantics")
    assert scalar_values["internal_reference_execution_enabled"] is True
    assert "occupancy_population_and_altloc_semantics_not_interpreted" in scalar_values["blockers"]
    ...
    assert topology["current_state"] == ("bounded_component_bonds_and_identity_connection_topology")
    ...
    assert "atom_element_charge_and_aromaticity_not_crosschecked" in topology["blockers"]

    preparation = rows[MMCIF_NONPOLY_PREPARATION_CAPABILITY_ID]
    assert preparation["current_state"] == ("bounded_neutral_acyclic_coh_graph_preparation_and_parameterability_report")
    assert preparation["internal_reference_execution_enabled"] is True
    assert "source_declared_biological_assembly_preparation_not_supported" in (preparation["blockers"])
    assert "source_declared_observation_gap_preparation_not_supported" in (preparation["blockers"])
    ...
    assert "canonical_all_atom_system_not_bound_to_preparation_report" in (preparation["blockers"])

    hydrogen_coordinates = rows[MMCIF_NONPOLY_HYDROGEN_COORDINATE_CAPABILITY_ID]
    assert hydrogen_coordinates["current_state"] == ("bounded_graph_bound_fixed_parent_offset_angstrom_coordinates")
    assert hydrogen_coordinates["internal_reference_execution_enabled"] is True
    assert "fixed_parent_offset_does_not_interpret_neighbor_geometry" in (hydrogen_coordinates["blockers"])
    assert "hydrogen_bond_length_not_calibrated" in (hydrogen_coordinates["blockers"])
    assert "partial_charge_assignment_not_performed" in (hydrogen_coordinates["blockers"])

    all_atom_system = rows[MMCIF_NONPOLY_ALL_ATOM_SYSTEM_CAPABILITY_ID]
    assert all_atom_system["current_state"] == ("bounded_instance_canonical_all_atom_system_materialization")
    assert all_atom_system["internal_reference_execution_enabled"] is True
    assert "intercomponent_covalent_connections_not_materialized" in (all_atom_system["blockers"])
    assert "intercomponent_coordination_preserved_as_metadata_only" in (all_atom_system["blockers"])
    assert "reviewed_parameter_source_binding_is_separate_capability" in (all_atom_system["blockers"])
    assert "partial_charge_assignment_is_separate_capability" in (all_atom_system["blockers"])
    assert "canonical_all_atom_round_trip_is_separate_capability" in (all_atom_system["blockers"])
    ...
    assert "partial_charge_assignment_is_separate_capability" in (parameter_binding["blockers"])
    ...
    assert round_trip["current_state"] == ("canonical_json_all_atom_identity_round_trip_receipts")
    ...
    assert "caller_supplied_partial_charges_not_scientifically_validated" in (round_trip["blockers"])
    ...
    assert "exact_pubchem_cid_176_neutral_acetic_acid_graph_only" in (ph_protonation["blockers"])
    assert "source_structure_identity_not_authenticated" in (ph_protonation["blockers"])
    assert "ambiguous_population_abstains_below_90_percent_dominance" in (ph_protonation["blockers"])
    assert "bounded_cid_177_11199_tautomer_selection_is_separate_capability" in (ph_protonation["blockers"])
    ...
    assert ph_corpus["current_state"] == ("frozen_7_case_pubchem_identity_supported_abstention_and_failure_corpus")
    ...
    assert "tautomer_selection_evidence_is_separate_bounded_pair_corpus" in (ph_corpus["blockers"])

    tautomer = rows[MMCIF_NONPOLY_TAUTOMER_SELECTION_CAPABILITY_ID]
    assert tautomer["current_state"] == (
        "bounded_pubchem_cid_177_11199_reference_canonical_tautomer_selection_with_generated_hydrogen_transfer"
    )
    ...
    assert tautomer_corpus["current_state"] == ("frozen_6_case_pubchem_identity_supported_and_failure_tautomer_corpus")
    assert tautomer_corpus["internal_reference_execution_enabled"] is True
    assert "pubchem_source_specific_license_review_remains" in (tautomer_corpus["blockers"])
    assert "thermodynamic_and_population_evidence_missing" in (tautomer_corpus["blockers"])

    parameter_source = rows[PARAMETER_SOURCE_PROVENANCE_CAPABILITY_ID]
    assert parameter_source["current_state"] == ("reviewed_openff_sage_2_2_1_identity_license_scope_only")
    ...
    assert "partial_charge_assignment_is_separate_capability" in (parameter_source["blockers"])
    ...
    assert preparation_corpus["current_state"] == ("frozen_30_case_failure_complete_corpus_and_52_axis_coverage_ledger")
    assert preparation_corpus["internal_reference_execution_enabled"] is True
    assert "synthetic_base_corpus_plus_separate_real_world_ph_and_tautomer_corpora" in (preparation_corpus["blockers"])
    assert (
        "zero_classified_implementation_gaps_do_not_establish_scientific_readiness" in (preparation_corpus["blockers"])
    )
    ...
    assert h5_record["current_state"] == (
        "frozen_h5_parameter_origin_and_runtime_envelope_record_without_parameter_set_or_scientific_validation"
    )
    assert h5_record["internal_reference_execution_enabled"] is False
    assert "reviewed_sage_source_not_bound_to_runtime_parameter_values" in (h5_record["blockers"])
    assert "runtime_capacity_envelope_is_not_scientific_applicability_evidence" in (h5_record["blockers"])
    ...
    assert "reference_minimization_not_scientifically_validated" in (minimization["blockers"])
    ...
    assert "finite_difference_diagnostics_not_independent_scientific_validation" in (diagnostics["blockers"])
    assert "periodic_virial_cell_strain_derivative_not_implemented" in (diagnostics["blockers"])
    ...
    assert improper_constraint["current_state"] == (
        "bounded_versioned_improper_symmetric_constraint_projection_and_constrained_minimization_checkpoint_restart"
    )
    ...
    assert "harmonic_out_of_plane_improper_not_scientifically_validated" in (improper_constraint["blockers"])
    assert "equal_weight_distance_constraints_ignore_atomic_masses" in (improper_constraint["blockers"])
    assert "equal_weight_constrained_minimization_not_scientifically_validated" in (improper_constraint["blockers"])
    assert "independent_constrained_minimization_evidence_missing" in (improper_constraint["blockers"])
    ...
    assert "solvated_constrained_minimization_not_scientifically_validated" in (solvation["blockers"])
    assert "independent_solvated_minimization_evidence_missing" in (solvation["blockers"])
    ...
    assert "independent_analytic_oracle_not_implemented" not in validation_protocol["blockers"]
    assert "independent_scientific_review_missing" in validation_protocol["blockers"]
    assert "signed_independent_scientific_review_attestation_missing" in validation_protocol["blockers"]
    assert "trusted_independent_scientific_reviewer_key_not_provided" in validation_protocol["blockers"]
    assert "signed_execution_authorization_receipt_schema_not_frozen" not in validation_protocol["blockers"]
    assert "trusted_authorization_operator_key_not_provided" in validation_protocol["blockers"]
    assert "run_start_dependency_reverification_not_implemented" not in validation_protocol["blockers"]
    assert "authorization_nonce_not_atomically_reserved" in validation_protocol["blockers"]
    assert "execution_environment_contract_not_frozen" not in (validation_protocol["blockers"])
    ...
    assert "result_receipt_writer_not_implemented" not in (validation_protocol["blockers"])
    assert "production_validation_result_receipt_missing" in (validation_protocol["blockers"])
    assert "independent_result_review_missing" in validation_protocol["blockers"]
    assert "signed_independent_result_review_attestation_missing" in validation_protocol["blockers"]
    assert "trusted_independent_result_reviewer_key_not_provided" in validation_protocol["blockers"]
    assert (
    ...
    assert minimization_protocol["current_state"] == (
        "failure_inclusive_complete_coordinate_trace_checkpoint_restart_"
        "trajectory_comparison_contract_and_result_review_without_production_"
        "result_receipt_or_review"
    )
    assert minimization_protocol["internal_reference_execution_enabled"] is False
    assert "materializer_definition_is_not_validation_result_evidence" in (minimization_protocol["blockers"])
    assert "fixture_materializer_not_implemented" not in (minimization_protocol["blockers"])
    assert "independent_minimization_reference_is_not_validation_result_evidence" in (minimization_protocol["blockers"])
    assert "independent_minimization_reference_not_independently_reviewed" in (minimization_protocol["blockers"])
    assert "signed_independent_scientific_review_attestation_missing" in (minimization_protocol["blockers"])
    assert "trusted_independent_scientific_reviewer_key_not_provided" in (minimization_protocol["blockers"])
    assert "production_trust_store_not_provisioned" in (minimization_protocol["blockers"])
    assert "independent_minimization_reference_not_bound" not in (minimization_protocol["blockers"])
    assert "production_result_receipt_missing" in (minimization_protocol["blockers"])
    assert "signed_independent_result_review_attestation_missing" in (minimization_protocol["blockers"])
    assert "two_cpu_host_reproducibility_missing" in (minimization_protocol["blockers"])
    assert "coordinate_trace_not_retained_in_result_receipt" not in (minimization_protocol["blockers"])
    assert "trajectory_level_minimization_comparison_missing" not in (minimization_protocol["blockers"])
    assert "signed_execution_authorization_receipt_schema_not_frozen" not in (minimization_protocol["blockers"])
    assert "execution_environment_receipt_missing" in (minimization_protocol["blockers"])
    assert "run_start_dependency_reverification_not_implemented" not in (minimization_protocol["blockers"])
    assert "validation_runner_not_implemented" not in (minimization_protocol["blockers"])
    assert "result_receipt_writer_not_implemented" not in (minimization_protocol["blockers"])
    assert "scientific_validation_missing" in minimization_protocol["blockers"]
    assert "posebusters_benchmark_equivalence_not_established" in public_protocol["blockers"]
...
def test_engine_v2_docs_pin_legacy_count_and_declared_witness_scope() -> None:
    status = Path("docs/engine_v2_status.md").read_text(encoding="utf-8")
    policy = Path("docs/engine_v2_public_api.md").read_text(encoding="utf-8")
    ...
    assert "distinct caller-pinned declared" in bundle
    assert "independent fault domains" not in status
    assert "independent fault domains" not in policy
    assert "independent fault domains" not in readiness
    assert "independent fault domains" not in bundle
...
def test_main_integration_workflow_targets_main_and_complete_v2_suite() -> None:
    ...
    assert "FROZEN_VALIDATION_PRODUCTION_RESERVATION_CUSTODY_EXTENSION_CONTRACT_SHA256" in source
    assert "FROZEN_VALIDATION_PRODUCTION_RESERVATION_REGISTRY_PROOF_CONTRACT_SHA256" in source
    assert "VALIDATION_RUNTIME_INTEGRITY_BOUND_RESERVATION_REGISTRY_PROOF_CONTRACT_SHA256" in source
    assert "FROZEN_VALIDATION_PRODUCTION_RESERVATION_AUTHENTICATED_HEAD_RECEIPT_CONTRACT_SHA256" in source
    assert "VALIDATION_RUNTIME_INTEGRITY_BOUND_RESERVATION_AUTHENTICATED_HEAD_RECEIPT_CONTRACT_SHA256" in source
    assert "FROZEN_VALIDATION_PRODUCTION_RESERVATION_LATER_HEAD_CONSISTENCY_CONTRACT_SHA256" in source
    ...
    assert "FROZEN_REFERENCE_MINIMIZATION_VALIDATION_RUN_START_CONTRACT_SHA256" in source
    assert "FROZEN_REFERENCE_MINIMIZATION_VALIDATION_RESULT_WRITER_CONTRACT_SHA256" in source
    assert "FROZEN_REFERENCE_MINIMIZATION_VALIDATION_RESULT_REVIEW_CONTRACT_SHA256" in source
    assert "FROZEN_REFERENCE_MINIMIZATION_VALIDATION_TRAJECTORY_COMPARISON_CONTRACT_SHA256" in source


def test_cpu_reference_validation_workflow_covers_both_result_reviews() -> None:
    source = Path(".github/workflows/ci-engine-v2-cpu-reference-validation-protocol.yml").read_text(encoding="utf-8")
    ...
    assert "test_engine_v2_reference_minimization_validation_trajectory_comparison.py" in source
    assert "reference_minimization_validation_trajectory_comparison.py" in source
    assert "FROZEN_REFERENCE_MINIMIZATION_VALIDATION_RESULT_REVIEW_CONTRACT_SHA256" in source
    assert "reference_minimization_validation_result_review_contract_decision" in source
    assert "reference_minimization_validation_trajectory_comparison_contract_decision" in source
    ...
    assert "FROZEN_VALIDATION_PRODUCTION_EVIDENCE_CUSTODY_CONTRACT_SHA256" in source
    assert "test_engine_v2_validation_production_review_authorization_custody_extension.py" in source
    assert "test_engine_v2_validation_production_reservation_custody_extension.py" in source
    assert "test_engine_v2_validation_production_reservation_registry_proof.py" in source
