...
from tools import run_wetlab_dengue_ns2b_ns3_exploratory_retry as mod


def test_run_wetlab_dengue_ns2b_ns3_exploratory_retry_refreshes_lane_before_launch(monkeypatch, tmp_path) -> None:
    ...
    assert captured["target_id"] == "Dengue NS2B-NS3 protease"
    assert captured["shard_id"] == "09_of_20"
    assert captured["command_kind"] == "throughput_preflight_tuned_gate45"
    assert payload["summary"]["lane_label"] == "exploratory_gate4.5_followup"
    assert payload["summary"]["lane_phase"] == "followup"
