...
from tools.verify_engine_v2_mixed64_scorer_validity_ranking_v3 import (
    DEFAULT_POLICY_PATH,
    Mixed64ScorerValidityRankingPolicyVerificationError,
    verify_policy,
)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (("scoring", "backend"), "rust_cpu_required"),
        (("scoring", "maximum_batch_size"), 65),
        (("scoring", "result_dependent_retry_allowed"), True),
        (("ranking", "primary_includes_pose_invalid"), False),
...
def test_policy_tamper_fails_closed(
    ...
    with pytest.raises(Mixed64ScorerValidityRankingPolicyVerificationError):
...
def test_cli_runs_in_isolated_mode_outside_repository(tmp_path: Path) -> None:
    tool = (
        ...
        / "tools"
        / "verify_engine_v2_mixed64_scorer_validity_ranking_v3.py"
    )
