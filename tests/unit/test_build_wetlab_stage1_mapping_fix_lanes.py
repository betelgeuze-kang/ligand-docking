...
from tools import build_wetlab_stage1_mapping_fix_lanes as mod


def test_build_wetlab_stage1_mapping_fix_lanes_surfaces_mpro_and_tcruzi_only() -> None:
    retry_preset = {
        ...
        "rows": [
            ...
            },
            {
                ...
                "representative_stage6_failure_shard_id": "01_of_20",
            },
        ],
    }
    execution_queue = {
    ...
    portfolio = {
        "rows": [
            ...
            {"target_id": "ALK2", "domain_family": "kinase"},
        ]
    }
    ...
    assert summary["status"] == "wetlab_stage1_mapping_fix_lanes_ready"
    assert summary["target_count"] == 2
    assert summary["ready_target_count"] == 2
    assert summary["ready_targets"] == "SARS-CoV-2 Mpro; T. cruzi PDE"
    assert summary["stage1_mapping_fix_target_count"] == 2
    assert len(lane_outputs) == 2

    rows = payload["rows"]
    assert [row["target_id"] for row in rows] == ["SARS-CoV-2 Mpro", "T. cruzi PDE"]
    assert all(row["lane_kind"] == "stage1_mapping_fix_lane" for row in rows)
    assert rows[0]["shard_id"] == "01_of_20"
    assert rows[1]["shard_id"] == "07_of_20"
    assert all("run_wetlab_mapping_fix_retry.py" in row["runner_command"] for row in rows)


def test_build_wetlab_stage1_mapping_fix_lanes_omits_targets_when_stage6_tuning_is_ready() -> None:
    payload, lane_outputs = mod.build_payload(
    ...
    assert summary["stage1_mapping_fix_target_count"] == 0
    assert payload["rows"] == []
    assert lane_outputs == []
