...
bool is_available(int32_t device_ordinal) noexcept {
    ...
    const int32_t status = bg_hip_safe_provider_is_available_v1(
        device_ordinal, &available, error, sizeof(error));
    return status == BG_STATUS_OK && available == UINT8_C(1);
#else
...
bg_status evaluate(
    ...
    if (out_evaluation == nullptr) {
        return fail(BG_STATUS_INVALID_ARGUMENT, "hip_safe evaluation output is null");
    }
#if !BG_HAS_HIP_SAFE_PROVIDER
    static_cast<void>(device_ordinal);
    static_cast<void>(system);
    static_cast<void>(forcefield);
    static_cast<void>(compute_forces);
    return fail(
        BG_STATUS_BACKEND_UNAVAILABLE,
        "hip_safe provider is not compiled; CPU fallback is forbidden");
#else
    static_assert(std::is_standard_layout_v<bg_forcefield::Pair>);
    static_assert(std::is_standard_layout_v<bg_forcefield::PairScale>);
    static_assert(sizeof(bg_forcefield::Pair) == sizeof(bg_hip_safe_pair_v1));
    static_assert(alignof(bg_forcefield::Pair) == alignof(bg_hip_safe_pair_v1));
    static_assert(offsetof(bg_forcefield::Pair, atom_i) ==
                  offsetof(bg_hip_safe_pair_v1, atom_i));
    static_assert(offsetof(bg_forcefield::Pair, atom_j) ==
                  offsetof(bg_hip_safe_pair_v1, atom_j));
    static_assert(
        sizeof(bg_forcefield::PairScale) == sizeof(bg_hip_safe_pair_scale_v1));
    static_assert(
        alignof(bg_forcefield::PairScale) == alignof(bg_hip_safe_pair_scale_v1));
    static_assert(offsetof(bg_forcefield::PairScale, pair) == 0);
    static_assert(offsetof(bg_forcefield::PairScale, lennard_jones) ==
                  offsetof(bg_hip_safe_pair_scale_v1, lennard_jones));
    static_assert(offsetof(bg_forcefield::PairScale, coulomb) ==
                  offsetof(bg_hip_safe_pair_scale_v1, coulomb));

    bg_hip_safe_system_v1 provider_system{};
    provider_system.struct_size =
        static_cast<uint32_t>(sizeof(provider_system));
    provider_system.abi_version = BG_HIP_SAFE_PROVIDER_ABI_VERSION;
    ...
    bg_hip_safe_forcefield_v1 provider_forcefield{};
    provider_forcefield.struct_size =
        static_cast<uint32_t>(sizeof(provider_forcefield));
    provider_forcefield.abi_version = BG_HIP_SAFE_PROVIDER_ABI_VERSION;
    ...
    provider_forcefield.exclusions =
        reinterpret_cast<const bg_hip_safe_pair_v1 *>(
    ...
    provider_forcefield.pair_scales =
        reinterpret_cast<const bg_hip_safe_pair_scale_v1 *>(
    ...
    bg_hip_safe_energy_v1 provider_energy{};
    provider_energy.struct_size =
        static_cast<uint32_t>(sizeof(provider_energy));
    provider_energy.abi_version = BG_HIP_SAFE_PROVIDER_ABI_VERSION;
    bg_hip_safe_force_output_v1 provider_forces{};
    provider_forces.struct_size =
        static_cast<uint32_t>(sizeof(provider_forces));
    provider_forces.abi_version = BG_HIP_SAFE_PROVIDER_ABI_VERSION;
    ...
    const int32_t raw_status = bg_hip_safe_evaluate_v1(
        device_ordinal,
        &provider_system,
        &provider_forcefield,
    ...
    if (status != BG_STATUS_OK) {
        provider_error[BG_HIP_SAFE_ERROR_CAPACITY - 1U] = '\0';
        const char *message = provider_error[0] == '\0'
                                  ? "hip_safe provider failed without a diagnostic"
                                  : provider_error;
