...
void reserve_direct_ewald_composite_ewald_parent_force_scratch(
    ...
    std::size_t capacity) {
    assert(simulation != nullptr);
    simulation->ewald_parent_evaluation_scratch.forces.reserve(capacity);
}
...
direct_ewald_composite_force_scratch_snapshot(
    ...
    const bg_simulation::ParticleVectorScratch &scratch =
        simulation->simulation->force_evaluation_scratch;
    DirectEwaldCompositeForceScratchSnapshot snapshot;
    snapshot.addresses = {
        scratch.x.data(),
        scratch.y.data(),
        scratch.z.data(),
...
direct_ewald_composite_ewald_parent_force_scratch_snapshot(
    ...
    const std::vector<std::array<double, 3>> &scratch =
        simulation->ewald_parent_evaluation_scratch.forces;
    DirectEwaldCompositeEwaldParentForceScratchSnapshot snapshot;
