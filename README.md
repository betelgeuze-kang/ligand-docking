...
The current PDE selected all-atom path has no remaining hard block, and the tracked accuracy scorecard has no blocked metric rows. The science accuracy frontier is restricted-ready, but broad wetlab/platform, broad GPCR, and OpenMM/Schrodinger-class parity claims still stay separate until prospective wetlab evidence, formal broad-claim review, scorer/router promotion gates, R9 public benchmark evidence receipts, and broader platform guardrails are available; the GPCR target-held-out/guarded-100k inputs are now tracked as ready but not claim-approved. The R9 public benchmark work-order is seeded from local PDBBind/CASF pose results (`seeded_rows=8`, `prefilled_operator_fields=40`, `pending_operator_fields=32`, `experimental_deltaG_prefilled=8`) with pending fields split as license/DockQ/lDDT-PLI/internal refine free-energy = `8/8/8/8`; experimental ΔG pending is `0`, ligand pose artifacts are present for all `8` rows, and the local CASF tar archives contain `23062` ligand-pose members but `0` receptor-coordinate members, so native receptor/complex coordinate rows, interaction-metric sources, and internal-ΔG sources are still missing `8/8/8`. Candidate expansion, metric diagnosis, atomization, parameterization, and local minimization evidence remain separated so no single strong-looking energy row can be over-promoted.
