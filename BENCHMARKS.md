...
- The next DRD2 work is to rework the v11/v12 feature contract after complete true-base frozen replay. `all_basic` currently creates saturated synthetic-anchor decoy support, so fix anchor portability or add a label-free saturated-anchor penalty before any guarded 100k rerun. Use `gpcr_core_family_anchor_rescore_v2` only as feature donor/baseline and do not generalize `gpcr_basic_amine_proxy` into broad GPCR/basic-amine wording.
