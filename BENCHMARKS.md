...
아래 수치를 일반 상용 플랫폼 전체, 일반 GPCR-family scale-up, transporter, CA2/PXR, broader IDP promotion, unattended decision-making claim으로 확장하면 안 됩니다. GPCR positive freeze, frozen-candidate scoreability, family-held-out scorecard, and leakage audit are green, but the current blocker in the v2 replay lane is CI-low stability, not point PR-AUC. The v3 family-anchor CI-stability packet `runs/gpcr_residual_prototype_spec_family_anchor_ci_stability_v3.json` is diagnostic-only (`prototype_mode=shadow_only`, `scorer_apply_allowed=false`) and exists only to explain why the v2 shadow CI-low remains below `0.45`. If a v4 scorer is still unmerged, treat it only as the next shadow/guarded-comparison direction, not completed evidence. The latest r2 row-level evidence only recovers HTR2A/OPRM1 and leaves DRD2 buried at global rank `18923` / target rank `5315` in `runs/external_validation_2026-05-03_family_balanced_frozen_r2_set1_core_blind_gpcr_core_full_p0_n100000_r1_stage5_ranking_rows.csv`. The guarded r2 base evidence is `PR-AUC=0.5186945103743427`, `ranking_pr_auc_ci_low=0.1485815545422209`, `top20=0.25`; the refreshed `gpcr_core_family_anchor_rescore_v2` shadow replay is recorded in `runs/gpcr_family_anchor_v2_shadow_replay_eval_current.json` and reaches `PR-AUC=0.5767474245351905`, `ranking_pr_auc_ci_low=0.21066694653866244`, `top20=0.25`, DRD2 `global_rank=8562` / `target_rank=2435`. That replay is only the next guarded-comparison design step toward target-internal pairwise diagnostic/guarded validation prep for the hard-decoy lane, not claim evidence. The base pairwise diagnostic slice still has `decoys_above_positive_count=5314`, `pairwise_win_rate=0.46854685468546853`, and `ready_for_guarded_apply=false`; the refreshed shadow projection reduces DRD2 decoys-above-positive to `2434` and raises pairwise win-rate to `0.7565756575657565`, but guarded apply remains blocked because shadow CI-low is still below the `0.45` threshold. `claim_promotion_allowed=false` stays in force, and delivery/router/platform claim promotion remains forbidden.

For the GPCR non-ADRB2 positive freeze workflow, `config/gpcr_non_adrb2_positive_candidates_v1.csv` is the curated candidate input schema. The intended handoff is candidate CSV -> `runs/gpcr_non_adrb2_positive_candidates_leakage_audit_current.json` -> `runs/gpcr_positive_coverage_freeze_packet_current.json` -> `runs/gpcr_frozen_candidate_profile_support_current/summary.json` -> `runs/gpcr_frozen_candidate_scoreability_current.json` -> `runs/gpcr_family_heldout_scorecard_current.json` / `runs/gpcr_family_heldout_scorecard_guardrail_current.json` -> `runs/gpcr_guarded_100k_rerun_readiness_current.json`; the current freeze packet is `frozen=true` with `positive_count=9`, `new_non_adrb2_positive_count=3`, `distinct_positive_gpcr_target_count=4`, and `leakage_audit_pass=true`. Profile support is `profile_ready=true`, scoreability is `pass=true` with `freeze_positive_count=9` and `profile_positive_count=9`, and readiness now has `launch_eligible=true` / `claim_review_eligible=false`. The next claim-locked diagnostic order is DRD2 pose/physics -> conserved anchor proxy -> prior gating -> target-internal pairwise diagnostic/guarded validation prep for the hard-decoy lane -> coverage expansion; use `runs/gpcr_guarded_100k_rank_failure_diagnostics_current.json` / `drd2_pose_physics_diagnostics` as the current proxy hook and keep downstream artifacts compare-only. The helper command is `python3 tools/build_gpcr_frozen_candidate_profile_support.py --native-source-csv config/gpcr_non_adrb2_native_sources_v1.csv`, followed by scoreability and readiness refresh.
...
The latest r2 rank evidence is `runs/external_validation_2026-05-03_family_balanced_frozen_r2_set1_core_blind_gpcr_core_full_p0_n100000_r1_stage5_ranking_summary.md` plus `..._stage5_ranking_rows.csv`. HTR2A and OPRM1 recover to target-rank 1 (`global_rank=2` and `global_rank=6`), while DRD2 remains buried at `global_rank=18923` / `target_rank=5315`. The refreshed diagnostic packet adds DRD2 atom-anchor evidence under `runs/gpcr_guarded_100k_rank_failure_diagnostics_current.json` / `drd2_pose_physics_diagnostics`: the DRD2 positive maintains the native acidic anchor (`Asp114`) with mean distance about `3.25 A`, but the top DRD2 decoy cluster is even closer on average (`2.48 A`). This means the blocker is not simple anchor absence; it is decoy over-anchoring / ligand-physics-prior separation. Keep this as diagnostic-only evidence. The refreshed `gpcr_core_family_anchor_rescore_v2` replay adds target-agnostic `gpcr_basic_amine_proxy`, conserved-anchor/prior gating, and anchor-chemistry mismatch pressure on top of the prior r2 active score; local shadow replay improves the point estimate from `PR-AUC=0.5186945103743427` to `0.5767474245351905`, computes `ranking_pr_auc_ci_low=0.21066694653866244`, keeps top20 at `0.25`, and moves DRD2 from `global_rank=18923` to `8562`. It remains the next guarded-comparison design step toward target-internal pairwise diagnostic/guarded validation prep for the hard-decoy lane, not claim evidence. The base pairwise diagnostic slice still has `decoys_above_positive_count=5314`, `pairwise_win_rate=0.46854685468546853`, and `ready_for_guarded_apply=false`; the refreshed shadow projection improves that DRD2 slice to `decoys_above_positive_count=2434` and `pairwise_win_rate=0.7565756575657565`, but CI-low remains below the `0.45` claim-review threshold. The next implementation remains claim-locked diagnostics/shadow/replay in the order DRD2 pose/physics -> conserved anchor proxy -> prior gating -> target-internal pairwise diagnostic/guarded validation prep for the hard-decoy lane -> coverage expansion; do not rerun the same packet as claim evidence, and do not use full 100k reruns, threshold relaxation, target identity feature, or fake pass to force a pass. Delivery/router/platform claim promotion remains forbidden and `claim_promotion_allowed=false` stays in force.
...
- `gpcr_core_family_anchor_rescore_v2`는 현재 shadow replay evidence입니다. `gpcr_basic_amine_proxy`는 9/9 positives를 잡지만 decoy도 `19959/39991`개가 basic amine을 가져서 단독 claim feature가 아닙니다. Anchor-chemistry mismatch pressure lowers the DRD2 hard-decoy shadow slice from `5314` to `2434` decoys above the positive. The replay signal crosses the PR-AUC point threshold (`0.5767474245351905`) but bootstrap `ranking_pr_auc_ci_low=0.21066694653866244` remains below `0.45`, so it only opens the next guarded-comparison design step and does not allow claim promotion.
