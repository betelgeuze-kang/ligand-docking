...
현재 로컬 납품 claim 범위는 제한된 `kinase`, `gpcr`, `ion_channel`에만 적용합니다. 2026-05-18 산출물 기준 restricted local verdict는 `delivery_ready=true`, `verdict=delivery_ready`, `p0_blocker_count=0`이며 `local_engine_commercialization_queue`와 같은 green 상태입니다. tracked ligand scale-up suite도 `commercialization_ready_suite_count=3/3` / `pending_suite_ids=[]`입니다. 이는 broad platform, transporter, CA2/PXR, broader IDP promotion, unattended decision-making, 또는 GPCR-family/router 일반화 claim이 아닙니다.
...
아래 수치를 일반 상용 플랫폼 전체, 일반 GPCR-family/router 일반화, transporter, CA2/PXR, broader IDP promotion, unattended decision-making claim으로 확장하면 안 됩니다. 2026-05-13의 `/goal` accounting closure는 audit baseline으로 남기지만, 현재 delivery source of truth는 `runs/local_delivery_verdict_gate_current.json`과 `runs/local_engine_commercialization_queue_current.json`입니다. 최신 queue는 `queue_clear=true`, `blocked_count=0`, `top_priority_status=keep_green`이고 verdict도 `delivery_ready`입니다. Transporter negative placeholders는 authoritative apply gate로 accounting closure 처리되었고, AQP1 kcal은 functional IC50-derived surrogate로만 사용합니다. GPCR family/router 실험들은 legacy comparison/reject evidence로 보존하며, 제한된 1M scale-up package와 최신 A1 ranking-parity green 상태를 broad GPCR/router/platform claim으로 승격하지 않습니다.

`/goal` 이후의 active benchmark lane인 상용툴 정확도 parity도 green으로 갱신됐습니다. `runs/accuracy_parity_scorecard_current.md`는 `status=green`이며 row mix는 `5 pass / 0 restricted_pass / 0 blocked / 0 missing`입니다. OpenMM 11-target strict release, structure deterministic true-metric materialization, PDE selected all-atom/atomized local-min evidence, 그리고 GPCR A1 independent repeat가 scorecard에 반영됐습니다. 최신 A1 repeat ranking artifact는 PR-AUC `0.8718530390764964`, PR CI-low `0.7611678630724843`, top20 `1.00`, blockers `[]`입니다. Scorer deployment, router, broad platform wording은 여전히 별도 guardrail입니다.
...
| Verdict gate | Green / delivery-ready for restricted local scope | `delivery_ready=true`, `verdict=delivery_ready`, `p0_blocker_count=0`, `hard_blocker_count=0`, `commercialization_queue_clear=true` | `runs/local_delivery_verdict_gate_current.json` |
...
| Commercialization queue | Clear / keep-green | `queue_clear=true`, `blocked_count=0`, `row_count=5`, `top_priority_id=nightly_reliability`, `top_priority_status=keep_green` | `runs/local_engine_commercialization_queue_current.json` |
...
현재 GPCR triage packet의 legacy 후보들은 comparison-only/reject evidence로 유지합니다. 별도로 최신 A1 independent repeat는 tracked ranking-parity gate를 통과했지만, scorer deployment/router/platform promotion은 별도 guardrail입니다.
Legacy failure analysis는 raw ranking rows를 다시 읽어 `source_rows_available=true`로 계산되며, 기존 score column `7`개 중 어느 것도 broad-family core gate를 회복하지 못했던 기록입니다. 주요 root-cause tag는 `donor_prior_decoy_intrusion`, `weak_contact_prior_mismatch`, `affinity_hint_md_support_mismatch`입니다.
...
| GPCR A1 independent repeat crossfit rank rescue | Pass, tracked ranking parity | `0.8718530390764964` | `0.7611678630724843` | `1.00` | `34` | `binding_score_composite_v7_coverage_v2_crossfit_rank_rescue_shadow` | independent repeat completed with blockers `[]`; ranking-parity claim is ready for the tracked scorecard, while router/platform promotion remains separate |
...
ADRB2 pharmacophore candidate는 target-specific beta-blocker/aryloxypropanolamine SMARTS reward를 사용합니다. 현재 ChEMBL50 ADRB2 후보는 `coverage-only/review-required`입니다. Non-leaky GPCR +3 requirement and family-held-out scorecard are now green (`gpcr_positive_count=9`, `gpcr_distinct_positive_target_count=4`), and the newer A1 independent repeat passes tracked ranking parity, but GPCR-family/router-level promotion is still separate until deployment and broad-family guardrails clear.
...
| --- | --- | --- | --- |
| Scale-up benchmark claim safety | Restricted scope green | `claim_safe=true`, status `claim_safe_size_shift_speed_diagnostic` for 1M; equal-size A/B owns throughput wording | `runs/ligand_scaleup_1m_benchmark_summary_current.json` |
| Guardrails | Quality green, speed diagnostic | no pass-to-fail transitions; worst PR-AUC delta `-0.0727` on `gpcr_chembl50_full` still clears absolute floor with candidate PR-AUC `0.8093`; top20 baseline unavailable and 1M speed retained as diagnostic | `runs/ligand_scaleup_1m_benchmark_summary_current.json` |
...
- GPCR recovery candidates (`gpcr_core_decoy_intrusion_v1`, `gpcr_core_linear_rescore_v1`, `gpcr_core_mismatch_contact_rescore_v1`, and `gpcr_core_structure_support_rescore_v1`) stay comparison-only shadow/guarded-apply candidates; GPCR-family/router claim promotion remains forbidden until the same full 100k gate clears `ranking_pr_auc_ci_low >= 0.45` and `top20_hit_rate >= 0.20`. This is behind the current transporter negative-placeholder expansion blocker. Do not try to bypass this with full 100k reruns, threshold relaxation, target identity feature, or fake pass, and do not generalize the v2 basic-amine feature donor into broad GPCR/basic-amine wording.
