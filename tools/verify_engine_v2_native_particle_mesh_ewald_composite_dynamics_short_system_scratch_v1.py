#!/usr/bin/env python3
"""Verify bounded persistent short-system scratch reuse for particle-mesh Ewald dynamics."""
...
REQUIRED_TRIGGER_PATHS = (
    ...
    "docs/engine_v2_native_particle_mesh_ewald_composite_dynamics_force_scratch_v1.md",
    ...
    "tools/verify_engine_v2_native_particle_mesh_ewald_composite_dynamics_force_scratch_v1.py",
...
EXPECTED_TEST_SOURCE_SHA256 = (
    "6a72a90d09c9c77153a0f22b3d8337c10069a01387bfb120dc69c8c52cf4103a"
)
EXPECTED_TEST_HELPER_SOURCE_SHA256 = (
    "47da93d38e1b423344f4266b12dfbd706b62cf57c77c6c88a85a0fc9c921020f"
)
EXPECTED_TEST_HELPER_HEADER_SHA256 = (
    "71c9f55093beab62f7067df3f7df829daa1d6efda107d36c290f45a6b5a22719"
)
EXPECTED_PRODUCTION_SOURCE_SHA256 = {
    "particle_mesh_ewald_composite.cpp":
        "b32ad43569372b9502862597de96f62152f1d6da93bd4230c27814ead762da60",
    "particle_mesh_ewald_composite_dynamics.cpp":
        "cce79d0eeafb896d646734b1461f490b7839b4397c89d2524493e730183298ed",
    "particle_mesh_ewald_composite_dynamics.hpp":
        "760594d7c9dc9f35462057cb7632ae9c50e139b7d1b5644bb33e3a1843a304f1",
    "particle_mesh_ewald_composite_evaluator.hpp":
        "b3544aeed2460d056dde1a4629f55cba6c184124114b344d543b9e3ff6fd3e18",
}
    ...
    steps:
      - name: Check out exact evidence graph
        ...
        with:
          ...
          clean: true
          fetch-depth: 0
      - name: Materialize exact PR 447 architecture and PR 445 target
        shell: bash
        run: |
          set -euo pipefail
...
EXPECTED_NATIVE_JOB_BODY = """    runs-on: ubuntu-latest
    ...
    steps:
      ...
      - name: Synthetic release and sanitizer regressions
        run: |
          cmake -S . -B build/pme-short-system-scratch-release -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF
          cmake --build build/pme-short-system-scratch-release --target betelgeuze_engine_particle_mesh_ewald_composite_dynamics --parallel 2
          ctest --test-dir build/pme-short-system-scratch-release -R '^betelgeuze_engine_(particle_mesh_ewald_composite_dynamics|export_allowlist)$' --output-on-failure
          cmake -S . -B build/pme-short-system-scratch-sanitize -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF -DCMAKE_C_FLAGS='-fsanitize=address,undefined -fno-omit-frame-pointer' -DCMAKE_CXX_FLAGS='-fsanitize=address,undefined -fno-omit-frame-pointer'
          cmake --build build/pme-short-system-scratch-sanitize --target betelgeuze_engine_particle_mesh_ewald_composite_dynamics --parallel 2
          ASAN_OPTIONS=detect_leaks=1 UBSAN_OPTIONS=halt_on_error=1 ctest --test-dir build/pme-short-system-scratch-sanitize -R '^betelgeuze_engine_particle_mesh_ewald_composite_dynamics$' --output-on-failure
"""

EXPECTED_RUST_JOB_BODY = """    runs-on: ubuntu-latest
    timeout-minutes: 35
    env:
      CARGO_TARGET_DIR: ${{ github.workspace }}/build/pme-short-system-scratch-cargo
    steps:
...
EXPECTED_MACOS_JOB_BODY = """    runs-on: macos-15
    ...
    steps:
      ...
      - name: Exact export regression
        run: |
          cmake -S . -B build/pme-short-system-scratch-macos -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF
          cmake --build build/pme-short-system-scratch-macos --target betelgeuze_engine --parallel 2
          ctest --test-dir build/pme-short-system-scratch-macos -R '^betelgeuze_engine_export_allowlist$' --output-on-failure
"""
...
def expected_workflow_document() -> str:
    ...
    preamble = (
        "name: ci-engine-v2-native-pme-composite-dynamics-"
        ...
        "  contents: read\n\n"
        "concurrency:\n"
        "  group: ci-engine-v2-native-pme-composite-dynamics-"
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            "particle_mesh_ewald_composite_dynamics_owner_persistent_short_system_scratch_"
            "current_sources_tests_evidence_target_and_architecture_predecessors"
        ),
...
def build_profile(manifest_raw: bytes) -> dict:
    ...
    return {
        ...
        "validation": {
            ...
            "workflow_target_and_architecture_heads_explicitly_fetched": True,
        },
...
""",
        """      - name: Materialize exact PR 447 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 3f68361985e57f4c5ee547ba690f4a6859bd8b34^{tree})" = "1fc25fdc53ae1d2303a6fe6abe279e520fc13f12"
          ...
          test "$(git rev-parse FETCH_HEAD^{tree})" = "1fc25fdc53ae1d2303a6fe6abe279e520fc13f12"
""",
        "predecessor workflow reviewed-head materialization",
    )
...
""",
        """      - name: Verify exact frozen PR 447 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=3f68361985e57f4c5ee547ba690f4a6859bd8b34
...
""",
        "predecessor workflow detached verification",
    )
...
PARTICLE_MESH_EWALD_SHORT_SYSTEM_SCRATCH_EVIDENCE_PRESENT = (
    ROOT
    / "config/engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "short_system_scratch_profile_v1.json"
).is_file()
pytestmark = pytest.mark.skipif(
    PARTICLE_MESH_EWALD_SHORT_SYSTEM_SCRATCH_EVIDENCE_PRESENT,
    reason=(
        "direct-Ewald composite dynamics short-system scratch evidence is verified "
        "from its exact frozen object after particle-mesh Ewald short-system scratch "
        "evidence is present"
    ),
)
...
def test_exact_profile_manifest_and_contracts() -> None:
"""
    return replace_exact(frozen, old, new, "predecessor unit skip")
...
def require_short_system_scratch_contract(root: Path = ROOT) -> None:
    ...
    transforms = (
        (
            "particle_mesh_ewald_composite.cpp",
            expected_particle_mesh_ewald_composite_source,
        ),
        (
            "particle_mesh_ewald_composite_dynamics.cpp",
    ...
    stateful_region = composite_source.split(
        "    bg_system *short_system = short_system_scratch;\n", 1
    )[1].split("\n\n    cpu::Evaluation", 1)[0]
    ...
    for axis in ("x", "y", "z"):
        ...
        if composite_source.count(token) != 1:
            fail(f"position-{axis} in-place refresh contract drift")
    if composite_source.count(
        "return !double_bits_equal(charge, 0.0);"
    ) != 1:
    ...
    test_tokens = (
        ...
        "BG_BACKEND_RUST_CPU",
        "new PME short-system scratch layout was invalid",
