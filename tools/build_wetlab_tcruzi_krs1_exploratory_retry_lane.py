...
DEFAULT_OUT_MD = "runs/wetlab_tcruzi_krs1_exploratory_retry_lane_current.md"
...
def _gate_label(threshold: float) -> str:
    return "gate5.1" if abs(threshold - 5.1) < 0.01 else "gate5.5"
...
def build_payload(
    ...
    status = "wetlab_tcruzi_krs1_exploratory_retry_lane_running" if running else "wetlab_tcruzi_krs1_exploratory_retry_lane_ready"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the T. cruzi KRS1 exploratory observed-band retry lane.")
...
def main() -> None:
    ...
    write_artifact(
        args.out_md,
        "Wet-Lab T. cruzi KRS1 Exploratory Retry Lane",
        build_payload(
