...
DEFAULT_GO_NO_GO_JSON = "runs/tcruzi_krs1_go_no_go_card_current.json"
DEFAULT_PROGRESS_JSON = "runs/tcruzi_krs1_live_progress_current.json"
DEFAULT_RESULT_JSON = "runs/tcruzi_krs1_result_summary_current.json"
DEFAULT_OUT_MD = "runs/tcruzi_krs1_run_record_current.md"
...
def _resolve_run_state(requested_state: str, upstream_gate_open: bool) -> str:
    ...
    if not upstream_gate_open and requested_state in {RUN_STATE_READY, RUN_STATE_RUNNING, RUN_STATE_RESULT_READY, RUN_STATE_EXPLICIT_HOLD}:
        raise ValueError("T. cruzi KRS1 run state cannot advance past blocked_on_previous_review while the upstream DprE1 gate is still closed.")
...
def _queue_status_for_run_state(run_state: str) -> str:
    ...
    raise ValueError(f"Unsupported T. cruzi KRS1 run state: {run_state}")


def build_payload(launch_payload: dict[str, Any], result_review_payload: dict[str, Any], successor_launch_payload: dict[str, Any] | None, go_no_go_payload: dict[str, Any], live_progress: dict[str, Any] | None = None, result_summary: dict[str, Any] | None = None, run_state: str = RUN_STATE_AUTO) -> dict:
    launch_s = _summary(launch_payload)
    review_s = _summary(result_review_payload)
    successor_s = _summary(successor_launch_payload)
    go_s = _summary(go_no_go_payload)
    progress_s = _summary(live_progress)
    result_s = _summary(result_summary)

    upstream_gate_open = bool(review_s.get("tcruzi_krs1_gate_open", review_s.get("execution_gate_open", False)))
    upstream_gate_state = str(review_s.get("tcruzi_krs1_review_state", review_s.get("queue_status_now", ""))).strip() or RUN_STATE_BLOCKED
    ...
    successor_gate_state = "open_for_lrrk2_execution" if successor_gate_open else "blocked_until_tcruzi_krs1_result_ready_or_explicit_hold"
    ...
    if successor_gate_open:
        next_required_step = "T. cruzi KRS1 now satisfies the successor-open rule. LRRK2 may move into the ready_after_previous_review slot on the next wave2 refresh."
    elif execution_state == RUN_STATE_RUNNING:
        next_required_step = "Keep T. cruzi KRS1 as the active Wave 2 slot and leave LRRK2 blocked until this run reaches result_ready or explicit_hold."
    elif execution_state == RUN_STATE_READY:
        next_required_step = "Launch T. cruzi KRS1 from the fourth Wave 2 slot now, then use the resulting outcome to open the LRRK2 successor gate."
    else:
        next_required_step = "Keep T. cruzi KRS1 blocked until the DprE1 release gate opens, and keep LRRK2 prep-only until T. cruzi KRS1 later reaches result_ready or explicit_hold."

    rows = [
        {"checkpoint_kind": "launch_packet", "artifact_path": "runs/tcruzi_krs1_launch_packet_current.md", "checkpoint_state": str(launch_s.get("status", "")).strip(), "queue_effect": "fix_fourth_wave2_serialized_slot"},
        {"checkpoint_kind": "upstream_dpre1_review", "artifact_path": "runs/dpre1_result_review_current.md", "checkpoint_state": upstream_gate_state, "queue_effect": "open_only_after_dpre1_result_ready_or_explicit_hold"},
        {"checkpoint_kind": "live_progress", "artifact_path": "runs/tcruzi_krs1_live_progress_current.md", "checkpoint_state": progress_status or "not_detected", "queue_effect": active_stage_label or "no_live_progress_artifact"},
        {"checkpoint_kind": "result_summary", "artifact_path": "runs/tcruzi_krs1_result_summary_current.md", "checkpoint_state": result_status or "not_detected", "queue_effect": "capture_result_ready_or_explicit_hold"},
        {"checkpoint_kind": "go_no_go_card", "artifact_path": "runs/tcruzi_krs1_go_no_go_card_current.md", "checkpoint_state": str(go_s.get("status", "")).strip(), "queue_effect": "successor_release_rule_frozen"},
        {"checkpoint_kind": "run_execution", "artifact_path": "runs/tcruzi_krs1_run_record_current.md", "checkpoint_state": execution_state, "queue_effect": queue_status_now},
        {"checkpoint_kind": "successor_gate", "artifact_path": "runs/lrrk2_launch_packet_current.md", "checkpoint_state": successor_gate_state, "queue_effect": "open_only_after_tcruzi_krs1_result_ready_or_explicit_hold"},
    ]

    return {"summary": {
        "status": "tcruzi_krs1_run_record_ready",
        ...
        "tcruzi_krs1_review_status": str(review_s.get("status", "")).strip(),
        ...
        "successor_target": str(successor_s.get("target_id", "LRRK2")).strip() or "LRRK2",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the T. cruzi KRS1 run-record surface.")
...
def main() -> None:
    args = parse_args()
    payload = build_payload(load_json(args.launch_json), load_json(args.result_review_json), maybe_load_json(args.successor_launch_json), load_json(args.go_no_go_json), maybe_load_json(args.progress_json), maybe_load_json(args.result_json), args.run_state)
    write_artifact(DEFAULT_OUT_MD, "T. cruzi KRS1 Run Record", payload)
