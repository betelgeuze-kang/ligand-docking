#!/usr/bin/env python3
"""Verify the merge-anchored archive for native fixed64 CPU profile v4."""
...
ARCHIVE_RELATIVE_PATH = Path(
    "config/engine_v2_native_fixed64_cpu_profile_v4_archive.json"
)
PROFILE_ID = "engine_v2_native_fixed64_cpu_synthetic_v4"
PROFILE_SHA256 = "09a7d27446c7b6283798c6f1c6ba6a3c36ac63582c65b4897657e409ed4536c8"
ARCHIVE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_fixed64_cpu_profile_archive/1.0.0"
)
MERGE_COMMIT_OID = "5b6e007466542f616348fa83dc57deaac3650df9"
REVIEWED_HEAD_OID = "8859c3b6e138cd4a969209b749db16d8980318c1"
TRANSITIVE_SOURCE_MANIFEST_SHA256 = (
    "757dbe9270e2494e3eb420e14d00788da38dba7d1dafdcfbf9d5f64ce2a9a729"
)
...
class NativeFixed64CPUProfileV4ArchiveError(ValueError):
    """The immutable v4 archive failed closed."""


def _fail(message: str) -> NoReturn:
    raise NativeFixed64CPUProfileV4ArchiveError(message)
...
def _load_canonical_object(raw: bytes, *, label: str) -> dict[str, object]:
    try:
        ...
        raise NativeFixed64CPUProfileV4ArchiveError(
...
def require_archived_profile_v4(
    profile_raw: bytes, archive_raw: bytes
) -> dict[str, object]:
    profile = _load_canonical_object(profile_raw, label="v4 profile")
    archive = _load_canonical_object(archive_raw, label="v4 archive")
    profile_sha256 = hashlib.sha256(profile_raw).hexdigest()
    if profile_sha256 != PROFILE_SHA256:
        _fail("v4 profile bytes changed from the merge-anchored identity")
    if (
        ...
        _fail("v4 profile identity or terminal state changed")
    ...
    if (
        ...
        _fail("v4 profile authority is not entirely false")
    ...
    if set(archive) != {
        ...
        _fail("v4 archive field set changed")
    ...
    if (
        ...
        or archive["status"] != "archived_frozen_superseded_by_v5"
        ...
        _fail("v4 archive evidence changed")
    return archive
...
def main() -> int:
    ...
    archive = require_archived_profile_v4(profile_raw, archive_raw)
    print(
        json.dumps(
            {
