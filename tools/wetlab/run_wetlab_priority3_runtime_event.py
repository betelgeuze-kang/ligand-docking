...
DEFAULT_LOG_PATH = ROOT / "runs/wetlab_priority3_runtime_event_log.jsonl"
...
def _rebuild_support_artifacts(python_bin: str) -> None:
    subprocess.run([python_bin, "tools/build_wetlab_priority3_protein_run_queue.py"], cwd=ROOT, check=True)
    subprocess.run([python_bin, "tools/build_wetlab_priority3_runtime_runbook.py"], cwd=ROOT, check=True)
    subprocess.run([python_bin, "tools/build_wetlab_priority3_execution_console.py"], cwd=ROOT, check=True)
...
def apply_and_log_event(
    *,
    ...
    write_artifact(runtime_event_mod.DEFAULT_OUT_MD, "Wet-Lab Priority3 Runtime Event", payload)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apply and log a priority3 runtime event, then rebuild the execution console.")
