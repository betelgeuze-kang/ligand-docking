...
DEFAULT_OUT_MD = "runs/wetlab_wave1_tail_runtime_event_current.md"
...
def apply_runtime_event(
    *,
    ...
    _run(["tools/wetlab/build_wetlab_wave1_tail_gate_refresh.py"], python_bin)
...
def build_payload(event_result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "summary": {
            "status": "wetlab_wave1_tail_runtime_event_applied",
            ...
            "next_required_step": "If this target is still running, keep updating it with start/heartbeat events. If it completed or hit an explicit hold, read the downstream gate artifact to confirm the next serialized target opened as expected.",
        },
        "structured": {
            "progress_command": event_result["progress_command"],
            "result_command": event_result["result_command"],
            "refresh_command": "tools/wetlab/build_wetlab_wave1_tail_gate_refresh.py",
        },
        "rows": [
            {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apply a runtime event to a Wave 1 tail serialized wet-lab target and refresh downstream gates.")
...
def main() -> None:
    ...
    write_artifact(DEFAULT_OUT_MD, "Wet-Lab Wave1 Tail Runtime Event", payload)
