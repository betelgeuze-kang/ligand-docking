...
DEFAULT_THROUGHPUT_BRIDGE_JSON = "runs/wetlab_tcruzi_pde_stage6_tuning_bridge_current.json"
DEFAULT_STAGE6_TUNING_SURFACE_JSON = "runs/wetlab_tcruzi_pde_stage6_tuning_surface_current.json"
DEFAULT_LEGACY_MAPPING_FIX_LANE_JSON = "runs/tcruzi_pde_mapping_fix_retry_lane_current.json"
DEFAULT_OUT_MD = "runs/wetlab_tcruzi_pde_exploratory_retry_lane_current.md"
...
def build_payload(
    ...
    selected_row = _find_bridge_row(throughput_bridge_payload, "throughput_preflight_tuned_gate51")
    selected_execute_row = _find_bridge_row(throughput_bridge_payload, "throughput_execute_tuned_gate51")
    ...
    status = "wetlab_tcruzi_pde_exploratory_retry_lane_running" if running else "wetlab_tcruzi_pde_exploratory_retry_lane_ready"
    next_step = (
        f"Watch {TARGET_ID} {shard_id} through the primary watcher; keep the default lane closed until this gate5.1 candidate retry lands clean or is held again."
        if running and shard_id
        else f"Run the {TARGET_ID} exploratory gate5.1 retry for {shard_id}; use gate5.1 as the immediately runnable family for the observed {float(tuning_summary.get('recommended_observed_threshold_A', 0.0) or 0.0):.1f}A band and keep the default lane closed until the result is reviewed."
    ...
    return {
        ...
        "structured": {
            ...
            "throughput_bridge_artifact": "runs/wetlab_tcruzi_pde_stage6_tuning_bridge_current.md",
            "stage6_tuning_surface_artifact": "runs/wetlab_tcruzi_pde_stage6_tuning_surface_current.md",
            "legacy_mapping_fix_lane_artifact": "runs/tcruzi_pde_mapping_fix_retry_lane_current.md",
        },
        "rows": [
            {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the T. cruzi PDE exploratory retry lane.")
...
def main() -> None:
    ...
    write_artifact(
        args.out_md,
        "Wet-Lab T. cruzi PDE Exploratory Retry Lane",
        build_payload(
