...
DEFAULT_LANE_JSON = "runs/wetlab_dengue_ns2b_ns3_protease_exploratory_retry_lane_current.json"
...
DEFAULT_OUT_MD = "runs/wetlab_dengue_ns2b_ns3_protease_exploratory_retry_runner_current.md"
...
def run(
    *,
    ...
    if refresh_lane:
        ...
        write_artifact(lane_json.replace(".json", ".md"), "Wet-Lab Dengue NS2B-NS3 Exploratory Retry Lane", lane_payload)
    else:
    ...
    if not bool(summary.get("ready_for_manual_retry", False)):
        raise SystemExit("Dengue NS2B-NS3 exploratory retry lane is not ready_for_manual_retry")
    if not selected_shard or not selected_kind:
        raise SystemExit("Dengue NS2B-NS3 exploratory retry lane is missing shard_id or selected_command_kind")
    ...
    payload = {
        "summary": {
            "status": "wetlab_dengue_ns2b_ns3_protease_exploratory_retry_runner_ready",
            "target_id": target_id,
            "shard_id": selected_shard,
            "selected_command_kind": selected_kind,
            "lane_label": str(summary.get("lane_label", "")).strip(),
            "lane_phase": str(summary.get("lane_phase", "")).strip(),
            "guarded_launch_completed": True,
            "next_required_step": f"Watch {target_id} {selected_shard} through the primary watcher; keep the default lane closed until this gate4.5 {str(summary.get('lane_phase', '')).strip() or 'candidate'} retry lands clean or is held again.",
        },
        "structured": {
            "exploratory_retry_lane_artifact": "runs/wetlab_dengue_ns2b_ns3_protease_exploratory_retry_lane_current.md",
    ...
    write_artifact(DEFAULT_OUT_MD, "Wet-Lab Dengue NS2B-NS3 Exploratory Retry Runner", payload)
    return payload


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch the Dengue NS2B-NS3 exploratory retry lane.")
