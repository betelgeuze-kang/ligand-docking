...
DEFAULT_OUT_MD = "runs/wetlab_dpre1_guarded_operator_packet_current.md"
...
def _decision_case_for_command(command_kind: str) -> str:
    if "gate51" in _text(command_kind):
        return "dpre1_guarded_gate51_review_candidate"
    if _is_tuned_command(command_kind):
        return "dpre1_guarded_tuned_review_candidate"
    return "dpre1_guarded_review_required"
...
def _next_required_step(command_kind: str, shard_id: str) -> str:
    if "gate51" in _text(command_kind):
        return (
            f"Use the DprE1 guarded gate5.1 operator packet as the review unit for {shard_id}, keep the default lane closed, and review the gate5.1 exploratory retry before any reopen decision."
            if shard_id
            else "Use the DprE1 guarded gate5.1 operator packet, keep the default lane closed, and review the gate5.1 exploratory retry before any reopen decision."
        )
    if _is_tuned_command(command_kind):
        return (
            f"Use the DprE1 guarded tuned operator packet as the review unit for {shard_id}, keep the default lane closed, and review the tuned branch before any reopen decision."
            if shard_id
            else "Use the DprE1 guarded tuned operator packet, keep the default lane closed, and review the tuned branch before any reopen decision."
        )
    return (
        f"Use the DprE1 guarded operator packet as the stage6 review unit for {shard_id}, keep the default lane closed, and select a tuned retry preset before any reopen decision."
        if shard_id
        else "Use the DprE1 guarded operator packet as the stage6 review unit, keep the default lane closed, and select a tuned retry preset before any reopen decision."
    )
...
def build_payload(
    ...
    return {
        "summary": {
            "status": "wetlab_dpre1_guarded_operator_packet_ready" if packet_ready else "wetlab_dpre1_guarded_operator_packet_pending",
            "target_id": TARGET_ID,
            "shard_id": shard_id,
            "surface_label": "dpre1_guarded_operator_packet",
            ...
            "review_unit_label": "guarded gate5.1 operator packet" if _is_tuned_command(command_kind) else "guarded stage6 operator packet",
        ...
        "rows": [
            {
                "row_kind": "dpre1_guarded_operator_packet_row",
                "target_id": TARGET_ID,
                "shard_id": shard_id,
                "packet_scope": packet_scope,
                "selected_command_kind": command_kind,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the DprE1 guarded operator packet.")
...
def main() -> None:
    ...
    write_artifact(args.out_md, "Wet-Lab DprE1 Guarded Operator Packet", payload)
