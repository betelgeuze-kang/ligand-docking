...
DEFAULT_LANE_JSON = "runs/wetlab_tcruzi_pde_exploratory_retry_lane_current.json"
...
DEFAULT_OUT_MD = "runs/wetlab_tcruzi_pde_exploratory_retry_runner_current.md"
...
def run(
    *,
    ...
    if not bool(summary.get("ready_for_manual_retry", False)):
        raise SystemExit("T. cruzi PDE exploratory retry lane is not ready_for_manual_retry")
    if not selected_shard:
        raise SystemExit("no shard_id available for T. cruzi PDE exploratory retry")
    if not selected_kind:
        raise SystemExit("no selected command kind available for T. cruzi PDE exploratory retry")
    ...
    payload = {
        "summary": {
            "status": "wetlab_tcruzi_pde_exploratory_retry_runner_ready",
            "target_id": target_id,
            "shard_id": selected_shard,
            "selected_command_kind": selected_kind,
            "guarded_launch_completed": True,
            "next_required_step": f"Watch {target_id} {selected_shard} through the primary watcher; keep the default lane closed until this gate5.1 candidate retry lands clean or is held again.",
        },
        "structured": {
            "exploratory_retry_lane_artifact": "runs/wetlab_tcruzi_pde_exploratory_retry_lane_current.md",
    ...
    write_artifact(DEFAULT_OUT_MD, "Wet-Lab T. cruzi PDE Exploratory Retry Runner", payload)
    return payload


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch the T. cruzi PDE exploratory retry lane.")
