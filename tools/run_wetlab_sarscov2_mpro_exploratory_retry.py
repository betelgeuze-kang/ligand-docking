...
DEFAULT_LANE_JSON = "runs/wetlab_sarscov2_mpro_exploratory_retry_lane_current.json"
...
DEFAULT_OUT_MD = "runs/wetlab_sarscov2_mpro_exploratory_retry_runner_current.md"
...
def run(
    *,
    ...
    if not bool(summary.get("ready_for_manual_retry", False)):
        raise SystemExit("SARS-CoV-2 Mpro exploratory retry lane is not ready_for_manual_retry")
    if not selected_shard:
        raise SystemExit("no shard_id available for SARS-CoV-2 Mpro exploratory retry")
    if not selected_kind:
        raise SystemExit("no selected command kind available for SARS-CoV-2 Mpro exploratory retry")
    ...
    payload = {
        "summary": {
            "status": "wetlab_sarscov2_mpro_exploratory_retry_runner_ready",
            "target_id": target_id,
            "shard_id": selected_shard,
            "selected_command_kind": selected_kind,
            "guarded_launch_completed": True,
            "next_required_step": f"Watch {target_id} {selected_shard} through the primary watcher; keep the default lane closed until this gate4.5 candidate retry lands clean or is held again.",
        },
        "structured": {
            "exploratory_retry_lane_artifact": "runs/wetlab_sarscov2_mpro_exploratory_retry_lane_current.md",
    ...
    write_artifact(DEFAULT_OUT_MD, "Wet-Lab SARS-CoV-2 Mpro Exploratory Retry Runner", payload)
    return payload


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch the SARS-CoV-2 Mpro exploratory retry lane.")
