...
DEFAULT_SLICE_SCORES_CSV = "runs/gpcr_drd2_valid_anchor_discriminator_slice_input_scores_current.csv"
DEFAULT_SPEC_JSON = "runs/gpcr_residual_prototype_spec_cationic_weakbase_rescue_shadow_v11_current.json"
DEFAULT_REPLAY_SCORES_CSV = "runs/gpcr_cationic_weakbase_rescue_v11_shadow_replay_scores_current.csv"
DEFAULT_REPLAY_SUMMARY_JSON = "runs/gpcr_cationic_weakbase_rescue_v11_shadow_replay_summary_current.json"
DEFAULT_REPLAY_SUMMARY_MD = "runs/gpcr_cationic_weakbase_rescue_v11_shadow_replay_summary_current.md"
DEFAULT_REVIEW_JSON = "runs/gpcr_cationic_weakbase_rescue_v11_shadow_replay_review_current.json"
DEFAULT_REVIEW_MD = "runs/gpcr_cationic_weakbase_rescue_v11_shadow_replay_review_current.md"
DEFAULT_OUT_JSON = "runs/gpcr_drd2_valid_anchor_discriminator_slice_replay_packet_current.json"
DEFAULT_OUT_MD = "runs/gpcr_drd2_valid_anchor_discriminator_slice_replay_packet_current.md"
SPEC_VARIANT = "gpcr_core_cationic_weakbase_rescue_shadow_v11"
        ...
        out.append(
            {
                ...
                "false_valid_anchor_discriminator_pressure": _float(row.get("false_valid_anchor_discriminator_pressure")),
                "window_like_nonbasic_pressure": _float(row.get("window_like_nonbasic_pressure")),
                "weak_base_rescue_support_pressure": _float(row.get("weak_base_rescue_support_pressure")),
                "weak_base_rescue_gate": _float(row.get("weak_base_rescue_gate")),
                ...
                "cationic_center_basic_atom_count": _float(row.get("cationic_center_basic_atom_count")),
...
def build_packet(
    *,
    ...
    rebuild_slice: bool = True,
    generated_at_local: str | None = None,
) -> dict[str, Any]:
    if rebuild_slice:
        _run([sys.executable, "tools/gpcr_replay/build_gpcr_drd2_hard_decoy_slice_packet.py"])
    ...
    spec_payload = build_spec_payload(variant=SPEC_VARIANT)
    ...
    summary = {
        "packet_type": "gpcr_drd2_valid_anchor_discriminator_slice_replay_packet",
        ...
        "false_valid_anchor_discriminator_mean_pressure": (
            sum(_float(row.get("false_valid_anchor_discriminator_pressure")) for row in slice_rows) / len(slice_rows)
        ),
        ...
        "next_required_step": (
            "Valid-anchor discriminator v11 slice replay is green under claim lock. Resume mount stage2 "
            "regeneration, build frozen-row feature caches, then rerun guarded full-100k claim review."
            if status == "selected_slice_shadow_green_claim_locked"
            else "Refine false-valid-anchor pressure or weak-base rescue gating on the repaired slice before "
            "attempting frozen-row feature cache build or full 100k claim review."
        ),
    }
    return {
        "summary": summary,
...
def _write_markdown(path_like: str | Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# GPCR DRD2 Valid-Anchor Discriminator Slice Replay Packet",
        "",
        f"- status: `{summary['status']}`",
        f"- spec_variant: `{summary['spec_variant']}`",
        f"- slice_row_count: `{summary['slice_row_count']}`",
        f"- false_valid_anchor_discriminator_mean_pressure: `{summary['false_valid_anchor_discriminator_mean_pressure']}`",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run valid-anchor discriminator v11 weak-base rescue replay on repaired DRD2 slice."
    )
    ...
    parser.add_argument("--no-rebuild-slice", action="store_true")
...
def main() -> None:
    ...
    payload = build_packet(
        ...
        rebuild_slice=not args.no_rebuild_slice,
    )
