...
DEFAULT_LANE_JSON = "runs/wetlab_lbdhodh_exploratory_retry_lane_current.json"
...
DEFAULT_OUT_MD = "runs/wetlab_lbdhodh_exploratory_retry_runner_current.md"
...
def run(
    *,
    ...
    if not bool(summary.get("ready_for_manual_retry", False)):
        raise SystemExit("DHODH exploratory retry lane is not ready_for_manual_retry")
    if not selected_shard:
        raise SystemExit("no shard_id available for DHODH exploratory retry")
    if not selected_kind:
        raise SystemExit("no selected command kind available for DHODH exploratory retry")
    ...
    payload = {
        "summary": {
            "status": "wetlab_lbdhodh_exploratory_retry_runner_ready",
            "target_id": target_id,
            "shard_id": selected_shard,
            "selected_command_kind": selected_kind,
            "guarded_launch_completed": True,
            "next_required_step": f"Watch {target_id} {selected_shard} through the primary watcher; keep the default lane closed until this {_gate_label(selected_kind)} candidate retry lands clean or is held again.",
        },
        "structured": {
    ...
    write_artifact(DEFAULT_OUT_MD, "Wet-Lab DHODH Exploratory Retry Runner", payload)
    return payload


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch the DHODH exploratory observed-band retry lane.")
