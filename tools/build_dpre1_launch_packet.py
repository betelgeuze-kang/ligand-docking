...
DEFAULT_RENDER_SUITE_JSON = "runs/dpre1_render_suite_current.json"
DEFAULT_CONDITION_CARD_JSON = "runs/dpre1_condition_card_current.json"
...
DEFAULT_OUT_MD = "runs/dpre1_launch_packet_current.md"
...
def build_payload(render_suite: dict, condition_card: dict, repurposing_fill_map: dict | None = None, novelty_fill_map: dict | None = None) -> dict:
    ...
    rep_rows = [dict(row) for row in ((repurposing_fill_map or {}).get("rows", []) or []) if str(row.get("target_id", "")).strip() == "DprE1"]
    nov_rows = [dict(row) for row in ((novelty_fill_map or {}).get("rows", []) or []) if str(row.get("target_id", "")).strip() == "DprE1"]
    ...
    return {"summary": {
        ...
        "partner_track_id": str(suite_s.get("partner_track_id", "TB_Alliance")).strip() or "TB_Alliance",
        "render_suite_status": str(suite_s.get("status", "")).strip(),
        "export_status": "dpre1_tb_alliance_export_ready",
        "required_artifact_count": len(rows),
        "primary_biochemical_arm": str(condition_s.get("primary_biochemical_arm", "")).strip(),
        "whole_cell_arm": str(condition_s.get("whole_cell_arm", "")).strip(),
        ...
        "execution_goal": "Open DprE1 only after Dengue resolves and the biochemical-plus-whole-cell packet is fully filled.",
        "blocking_rule": "Do not start DprE1 live execution until Dengue NS2B-NS3 protease resolves and both repurposing and novelty lanes are filled.",
        "next_target_on_success": "T. cruzi KRS1",
        "next_target_on_hold": "T. cruzi KRS1",
        "headline": "DprE1 packet designed to keep direct biochemical evidence ahead of broader TB whole-cell interpretation.",
        "next_required_step": "Keep DprE1 as the third Wave 2 slot and wait for Dengue resolution before launch." if launch_readiness == "ready_for_serialized_execution" else "Keep DprE1 queued behind Dengue, but do not launch until the compound lanes are fully filled.",
    }, "rows": rows}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the DprE1 serialized launch packet for Wave 2.")
...
def main() -> None:
    ...
    write_artifact(DEFAULT_OUT_MD, "DprE1 Launch Packet", payload)
