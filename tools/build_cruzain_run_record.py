...
DEFAULT_ASSAY_JSON = "runs/cruzain_assay_packet_current.json"
DEFAULT_GO_NO_GO_JSON = "runs/cruzain_go_no_go_card_current.json"
DEFAULT_PROGRESS_JSON = "runs/cruzain_live_progress_current.json"
DEFAULT_RESULT_JSON = "runs/cruzain_result_summary_current.json"
DEFAULT_OUT_MD = "runs/cruzain_run_record_current.md"
...
def _result_class(result_summary: dict[str, Any], result_review_ready: bool, explicit_hold: bool) -> str:
    ...
    if "hold_partial_cleanliness" in details or "hold" in details:
        return "hold_partial_cleanliness"
    if "promote_clean_cruzain_favored" in details or "promote" in details:
        return "promote_clean_cruzain_favored"
    if decision_case:
...
def build_payload(
    ...
    rows = [
        {"record_item": "launch_packet", "artifact_path": "runs/cruzain_launch_packet_current.md", "current_signal": launch_status or "missing_launch_packet", "detail": "serialized_first_slot_in_next3", "record_role": "freeze_first_next3_execution_contract"},
        {"record_item": "assay_packet", "artifact_path": "runs/cruzain_assay_packet_current.md", "current_signal": assay_status or "missing_assay_packet", "detail": "first_pass_stack_ready", "record_role": "define_bounded_primary_assay_stack"},
        {"record_item": "live_progress", "artifact_path": "runs/cruzain_live_progress_current.md", "current_signal": progress_status or "not_detected", "detail": active_stage_label or "no_live_progress_artifact", "record_role": "track_active_execution_until_result_summary_lands"},
        {"record_item": "result_summary", "artifact_path": "runs/cruzain_result_summary_current.md", "current_signal": result_status or "not_detected", "detail": result_class, "record_role": "capture_result_ready_or_explicit_hold"},
        {"record_item": "go_no_go_card", "artifact_path": "runs/cruzain_go_no_go_card_current.md", "current_signal": go_status or "missing_go_no_go_card", "detail": result_class if result_review_ready or explicit_hold else "classification_rules_ready", "record_role": "classify_clean_vs_host_like_vs_reactive_outcomes"},
    ]

    if explicit_hold:
        next_required_step = "Review the explicit Cruzain hold against the go/no-go card, record the blocking reason in the result summary, and keep PLpro blocked until the hold resolves."
    elif result_review_ready:
        next_required_step = "Use the go/no-go card to classify the completed Cruzain outcome, freeze the result summary, and then refresh the PLpro gate review."
    elif run_started:
        next_required_step = "Keep the live progress artifact current until the Cruzain result summary lands; PLpro stays blocked while execution is active."
    else:
        next_required_step = "Launch Cruzain from the first next3 slot once priority3 is resolved, then start recording live progress once the protease stack is running."
    ...
    return {
        "summary": {
            ...
            "go_no_go_card_status": go_status,
            "live_progress_detected": bool(progress_s),
            "result_summary_detected": bool(result_s),
        ...
        "structured": {
            ...
            "progress_artifact_optional": "runs/cruzain_live_progress_current.md",
            "result_artifact_optional": "runs/cruzain_result_summary_current.md",
            "classification_policy": "interpret_completed_results_with_the_go_no_go_card_before_opening_plpro",
        },
        "rows": rows,
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Cruzain run-record slice from launch, progress, and result-facing artifacts.")
...
def main() -> None:
    args = parse_args()
    payload = build_payload(load_json(args.launch_json), load_json(args.assay_json), load_json(args.go_no_go_json), _load_json_if_exists(args.progress_json), _load_json_if_exists(args.result_json))
    write_artifact(DEFAULT_OUT_MD, "Cruzain Run Record", payload)
