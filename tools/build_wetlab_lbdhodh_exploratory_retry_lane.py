...
DEFAULT_OUT_MD = "runs/wetlab_lbdhodh_exploratory_retry_lane_current.md"
...
def build_payload(
    ...
    status = (
        "wetlab_lbdhodh_exploratory_retry_lane_running"
        if running
        else "wetlab_lbdhodh_exploratory_retry_lane_ready"
    )
    ...
    next_step = (
        ...
        f"Run the {TARGET_ID} exploratory {_gate_label(selected_threshold)} candidate retry for {shard_id}; this is the immediately runnable family for the observed {float(tuning_summary.get('recommended_observed_threshold_A', 0.0) or 0.0):.2f}A DHODH band, so keep the default auto lane closed until the result is reviewed."
    ...
    return {
        "summary": {
            ...
            "recommended_retry_mode": "exploratory_gate51_candidate" if abs(selected_threshold - 5.1) < 0.01 else "exploratory_gate55_candidate",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the DHODH exploratory observed-band retry lane.")
...
def main() -> None:
    ...
    write_artifact(
        args.out_md,
        "Wet-Lab DHODH Exploratory Retry Lane",
        build_payload(
