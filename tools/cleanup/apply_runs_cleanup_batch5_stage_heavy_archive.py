...
from tools.build_runs_cleanup_batch5_stage_heavy_review_manifest import _matches
from tools.builder_table_utils import write_csv_rows

ROOT = Path(__file__).resolve().parents[2]
DEFAULT_MANIFEST_JSON = "runs/runs_cleanup_batch5_stage_heavy_review_manifest_current.json"
DEFAULT_SIGNOFF_JSON = "runs/runs_cleanup_batch5_family_signoff_note_current.json"
DEFAULT_ARCHIVE_ROOT = "runs/archive/runs_cleanup_batch5_stage_heavy_current"
DEFAULT_OUT_JSON = "runs/runs_cleanup_batch5_stage_heavy_apply_report_current.json"
DEFAULT_OUT_CSV = "runs/runs_cleanup_batch5_stage_heavy_apply_report_current.csv"
DEFAULT_OUT_MD = "runs/runs_cleanup_batch5_stage_heavy_apply_report_current.md"
VALID_GROUP_IDS = {"stage2_traj_manifest_bundle", "stage3_scores_bundle"}
APPROVE_SIGNOFF = "approve_archive_after_sampling"
...
def apply_manifest(
    ...
    *,
    ...
    family_signoff = {
        str(row.get("family_id", "")).strip(): str(row.get("signoff_recommendation", "")).strip()
        for row in signoff_note.get("rows", []) or []
    }
    ...
    for manifest_row in manifest_rows:
        ...
        out_row = {
            ...
            "signoff_recommendation": family_signoff.get(family_id, ""),
            "status": "",
        }

        if recommended_disposition != "review_for_archive_after_sampling":
            out_row["archive_subdir"] = ""
            out_row["status"] = "skipped_non_batch5_review_row"
            rows.append(out_row)
            skipped_row_count += 1
            continue

        eligible_review_row_count += 1
        ...
        if family_signoff.get(family_id) != APPROVE_SIGNOFF:
            out_row["status"] = "family_signoff_missing_or_not_approved"
            rows.append(out_row)
            skipped_row_count += 1
            continue
    ...
    out_summary = {
        "status": "runs_cleanup_batch5_stage_heavy_apply_report_ready",
        "runs_dir": str(root),
        "source_manifest_status": str(summary.get("status", "unknown")),
        "source_signoff_status": str(signoff_note.get("summary", {}).get("status", "unknown")),
        "archive_root": str(archive_root_path),
        "manifest_row_count": len(manifest_rows),
        "eligible_review_row_count": eligible_review_row_count,
        ...
        "next_required_step": "Rebuild the runs cleanup audit, then freeze a review-only manifest for idp_3bead_holdout stale prefixes before touching current accepted IDP lanes.",
    }
...
def _write_markdown(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        "| family_id | group_id | stage_id | signoff_recommendation | expected_match_count | matched_file_count | moved_file_count | moved_size_mb | archive_subdir | status |",
        "| --- | --- | --- | --- | ---: | ---: | ---: | ---: | --- | --- |",
    ]
    for row in payload["rows"]:
        lines.append(
            f"| `{row['family_id']}` | `{row['group_id']}` | `{row['stage_id']}` | `{row['signoff_recommendation']}` | `{row['expected_match_count']}` | `{row['matched_file_count']}` | `{row['moved_file_count']}` | `{row['moved_size_mb']}` | `{row['archive_subdir']}` | `{row['status']}` |"
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apply batch5 heavy-bundle cleanup after family signoff.")
    parser.add_argument("--manifest-json", default=DEFAULT_MANIFEST_JSON)
    parser.add_argument("--signoff-json", default=DEFAULT_SIGNOFF_JSON)
