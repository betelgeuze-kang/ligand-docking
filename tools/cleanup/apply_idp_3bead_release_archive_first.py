...
DEFAULT_MANIFEST_JSON = "runs/idp_3bead_release_archive_first_manifest_current.json"
DEFAULT_ARCHIVE_ROOT = "runs/archive/idp_3bead_release_archive_first_current"
DEFAULT_OUT_JSON = "runs/idp_3bead_release_archive_first_apply_report_current.json"
DEFAULT_OUT_CSV = "runs/idp_3bead_release_archive_first_apply_report_current.csv"
DEFAULT_OUT_MD = "runs/idp_3bead_release_archive_first_apply_report_current.md"
...
def apply_manifest(manifest: dict[str, Any], *, archive_root: str = DEFAULT_ARCHIVE_ROOT) -> dict[str, Any]:
    ...
    summary = {
        "status": "idp_3bead_release_archive_first_apply_report_ready",
        ...
        "next_required_step": "Compress the release archive root for real disk recovery, then rebuild the cleanup audit and keep only the baseline release pack in active root.",
    }
...
def _write_markdown(path: Path, payload: dict[str, Any]) -> None:
    s = payload["summary"]
    lines = [
        "# IDP 3-Bead Release Archive-First Apply Report",
        "",
        f"- status: `{s['status']}`",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Archive-first apply for stale idp_3bead_release smoke prefixes.")
