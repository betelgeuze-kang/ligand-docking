...
DEFAULT_RESULT_REVIEW_JSON = "runs/tcruzi_krs1_result_review_current.json"
DEFAULT_RESULT_SUMMARY_JSON = "runs/tcruzi_krs1_result_summary_current.json"
DEFAULT_LAUNCH_PACKET_JSON = "runs/tcruzi_krs1_launch_packet_current.json"
...
DEFAULT_GUARDED_OPERATOR_PACKET_JSON = "runs/wetlab_tcruzi_krs1_guarded_operator_packet_current.json"
DEFAULT_GUARDED_BRANCH_SUMMARY_JSON = "runs/wetlab_tcruzi_krs1_guarded_branch_summary_current.json"
DEFAULT_RUN_RECORD_JSON = "runs/tcruzi_krs1_run_record_current.json"
DEFAULT_OUT_MD = "runs/wetlab_tcruzi_krs1_branch_review_surface_current.md"
...
def build_payload(
    ...
    review_ready = _text(review_s.get("status")) == "tcruzi_krs1_result_review_ready"
    result_ready = _text(result_s.get("status")) == "completed"
    launch_ready = _text(launch_s.get("status")) == "tcruzi_krs1_launch_packet_ready"
    tuning_ready = _text(tuning_s.get("status")) == "wetlab_tcruzi_krs1_stage6_tuning_surface_ready"
    lane_ready = _text(lane_s.get("status")) == "wetlab_tcruzi_krs1_exploratory_retry_lane_ready"
    guarded_packet_ready = _text(guarded_packet_s.get("status")).startswith("wetlab_tcruzi_krs1_guarded_operator_packet_")
    guarded_branch_ready = _text(guarded_branch_s.get("status")).startswith("wetlab_tcruzi_krs1_guarded_branch_summary_")
    guarded_packet_validated = bool(guarded_packet_s.get("branch_validated", False)) or "validated" in _text(
        guarded_packet_s.get("status")
    )
    guarded_branch_validated = bool(guarded_branch_s.get("branch_validated", False)) or "validated" in _text(
    ...
    decision_source_priority = (
        "guarded_branch_summary"
        if branch_validated
        else "guarded_operator_packet"
        if guarded_packet_ready
        else "result_summary"
    )

    successor_target = _text(review_s.get("successor_target"), run_s.get("successor_target"), "LRRK2")
    successor_gate_state = _text(
        "open_for_lrrk2_execution" if branch_validated else "",
        "blocked_pending_tcruzi_krs1_guarded_review" if guarded_branch_ready or guarded_packet_ready else "",
        review_s.get("successor_gate_state"),
        run_s.get("successor_gate_state"),
        "open_for_lrrk2_execution",
    )
    next_required_step = _text(
        guarded_branch_s.get("next_required_step") if guarded_branch_validated else "",
        guarded_packet_s.get("next_required_step") if guarded_packet_validated else "",
    ...
    if not next_required_step:
        next_required_step = (
            f"Run the T. cruzi KRS1 exploratory gate5.1 retry for {_text(lane_s.get('shard_id'), tuning_s.get('next_retry_shard_id'))}; "
    ...
    branch_review_artifact = (
        "runs/wetlab_tcruzi_krs1_guarded_branch_summary_current.md"
        if guarded_branch_ready
        else "runs/tcruzi_krs1_result_review_current.md"
    )
    decision_artifact = (
        "runs/wetlab_tcruzi_krs1_guarded_branch_summary_current.md"
        if branch_validated
        else
        "runs/wetlab_tcruzi_krs1_guarded_operator_packet_current.md"
        if guarded_packet_ready
        else "runs/tcruzi_krs1_result_summary_current.md"
    )
    guarded_review_active = (guarded_branch_ready or guarded_packet_ready) and not branch_validated
    ...
    rows = [
        {
            ...
            "queue_phrase": _text(
                ...
                guarded_packet_s.get("next_required_step"),
                review_s.get("next_required_step"),
                "T. cruzi KRS1 stays in a guarded review branch until the gate5.1 exploratory retry is reviewed.",
            ),
            "gate_status": _text(
                guarded_branch_s.get("status"),
                guarded_packet_s.get("status"),
        ...
        },
        {
            ...
            "queue_phrase": _text(
                guarded_branch_s.get("next_required_step") if branch_validated else "",
                guarded_packet_s.get("next_required_step"),
                result_s.get("next_required_step"),
                "T. cruzi KRS1 guarded review keeps the default lane closed until a tuned retry decision is reviewed.",
            ),
            "gate_status": _text(
                guarded_branch_s.get("status") if branch_validated else "",
                guarded_packet_s.get("status"),
                result_s.get("status"),
                default="missing",
            ),
        },
        {
            ...
            "queue_phrase": _text(
                (
                    "Keep the T. cruzi KRS1 default lane closed while the validated guarded gate5.1 branch allows LRRK2 successor continuation."
                    if branch_validated
                    else ""
                ),
                "Keep the T. cruzi KRS1 default lane closed and leave launch sequencing paused while the guarded review branch is active."
                if guarded_review_active
                else "",
                launch_s.get("next_required_step"),
                "T. cruzi KRS1 launch packet remains paused while the guarded review branch is active.",
            ),
        ...
        },
        {
            ...
            "source_artifact": "runs/wetlab_tcruzi_krs1_stage6_tuning_surface_current.md",
            "queue_phrase": _text(
                tuning_s.get("next_required_step"),
                "Use the observed 5.05A band as the immediately runnable family for T. cruzi KRS1 stage6 tuning.",
            ),
        ...
        },
        {
            ...
            "source_artifact": "runs/wetlab_tcruzi_krs1_exploratory_retry_lane_current.md",
            "queue_phrase": _text(
                lane_s.get("next_required_step"),
                "Use the T. cruzi KRS1 exploratory gate5.1 candidate lane as the active guarded review path while the successor gate stays blocked.",
            ),
        ...
        },
        {
            ...
            "source_artifact": "runs/lrrk2_launch_packet_current.md",
            "queue_phrase": _text(
                (
                    "LRRK2 may continue now that the T. cruzi KRS1 guarded gate5.1 branch is validated; keep the KRS1 default lane closed."
                    if branch_validated
                    else ""
                ),
                "LRRK2 stays blocked behind T. cruzi KRS1 until the guarded review branch is cleared."
                if guarded_review_active
                else "",
                successor_gate_state,
                "LRRK2 stays blocked behind the active T. cruzi KRS1 guarded review branch.",
            ),
    ...
    return {
        "summary": {
            ...
            "target_id": TARGET_ID,
            "branch_label": _text(guarded_branch_s.get("branch_label"), "tcruzi_krs1_guarded_review_branch"),
            "branch_state": _text(
                guarded_branch_s.get("branch_state"),
                review_s.get("tcruzi_krs1_review_state"),
                "guarded_gate51_validated_default_lane_closed" if branch_validated else "guarded_stage6_review_default_lane_closed",
            ),
            "source_priority": source_priority,
            "decision_source_priority": decision_source_priority,
            "serialized_queue_rank": _safe_int(review_s.get("serialized_queue_rank", launch_s.get("serialized_queue_rank", 4)), 4),
            "serialized_run_order": (
                "lrrk2_successor_continuation_allowed"
                if branch_validated
                else
                "guarded_review_hold"
                if guarded_review_active
            ...
            "partner_track_id": _text(review_s.get("partner_track_id"), result_s.get("partner_track_id"), launch_s.get("partner_track_id"), "DNDi_Chagas_backup"),
            "result_review_status": _text(guarded_branch_s.get("status"), guarded_packet_s.get("status"), review_s.get("status"), "tcruzi_krs1_result_review_ready"),
            "result_summary_status": _text(
                guarded_branch_s.get("status") if branch_validated else "",
                guarded_packet_s.get("status"),
                result_s.get("status"),
                "completed",
            ),
            "launch_packet_status": _text(launch_s.get("status"), "tcruzi_krs1_launch_packet_ready"),
            "run_record_status": _text(run_s.get("status"), "tcruzi_krs1_run_record_ready"),
        ...
        "structured": {
            "result_review_artifact": "runs/tcruzi_krs1_result_review_current.md",
            "result_summary_artifact": "runs/tcruzi_krs1_result_summary_current.md",
            "launch_packet_artifact": "runs/tcruzi_krs1_launch_packet_current.md",
            "stage6_tuning_surface_artifact": "runs/wetlab_tcruzi_krs1_stage6_tuning_surface_current.md",
            "exploratory_retry_lane_artifact": "runs/wetlab_tcruzi_krs1_exploratory_retry_lane_current.md",
            "guarded_operator_packet_artifact": "runs/wetlab_tcruzi_krs1_guarded_operator_packet_current.md",
            "guarded_branch_summary_artifact": "runs/wetlab_tcruzi_krs1_guarded_branch_summary_current.md",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the T. cruzi KRS1 branch review surface.")
...
def main() -> None:
    ...
    write_artifact(args.out_md, "Wet-Lab T. cruzi KRS1 Branch Review Surface", payload)
