...
from tools import build_wetlab_wave1_tail_runtime_event as runtime_event_mod
...
def _rebuild_support_artifacts(python_bin: str) -> None:
    subprocess.run([python_bin, "tools/build_wetlab_wave1_tail_runtime_runbook.py"], cwd=ROOT, check=True)
    subprocess.run([python_bin, "tools/build_wetlab_wave1_tail_execution_console.py"], cwd=ROOT, check=True)
