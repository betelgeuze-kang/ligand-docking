...
from tools.wetlab import build_wetlab_wave1_tail_runtime_event as runtime_event_mod
from tools.wetlab_target_render_utils import write_artifact

ROOT = Path(__file__).resolve().parents[1]
DEFAULT_LOG_PATH = ROOT / "runs/wetlab_wave1_tail_runtime_event_log.jsonl"
...
def _rebuild_support_artifacts(python_bin: str) -> None:
    subprocess.run([python_bin, "tools/wetlab/wetlab/build_wetlab_wave1_tail_runtime_runbook.py"], cwd=ROOT, check=True)
    subprocess.run([python_bin, "tools/wetlab/wetlab/build_wetlab_wave1_tail_execution_console.py"], cwd=ROOT, check=True)
...
def apply_and_log_event(
    *,
    ...
    write_artifact(runtime_event_mod.DEFAULT_OUT_MD, "Wet-Lab Wave1 Tail Runtime Event", payload)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apply and log a Wave 1 tail runtime event, then rebuild the execution console.")
