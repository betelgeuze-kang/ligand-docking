...
PACKETS = {
    "core": {
        "reference_csv": "config/ligand_binding_reference_blind_ca2_zn_v1.csv",
        "eval_split_csv": "config/ligand_eval_splits_blind_ca2_zn_v1.csv",
        "ligand_meta_csv": "config/ligand_meta_blind_ca2_zn_v1.csv",
    },
    "ood": {
        "reference_csv": "config/ligand_binding_reference_blind_ca2_zn_chembl50_v1.csv",
        "eval_split_csv": "config/ligand_eval_splits_blind_ca2_zn_chembl50_v1.csv",
        "ligand_meta_csv": "config/ligand_meta_blind_ca2_zn_chembl50_v1.csv",
    },
}
...
def _queue_row(packet_name: str, ligand_id: str, ref: dict[str, str], split: dict[str, str], meta: dict[str, str]) -> dict[str, Any]:
    ...
    packet_step = f"{packet_name}_{binder_label}_{slot_index}".strip("_")
    return {
        "packet": packet_name,
        ...
        "notes": f"Replace placeholder {packet_name} {binder_label} slot {slot_index} with a curated CA2 ligand row.",
    }
...
def build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "summary": {
            ...
            "next_required_step": "Replace every placeholder slot in the queue with a curated CA2 ligand_id/provenance/meta row before attempting a runnable packet.",
        },
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a CA2 placeholder packet fill queue.")
    parser.add_argument("--out-json", default="runs/ca2_packet_fill_queue_current.json")
    parser.add_argument("--out-csv", default="runs/ca2_packet_fill_queue_current.csv")
    parser.add_argument("--out-md", default="runs/ca2_packet_fill_queue_current.md")
