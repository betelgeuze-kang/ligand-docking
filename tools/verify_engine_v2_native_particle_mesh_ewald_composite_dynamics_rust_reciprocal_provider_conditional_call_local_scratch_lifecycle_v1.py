#!/usr/bin/env python3
"""Verify conditional native PME Rust-adapter call-local scratch lifetime."""
...
SCHEMA_ID = (
    ...
    "rust_reciprocal_provider_conditional_call_local_scratch_lifecycle_"
    "profile/1.0.0"
)
SOURCE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_particle_mesh_ewald_composite_dynamics_"
...
PREDECESSOR = {
    "pull_request": 473,
    "reviewed_head": "6acdd4dc35d850e895902395f346f4a1e67db9b9",
    "merge_commit": "1ad469f2dc7adcde2b70279e6d0573b56460bc60",
    "merge_tree": "842d279a41dbb5cee81f321021e8489fc0739ddf",
    "profile_path": PREDECESSOR_PROFILE_RELATIVE_PATH.as_posix(),
    "profile_sha256": (
        "31c60474f083d2210d5542e52cee9cbad3fcb1da6528d6d865f01f4a4f946da2"
    ),
    "source_manifest_path": PREDECESSOR_MANIFEST_RELATIVE_PATH.as_posix(),
    "source_manifest_sha256": (
        "e5fa5f42e811582911cd260851ee76a9594eb4c57b657900ce5310740c127e81"
    ),
    "source_manifest_entry_count": 369,
}
...
EXPECTED_PREDECESSOR_WORKFLOW_SHA256 = (
    "3fe571610f75f55a19197ee41ce1633ca20c53b55027dc5a174ad44c0552a655"
)
EXPECTED_PREDECESSOR_UNIT_SHA256 = (
    "01393bdcc73dd792c2d453d9790dbc68ef594b1b3fbd53be13a32cbee3861720"
)
...
EXPECTED_SUCCESSOR_IMPLEMENTATION_SHA256 = {
    ...
    ADAPTER_TEST_RELATIVE_PATH: "4e106c951bb0bd666909a0cadcf703d34c0326519106ca9f7b70ddc07da3bf03",
...
def expected_predecessor_workflow() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_WORKFLOW_SHA256:
        fail("exact frozen PR 473 workflow digest drift")
    ...
    for predecessor_path, successor_path in evidence_wiring:
        ...
        if expected.count(anchor) != 2:
            fail("exact frozen PR 473 workflow trigger anchor drift: %s" % predecessor_path)
    ...
    old_region = source_region(
        expected,
        "      - name: Materialize exact PR 472 target and reviewed head\n",
        "\n\n  native-linux:\n",
        "exact frozen PR 473 immutable-evidence predecessor block",
    )
    new_region = """      - name: Materialize exact PR 473 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 1ad469f2dc7adcde2b70279e6d0573b56460bc60^{tree})" = "842d279a41dbb5cee81f321021e8489fc0739ddf"
          git merge-base --is-ancestor 1ad469f2dc7adcde2b70279e6d0573b56460bc60 HEAD
          git fetch --no-tags --depth=1 origin refs/pull/473/head
          test "$(git rev-parse FETCH_HEAD)" = "6acdd4dc35d850e895902395f346f4a1e67db9b9"
          test "$(git rev-parse FETCH_HEAD^{tree})" = "842d279a41dbb5cee81f321021e8489fc0739ddf"
      - name: Verify exact frozen PR 473 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=1ad469f2dc7adcde2b70279e6d0573b56460bc60
          frozen_tree=842d279a41dbb5cee81f321021e8489fc0739ddf
          ...
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_stateless_energy_all_scratch_profile_v1.json >/dev/null
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_stateless_energy_all_scratch_profile_v1_sources.json >/dev/null
          python3 -m pip install pytest==8.3.5
          python3 -m tools.verify_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_stateless_energy_all_scratch_v1
          python3 -m pytest -q tests/unit/test_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_stateless_energy_all_scratch_v1.py
          restore
          trap - EXIT"""
    return replace_once(
        expected,
        old_region,
        new_region,
        "exact frozen PR 473 immutable-evidence block",
    )
...
def expected_predecessor_unit() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_UNIT_SHA256:
        fail("exact frozen PR 473 unit digest drift")
    expected = frozen_raw.decode()
    expected = replace_once(
        expected,
        "from pathlib import Path\n",
        "from pathlib import Path\n\nimport pytest\n",
        "exact frozen PR 473 unit pytest import",
    )
...
pytestmark = pytest.mark.skipif(
    ...
    return replace_once(
        ...
        skip,
        "exact frozen PR 473 unit successor skip",
    )
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            ...
            "conditional_call_local_scratch_lifecycle_current_sources_tests_evidence_"
            "pr473_target"
        ),
...
def build_profile(manifest_raw: bytes) -> dict:
    ...
    return {
        ...
        "implementation": {
            "scope_is_only_native_provider_force_scratch_selection_lifecycle": True,
        ...
        "validation": {
            ...
            "source_manifest_entry_count_exact": 375,
            "pull_request_trigger_path_count_exact": 192,
            "push_trigger_path_count_exact": 192,
            ...
            "predecessor_adapter_exact_optional_transform": True,
            "conditional_optional_selection_source_exact": True,
            ...
            "provider_dispatch_validation_and_commit_exact_predecessor_bytes": True,
...
def require_workflow_contract(root: Path = ROOT) -> None:
    ...
    if predecessor != expected_predecessor_workflow():
        fail("PR 473 predecessor workflow is not the exact frozen-object transformation")
    for workflow, expected_name in (
        ...
        if len(pull_paths) != 192 or len(set(pull_paths)) != 192 or push_paths != pull_paths:
            fail("workflow 192-path symmetric trigger contract drift: %s" % expected_name)
    ...
    release_region = source_region(
        successor,
        "          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-conditional-call-local-scratch-lifecycle-release ",
        "          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-conditional-call-local-scratch-lifecycle-sanitize ",
    ...
    for token in (
        ...
        if token not in predecessor:
            fail("PR 473 predecessor workflow freeze drift: %s" % token)
...
def require_predecessor_unit_freeze(root: Path = ROOT) -> None:
    ...
    if unit != expected_predecessor_unit():
        fail("PR 473 predecessor unit is not the exact frozen-object transformation")
    constants = {
        node.value
    ...
    for token in (
        ...
        if unit.count(token) != 1:
            fail("PR 473 predecessor unit freeze drift: %s" % token)
    if PROFILE_RELATIVE_PATH.as_posix() not in constants:
        fail("PR 473 predecessor unit successor-profile path drift")
    if not any("exact frozen PR 473 object" in value for value in constants):
        fail("PR 473 predecessor unit frozen-object reason drift")
    if "is_file()" not in unit:
        fail("PR 473 predecessor unit skip is not successor-profile scoped")
...
def require_exact_source_hashes(root: Path = ROOT) -> None:
    ...
    for relative in IMPLEMENTATION_DELTA_PATHS:
        ...
        if sha(predecessor_raw) != EXPECTED_PREDECESSOR_IMPLEMENTATION_SHA256[relative]:
            fail("frozen PR 473 implementation digest drift: %s" % relative)
    ...
    expected_adapter = replace_once(
        ...
        "#include <cstdint>\n#include <type_traits>\n",
        "#include <cstdint>\n#include <optional>\n#include <type_traits>\n",
        "conditional scratch optional include",
    )
    expected_adapter = replace_once(
        expected_adapter,
...
""",
        "conditional call-local scratch selection",
    )
    for relative in (ADAPTER_RELATIVE_PATH, VENDOR_ADAPTER_RELATIVE_PATH):
        if (root / relative).read_text() != expected_adapter:
            fail("adapter is not the exact PR 473 optional-lifecycle transform: %s" % relative)
...
def require_native_contract(root: Path = ROOT) -> None:
    ...
    if rollback != frozen_rollback:
        fail("reusable Evaluation rollback guard changed from exact PR 473 bytes")
    ...
    dispatch = source_region(
        ...
        "    std::int32_t raw_status;",
        "    const bg_status status = normalize_provider_status(raw_status);",
        "successor provider dispatch",
    )
    frozen_dispatch = source_region(
        frozen_body,
        "    std::int32_t raw_status;",
        "    const bg_status status = normalize_provider_status(raw_status);",
        "frozen provider dispatch",
    )
    if dispatch != frozen_dispatch:
        fail("provider dispatch changed from exact PR 473 bytes")
    ...
    if validation_and_commit != frozen_validation_and_commit:
        fail("provider validation or external commit changed from exact PR 473 bytes")
    for token in (
    ...
    if destructor != frozen_destructor:
        fail("ProviderForceScratch destructor changed from exact PR 473 bytes")
...
def require_test_contract(root: Path = ROOT) -> None:
    ...
    if transactional_peer != frozen_transactional_peer:
        fail("raw-public transactional force-output peer block changed from PR 473")
    ...
    if stateless_predecessor_tests != frozen_stateless_predecessor_tests:
        fail("stateless lifecycle tests changed from exact PR 473 bytes")
...
def require_docs_contract(root: Path = ROOT) -> None:
    ...
    for token in (
        "native C++ adapter ownership selection for `ProviderForceScratch`",
        "function-scope `std::optional<ProviderForceScratch>`",
        "emplaced exactly once only for stateless calls",
        "Reusable calls leave it disengaged",
        "function-scope optional remains alive through force-channel preparation, provider dispatch, status normalization, energy and force finiteness checks, force copying, and external result commit",
        "zero destroy callbacks before external owner scope exit after six reusable calls",
        "forceful success, typed failure, and rejected non-finite force success",
        "energy-only success and typed failure",
        "provider-force-source success",
        "three explicit external-owner scopes exits",
        "each run exactly once",
        "only owner lifetime and unused call-local destroy-callback elision",
        "five adapter branches remain distinct and use the same two private provider symbols",
        "frozen from exact PR 473",
        "No Rust function, private header declaration, public symbol, provider ABI, status ABI, checkpoint format, production caller, or composite test changes",
        ...
        "optional still reserves inline object storage",
        "No allocation-free, allocation-count, heap-allocation-elision, provider-allocation-elision, stack-storage reduction, object-size reduction, persistent-reuse, cross-call-reuse, performance, peak-memory, acceleration, scientific-equivalence, molecular, HIP, or product claim",
        "Destroy callback elision is not a performance claim",
        "external_reservation_provider_not_operational",
        "historical_execution_operational_authority_false",
