...
def _exact_requirements(
    ...
    *,
    ...
    for line_number, raw_line in enumerate(
        _read_text(path, name=name).splitlines(),
        start=1,
    ):
        line = raw_line.strip()
        ...
        if line.startswith("-r "):
            ...
            if not include or any(
                part in {"", ".", ".."} for part in Path(include).parts
            ):
...
def _dockerfile_executable_text(dockerfile: str) -> str:
...
def verify_supply_chain(repo_root: Path) -> str:
    ...
    for forbidden in (
        ...
        if forbidden in executable_dockerfile:
            raise ProductSupplyChainError(
    ...
    if "!runs" in dockerignore:
        raise ProductSupplyChainError(
            "runtime evidence artifacts entered build context"
        )
