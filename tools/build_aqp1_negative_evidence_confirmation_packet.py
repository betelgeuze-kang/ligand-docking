...
def build_payload(
    ...
    *,
    ...
    primary_anchor_pmid = _text(acquisition_summary.get("primary_anchor_pmid")) or PRIMARY_ANCHOR_PMID
    primary_anchor_url = (
        ...
        rows.append(
            {
                ...
                "decision_rationale": (
                    "Use the 2012 reinvestigation as the exact-source confirmation anchor, treat the 2025 acetazolamide paper as positive boundary context only, "
...
def _write_markdown(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "## Confirmation Rows",
        "",
        "| confirmation_rank | packet_step | current_ligand_id | primary_anchor_pmid | positive_boundary_pmid | confirmation_decision |",
        "| ---: | --- | --- | --- | --- | --- |",
    ]
    for row in payload["rows"]:
        lines.append(
            f"| {row['confirmation_rank']} | `{row['packet_step']}` | `{row['current_ligand_id']}` | "
            f"`{row['primary_anchor_pmid']}` | `{row['positive_boundary_pmid']}` | `{row['confirmation_decision']}` |"
        )
