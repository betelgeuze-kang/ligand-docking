#!/usr/bin/env python3
"""Verify native PME Rust-adapter force-descriptor branch localization."""
...
WORKFLOW_STEM = (
    ...
    "rust-reciprocal-provider-force-descriptor-branch-localization"
)
PREDECESSOR_STEM = (
    "engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_direct_force_descriptor_binding"
)
PREDECESSOR_WORKFLOW_STEM = (
    "ci-engine-v2-native-particle-mesh-ewald-composite-dynamics-"
    "rust-reciprocal-provider-direct-force-descriptor-binding"
)
...
SCHEMA_ID = (
    ...
    "rust_reciprocal_provider_force_descriptor_branch_localization_"
    "profile/1.0.0"
)
SOURCE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_particle_mesh_ewald_composite_dynamics_"
...
PREDECESSOR = {
    "pull_request": 477,
    "reviewed_head": "1bff4017f9354e5f068c63475525220ddec99850",
    "merge_commit": "78a5ba2e0cc1cc6a8b71deca134ced017d6bbe51",
    "merge_tree": "35073ef5ad542e1add4ed64fe0752a965b333b08",
    "profile_path": PREDECESSOR_PROFILE_RELATIVE_PATH.as_posix(),
    "profile_sha256": (
        "d96e9cf8121f3cc2074a8976267eb9250861c90df718ecd4fefb067df170edf1"
    ),
    "source_manifest_path": PREDECESSOR_MANIFEST_RELATIVE_PATH.as_posix(),
    "source_manifest_sha256": (
        "173a34a33a2784914b6c0a7f97ba2bf7d14b6e3264ddcd166acc49942236988d"
    ),
    "source_manifest_entry_count": 393,
}
...
EXPECTED_PREDECESSOR_WORKFLOW_SHA256 = (
    "87d0ae5c132a05c8f2f1717c1d613bd3549e29344c333557269bfb8e2f85c7d7"
)
EXPECTED_PREDECESSOR_UNIT_SHA256 = (
    "a1af69bcd270b9ba6544988640bd3db5c53e359815c0fc251fcdc768c935eeb4"
)
EXPECTED_PREDECESSOR_IMPLEMENTATION_SHA256 = {
    ADAPTER_RELATIVE_PATH: "b4fa710e2c2a55a337ffb89c4984870d3a34fce248638dd8818970a8f98d2371",
    VENDOR_ADAPTER_RELATIVE_PATH: "b4fa710e2c2a55a337ffb89c4984870d3a34fce248638dd8818970a8f98d2371",
}
EXPECTED_SUCCESSOR_IMPLEMENTATION_SHA256 = {
    ADAPTER_RELATIVE_PATH: "2bd60c2a03a2c3520708eca21ce713ffbf8e4809565286991102a505aef18bc6",
    VENDOR_ADAPTER_RELATIVE_PATH: "2bd60c2a03a2c3520708eca21ce713ffbf8e4809565286991102a505aef18bc6",
}
...
def expected_predecessor_workflow() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_WORKFLOW_SHA256:
        fail("exact frozen PR 477 workflow digest drift")
    ...
    for predecessor_path, successor_path in evidence_wiring:
        ...
        if expected.count(anchor) != 2:
            fail("exact frozen PR 477 workflow trigger anchor drift: %s" % predecessor_path)
    ...
    old_region = source_region(
        expected,
        "      - name: Materialize exact PR 476 target and reviewed head\n",
        "\n\n  native-linux:\n",
        "exact frozen PR 477 immutable-evidence predecessor block",
    )
    new_region = """      - name: Materialize exact PR 477 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 78a5ba2e0cc1cc6a8b71deca134ced017d6bbe51^{tree})" = "35073ef5ad542e1add4ed64fe0752a965b333b08"
          git merge-base --is-ancestor 78a5ba2e0cc1cc6a8b71deca134ced017d6bbe51 HEAD
          git fetch --no-tags --depth=1 origin refs/pull/477/head
          test "$(git rev-parse FETCH_HEAD)" = "1bff4017f9354e5f068c63475525220ddec99850"
          test "$(git rev-parse FETCH_HEAD^{tree})" = "35073ef5ad542e1add4ed64fe0752a965b333b08"
      - name: Verify exact frozen PR 477 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=78a5ba2e0cc1cc6a8b71deca134ced017d6bbe51
          frozen_tree=35073ef5ad542e1add4ed64fe0752a965b333b08
          ...
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_direct_force_descriptor_binding_profile_v1.json >/dev/null
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_direct_force_descriptor_binding_profile_v1_sources.json >/dev/null
          python3 -m pip install pytest==8.3.5
          python3 -m tools.verify_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_direct_force_descriptor_binding_v1
          python3 -m pytest -q tests/unit/test_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_direct_force_descriptor_binding_v1.py
          restore
          trap - EXIT"""
    return replace_once(
        expected,
        old_region,
        new_region,
        "exact frozen PR 477 immutable-evidence block",
    )
...
def expected_predecessor_unit() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_UNIT_SHA256:
        fail("exact frozen PR 477 unit digest drift")
    expected = frozen_raw.decode()
    expected = replace_once(
        expected,
        "from pathlib import Path\n",
        "from pathlib import Path\n\nimport pytest\n",
        "exact frozen PR 477 unit pytest import",
    )
    skip = """ROOT = Path(__file__).resolve().parents[2]
PME_RUST_RECIPROCAL_PROVIDER_FORCE_DESCRIPTOR_BRANCH_LOCALIZATION_EVIDENCE_PRESENT = (
    ROOT
    / "config/engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_force_descriptor_branch_localization_profile_v1.json"
).is_file()
pytestmark = pytest.mark.skipif(
    PME_RUST_RECIPROCAL_PROVIDER_FORCE_DESCRIPTOR_BRANCH_LOCALIZATION_EVIDENCE_PRESENT,
    reason=(
        "direct force descriptor binding evidence is verified from its exact "
        "frozen PR 477 object after force descriptor branch localization "
        "evidence is present"
    ),
)"""
    return replace_once(
        ...
        skip,
        "exact frozen PR 477 unit successor skip",
    )
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            ...
            "force_descriptor_branch_localization_current_sources_tests_evidence_"
            "pr477_target"
        ),
...
def build_profile(manifest_raw: bytes) -> dict:
    ...
    return {
        ...
        "implementation": {
            "scope_is_only_native_force_descriptor_branch_localization": True,
            "outer_force_descriptor_declaration_removed": True,
            "standalone_force_descriptor_preparation_branch_removed": True,
            "force_descriptor_declaration_is_branch_local": True,
            "force_descriptor_preparation_and_force_dispatch_share_compute_forces_branch": True,
            "provider_error_descriptor_remains_common_to_both_dispatch_branches": True,
            "force_descriptor_pointer_temporary_removed": True,
            "force_descriptor_pointer_declaration_removed": True,
            "force_descriptor_pointer_assignment_removed": True,
            "force_symbol_binds_provider_forces_by_address_directly": True,
            "force_descriptor_direct_binding_preserved": True,
            "force_descriptor_direct_binding_only_in_compute_forces_branch": True,
            "energy_branch_has_zero_force_descriptor_arguments": True,
            "active_provider_force_scratch_is_non_null_reference_after_guard": True,
            "active_provider_force_scratch_reference_bound_once": True,
            "active_provider_force_scratch_pointer_reseating_removed": True,
            "active_provider_force_scratch_pointer_member_access_removed": True,
            "reusable_owner_pointer_parameter_preserved": True,
            "reusable_owner_null_guard_precedes_reference_binding": True,
            "reference_selection_uses_reuse_force_storage_only": True,
            "stateless_reference_binds_emplaced_call_local_owner": True,
            "reusable_reference_binds_external_owner": True,
            "two_symbol_dispatch_and_five_semantic_routes_preserved": True,
            ...
            "force_descriptor_preparation_precedes_force_symbol_call": True,
            ...
            "production_forceful_callers_reach_changed_force_symbol_branch": True,
            ...
            "nullability_elision_performance_improvement_claimed": False,
            "reference_binding_performance_improvement_claimed": False,
            "pointer_temporary_removal_performance_improvement_claimed": False,
            "direct_descriptor_binding_performance_improvement_claimed": False,
            "branch_localization_performance_improvement_claimed": False,
            "force_descriptor_scope_reduction_performance_improvement_claimed": False,
        ...
        "validation": {
            ...
            "source_manifest_entry_count_exact": 399,
            "pull_request_trigger_path_count_exact": 216,
            "push_trigger_path_count_exact": 216,
            ...
            "predecessor_adapter_exact_force_descriptor_branch_localization_transform": True,
            "provider_force_descriptor_declaration_count_exact": 1,
            "provider_force_descriptor_population_branch_count_exact": 1,
            "standalone_force_descriptor_preparation_branch_count_exact": 0,
            "force_branch_descriptor_declaration_count_exact": 1,
            "provider_force_descriptor_references_confined_to_force_branch": True,
            "energy_branch_provider_force_descriptor_reference_count_exact": 0,
            "relocated_force_descriptor_preparation_exact_predecessor_bytes": True,
            "force_descriptor_pointer_declaration_count_exact": 0,
            "force_descriptor_pointer_assignment_count_exact": 0,
            "force_descriptor_pointer_use_count_exact": 0,
            "force_symbol_direct_provider_forces_argument_count_exact": 1,
            "energy_symbol_force_descriptor_argument_count_exact": 0,
            "active_scratch_reference_declaration_count_exact": 1,
            "active_scratch_reference_member_access_count_exact": 18,
            "active_scratch_pointer_declaration_count_exact": 0,
            "active_scratch_pointer_member_access_count_exact": 0,
            "active_scratch_reseating_branch_count_exact": 0,
            "optional_emplacement_count_exact": 1,
            "reusable_owner_pointer_parameter_and_null_guard_preserved": True,
            "native_adapter_test_exact_predecessor_bytes": True,
            ...
            "adapter_dispatch_exact_after_branch_local_preparation_normalization": True,
            "energy_branch_exact_predecessor_bytes": True,
            "force_private_symbol_adapter_callsite_count_exact": 1,
            "energy_private_symbol_adapter_callsite_count_exact": 1,
...
def require_workflow_contract(root: Path = ROOT) -> None:
    ...
    if predecessor != expected_predecessor_workflow():
        fail("PR 477 predecessor workflow is not the exact frozen-object transformation")
    for workflow, expected_name in (
        ...
        if len(pull_paths) != 216 or len(set(pull_paths)) != 216 or push_paths != pull_paths:
            fail("workflow 216-path symmetric trigger contract drift: %s" % expected_name)
    ...
    release_region = source_region(
        successor,
        "          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-force-descriptor-branch-localization-release ",
        "          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-force-descriptor-branch-localization-sanitize ",
    ...
    for token in (
        ...
        if token not in predecessor:
            fail("PR 477 predecessor workflow freeze drift: %s" % token)
...
def require_predecessor_unit_freeze(root: Path = ROOT) -> None:
    ...
    if unit != expected_predecessor_unit():
        fail("PR 477 predecessor unit is not the exact frozen-object transformation")
    constants = {
        node.value
    ...
    for token in (
        ...
        if unit.count(token) != 1:
            fail("PR 477 predecessor unit freeze drift: %s" % token)
    if PROFILE_RELATIVE_PATH.as_posix() not in constants:
        fail("PR 477 predecessor unit successor-profile path drift")
    if not any("frozen PR 477 object" in value for value in constants):
        fail("PR 477 predecessor unit frozen-object reason drift")
    if "is_file()" not in unit:
        fail("PR 477 predecessor unit skip is not successor-profile scoped")
...
def require_exact_source_hashes(root: Path = ROOT) -> None:
    ...
    for relative in IMPLEMENTATION_DELTA_PATHS:
        ...
        if sha(predecessor_raw) != EXPECTED_PREDECESSOR_IMPLEMENTATION_SHA256[relative]:
            fail("frozen PR 477 implementation digest drift: %s" % relative)
    ...
    force_preparation_body = """        active_provider_force_scratch.x.resize(atom_count);
...
"""
    expected_adapter = replace_once(
        ...
        "",
        "outer force descriptor declaration removal",
    )
    expected_adapter = replace_once(
        expected_adapter,
        "    if (compute_forces) {\n" + force_preparation_body + "    }\n",
        "",
        "standalone force descriptor preparation branch removal",
    )
    expected_adapter = replace_once(
        expected_adapter,
        ...
        "        bg_rust_particle_mesh_reciprocal_force_output_v1 provider_forces{};\n"
        + force_preparation_body,
        "force descriptor preparation branch localization",
    )
    for relative in (ADAPTER_RELATIVE_PATH, VENDOR_ADAPTER_RELATIVE_PATH):
        if (root / relative).read_text() != expected_adapter:
            fail("adapter is not the exact PR 477 force-descriptor branch-localization transform: %s" % relative)
    ...
    if sha(frozen_adapter_test) != EXPECTED_PREDECESSOR_ADAPTER_TEST_SHA256:
        fail("frozen PR 477 native adapter-test digest drift")
    if (root / ADAPTER_TEST_RELATIVE_PATH).read_bytes() != frozen_adapter_test:
        fail("native adapter test changed from exact PR 477 bytes")
...
def require_native_contract(root: Path = ROOT) -> None:
    ...
    if dispatch.count("&provider_energy, &provider_forces, &provider_error") != 1:
    ...
    force_branch = source_region(
        ...
        "if (compute_forces) {",
        "} else {",
        "adapter branch-local force all-scratch dispatch",
    )
    force_preparation = source_region(
        force_branch,
        "        bg_rust_particle_mesh_reciprocal_force_output_v1 provider_forces{};\n",
        "\n        raw_status = " + all_scratch_force,
        "branch-local provider-force descriptor preparation",
    )
    require_ordered_tokens(
        body,
        (
        ...
        "adapter guards, branch-local force preparation, dispatch, validation, and commit",
    )
    if body.count("if (reuse_force_storage && provider_force_scratch == nullptr)") != 1:
        fail("adapter reusable owner null guard drift")
    if body.count("bg_rust_particle_mesh_reciprocal_force_output_v1 provider_forces{};") != 1:
        fail("adapter provider-force descriptor declaration count drift")
    if body.count("&provider_forces") != 1:
        fail("adapter direct provider-force descriptor argument count drift")
    if body.count("provider_forces") != force_branch.count("provider_forces"):
        fail("provider-force descriptor escaped the force branch")
    if "force_pointer" in body:
        fail("adapter retained the force descriptor pointer temporary")
    for token in (
        "bg_rust_particle_mesh_reciprocal_force_output_v1 provider_forces{};",
        "active_provider_force_scratch.x.resize(atom_count);",
        "active_provider_force_scratch.y.resize(atom_count);",
        ...
        if force_preparation.count(token) != 1:
            fail("branch-local provider-force descriptor preparation drift: %s" % token)
    ...
    for token in (
        ...
        if token not in local_scratch_setup:
            fail("active scratch reference selection drift: %s" % token)
    ...
    if body.count("ProviderForceScratch &active_provider_force_scratch =") != 1:
        fail("adapter active scratch reference declaration count drift")
    if body.count("active_provider_force_scratch.") != 18:
        fail("adapter active scratch reference-access count drift")
    for forbidden in (
    ...
    for token in (
        ...
        if token not in force_branch:
            fail("branch-local force descriptor routing drift: %s" % token)
    ...
    if body.count("provider_forces") != force_branch.count("provider_forces"):
        fail("energy/common adapter scope retained a force descriptor reference")
    ...
    if rollback != frozen_rollback:
        fail("reusable Evaluation rollback guard changed from exact PR 477 bytes")
    ...
    frozen_force_preparation = source_region(
        frozen_body,
        "    if (compute_forces) {\n        active_provider_force_scratch.x.resize(atom_count);",
        "\n    }\n\n    bg_rust_particle_mesh_reciprocal_error_v1 provider_error{};",
        "frozen provider-force descriptor preparation",
    )
    frozen_force_preparation_body = replace_once(
        frozen_force_preparation,
        "    if (compute_forces) {\n",
        "",
        "frozen standalone force descriptor preparation wrapper",
    )
    expected_force_preparation = (
        "        bg_rust_particle_mesh_reciprocal_force_output_v1 provider_forces{};\n"
        + frozen_force_preparation_body
    )
    if force_preparation != expected_force_preparation:
        fail("force descriptor preparation changed while moving into the force branch")
    frozen_dispatch = source_region(
        frozen_body,
        "    std::int32_t raw_status;",
        "    const bg_status status = normalize_provider_status(raw_status);",
        "frozen two-symbol compute-forces dispatch",
    )
    normalized_dispatch = replace_once(
        dispatch,
        force_preparation + "\n",
        "",
        "branch-local force descriptor preparation normalization",
    )
    if normalized_dispatch != frozen_dispatch:
        fail("dispatch changed beyond force descriptor preparation localization")
    frozen_energy_branch = source_region(
        frozen_dispatch,
        "} else {",
        "\n    }\n",
        "frozen force-free all-scratch branch",
    )
    if energy_branch != frozen_energy_branch:
        fail("energy branch changed from exact PR 477 bytes")
    ...
    if validation_and_commit != frozen_validation_and_commit:
        fail("provider validation or external commit changed from exact PR 477 bytes")
    for token in (
    ...
    if destructor != frozen_destructor:
        fail("ProviderForceScratch destructor changed from exact PR 477 bytes")
...
def require_test_contract(root: Path = ROOT) -> None:
    ...
    if transactional_peer != frozen_transactional_peer:
        fail("raw-public transactional force-output peer block changed from PR 477")
    ...
    if stateless_predecessor_tests != frozen_stateless_predecessor_tests:
        fail("stateless lifecycle tests changed from exact PR 477 bytes")
...
def require_docs_contract(root: Path = ROOT) -> None:
    ...
    for token in (
        "changes only the lexical scope of the native C++ adapter's already directly bound `provider_forces` descriptor",
        "exact PR 477 predecessor declared the descriptor in common scope, populated it in a standalone `if (compute_forces)` preparation branch",
        "removes the common-scope declaration and standalone preparation branch",
        "same exact whole-file transform of the exact PR 477 predecessor",
        "removing the unique outer descriptor declaration, removing the unique standalone preparation branch, and inserting the descriptor declaration plus that exact preparation body at the start of the force dispatch branch",
        "No other adapter byte may change",
        "Inside the single `if (compute_forces)` dispatch branch, the descriptor is zero-initialized exactly once",
        "Every `provider_forces` variable reference is confined to that branch",
        "energy branch is exact PR 477 bytes and has zero `provider_forces` references",
        "common `provider_error` descriptor and `raw_status` remain available to both branches",
        "dispatch region is not claimed byte-identical because it now contains the branch-local descriptor preparation",
        "Removing only that preparation from the successor dispatch must reproduce the exact PR 477 dispatch",
        "relocated preparation must reproduce the exact predecessor preparation bytes",
        "post-dispatch status normalization, typed-error handling, energy and force finiteness validation, rollback, force copying, and success-only external commit remain exact PR 477 bytes",
        "provider-force-source guard and reusable-owner null guard still precede the non-null active scratch reference binding and dispatch",
        "one reference with 18 member accesses",
        "rollback guard and `ProviderForceScratch` destructor are exact PR 477 bytes",
        "two private provider symbols and five semantic route classes remain unchanged",
        "native fake-provider adapter test is unchanged from exact PR 477 bytes",
        "six reusable zero-destroy checks, three external-owner scope destroy checks, and six stateless call-local lifecycle checks",
        "No Rust function, private header declaration, provider symbol, public symbol, provider ABI, status ABI, checkpoint format, production caller, composite source, or composite test changes",
        ...
        "predecessor workflow detaches the exact PR 477 merge object",
        "source-scope/type-state hardening only",
        "No allocation-free, allocation-count, allocation-behavior, heap-allocation-elision, provider-allocation-elision, stack-storage reduction, scratch-footprint reduction, object-size reduction, peak-memory reduction, branch-localization performance improvement, force-descriptor scope-reduction performance improvement, performance, acceleration, scientific-equivalence, molecular, HIP, product, or operational claim",
        "Shortening a descriptor's lexical scope is not a runtime performance claim",
        "external_reservation_provider_not_operational",
        "historical_execution_operational_authority_false",
