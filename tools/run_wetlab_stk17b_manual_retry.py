...
DEFAULT_LANE_JSON = "runs/wetlab_stk17b_manual_retry_lane_current.json"
...
DEFAULT_OUT_MD = "runs/wetlab_stk17b_manual_retry_runner_current.md"
...
def run(
    *,
    ...
    if not bool(summary.get("ready_for_manual_retry", False)):
        raise SystemExit("STK17B manual retry lane is not ready_for_manual_retry")
    if not selected_shard:
        raise SystemExit("no shard_id available for STK17B manual retry")
    if not selected_kind:
        raise SystemExit("no selected command kind available for STK17B manual retry")
    ...
    payload = {
        "summary": {
            "status": "wetlab_stk17b_manual_retry_runner_ready",
            "target_id": target_id,
            "shard_id": selected_shard,
    ...
    write_artifact(DEFAULT_OUT_MD, "STK17B (DRAK2) Manual Retry Runner", payload)
    return payload


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch the guarded manual retry lane for STK17B (DRAK2).")
