...
def build_goal_release_decision_gate(
    *,
    ...
    no_goal_blockers = _text(rollup.get("status")) == "goal_readiness_ready"
    ...
    rows = [
        ...
        _row(
            ...
            observed=_text(rollup.get("status")) or "missing",
            required="goal_readiness_ready",
            passed=no_goal_blockers,
            reason="The top-level rollup must be clear before the full objective can be treated as release-ready.",
        ),
        _row(
            lane_id="goal_release",
    ...
    if not no_goal_blockers:
        next_required_items.append("top-level readiness rollup")
