#!/usr/bin/env python3
"""Verify call-local stateless forceful PME Rust-adapter all-scratch routing."""
...
SCHEMA_ID = (
    ...
    "rust_reciprocal_provider_stateless_forceful_all_scratch_"
    "profile/1.0.0"
)
SOURCE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_particle_mesh_ewald_composite_dynamics_"
...
PREDECESSOR = {
    "pull_request": 471,
    "reviewed_head": "a857f6fbd8281adcbbd093b73809385169e09db4",
    "merge_commit": "48301bf56324a7e625b1c2c7019310771aefe7f3",
    "merge_tree": "660e4b2bd0ffdf483f7c9bc0ee5b7b9197e95e03",
    "profile_path": PREDECESSOR_PROFILE_RELATIVE_PATH.as_posix(),
    "profile_sha256": (
        "0d3c613d510b6b3cb5f40351d1e5d6ece7317f8381415bc8ee7008cb4f156e8a"
    ),
    "source_manifest_path": PREDECESSOR_MANIFEST_RELATIVE_PATH.as_posix(),
    "source_manifest_sha256": (
        "83da74c4ac551ab2a12836705735af4d62c25a3e929e8d29bab79d70a29842d8"
    ),
    "source_manifest_entry_count": 357,
}
...
EXPECTED_PREDECESSOR_WORKFLOW_SHA256 = (
    "882d8ef4e714f78ed0bb8c31c5f1e53a29b91ac97cb8a4991b50e4f60585e7d4"
)
EXPECTED_PREDECESSOR_UNIT_SHA256 = (
    "4cea3a257d74a29e222e4136bad904daa2d579fb4467a35b308733750acad4bc"
)
...
def expected_predecessor_workflow() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_WORKFLOW_SHA256:
        fail("exact frozen PR 471 workflow digest drift")
    ...
    for predecessor_path, successor_path in evidence_wiring:
        ...
        if expected.count(anchor) != 2:
            fail("exact frozen PR 471 workflow trigger anchor drift: %s" % predecessor_path)
    ...
    old_region = source_region(
        expected,
        "      - name: Materialize exact PR 470 target and reviewed head\n",
        "\n\n  native-linux:\n",
        "exact frozen PR 471 immutable-evidence predecessor block",
    )
    new_region = """      - name: Materialize exact PR 471 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 48301bf56324a7e625b1c2c7019310771aefe7f3^{tree})" = "660e4b2bd0ffdf483f7c9bc0ee5b7b9197e95e03"
          git merge-base --is-ancestor 48301bf56324a7e625b1c2c7019310771aefe7f3 HEAD
          git fetch --no-tags --depth=1 origin refs/pull/471/head
          test "$(git rev-parse FETCH_HEAD)" = "a857f6fbd8281adcbbd093b73809385169e09db4"
          test "$(git rev-parse FETCH_HEAD^{tree})" = "660e4b2bd0ffdf483f7c9bc0ee5b7b9197e95e03"
      - name: Verify exact frozen PR 471 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=48301bf56324a7e625b1c2c7019310771aefe7f3
          frozen_tree=660e4b2bd0ffdf483f7c9bc0ee5b7b9197e95e03
          ...
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_owner_reusable_forceful_all_scratch_reuse_profile_v1.json >/dev/null
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_owner_reusable_forceful_all_scratch_reuse_profile_v1_sources.json >/dev/null
          python3 -m pip install pytest==8.3.5
          python3 -m tools.verify_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_owner_reusable_forceful_all_scratch_reuse_v1
          python3 -m pytest -q tests/unit/test_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_owner_reusable_forceful_all_scratch_reuse_v1.py
          restore
          trap - EXIT"""
    return replace_once(
        expected,
        old_region,
        new_region,
        "exact frozen PR 471 immutable-evidence block",
    )
...
def expected_predecessor_unit() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_UNIT_SHA256:
        fail("exact frozen PR 471 unit digest drift")
    expected = frozen_raw.decode()
    expected = replace_once(
        expected,
        "from pathlib import Path\n",
        "from pathlib import Path\n\nimport pytest\n",
        "exact frozen PR 471 unit pytest import",
    )
    skip = """ROOT = Path(__file__).resolve().parents[2]
PME_RUST_RECIPROCAL_PROVIDER_STATELESS_FORCEFUL_ALL_SCRATCH_EVIDENCE_PRESENT = (
    ROOT
    / "config/engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_stateless_forceful_all_scratch_profile_v1.json"
).is_file()
pytestmark = pytest.mark.skipif(
    PME_RUST_RECIPROCAL_PROVIDER_STATELESS_FORCEFUL_ALL_SCRATCH_EVIDENCE_PRESENT,
    reason=(
        "owner reusable forceful all-scratch reuse evidence is verified from "
        "its exact frozen PR 471 object after stateless forceful all-scratch "
        "evidence is present"
    ),
)"""
    return replace_once(
        ...
        skip,
        "exact frozen PR 471 unit successor skip",
    )
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            ...
            "stateless_forceful_all_scratch_current_sources_tests_evidence_"
            "pr471_target"
        ),
...
def build_profile(manifest_raw: bytes) -> dict:
    ...
    return {
        ...
        "implementation": {
            "scope_is_only_native_stateless_forceful_adapter_route": True,
            "stateless_forceful_all_three_scratch_route_enabled": True,
            "stateless_forceful_direct_route_removed_from_adapter": True,
            ...
            "adapter_uses_three_unique_provider_symbols": True,
            "call_local_provider_force_scratch_selected_for_stateless_calls": True,
            "stateless_forceful_call_local_reciprocal_workspace_initially_empty": True,
            "stateless_forceful_call_local_neutrality_sort_scratch_initially_empty": True,
            "stateless_forceful_call_local_particle_assignment_scratch_initially_empty": True,
            "stateless_forceful_three_descriptors_initially_exact_all_zero": True,
            "stateless_forceful_three_descriptors_pairwise_distinct": True,
            "stateless_forceful_call_local_force_xyz_used": True,
            "stateless_forceful_three_descriptors_destroyed_before_return": True,
            "stateless_forceful_matching_destroy_callbacks_exactly_once_each": True,
            "stateless_forceful_lifecycle_tested_on_success_typed_failure_and_nonfinite_success": True,
            ...
            "call_local_force_xyz_payload_is_derived_and_nontransactional": True,
            ...
            "stateless_late_typed_failure_exact_evaluation_rollback_tested": True,
            "stateless_nonfinite_force_success_exact_evaluation_rollback_tested": True,
            "call_local_force_xyz_may_change_on_late_error_or_nonfinite_success": True,
            "production_composite_caller_exact_predecessor_bytes": True,
            "production_stateless_forceful_caller_reaches_changed_adapter_route": True,
            "production_stateless_forceful_caller_tests_enabled": True,
            ...
            "call_local_force_xyz_transactionality_claimed": False,
        ...
        "validation": {
            ...
            "source_manifest_entry_count_exact": 363,
            "pull_request_trigger_path_count_exact": 180,
            "push_trigger_path_count_exact": 180,
            ...
            "five_branch_three_symbol_adapter_dispatch_exact": True,
            "stateless_forceful_three_call_local_empty_descriptors_exact": True,
            "stateless_forceful_destroy_callbacks_exact_once_before_return": True,
            "stateless_forceful_success_error_nonfinite_lifecycle_tests_exact": True,
            "stateless_forceful_exact_evaluation_rollback_tests_exact": True,
            "reusable_forceful_predecessor_coverage_preserved": True,
            ...
            "release_and_sanitizer_cover_stateless_reciprocal_pme_and_composite_callers": True,
...
def require_workflow_contract(root: Path = ROOT) -> None:
    ...
    if predecessor != expected_predecessor_workflow():
        fail("PR 471 predecessor workflow is not the exact frozen-object transformation")
    for workflow, expected_name in (
        ...
        if len(pull_paths) != 180 or len(set(pull_paths)) != 180 or push_paths != pull_paths:
            fail("workflow 180-path symmetric trigger contract drift: %s" % expected_name)
    ...
    release_region = source_region(
        successor,
        "          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-stateless-forceful-all-scratch-release ",
        "          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-stateless-forceful-all-scratch-sanitize ",
    ...
    for token in (
        ...
        if token not in predecessor:
            fail("PR 471 predecessor workflow freeze drift: %s" % token)
...
def require_predecessor_unit_freeze(root: Path = ROOT) -> None:
    ...
    if unit != expected_predecessor_unit():
        fail("PR 471 predecessor unit is not the exact frozen-object transformation")
    constants = {
        node.value
    ...
    for token in (
        ...
        if unit.count(token) != 1:
            fail("PR 471 predecessor unit freeze drift: %s" % token)
    if PROFILE_RELATIVE_PATH.as_posix() not in constants:
        fail("PR 471 predecessor unit successor-profile path drift")
    if not any("exact frozen PR 471 object" in value for value in constants):
        fail("PR 471 predecessor unit frozen-object reason drift")
    if "is_file()" not in unit:
        fail("PR 471 predecessor unit skip is not successor-profile scoped")
...
def require_exact_source_hashes(root: Path = ROOT) -> None:
    ...
    for relative in IMPLEMENTATION_DELTA_PATHS:
        ...
        if sha(predecessor_raw) != EXPECTED_PREDECESSOR_IMPLEMENTATION_SHA256[relative]:
            fail("frozen PR 471 implementation digest drift: %s" % relative)
...
def require_native_contract(root: Path = ROOT) -> None:
    ...
    for symbol in (energy_all_scratch, transactional):
    ...
    require_ordered_tokens(
        ...
        "adapter five-branch three-symbol dispatch, finite preflight, and commit",
    )
    ...
    if rollback != frozen_rollback:
        fail("reusable Evaluation rollback guard changed from exact PR 471 bytes")
    for token in (
    ...
    require_ordered_tokens(
        ...
        "public reciprocal stateless forceful caller",
    )
    ...
    require_ordered_tokens(
        ...
        "particle-mesh Ewald stateless forceful caller",
    )
...
def require_test_contract(root: Path = ROOT) -> None:
    ...
    for token in (
        ...
        "triple_scratch_descriptors_were_empty",
        "triple_scratch_descriptors_were_distinct",
        ...
        if token not in adapter_test:
            fail("native stateless/reusable forceful lifecycle test drift: %s" % token)
    stateless_success_test = source_region(
        adapter_test,
        ...
        "stateless-forceful success and transactional-peer test",
    )
    if stateless_success_test.count(
        "require_only_stateless_all_scratch_force_route();"
    ) != 1 or stateless_success_test.count(
        "require_stateless_all_scratch_lifecycle();"
    ) != 1:
        fail("stateless-forceful success lifecycle assertion count drift")
    stateless_late_test = source_region(
        adapter_test,
        "void verify_late_stateless_all_scratch_failure_preserves_adapter_output() {",
        "\nvoid verify_nonfinite_stateless_all_scratch_success_preserves_adapter_output() {",
        "stateless-forceful late typed-failure test",
    )
    stateless_nonfinite_test = source_region(
        adapter_test,
        "void verify_nonfinite_stateless_all_scratch_success_preserves_adapter_output() {",
        "\nvoid verify_reusable_forceful_all_scratch_branch_and_transactionality() {",
        "stateless-forceful non-finite success test",
    )
    for region, label in (
        (stateless_late_test, "late typed failure"),
        (stateless_nonfinite_test, "non-finite success"),
    ):
        if region.count("require_only_stateless_all_scratch_force_route();") != 1:
            fail("stateless-forceful %s route assertion count drift" % label)
        if region.count("require_stateless_all_scratch_lifecycle();") != 1:
            fail("stateless-forceful %s lifecycle assertion count drift" % label)
        if region.count("require_same_snapshot(output, before);") != 1:
            fail("stateless-forceful %s Evaluation rollback drift" % label)
    ...
    if transactional_peer != frozen_transactional_peer:
        fail("raw-public transactional force-output peer block changed from PR 471")
    ...
    frozen_reusable_forceful_test = source_region(
        frozen_adapter_test,
        "void verify_reusable_forceful_all_scratch_branch_and_transactionality() {",
        "\nvoid verify_reusable_energy_workspace_branch_and_transactionality() {",
        "frozen reusable-forceful all-scratch adapter test",
    )
    if reusable_forceful_test != frozen_reusable_forceful_test:
        fail("reusable-forceful predecessor test changed from exact PR 471 bytes")
...
def require_docs_contract(root: Path = ROOT) -> None:
    ...
    for token in (
        "native C++ adapter's stateless force-producing branch",
        PRIVATE_SYMBOL,
        "five adapter branches remain distinct while using three unique provider symbols",
        "direct-force and workspace-only force ABIs remain available",
        "zero native-adapter call sites",
        "exact all-zero EMPTY values before provider dispatch",
        "pairwise distinct",
        "destroy callbacks exactly once each before the adapter returns",
        "on success, typed failure, and rejected non-finite success",
        ...
        "call-local force x/y/z, reciprocal workspace, neutrality-sort scratch, and particle-assignment scratch are derived and nontransactional",
        "cold preparation order remains neutrality sorting, particle assignment, then reciprocal workspace",
        "production stateless forceful caller reaches the changed adapter branch",
        "native fake provider checks dispatch, initially EMPTY descriptors, exact destruction, force writes, and Evaluation rollback only",
        "existing private all-scratch force symbol in the normal symbol table and absent from dynamic exports",
        "No allocation-free, persistent-reuse, cross-call-reuse, performance, acceleration, scientific-equivalence, molecular, HIP, or product claim",
        "external_reservation_provider_not_operational",
        "historical_execution_operational_authority_false",
