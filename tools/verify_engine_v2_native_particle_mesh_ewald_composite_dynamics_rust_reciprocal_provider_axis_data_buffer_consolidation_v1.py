#!/usr/bin/env python3
"""Verify one call-local reciprocal-axis backing buffer split into three slices."""
...
SCHEMA_ID = (
    ...
    "rust_reciprocal_provider_axis_data_buffer_consolidation_profile/1.0.0"
)
SOURCE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_axis_data_buffer_consolidation_sources/1.0.0"
)
PUBLIC_PROFILE_ID = (
...
PREDECESSOR = {
    "pull_request": 460,
    "reviewed_head": "312717e57f3e1b2aed9eeb0da52c4544efff057b",
    "merge_commit": "759cbd3abeda74bc385c6a5091cea834f4f06458",
    "merge_tree": "7343cfee07e40a16460e82ff423e2c9ef353342b",
    "profile_path": PREDECESSOR_PROFILE_RELATIVE_PATH.as_posix(),
    "profile_sha256": (
        "0c8419c5e3ae3b074ee85e65aff3aadc98fa3c92794793a7cecdca4241341c5a"
    ),
    "source_manifest_path": PREDECESSOR_MANIFEST_RELATIVE_PATH.as_posix(),
    "source_manifest_sha256": (
        "c1a751dcc0686188bb4e3bdf16567eb469a419c0d42fd21e91b14d20d6c4de5e"
    ),
    "source_manifest_entry_count": 290,
}
...
EXPECTED_PREDECESSOR_WORKFLOW_SHA256 = (
    "bb5f79990e75c8ee6ec990ad8435208103f873169792cf47377c029143ad377a"
)
EXPECTED_PREDECESSOR_UNIT_SHA256 = (
    "7967a73db378a3914b7a1674366639ef62486fc7763df55510c5c402c2f0223a"
)
    ...
    steps:
      - name: Check out exact evidence graph
        ...
        with:
          ...
          clean: true
          fetch-depth: 0
      - name: Materialize exact PR 460 target, PR 453 architecture, PR 440 inherited evaluator, and PR 380 direct-output precedent
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 759cbd3abeda74bc385c6a5091cea834f4f06458^{tree})" = "7343cfee07e40a16460e82ff423e2c9ef353342b"
          git merge-base --is-ancestor 759cbd3abeda74bc385c6a5091cea834f4f06458 HEAD
          git fetch --no-tags --depth=1 origin refs/pull/460/head
          test "$(git rev-parse FETCH_HEAD)" = "312717e57f3e1b2aed9eeb0da52c4544efff057b"
          test "$(git rev-parse FETCH_HEAD^{tree})" = "7343cfee07e40a16460e82ff423e2c9ef353342b"
          test "$(git rev-parse 35a8f0b0ba0e079bc2a1edee15d19ef2c2823f2a^{tree})" = "b22c5fd115a5c8e28856872df57127ecdd28d9b5"
          git merge-base --is-ancestor 35a8f0b0ba0e079bc2a1edee15d19ef2c2823f2a 759cbd3abeda74bc385c6a5091cea834f4f06458
          ...
          git merge-base --is-ancestor 6662f1b53829930a93de0f298b820d5a367cc3dc 759cbd3abeda74bc385c6a5091cea834f4f06458
...
EXPECTED_NATIVE_JOB_BODY = """    runs-on: ubuntu-latest
    ...
    steps:
      ...
      - name: Synthetic release and sanitizer regressions
        run: |
          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-axis-data-buffer-consolidation-release -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF
          cmake --build build/particle-mesh-ewald-rust-reciprocal-provider-axis-data-buffer-consolidation-release --target betelgeuze_engine_particle_mesh_reciprocal betelgeuze_engine_particle_mesh_ewald_composite_dynamics --parallel 2
          ctest --test-dir build/particle-mesh-ewald-rust-reciprocal-provider-axis-data-buffer-consolidation-release -R '^betelgeuze_engine_(particle_mesh_reciprocal|particle_mesh_ewald_composite_dynamics|export_allowlist)$' --output-on-failure
          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-axis-data-buffer-consolidation-sanitize -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF -DCMAKE_C_FLAGS='-fsanitize=address,undefined -fno-omit-frame-pointer' -DCMAKE_CXX_FLAGS='-fsanitize=address,undefined -fno-omit-frame-pointer'
          cmake --build build/particle-mesh-ewald-rust-reciprocal-provider-axis-data-buffer-consolidation-sanitize --target betelgeuze_engine_particle_mesh_reciprocal betelgeuze_engine_particle_mesh_ewald_composite_dynamics --parallel 2
          ASAN_OPTIONS=detect_leaks=1 UBSAN_OPTIONS=halt_on_error=1 ctest --test-dir build/particle-mesh-ewald-rust-reciprocal-provider-axis-data-buffer-consolidation-sanitize -R '^betelgeuze_engine_(particle_mesh_reciprocal|particle_mesh_ewald_composite_dynamics)$' --output-on-failure
"""

EXPECTED_RUST_JOB_BODY = """    runs-on: ubuntu-latest
    timeout-minutes: 35
    env:
      CARGO_TARGET_DIR: ${{ github.workspace }}/build/particle-mesh-ewald-rust-reciprocal-provider-axis-data-buffer-consolidation-cargo
    steps:
...
EXPECTED_MACOS_JOB_BODY = """    runs-on: macos-15
    ...
    steps:
      ...
      - name: Exact export regression
        run: |
          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-axis-data-buffer-consolidation-macos -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF
          cmake --build build/particle-mesh-ewald-rust-reciprocal-provider-axis-data-buffer-consolidation-macos --target betelgeuze_engine --parallel 2
          ctest --test-dir build/particle-mesh-ewald-rust-reciprocal-provider-axis-data-buffer-consolidation-macos -R '^betelgeuze_engine_export_allowlist$' --output-on-failure
"""
...
def expected_workflow_document() -> str:
    ...
    preamble = (
        ...
        "rust-reciprocal-provider-axis-data-buffer-consolidation\n\n"
        "on:\n"
        "  pull_request:\n"
        "    paths:\n"
        f"{trigger_paths}"
        ...
        "rust-reciprocal-provider-axis-data-buffer-consolidation-${{ github.ref }}\n"
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            ...
            "axis_data_buffer_consolidation_current_sources_tests_evidence_target_"
...
def build_profile(manifest_raw: bytes) -> dict:
    ...
    return {
        ...
        "profile_id": (
            ...
            "rust_reciprocal_provider_axis_data_buffer_consolidation_development_v1"
        ),
        "roadmap_issue": 434,
        ...
        "validation": {
            ...
            "predecessor_fft_line_scratch_contract_inherited": True,
...
def expected_frozen_predecessor_workflow(frozen: str) -> str:
    ...
    old_materialize = source_region(
        expected,
        "      - name: Materialize exact PR 459 target, PR 453 architecture, "
        "PR 440 inherited evaluator, and PR 380 direct-output precedent\n",
        "      - name: Verify bounded successor evidence\n",
        "frozen predecessor materialization",
    )
    new_materialize = """      - name: Materialize exact PR 460 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 759cbd3abeda74bc385c6a5091cea834f4f06458^{tree})" = "7343cfee07e40a16460e82ff423e2c9ef353342b"
...
"""
    new_verify = """      - name: Verify exact frozen PR 460 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=759cbd3abeda74bc385c6a5091cea834f4f06458
          frozen_tree=7343cfee07e40a16460e82ff423e2c9ef353342b
...
def expected_frozen_predecessor_unit(frozen: str) -> str:
    ...
    addition = """PME_RUST_RECIPROCAL_PROVIDER_AXIS_DATA_BUFFER_CONSOLIDATION_EVIDENCE_PRESENT = (
    ROOT
    / "config/engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_axis_data_buffer_consolidation_profile_v1.json"
).is_file()
...
EXPECTED_RUST_RECIPROCAL_SHA256 = (
    "14ce6f5d8d1c94ff94bcf19e79d46aca20e20eda4f750b397ecef7788d90d6b9"
)
...
def require_rust_reciprocal_provider_axis_data_buffer_consolidation_contract(
    root: Path = ROOT,
) -> None:
    if IMPLEMENTATION_DELTA_PATHS != (RUST_RECIPROCAL_RELATIVE_PATH,):
        fail("reciprocal-axis buffer consolidation production path set drift")
    ...
    if sha(rust_raw) != EXPECTED_RUST_RECIPROCAL_SHA256:
        fail("exact reciprocal-axis buffer consolidation Rust source drift")
    if git(
        "show", f"{merge}:{RUST_RECIPROCAL_RELATIVE_PATH.as_posix()}"
    ).stdout == rust_raw:
        fail("reciprocal-axis buffer consolidation Rust source did not change")
...
"""
    ...
    for token in (
        ...
        if direct_prewrite_test.count(token) != 1:
...
def require_contracts(root: Path = ROOT) -> None:
    ...
    require_rust_reciprocal_provider_axis_data_buffer_consolidation_contract(root)
