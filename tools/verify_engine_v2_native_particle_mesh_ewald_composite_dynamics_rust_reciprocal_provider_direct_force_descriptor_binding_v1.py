#!/usr/bin/env python3
"""Verify native PME Rust-adapter direct force-descriptor binding."""
...
PREDECESSOR_STEM = (
    "engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_active_scratch_reference"
)
PREDECESSOR_WORKFLOW_STEM = (
    "ci-engine-v2-native-particle-mesh-ewald-composite-dynamics-"
    "rust-reciprocal-provider-active-scratch-reference"
)
...
SCHEMA_ID = (
    ...
    "rust_reciprocal_provider_direct_force_descriptor_binding_"
    "profile/1.0.0"
)
SOURCE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_particle_mesh_ewald_composite_dynamics_"
...
PREDECESSOR = {
    "pull_request": 476,
    "reviewed_head": "f19fbc07098eec4e92119908b64d5f41fe206ee0",
    "merge_commit": "26b41a2952a8f2643e67197c4425fdf487d1304a",
    "merge_tree": "9af8cf67dcec59ccf209f512c75cd921687b86fa",
    "profile_path": PREDECESSOR_PROFILE_RELATIVE_PATH.as_posix(),
    "profile_sha256": (
        "335a9535fe1ce60db74fe3e6d3f149f266fa1af6f28eed684131a86099b22879"
    ),
    "source_manifest_path": PREDECESSOR_MANIFEST_RELATIVE_PATH.as_posix(),
    "source_manifest_sha256": (
        "4acd8e872ccbb564dd0de97f8f51572694f30a0e26433f14c2a618e3b9ea6c61"
    ),
    "source_manifest_entry_count": 387,
}
...
EXPECTED_PREDECESSOR_WORKFLOW_SHA256 = (
    "4d5f94eabda48be7f367d6681098509e0c257ea5a6d98d8925fc95532fb7ce0f"
)
EXPECTED_PREDECESSOR_UNIT_SHA256 = (
    "2a7c72c1d364e42d9259d067c5946f1aeed47a12f20ee37a7349c4c1431f15ac"
)
EXPECTED_PREDECESSOR_IMPLEMENTATION_SHA256 = {
    ADAPTER_RELATIVE_PATH: "167799f4d7a13e9ef2b03ffab2dff3ec5469adda67e93992b28a0d32ba92f72c",
    VENDOR_ADAPTER_RELATIVE_PATH: "167799f4d7a13e9ef2b03ffab2dff3ec5469adda67e93992b28a0d32ba92f72c",
}
...
def expected_predecessor_workflow() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_WORKFLOW_SHA256:
        fail("exact frozen PR 476 workflow digest drift")
    ...
    for predecessor_path, successor_path in evidence_wiring:
        ...
        if expected.count(anchor) != 2:
            fail("exact frozen PR 476 workflow trigger anchor drift: %s" % predecessor_path)
    ...
    old_region = source_region(
        expected,
        "      - name: Materialize exact PR 475 target and reviewed head\n",
        "\n\n  native-linux:\n",
        "exact frozen PR 476 immutable-evidence predecessor block",
    )
    new_region = """      - name: Materialize exact PR 476 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 26b41a2952a8f2643e67197c4425fdf487d1304a^{tree})" = "9af8cf67dcec59ccf209f512c75cd921687b86fa"
          git merge-base --is-ancestor 26b41a2952a8f2643e67197c4425fdf487d1304a HEAD
          git fetch --no-tags --depth=1 origin refs/pull/476/head
          test "$(git rev-parse FETCH_HEAD)" = "f19fbc07098eec4e92119908b64d5f41fe206ee0"
          test "$(git rev-parse FETCH_HEAD^{tree})" = "9af8cf67dcec59ccf209f512c75cd921687b86fa"
      - name: Verify exact frozen PR 476 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=26b41a2952a8f2643e67197c4425fdf487d1304a
          frozen_tree=9af8cf67dcec59ccf209f512c75cd921687b86fa
          ...
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_active_scratch_reference_profile_v1.json >/dev/null
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_active_scratch_reference_profile_v1_sources.json >/dev/null
          python3 -m pip install pytest==8.3.5
          python3 -m tools.verify_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_active_scratch_reference_v1
          python3 -m pytest -q tests/unit/test_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_active_scratch_reference_v1.py
          restore
          trap - EXIT"""
    return replace_once(
        expected,
        old_region,
        new_region,
        "exact frozen PR 476 immutable-evidence block",
    )
...
def expected_predecessor_unit() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_UNIT_SHA256:
        fail("exact frozen PR 476 unit digest drift")
    expected = frozen_raw.decode()
    expected = replace_once(
        expected,
        "from pathlib import Path\n",
        "from pathlib import Path\n\nimport pytest\n",
        "exact frozen PR 476 unit pytest import",
    )
    skip = """ROOT = Path(__file__).resolve().parents[2]
PME_RUST_RECIPROCAL_PROVIDER_DIRECT_FORCE_DESCRIPTOR_BINDING_EVIDENCE_PRESENT = (
    ROOT
    / "config/engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_direct_force_descriptor_binding_profile_v1.json"
).is_file()
pytestmark = pytest.mark.skipif(
    PME_RUST_RECIPROCAL_PROVIDER_DIRECT_FORCE_DESCRIPTOR_BINDING_EVIDENCE_PRESENT,
    reason=(
        "active scratch reference evidence is verified from its exact frozen "
        "PR 476 object after direct force descriptor binding evidence is present"
    ),
)"""
    return replace_once(
        expected,
        ...
        skip,
        "exact frozen PR 476 unit successor skip",
    )
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            ...
            "direct_force_descriptor_binding_current_sources_tests_evidence_"
            "pr476_target"
        ),
...
def build_profile(manifest_raw: bytes) -> dict:
    ...
    return {
        ...
        "implementation": {
            "scope_is_only_native_force_symbol_direct_descriptor_binding": True,
            ...
            "force_descriptor_preparation_precedes_dispatch": True,
        ...
        "validation": {
            ...
            "source_manifest_entry_count_exact": 393,
            "pull_request_trigger_path_count_exact": 210,
            "push_trigger_path_count_exact": 210,
            ...
            "predecessor_adapter_exact_three_point_force_descriptor_transform": True,
            "provider_force_descriptor_declaration_count_exact": 1,
            "provider_force_descriptor_population_branch_count_exact": 1,
            "force_descriptor_preparation_exact_after_pointer_assignment_removal": True,
            ...
            "two_branch_two_symbol_adapter_dispatch_exact_after_force_argument_normalization": True,
            "force_private_symbol_adapter_callsite_count_exact": 1,
            "energy_private_symbol_adapter_callsite_count_exact": 1,
...
def require_workflow_contract(root: Path = ROOT) -> None:
    ...
    if predecessor != expected_predecessor_workflow():
        fail("PR 476 predecessor workflow is not the exact frozen-object transformation")
    for workflow, expected_name in (
        ...
        if len(pull_paths) != 210 or len(set(pull_paths)) != 210 or push_paths != pull_paths:
            fail("workflow 210-path symmetric trigger contract drift: %s" % expected_name)
    ...
    release_region = source_region(
        successor,
        "          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-direct-force-descriptor-binding-release ",
        "          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-direct-force-descriptor-binding-sanitize ",
    ...
    for token in (
        ...
        if token not in predecessor:
            fail("PR 476 predecessor workflow freeze drift: %s" % token)
...
def require_predecessor_unit_freeze(root: Path = ROOT) -> None:
    ...
    if unit != expected_predecessor_unit():
        fail("PR 476 predecessor unit is not the exact frozen-object transformation")
    constants = {
        node.value
    ...
    for token in (
        ...
        if unit.count(token) != 1:
            fail("PR 476 predecessor unit freeze drift: %s" % token)
    if PROFILE_RELATIVE_PATH.as_posix() not in constants:
        fail("PR 476 predecessor unit successor-profile path drift")
    if not any("exact frozen PR 476 object" in value for value in constants):
        fail("PR 476 predecessor unit frozen-object reason drift")
    if "is_file()" not in unit:
        fail("PR 476 predecessor unit skip is not successor-profile scoped")
...
def require_exact_source_hashes(root: Path = ROOT) -> None:
    ...
    for relative in IMPLEMENTATION_DELTA_PATHS:
        ...
        if sha(predecessor_raw) != EXPECTED_PREDECESSOR_IMPLEMENTATION_SHA256[relative]:
            fail("frozen PR 476 implementation digest drift: %s" % relative)
    ...
    if sha(frozen_adapter_test) != EXPECTED_PREDECESSOR_ADAPTER_TEST_SHA256:
        fail("frozen PR 476 native adapter-test digest drift")
    if (root / ADAPTER_TEST_RELATIVE_PATH).read_bytes() != frozen_adapter_test:
        fail("native adapter test changed from exact PR 476 bytes")
...
def require_native_contract(root: Path = ROOT) -> None:
    ...
    force_preparation = source_region(
        ...
        "provider-force descriptor preparation",
    )
    if force_preparation.count("if (compute_forces) {") != 1:
        fail("provider-force descriptor preparation branch count drift")
    for token in (
        ...
        if force_preparation.count(token) != 1:
            fail("provider-force descriptor preparation drift: %s" % token)
    ...
    if rollback != frozen_rollback:
        fail("reusable Evaluation rollback guard changed from exact PR 476 bytes")
    ...
    expected_force_preparation = replace_once(
        frozen_force_preparation,
        "\n        force_pointer = &provider_forces;",
        "",
        "frozen force descriptor pointer assignment",
    )
    if force_preparation != expected_force_preparation:
        fail("force descriptor preparation changed beyond pointer assignment removal")
    ...
    direct_force_argument = "&provider_energy, &provider_forces, &provider_error"
    if dispatch.count(direct_force_argument) != 1:
        fail("successor direct force-descriptor argument count drift")
    normalized_dispatch = dispatch.replace(
        direct_force_argument,
        "&provider_energy, force_pointer, &provider_error",
        1,
    )
    if normalized_dispatch != frozen_dispatch:
        fail("two-symbol dispatch changed beyond the direct force-descriptor argument")
    ...
    if validation_and_commit != frozen_validation_and_commit:
        fail("provider validation or external commit changed from exact PR 476 bytes")
    for token in (
    ...
    if destructor != frozen_destructor:
        fail("ProviderForceScratch destructor changed from exact PR 476 bytes")
...
def require_test_contract(root: Path = ROOT) -> None:
    ...
    if transactional_peer != frozen_transactional_peer:
        fail("raw-public transactional force-output peer block changed from PR 476")
    ...
    if stateless_predecessor_tests != frozen_stateless_predecessor_tests:
        fail("stateless lifecycle tests changed from exact PR 476 bytes")
...
def require_docs_contract(root: Path = ROOT) -> None:
    ...
    for token in (
        "changes only how the native C++ adapter supplies its already prepared `provider_forces` descriptor",
        "exact PR 476 predecessor declared a nullable local `force_pointer`",
        "removes that one declaration and one assignment and passes `&provider_forces` directly at the force-symbol call",
        "same exact three-point transform of the exact PR 476 predecessor",
        "deleting exactly the `force_pointer` declaration, deleting exactly its `&provider_forces` assignment, and replacing exactly the force-symbol `force_pointer` argument with `&provider_forces`",
        "No other adapter byte may change",
        "force symbol has exactly one adapter callsite and receives exactly one direct `&provider_forces` argument",
        "energy symbol has exactly one adapter callsite, its branch contains no force-descriptor argument",
        "not claimed byte-identical because its force argument changes",
        "equal the exact PR 476 dispatch after normalizing only `&provider_forces` back to `force_pointer`",
        "post-dispatch status normalization, typed-error handling, energy and force finiteness validation, rollback, force copying, and success-only external commit are required to be exact PR 476 bytes",
        "provider-force-source guard, reusable-owner null guard, active scratch reference binding, descriptor preparation, two provider symbols, five semantic route classes",
        "one non-null reference with 18 member accesses",
        "rollback guard and `ProviderForceScratch` destructor are exact PR 476 bytes",
        "native fake-provider adapter test is unchanged from exact PR 476 bytes",
        ...
        "predecessor workflow detaches the exact PR 476 merge object",
        "local binding simplification only",
        "No allocation-free, allocation-count, allocation-behavior, heap-allocation-elision, provider-allocation-elision, pointer-temporary-removal performance, direct-descriptor-binding performance, stack-storage reduction, scratch-footprint reduction, object-size reduction, peak-memory reduction, performance, acceleration, scientific-equivalence, molecular, HIP, product, or operational claim",
        "Removing the local pointer temporary is not a runtime performance claim",
        "external_reservation_provider_not_operational",
        "historical_execution_operational_authority_false",
