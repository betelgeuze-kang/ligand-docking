#!/usr/bin/env python3
"""Verify native PME Rust-adapter rollback output force-storage binding."""
...
SCHEMA_ID = (
    ...
    "rust_reciprocal_provider_evaluation_rollback_output_force_storage_binding_profile/1.0.0"
)
SOURCE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_evaluation_rollback_output_force_storage_binding_sources/1.0.0"
)
...
PREDECESSOR = {
    "pull_request": 482,
    "reviewed_head": "3f8e3f2acfdd3cbc3514feffa17e2e74e300598c",
    "merge_commit": "8decdf9ca7129bb5669d5217e611f52860ac779c",
    "merge_tree": "44e57663a0243d16b4805fe97c5655ee028f8c84",
    "profile_path": PREDECESSOR_PROFILE_RELATIVE_PATH.as_posix(),
    "profile_sha256": (
        "c130a1fb649972ff9d762a7d23c9c0e9599380b626681295b1cd691cc7f595a3"
    ),
    "source_manifest_path": PREDECESSOR_MANIFEST_RELATIVE_PATH.as_posix(),
    "source_manifest_sha256": (
        "42ac3a4ac8c9f9cc726ad58be2ef65f6d0e288273750f1d67d22ac4d938bf125"
    ),
    "source_manifest_entry_count": 423,
}

PREDECESSOR_EVIDENCE_SHA256 = {
    PREDECESSOR_WORKFLOW_RELATIVE_PATH: (
        "59ad45c19d6c812832d91d23e42761c53fe2eafe86c674875edcb792a06f9ab2"
    ),
    ...
    PREDECESSOR_DOC_RELATIVE_PATH: (
        "ff0f0457bb490c6a5ede86d6edd7571b5e7d0bcb7e6f0065de4101b5f529d08f"
    ),
    PREDECESSOR_UNIT_RELATIVE_PATH: (
        "9ed7fceba4fc61c2b63e6a39e0ae1b07f9ec301d80cbdeb58c0733cea9ea8d6b"
    ),
    PREDECESSOR_VERIFIER_RELATIVE_PATH: (
        "c8536d6cb9b33c265ad450436b68757de0093f5cf7e9acef645ef68d66017847"
    ),
}
...
def require_predecessor() -> dict:
    ...
    if canonical_bytes(profile) != profile_raw or canonical_bytes(manifest) != manifest_raw:
        fail("PR 482 predecessor evidence is not canonical JSON")
    rows = manifest.get("files")
    if not isinstance(rows, list) or len(rows) != 423:
        fail("PR 482 predecessor manifest count drift")
    if [row.get("path") for row in rows] != sorted(
        {row.get("path") for row in rows}
    ):
        fail("PR 482 predecessor manifest paths are not sorted and unique")
    for path in (
        PREDECESSOR_PROFILE_RELATIVE_PATH,
        PREDECESSOR_MANIFEST_RELATIVE_PATH,
        PREDECESSOR_DOC_RELATIVE_PATH,
        ...
        if (ROOT / path).read_bytes() != frozen_bytes(path):
            fail("checked-out PR 482 predecessor evidence drift: %s" % path)
    ...
    if git("cat-file", "-e", "%s^{commit}" % reviewed, check=False).returncode == 0:
        ...
        if reviewed_tree != PREDECESSOR["merge_tree"]:
            fail("PR 482 reviewed-head tree drift")
    return manifest
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            ...
            "evaluation_rollback_output_force_storage_binding_current_sources_tests_"
            "evidence_pr482_target"
        ),
...
def build_profile(manifest_raw: bytes, root: Path = ROOT) -> dict:
    ...
    implementation.update(
        {
            "scope_is_only_native_evaluation_rollback_output_force_storage_binding": True,
            "evaluation_rollback_output_force_storage_type_alias_exact": True,
            "evaluation_rollback_output_force_storage_type_derived_from_evaluation_member": True,
            "evaluation_rollback_output_force_storage_is_nullable_pointer": True,
            "evaluation_rollback_output_force_storage_pointer_bound_once": True,
            "evaluation_rollback_whole_output_pointer_parameter_removed": True,
            "evaluation_rollback_whole_output_pointer_member_removed": True,
            "evaluation_rollback_output_force_storage_uses_direct_pointer_dereference": True,
            "evaluation_rollback_callsite_passes_output_force_storage_address": True,
            "evaluation_rollback_callsite_address_formation_guarded_by_nonnull_output": True,
            "evaluation_rollback_activation_uses_compute_forces_reuse_and_nonnull_output": True,
            "evaluation_rollback_provider_force_source_null_output_short_circuits_address_formation": True,
            "evaluation_rollback_output_force_storage_pointer_is_sole_activation_and_commit_sentinel": True,
            "evaluation_rollback_commit_disarms_via_output_force_storage_pointer_only": True,
            "evaluation_rollback_retained_guard_state_is_force_storage_only": True,
            ...
            "evaluation_rollback_output_force_storage_binding_performance_improvement_claimed": False,
            "evaluation_rollback_output_force_storage_pointer_performance_improvement_claimed": False,
            "evaluation_rollback_output_force_storage_object_layout_equivalence_claimed": False,
            "evaluation_rollback_output_force_storage_runtime_lifetime_enforcement_claimed": False,
            "evaluation_rollback_output_pointer_narrowing_allocation_improvement_claimed": False,
    ...
    validation.update(
        {
            ...
            "predecessor_adapter_exact_evaluation_rollback_output_force_storage_binding_transform": True,
            "evaluation_rollback_output_force_storage_binding_source_exact": True,
            "adapter_outside_evaluation_rollback_output_force_storage_regions_exact_predecessor_bytes": True,
            ...
            "rollback_output_address_formation_safely_guarded": True,
            "provider_force_source_null_output_short_circuits_before_address_formation": True,
...
def expected_predecessor_workflow() -> str:
    ...
    for predecessor_path, successor_path in zip(
        ...
        if expected.count(anchor) != 2:
            fail("exact PR 482 workflow trigger anchor drift: %s" % predecessor_path)
        expected = expected.replace(
            anchor, anchor + '      - "%s"\n' % successor_path.as_posix()
        )
    old_region = source_region(
        expected,
        "      - name: Materialize exact PR 481 target and reviewed head\n",
        "\n\n  native-linux:\n",
        "exact PR 482 immutable-evidence block",
    )
    new_region = """      - name: Materialize exact PR 482 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 8decdf9ca7129bb5669d5217e611f52860ac779c^{tree})" = "44e57663a0243d16b4805fe97c5655ee028f8c84"
          git merge-base --is-ancestor 8decdf9ca7129bb5669d5217e611f52860ac779c HEAD
          git fetch --no-tags --depth=1 origin refs/pull/482/head
          test "$(git rev-parse FETCH_HEAD)" = "3f8e3f2acfdd3cbc3514feffa17e2e74e300598c"
          test "$(git rev-parse FETCH_HEAD^{tree})" = "44e57663a0243d16b4805fe97c5655ee028f8c84"
      - name: Verify exact frozen PR 482 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=8decdf9ca7129bb5669d5217e611f52860ac779c
          frozen_tree=44e57663a0243d16b4805fe97c5655ee028f8c84
          ...
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_evaluation_rollback_force_storage_binding_profile_v1.json >/dev/null
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_evaluation_rollback_force_storage_binding_profile_v1_sources.json >/dev/null
          python3 -m pip install pytest==8.3.5
          python3 -m tools.verify_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_evaluation_rollback_force_storage_binding_v1
          python3 -m pytest -q tests/unit/test_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_evaluation_rollback_force_storage_binding_v1.py
          restore
          trap - EXIT"""
    return replace_once(
        expected, old_region, new_region, "exact PR 482 predecessor workflow transform"
    )
...
def expected_predecessor_unit() -> str:
    ...
    expected = replace_once(
        ...
        "from pathlib import Path\n",
        "from pathlib import Path\n\nimport pytest\n",
        "exact PR 482 unit pytest import",
    )
    skip = """ROOT = Path(__file__).resolve().parents[2]
PME_RUST_RECIPROCAL_PROVIDER_EVALUATION_ROLLBACK_OUTPUT_FORCE_STORAGE_BINDING_EVIDENCE_PRESENT = (
    ROOT
    / "config/engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_evaluation_rollback_output_force_storage_binding_profile_v1.json"
).is_file()
pytestmark = pytest.mark.skipif(
    PME_RUST_RECIPROCAL_PROVIDER_EVALUATION_ROLLBACK_OUTPUT_FORCE_STORAGE_BINDING_EVIDENCE_PRESENT,
    reason=(
        "evaluation rollback force-storage binding evidence is verified from its exact "
        "frozen PR 482 object after rollback output force-storage binding evidence is present"
    ),
)"""
    return replace_once(
        expected,
        ...
        skip,
        "exact PR 482 unit successor skip",
    )


def expected_successor_workflow() -> str:
    old_hyphen = "rust-reciprocal-provider-evaluation-rollback-force-storage-binding"
    new_hyphen = "rust-reciprocal-provider-evaluation-rollback-output-force-storage-binding"
    old_underscore = "rust_reciprocal_provider_evaluation_rollback_force_storage_binding"
    new_underscore = "rust_reciprocal_provider_evaluation_rollback_output_force_storage_binding"
    expected = frozen_bytes(PREDECESSOR_WORKFLOW_RELATIVE_PATH).decode()
    ...
    replacements = (
        ("Materialize exact PR 481 target", "Materialize exact PR 482 target"),
        ("214f11daf45997826f142544bb02dc6c7831b8ee", PREDECESSOR["merge_commit"]),
        ("d8360b665efa6c6292ea7a690a3839f6200e2396", PREDECESSOR["merge_tree"]),
        ("refs/pull/481/head", "refs/pull/482/head"),
        ("bae72aefcf609029c45211ecaa28de7d86d8bd4d", PREDECESSOR["reviewed_head"]),
    )
...
def require_workflow_contract(root: Path = ROOT) -> None:
    ...
    if predecessor != expected_predecessor_workflow():
        fail("PR 482 predecessor workflow is not the exact frozen-object transform")
    if successor != expected_successor_workflow():
        fail("successor workflow is not the exact PR 482-derived transform")
    expected_names = {
    ...
    for path, expected_name in expected_names.items():
        ...
        if len(pull_paths) != 246 or len(set(pull_paths)) != 246:
            fail("workflow 246-path unique pull-request trigger drift: %s" % path)
    ...
    for token in (
        ...
        if token not in predecessor:
            fail("PR 482 predecessor workflow freeze drift: %s" % token)
...
def require_predecessor_unit_freeze(root: Path = ROOT) -> None:
    ...
    if source != expected_predecessor_unit():
        fail("PR 482 predecessor unit is not the exact frozen-object transform")
    ...
    if not any("PR 482 object" in value for value in constants):
        fail("PR 482 predecessor unit frozen-object reason drift")
    if source.count("pytest.mark.skipif(") != 1:
        fail("PR 482 predecessor unit skip drift")
...
def require_adapter_contract(root: Path = ROOT) -> None:
    ...
    if sha(frozen) != EXPECTED_PREDECESSOR_IMPLEMENTATION_SHA256:
        fail("exact PR 482 adapter digest drift")
    frozen_source = frozen.decode()
    expected = replace_once(
        frozen_source,
        OLD_ROLLBACK_CLASS,
        NEW_ROLLBACK_CLASS,
        "evaluation rollback class output force-storage binding",
    )
    expected = replace_once(
        expected,
        OLD_ROLLBACK_CONSTRUCTION,
        NEW_ROLLBACK_CONSTRUCTION,
        "evaluation rollback construction output force-storage binding",
    )
    for path in IMPLEMENTATION_DELTA_PATHS:
        ...
        if raw.decode() != expected:
            fail("adapter is not the exact PR 482 output force-storage transform: %s" % path)
    ...
    if rollback.count("Evaluation *output") != 0:
        fail("whole evaluation output pointer declaration remains")
    if rollback.count("EvaluationForceStorage *output_forces") != 2:
        fail("rollback output force-storage pointer declaration count drift")
    if rollback.count("output_(output)") != 0:
        fail("whole evaluation output pointer initializer remains")
    if rollback.count(
        ": output_forces_(output_forces), candidate_forces_(candidate_forces)"
    ) != 1:
        fail("rollback output force-storage pointer initializer count drift")
    if rollback.count("output_forces_ = nullptr;") != 1:
        fail("rollback output force-storage disarm count drift")
    if rollback.count("if (output_forces_ != nullptr)") != 2:
        fail("rollback output force-storage active-guard count drift")
    if rollback.count("output_->forces") != 0:
        fail("whole evaluation output member access remains")
    if rollback.count("= delete;") != 2:
        fail("rollback copy-deletion count drift")
    if rollback.count("candidate_forces_.swap(*output_forces_);") != 2:
    ...
    if construction.count("? out_evaluation : nullptr") != 0:
        fail("whole evaluation output callsite remains")
    if construction.count("&out_evaluation->forces") != 1:
        fail("rollback output force-storage address callsite count drift")
    activation = """compute_forces && reuse_force_storage && out_evaluation != nullptr
            ? &out_evaluation->forces
            : nullptr"""
    if construction.count(activation) != 1:
        fail("rollback callsite activation predicate drift")
    if construction.count("out_evaluation != nullptr") != 1:
        fail("rollback output non-null predicate count drift")
    ...
    frozen_dispatch = source_region(
        ...
        "    const bg_status status = normalize_provider_status([&]() -> std::int32_t {\n",
        "    provider_error.detail[",
        "exact PR 482 normalization-bound dispatch",
    )
    if dispatch != frozen_dispatch:
        fail("dispatch-status normalization region changed from exact PR 482")
    ...
    if sha(frozen_bytes(ADAPTER_TEST_RELATIVE_PATH)) != EXPECTED_PREDECESSOR_ADAPTER_TEST_SHA256:
        fail("exact PR 482 native adapter test digest drift")
    if sha(test_raw) != EXPECTED_PREDECESSOR_ADAPTER_TEST_SHA256:
        fail("native adapter test changed from exact PR 482 bytes")


def require_docs_contract(root: Path = ROOT) -> None:
    doc = (root / DOC_RELATIVE_PATH).read_text()
    required = (
        "exact PR 482 predecessor",
        "`using EvaluationForceStorage = decltype(Evaluation::forces)`",
        "`EvaluationForceStorage *output_forces_`",
        "`EvaluationForceStorage &candidate_forces_`",
        "passes `&out_evaluation->forces`",
        "`compute_forces && reuse_force_storage && out_evaluation != nullptr`",
        "short-circuits before the address is formed",
        "sole activation and commit sentinel",
        "does not provide runtime lifetime enforcement",
        "guard's retained state is force-storage-only",
