...
CASE_FIELDS = {
    "case_id", "preparation_success", "preparation_failure_code",
    "scored_candidate_count", "typed_failure_count",
    "proposal_oracle_recovered", "valid_proposal_oracle_recovered",
    "top1_recovered", "top5_recovered", "top1_valid", "top1_lane",
    "top1_slot_index", "top1_final_rmsd_angstrom",
    "best_final_rmsd_angstrom", "scoring_regret_angstrom", "source_sha256",
}
AGG_FIELDS = {
    "case_count", "preparation_success_count", "preparation_failure_count",
    "scored_case_count", "proposal_oracle_recovery_count",
    "valid_proposal_oracle_recovery_count", "top1_recovery_count",
    "top5_recovery_count", "invalid_top1_count",
    "top1_validity_unavailable_count", "mean_scoring_regret_angstrom",
    "preparation_failure_distribution", "typed_failure_distribution",
    "lane_contribution",
}
LANE_FIELDS = {
    "candidate_count", "typed_failure_count", "scored_count",
    "proposal_oracle_candidate_count", "valid_proposal_oracle_candidate_count",
    "final_native_like_candidate_count", "exact_valid_candidate_count",
    "top1_case_count", "top5_native_like_candidate_count",
}
METRICS = (
    "proposal_oracle_recovered", "valid_proposal_oracle_recovered",
    "top1_recovered", "top5_recovered",
)
COMPARE_FIELDS = {
    f"{direction}_{metric}_case_ids"
    for metric in METRICS for direction in ("new", "lost")
}
AUTH_FIELDS = {
    "reservation_authorized", "molecular_holdout_execution_authorized",
    "fresh_128_execution_authorized", "stage0_admission_authorized",
    "benchmark_claim_authorized", "scientific_claim_authorized",
    "product_authorized", "customer_pose_emission_authorized",
}
REPORT_FIELDS = {
    "schema_id", "profile_id", "profile_sha256",
    "profile_projection_sha256", "manifest_sha256",
    "manifest_projection_sha256", "fresh_registry_sha256",
    "development_repeatable", "result_informed_iteration_allowed",
    "candidate_denominator", "rmsd_threshold_angstrom", "current", "baseline",
...
def optional_boolean(value: Any, name: str) -> bool | None:
...
def finite(value: Any, name: str, lo: float = -math.inf) -> float:
...
def optional_finite(value: Any, name: str, lo: float = -math.inf) -> float | None:
    return None if value is None else finite(value, name, lo)
...
def case_id(value: Any, name: str) -> str:
...
def verify_case(raw: Any, index: int) -> dict[str, Any]:
    row = exact_object(raw, CASE_FIELDS, f"case[{index}]")
    cid = case_id(row["case_id"], f"case[{index}].case_id")
    prepared = boolean(row["preparation_success"], f"{cid}.preparation_success")
    scored = integer(row["scored_candidate_count"], f"{cid}.scored", 0, 64)
    failed = integer(row["typed_failure_count"], f"{cid}.failed", 0, 64)
    proposal = boolean(row["proposal_oracle_recovered"], f"{cid}.proposal")
    valid_proposal = boolean(
        row["valid_proposal_oracle_recovered"], f"{cid}.valid_proposal"
    )
    top1 = boolean(row["top1_recovered"], f"{cid}.top1")
    top5 = boolean(row["top5_recovered"], f"{cid}.top5")
    top1_valid = optional_boolean(row["top1_valid"], f"{cid}.top1_valid")
    top1_rmsd = optional_finite(
        row["top1_final_rmsd_angstrom"], f"{cid}.top1_rmsd", 0.0
    )
    best_rmsd = optional_finite(
        row["best_final_rmsd_angstrom"], f"{cid}.best_rmsd", 0.0
    )
    regret = optional_finite(row["scoring_regret_angstrom"], f"{cid}.regret", 0.0)
    sha(row["source_sha256"], f"{cid}.source_sha256")
    ...
    if not prepared:
        text(row["preparation_failure_code"], f"{cid}.failure_code")
        ...
        if any(row[key] is not None for key in (
            "top1_valid", "top1_lane", "top1_slot_index",
            "top1_final_rmsd_angstrom", "best_final_rmsd_angstrom",
        ...
        if scored == 0:
            ...
            text(row["top1_lane"], f"{cid}.top1_lane", 128)
            integer(row["top1_slot_index"], f"{cid}.top1_slot", 0, 63)
            ...
            if best_rmsd > top1_rmsd + 1e-12:
                raise D1ReportVerificationError(f"{cid}: best RMSD exceeds Top-1")
            if not math.isclose(
                regret, top1_rmsd - best_rmsd, rel_tol=0.0, abs_tol=1e-12
            ):
...
def distribution(value: Any, name: str) -> dict[str, int]:
    if type(value) is not dict or list(value) != sorted(value):
        raise D1ReportVerificationError(f"{name} must be a sorted object")
    result = {}
    for key, raw in value.items():
        result[text(key, f"{name}.key")] = integer(raw, f"{name}.{key}", 1, 2048)
    return result


def verify_lanes(
    value: Any, prepared: int, total_scored: int, total_failed: int, scored_cases: int
) -> None:
    if type(value) is not dict or list(value) != sorted(value):
    ...
    for lane, raw in value.items():
        ...
        counts = {
            key: integer(number, f"{lane}.{key}", 1, 2048)
            for key, number in raw.items()
        }
        totals.update(counts)
        if counts.get("scored_count", 0) + counts.get("typed_failure_count", 0) != counts["candidate_count"]:
            raise D1ReportVerificationError(f"lane {lane} denominator mismatch")
        if counts.get("valid_proposal_oracle_candidate_count", 0) > counts.get("proposal_oracle_candidate_count", 0):
            raise D1ReportVerificationError(f"lane {lane} valid proposal mismatch")
        for key in LANE_FIELDS - {"candidate_count", "scored_count", "typed_failure_count"}:
...
def verify_summary(raw: Any, name: str) -> dict[str, Any]:
    summary = exact_object(raw, {"cases", "aggregate"}, name)
    if type(summary["cases"]) is not list or len(summary["cases"]) != 32:
        raise D1ReportVerificationError(f"{name}: 32 case rows required")
    cases = [verify_case(row, index) for index, row in enumerate(summary["cases"])]
    ids = [row["case_id"] for row in cases]
    if len(set(ids)) != 32:
        raise D1ReportVerificationError(f"{name}: duplicate case IDs")
    aggregate = exact_object(summary["aggregate"], AGG_FIELDS, f"{name}.aggregate")
    ...
    expected = {
        ...
        "proposal_oracle_recovery_count": sum(row["proposal_oracle_recovered"] for row in cases),
        "valid_proposal_oracle_recovery_count": sum(row["valid_proposal_oracle_recovered"] for row in cases),
        "top1_recovery_count": sum(row["top1_recovered"] for row in cases),
        "top5_recovery_count": sum(row["top5_recovered"] for row in cases),
        "invalid_top1_count": sum(row["top1_valid"] is False for row in scored_cases),
        "top1_validity_unavailable_count": sum(row["top1_valid"] is None for row in scored_cases),
    }
    for key, wanted in expected.items():
        if integer(aggregate[key], f"{name}.{key}", 0, 32) != wanted:
            raise D1ReportVerificationError(f"{name}.{key} case-row mismatch")
    regrets = [row["scoring_regret_angstrom"] for row in scored_cases]
    observed_mean = optional_finite(
    ...
    if (wanted_mean is None) != (observed_mean is None) or (
        ...
        and not math.isclose(wanted_mean, observed_mean, rel_tol=0.0, abs_tol=1e-12)
    ):
    ...
    wanted_prep = dict(sorted(Counter(
        row["preparation_failure_code"] for row in cases
        if not row["preparation_success"]
    ).items()))
    if prep != wanted_prep:
        raise D1ReportVerificationError(f"{name}: preparation distribution mismatch")
    typed = distribution(aggregate["typed_failure_distribution"], f"{name}.typed")
    ...
    verify_lanes(
        aggregate["lane_contribution"], len(prepared), total_scored,
        total_failed, len(scored_cases)
    )
    return {"cases": cases, "aggregate": aggregate}


def metric_ids(summary: dict[str, Any], metric: str) -> set[str]:
    return {row["case_id"] for row in summary["cases"] if row[metric] is True}


def verify_baseline(raw: Any, current: dict[str, Any]) -> None:
    if raw is None:
        return
    ...
    sha(baseline["manifest_sha256"], "baseline.manifest_sha256")
    sha(baseline["manifest_projection_sha256"], "baseline.projection_sha256")
    other = verify_summary(baseline["summary"], "baseline.summary")
    ...
    for metric in METRICS:
        now, before = metric_ids(current, metric), metric_ids(other, metric)
        for direction, wanted in (
            ("new", sorted(now - before)), ("lost", sorted(before - now))
        ):
            key = f"{direction}_{metric}_case_ids"
            values = comparison[key]
            if type(values) is not list:
                raise D1ReportVerificationError(f"{key} must be list")
            observed = [case_id(value, key) for value in values]
...
def verify_report(path: Path) -> dict[str, Any]:
    ...
    exact_object(report, REPORT_FIELDS, "report")
    ...
    for key in (
        "profile_sha256", "profile_projection_sha256", "manifest_sha256",
        "manifest_projection_sha256", "fresh_registry_sha256", "report_sha256",
    ):
        sha(report[key], key)
    if report["candidate_denominator"] != 64:
        raise D1ReportVerificationError("candidate denominator changed")
    if finite(report["rmsd_threshold_angstrom"], "rmsd_threshold", 0.0) != 2.0:
        raise D1ReportVerificationError("RMSD threshold changed")
    if report["development_repeatable"] is not True or report["result_informed_iteration_allowed"] is not True:
        raise D1ReportVerificationError("development policy changed")
    authority = exact_object(report["authority"], AUTH_FIELDS, "authority")
    if any(value is not False for value in authority.values()):
        raise D1ReportVerificationError("authority escalated")
    current = verify_summary(report["current"], "current")
    verify_baseline(report["baseline"], current)
