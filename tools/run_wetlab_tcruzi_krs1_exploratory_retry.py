...
from tools.wetlab import build_wetlab_tcruzi_krs1_exploratory_retry_lane as lane_mod
from tools import run_wetlab_broad_screen_primary_runner as primary_runner_mod
from tools.wetlab_target_render_utils import load_json, write_artifact

TARGET_ID = "T. cruzi KRS1"
DEFAULT_LANE_JSON = "runs/wetlab_tcruzi_krs1_exploratory_retry_lane_current.json"
...
DEFAULT_OUT_MD = "runs/wetlab_tcruzi_krs1_exploratory_retry_runner_current.md"
...
def run(
    *,
    ...
    if refresh_lane:
        ...
        write_artifact(lane_json.replace(".json", ".md"), "Wet-Lab T. cruzi KRS1 Exploratory Retry Lane", lane_payload)
    else:
    ...
    if not bool(summary.get("ready_for_manual_retry", False)):
        raise SystemExit("T. cruzi KRS1 exploratory retry lane is not ready_for_manual_retry")
    if not selected_shard:
        raise SystemExit("no shard_id available for T. cruzi KRS1 exploratory retry")
    if not selected_kind:
        raise SystemExit("no selected command kind available for T. cruzi KRS1 exploratory retry")
    ...
    payload = {
        "summary": {
            "status": "wetlab_tcruzi_krs1_exploratory_retry_runner_ready",
            "target_id": target_id,
            "shard_id": selected_shard,
    ...
    write_artifact(DEFAULT_OUT_MD, "Wet-Lab T. cruzi KRS1 Exploratory Retry Runner", payload)
    return payload


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch the T. cruzi KRS1 exploratory observed-band retry lane.")
