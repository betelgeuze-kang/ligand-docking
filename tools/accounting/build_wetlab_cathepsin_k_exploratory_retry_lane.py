...
DEFAULT_OUT_MD = "runs/wetlab_cathepsin_k_exploratory_retry_lane_current.md"
...
def build_payload(
    ...
    status = "wetlab_cathepsin_k_exploratory_retry_lane_running" if running else "wetlab_cathepsin_k_exploratory_retry_lane_ready"
    next_step = (
        ...
        else f"Run the {TARGET_ID} exploratory gate4.5 {phase} retry for {shard_id}; use gate4.5 as the immediately runnable family for the observed {float(tuning_summary.get('recommended_observed_threshold_A', 0.0) or 0.0):.2f}A band and keep the default lane closed until the result is reviewed."
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Cathepsin K exploratory retry lane.")
...
def main() -> None:
    ...
    write_artifact(
        args.out_md,
        "Wet-Lab Cathepsin K Exploratory Retry Lane",
        build_payload(
