...
DEFAULT_REVIEW_JSON = "runs/idp_3bead_release_cleanup_review_manifest_current.json"
DEFAULT_OUT_JSON = "runs/idp_3bead_release_archive_first_manifest_current.json"
DEFAULT_OUT_CSV = "runs/idp_3bead_release_archive_first_manifest_current.csv"
DEFAULT_OUT_MD = "runs/idp_3bead_release_archive_first_manifest_current.md"
...
def build_payload(review_json: str) -> dict[str, Any]:
    ...
    summary = {
        "status": "idp_3bead_release_archive_first_manifest_ready",
        ...
        "next_required_step": "Archive these stale release-smoke prefixes as a batch, then compress the archive root to recover actual disk space.",
    }
...
def _write_markdown(path: Path, payload: dict[str, Any]) -> None:
    s = payload["summary"]
    lines = [
        "# IDP 3-Bead Release Archive-First Manifest",
        "",
        f"- status: `{s['status']}`",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build an archive-first manifest for stale idp_3bead_release prefixes.")
