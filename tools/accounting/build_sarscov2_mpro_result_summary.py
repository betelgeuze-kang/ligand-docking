...
DEFAULT_OUT_MD = "runs/sarscov2_mpro_result_summary_current.md"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write the canonical SARS-CoV-2 Mpro result-summary artifact.")
...
def build_payload(launch_payload: dict, go_no_go_payload: dict, status: str = "not_ready", decision_case: str = "", action: str = "", started_at: str = "", updated_at: str = "", completed_at: str = "", notes: str = "") -> dict:
    ...
    return build_result_summary_payload(
        ...
        launch_artifact="runs/sarscov2_mpro_launch_packet_current.md",
        launch_status=str(launch_s.get("status", "")).strip(),
        go_no_go_artifact="runs/sarscov2_mpro_go_no_go_card_current.md",
...
def main() -> None:
    ...
    write_artifact(DEFAULT_OUT_MD, "SARS-CoV-2 Mpro Result Summary", payload)
