...
from tools.accounting.build_npz_dynamics_cleanup_manifest import DEFAULT_OUT_JSON as DEFAULT_MANIFEST_JSON
from tools.accounting.build_storage_retention_manifest import _display, _human_size, _resolve
from tools.builder_table_utils import write_csv_rows

ROOT = Path(__file__).resolve().parents[2]
DEFAULT_OUT_JSON = "runs/npz_dynamics_cleanup_execution_current.json"
DEFAULT_OUT_CSV = "runs/npz_dynamics_cleanup_execution_current.csv"
DEFAULT_OUT_MD = "runs/npz_dynamics_cleanup_execution_current.md"
APPROVAL_TOKEN = "APPROVE_NPZ_DYNAMICS_CLEANUP"

CLAIM_BOUNDARY = (
    "NPZ/dynamics cleanup execution only deletes rows marked delete_recommended=true in the generated "
    "cleanup manifest, and only when --execute and the approval token are supplied. It does not delete "
    "referenced keep rows, review-required rows, git history, source code, models, CASP17 evidence roots, "
...
def apply_npz_dynamics_cleanup_manifest(
    *,
    root: str | Path = ROOT,
    ...
    failed_rows = [row for row in rows if row["status"] in {"delete_failed", "blocked_unsafe_path", "blocked_not_file"}]
    ...
    summary = {
        "packet_type": "npz_dynamics_cleanup_execution",
        "status": "npz_dynamics_cleanup_execution_complete" if delete_authorized and not failed_rows else "npz_dynamics_cleanup_execution_ready",
...
def _write_markdown(path_like: str | Path, payload: dict[str, Any], *, root: Path) -> None:
    ...
    s = payload["summary"]
    lines = [
        "# NPZ Dynamics Cleanup Execution",
        "",
        f"- status: `{s['status']}`",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apply an approval-gated NPZ/dynamics cleanup manifest.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    root = Path(args.root)
    payload = apply_npz_dynamics_cleanup_manifest(
        root=root,
