...
def _product_ai_architecture_lane(
    gap_path: str,
    gap_packet: dict[str, Any],
    backlog_path: str,
    backlog_packet: dict[str, Any],
) -> dict[str, Any]:
    gap = _summary(gap_packet)
    backlog = _summary(backlog_packet)
    all_gaps_closed = bool(gap.get("all_gaps_closed") is True)
    ...
    lane = _lane(
        ...
        path=f"{gap_path};{backlog_path}",
        packet=gap_packet if gap_packet and backlog_packet else {},
        observed_status=(
            f"{_text(gap.get('status')) or 'missing'};"
            f"all_gaps_closed={all_gaps_closed};"
    ...
    lane["open_gap_count"] = open_gap_count
...
def build_rollup(
    *,
    ...
    if product_ai_gate_present:
        rows.append(
            _product_ai_architecture_lane(
    ...
    summary = {
        ...
        "product_ai_architecture_gate_present": product_ai_gate_present,
        "product_ai_architecture_ready": (
            next((row for row in rows if row["lane_id"] == "product_ai_architecture"), {}).get("lane_status") == "evidence_ready"
            if product_ai_gate_present
            else None
        ),
        "product_ai_architecture_open_gap_count": _int(
            next((row for row in rows if row["lane_id"] == "product_ai_architecture"), {}).get("open_gap_count")
        ),
