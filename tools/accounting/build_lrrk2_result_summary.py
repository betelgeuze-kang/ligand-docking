...
DEFAULT_OUT_MD = 'runs/lrrk2_result_summary_current.md'


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Write the canonical LRRK2 result-summary artifact.')
...
def build_payload(launch_payload: dict, go_no_go_payload: dict, status: str = 'not_ready', decision_case: str = '', action: str = '', started_at: str = '', updated_at: str = '', completed_at: str = '', notes: str = '') -> dict:
    ...
    return build_result_summary_payload(
        ...
        launch_artifact='runs/lrrk2_launch_packet_current.md',
        launch_status=str(launch_s.get('status', '')).strip(),
        go_no_go_artifact='runs/lrrk2_go_no_go_card_current.md',
...
if __name__ == '__main__':
    args = parse_args()
    write_artifact(DEFAULT_OUT_MD, 'LRRK2 Result Summary', build_payload(load_json(args.launch_json), load_json(args.go_no_go_json), status=args.status, decision_case=args.decision_case, action=args.action, started_at=args.started_at, updated_at=args.updated_at, completed_at=args.completed_at, notes=args.notes))
