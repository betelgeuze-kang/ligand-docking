...
DEFAULT_OUTBOUND_EMAIL_DRAFT_JSON = "runs/cameo_outbound_email_draft_current.json"
DEFAULT_OUTBOUND_EMAIL_SEND_PREFLIGHT_JSON = "runs/cameo_outbound_email_send_preflight_current.json"
DEFAULT_OFFICIAL_RESULT_FETCH_PREFLIGHT_JSON = "runs/cameo_official_result_fetch_preflight_current.json"
...
def build_cameo_validation_operations_dossier(
    *,
    ...
    outbound_email_draft_packet: dict[str, Any] | None = None,
    outbound_email_send_preflight_packet: dict[str, Any] | None = None,
    official_result_fetch_preflight_packet: dict[str, Any] | None = None,
    ...
    outbound_email_draft_path: str = DEFAULT_OUTBOUND_EMAIL_DRAFT_JSON,
    outbound_email_send_preflight_path: str = DEFAULT_OUTBOUND_EMAIL_SEND_PREFLIGHT_JSON,
    official_result_fetch_preflight_path: str = DEFAULT_OFFICIAL_RESULT_FETCH_PREFLIGHT_JSON,
) -> dict[str, Any]:
    ...
    outbound_email_draft = _summary(outbound_email_draft_packet or {})
    outbound_email_send_preflight = _summary(outbound_email_send_preflight_packet or {})
    official_result_fetch_preflight = _summary(official_result_fetch_preflight_packet or {})
    ...
    outbound_email_draft_status = _text(outbound_email_draft.get("status"))
    outbound_email_draft_ready = (
        outbound_email_draft_status == "cameo_outbound_email_draft_ready"
        and _bool(outbound_email_draft.get("draft_eml_written"))
        and not _bool(outbound_email_draft.get("email_sent"))
        and not _bool(outbound_email_draft.get("smtp_connection_opened"))
        and not _bool(outbound_email_draft.get("outbound_email_enabled"))
        and not _bool(outbound_email_draft.get("external_state_mutated"))
    )
    outbound_email_send_preflight_status = _text(outbound_email_send_preflight.get("status"))
    outbound_email_send_preflight_ready = (
        outbound_email_send_preflight_status == "cameo_outbound_email_send_preflight_ready"
        and _bool(outbound_email_send_preflight.get("authorized_for_separate_operator_send"))
        and not _bool(outbound_email_send_preflight.get("email_sent"))
        and not _bool(outbound_email_send_preflight.get("smtp_connection_opened"))
        and not _bool(outbound_email_send_preflight.get("outbound_email_enabled"))
        and not _bool(outbound_email_send_preflight.get("external_state_mutated"))
    )
    official_result_fetch_preflight_status = _text(official_result_fetch_preflight.get("status"))
    official_result_fetch_preflight_ready = (
        official_result_fetch_preflight_status == "cameo_official_result_fetch_preflight_ready"
        and _bool(official_result_fetch_preflight.get("authorized_for_separate_operator_fetch"))
        and not _bool(official_result_fetch_preflight.get("network_request_opened"))
        and not _bool(official_result_fetch_preflight.get("official_results_fetched"))
        and not _bool(official_result_fetch_preflight.get("native_local_accuracy_used"))
        and not _bool(official_result_fetch_preflight.get("external_state_mutated"))
    )
    ...
    rows = [
        _row(
            ...
            recommended_action="Fill or repair operator CSV rows, then rerun cameo/build_cameo_operator_input_validation.py.",
            reason=(
        ...
        _row(
            ...
            status="ready" if official_result_fetch_preflight_ready else "blocked",
            source_status=official_result_fetch_preflight_status or "missing",
            blocker_count=(
                _int(official_result_fetch_preflight.get("blocker_count"))
                if official_result_fetch_preflight_packet
                else 1
            ),
            required_input="operator-approved official CAMEO result URL metadata for a separate no-claim fetch/retrieval step",
            source_artifact=official_result_fetch_preflight_path,
            recommended_action="Fill the fetch preflight CSV, then perform any official result retrieval as a separate operator-run step.",
            reason=(
                f"authorized_for_separate_operator_fetch={_bool(official_result_fetch_preflight.get('authorized_for_separate_operator_fetch'))}, "
                f"network_request_opened={_bool(official_result_fetch_preflight.get('network_request_opened'))}, "
                f"official_results_fetched={_bool(official_result_fetch_preflight.get('official_results_fetched'))}, "
                f"native_local_accuracy_used={_bool(official_result_fetch_preflight.get('native_local_accuracy_used'))}."
            ),
        ...
        _row(
            priority=8,
            stage="outbound_email_draft",
            status="ready" if outbound_email_draft_ready else "blocked",
            source_status=outbound_email_draft_status or "missing",
            blocker_count=_int(outbound_email_draft.get("blocker_count")) if outbound_email_draft_packet else 1,
            required_input="local .eml draft assembled from dry-run handoff attachments with SMTP/email send disabled",
            source_artifact=outbound_email_draft_path,
            recommended_action="Inspect the local .eml draft; actual outbound email remains approval-gated.",
            reason=(
                f"draft_eml_written={_bool(outbound_email_draft.get('draft_eml_written'))}, "
                f"email_sent={_bool(outbound_email_draft.get('email_sent'))}, "
                f"smtp_connection_opened={_bool(outbound_email_draft.get('smtp_connection_opened'))}, "
                f"outbound_email_enabled={_bool(outbound_email_draft.get('outbound_email_enabled'))}."
            ),
            official_result_required=official_result_required,
        ),
        _row(
            priority=9,
            stage="outbound_email_send_preflight",
            status="ready" if outbound_email_send_preflight_ready else "blocked",
            source_status=outbound_email_send_preflight_status or "missing",
            blocker_count=(
                _int(outbound_email_send_preflight.get("blocker_count")) if outbound_email_send_preflight_packet else 1
            ),
            required_input="operator-approved send metadata and SMTP profile for a separate send step, with no SMTP connection opened here",
            source_artifact=outbound_email_send_preflight_path,
            recommended_action="Fill the send preflight operator CSV after registration/email approval; actual email remains a separate operator-run action.",
            reason=(
                f"authorized_for_separate_operator_send={_bool(outbound_email_send_preflight.get('authorized_for_separate_operator_send'))}, "
                f"smtp_connection_opened={_bool(outbound_email_send_preflight.get('smtp_connection_opened'))}, "
                f"email_sent={_bool(outbound_email_send_preflight.get('email_sent'))}, "
                f"outbound_email_enabled={_bool(outbound_email_send_preflight.get('outbound_email_enabled'))}."
            ),
            official_result_required=official_result_required,
        ),
        _row(
    ...
    native_local_accuracy_used = any(
        ...
        for packet in [
            ...
            outbound_email_draft_packet or {},
            outbound_email_send_preflight_packet or {},
            official_result_fetch_preflight_packet or {},
        ]
    )
    ...
    summary = {
        ...
        "official_result_fetch_preflight_status": official_result_fetch_preflight_status,
        "official_result_fetch_preflight_ready": official_result_fetch_preflight_ready,
        "official_result_fetch_preflight_authorized": _bool(
            official_result_fetch_preflight.get("authorized_for_separate_operator_fetch")
        ),
        "official_result_fetch_preflight_network_request_opened": _bool(
            official_result_fetch_preflight.get("network_request_opened")
        ),
        "official_result_fetch_preflight_results_fetched": _bool(
            official_result_fetch_preflight.get("official_results_fetched")
        ),
        "official_result_fetch_preflight_artifact": official_result_fetch_preflight_path,
        ...
        "outbound_email_draft_status": outbound_email_draft_status,
        "outbound_email_draft_ready": outbound_email_draft_ready,
        "outbound_email_draft_eml_written": _bool(outbound_email_draft.get("draft_eml_written")),
        "outbound_email_draft_email_sent": _bool(outbound_email_draft.get("email_sent")),
        "outbound_email_draft_smtp_connection_opened": _bool(outbound_email_draft.get("smtp_connection_opened")),
        "outbound_email_draft_artifact": outbound_email_draft_path,
        "outbound_email_send_preflight_status": outbound_email_send_preflight_status,
        "outbound_email_send_preflight_ready": outbound_email_send_preflight_ready,
        "outbound_email_send_preflight_authorized": _bool(
            outbound_email_send_preflight.get("authorized_for_separate_operator_send")
        ),
        "outbound_email_send_preflight_email_sent": _bool(outbound_email_send_preflight.get("email_sent")),
        "outbound_email_send_preflight_smtp_connection_opened": _bool(
            outbound_email_send_preflight.get("smtp_connection_opened")
        ),
        "outbound_email_send_preflight_artifact": outbound_email_send_preflight_path,
...
def _write_markdown(path_like: str | Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- official_result_fetch_preflight_status: `{s['official_result_fetch_preflight_status']}`",
        f"- official_result_fetch_preflight_ready: `{s['official_result_fetch_preflight_ready']}`",
        f"- official_result_fetch_preflight_authorized: `{s['official_result_fetch_preflight_authorized']}`",
        f"- official_result_fetch_preflight_network_request_opened: `{s['official_result_fetch_preflight_network_request_opened']}`",
        f"- official_result_fetch_preflight_results_fetched: `{s['official_result_fetch_preflight_results_fetched']}`",
        ...
        f"- outbound_email_draft_status: `{s['outbound_email_draft_status']}`",
        f"- outbound_email_draft_ready: `{s['outbound_email_draft_ready']}`",
        f"- outbound_email_draft_eml_written: `{s['outbound_email_draft_eml_written']}`",
        f"- outbound_email_draft_email_sent: `{s['outbound_email_draft_email_sent']}`",
        f"- outbound_email_draft_smtp_connection_opened: `{s['outbound_email_draft_smtp_connection_opened']}`",
        f"- outbound_email_send_preflight_status: `{s['outbound_email_send_preflight_status']}`",
        f"- outbound_email_send_preflight_ready: `{s['outbound_email_send_preflight_ready']}`",
        f"- outbound_email_send_preflight_authorized: `{s['outbound_email_send_preflight_authorized']}`",
        f"- outbound_email_send_preflight_email_sent: `{s['outbound_email_send_preflight_email_sent']}`",
        f"- outbound_email_send_preflight_smtp_connection_opened: `{s['outbound_email_send_preflight_smtp_connection_opened']}`",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--outbound-email-draft-json", default=DEFAULT_OUTBOUND_EMAIL_DRAFT_JSON)
    parser.add_argument("--outbound-email-send-preflight-json", default=DEFAULT_OUTBOUND_EMAIL_SEND_PREFLIGHT_JSON)
    parser.add_argument("--official-result-fetch-preflight-json", default=DEFAULT_OFFICIAL_RESULT_FETCH_PREFLIGHT_JSON)
...
def main(argv: list[str] | None = None) -> None:
    ...
    payload = build_cameo_validation_operations_dossier(
        ...
        outbound_email_draft_packet=_read_json_if_present(args.outbound_email_draft_json),
        outbound_email_send_preflight_packet=_read_json_if_present(args.outbound_email_send_preflight_json),
        official_result_fetch_preflight_packet=_read_json_if_present(args.official_result_fetch_preflight_json),
        ...
        outbound_email_draft_path=args.outbound_email_draft_json,
        outbound_email_send_preflight_path=args.outbound_email_send_preflight_json,
        official_result_fetch_preflight_path=args.official_result_fetch_preflight_json,
    )
