...
DEFAULT_GO_NO_GO_JSON = "runs/dpre1_go_no_go_card_current.json"
DEFAULT_PROGRESS_JSON = "runs/dpre1_live_progress_current.json"
DEFAULT_RESULT_JSON = "runs/dpre1_result_summary_current.json"
...
DEFAULT_OUT_MD = "runs/dpre1_run_record_current.md"
...
def _resolve_run_state(requested_state: str, upstream_gate_open: bool) -> str:
    ...
    if not upstream_gate_open and requested_state in {RUN_STATE_READY, RUN_STATE_RUNNING, RUN_STATE_RESULT_READY, RUN_STATE_EXPLICIT_HOLD}:
        raise ValueError("DprE1 run state cannot advance past blocked_on_previous_review while the upstream Dengue gate is still closed.")
...
def _queue_status_for_run_state(run_state: str) -> str:
    ...
    raise ValueError(f"Unsupported DprE1 run state: {run_state}")
...
def build_payload(
    ...
    upstream_gate_open = bool(review_s.get("dpre1_gate_open", review_s.get("execution_gate_open", False)))
    upstream_gate_state = str(review_s.get("dpre1_review_state", review_s.get("queue_status_now", ""))).strip() or RUN_STATE_BLOCKED
    ...
    guarded_next_step = _first_text(guarded_s, "next_required_step") or _first_text(lane_s, "next_required_step")
    guarded_selected_command = _first_text(guarded_s, "selected_command_kind") or _first_text(lane_s, "selected_command_kind")
    try:
    ...
    queue_status_now = "blocked_pending_guarded_stage6_review" if guarded_active else _queue_status_for_run_state(execution_state)
    ...
    successor_gate_open = False if guarded_active else result_review_ready
    successor_gate_state = "blocked_pending_dpre1_guarded_review" if guarded_active else "open_for_tcruzi_krs1_execution" if successor_gate_open else "blocked_until_dpre1_result_ready_or_explicit_hold"
    successor_next_queue_state = "blocked_on_previous_review" if guarded_active else "ready_after_previous_review" if successor_gate_open else "blocked_on_previous_review"

    if guarded_active and guarded_next_step:
        next_required_step = guarded_next_step
    elif successor_gate_open:
        next_required_step = "DprE1 now satisfies the successor-open rule. T. cruzi KRS1 may move into the ready_after_previous_review slot on the next wave2 refresh."
    elif execution_state == RUN_STATE_RUNNING:
        next_required_step = "Keep DprE1 as the active Wave 2 slot and leave T. cruzi KRS1 blocked until this run reaches result_ready or explicit_hold."
    elif execution_state == RUN_STATE_READY:
        next_required_step = "Launch DprE1 from the third Wave 2 slot now, then use the resulting outcome to open the T. cruzi KRS1 successor gate."
    else:
        next_required_step = "Keep DprE1 blocked until the Dengue release gate opens, and keep T. cruzi KRS1 prep-only until DprE1 later reaches result_ready or explicit_hold."

    rows = [
        {"checkpoint_kind": "launch_packet", "artifact_path": "runs/dpre1_launch_packet_current.md", "checkpoint_state": str(launch_s.get("status", "")).strip(), "queue_effect": "fix_third_wave2_serialized_slot"},
        {"checkpoint_kind": "upstream_gate_review", "artifact_path": "runs/dpre1_result_review_current.md", "checkpoint_state": upstream_gate_state, "queue_effect": "open_only_after_dengue_resolution"},
        {"checkpoint_kind": "live_progress", "artifact_path": "runs/dpre1_live_progress_current.md", "checkpoint_state": progress_status or "not_detected", "queue_effect": active_stage_label or "no_live_progress_artifact"},
        {"checkpoint_kind": "result_summary", "artifact_path": "runs/dpre1_result_summary_current.md", "checkpoint_state": result_status or "not_detected", "queue_effect": "capture_result_ready_or_explicit_hold"},
        {"checkpoint_kind": "go_no_go_card", "artifact_path": "runs/dpre1_go_no_go_card_current.md", "checkpoint_state": str(go_s.get("status", "")).strip(), "queue_effect": "decision_context_frozen"},
        {"checkpoint_kind": "run_execution", "artifact_path": "runs/dpre1_run_record_current.md", "checkpoint_state": execution_state, "queue_effect": queue_status_now},
        {"checkpoint_kind": "successor_gate", "artifact_path": "runs/tcruzi_krs1_launch_packet_current.md", "checkpoint_state": successor_gate_state, "queue_effect": "open_only_after_dpre1_guarded_review_resolution" if guarded_active else "open_only_after_dpre1_result_ready_or_explicit_hold"},
    ]

    return {"summary": {
        "status": "dpre1_run_record_ready",
        "target_id": "DprE1",
        ...
        "dpre1_review_status": str(review_s.get("status", "")).strip(),
        ...
        "current_stage": "guarded_stage6_review_pending" if guarded_active else "manual_review_hold" if explicit_hold else "result_review_complete" if result_review_ready else active_stage_label if run_started else "launch_packet_frozen_pending_execution",
        "started_at": started_at,
        "last_update_at": last_update_at,
        "completed_at": completed_at,
        "successor_target": str(successor_s.get("target_id", "T. cruzi KRS1")).strip() or "T. cruzi KRS1",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the DprE1 run-record surface.")
...
def main() -> None:
    ...
    write_artifact(DEFAULT_OUT_MD, "DprE1 Run Record", payload)
