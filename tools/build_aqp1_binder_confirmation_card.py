...
DEFAULT_COMMIT_PACKET_JSON = "runs/aqp1_manual_verdict_commit_packet_current.json"
DEFAULT_UPDATE_SHEET_JSON = "runs/aqp1_binder_verdict_update_sheet_current.json"
DEFAULT_OUT_JSON = "runs/aqp1_binder_confirmation_card_current.json"
DEFAULT_OUT_CSV = "runs/aqp1_binder_confirmation_card_current.csv"
DEFAULT_OUT_MD = "runs/aqp1_binder_confirmation_card_current.md"
        ...
        rows.append(
            {
                ...
                "review_focus": str(row.get("review_focus", "")).strip(),
                "confirm_fields": "manual_verdict_update,manual_confidence_update,manual_decision_note",
                "staged_manual_verdict": str(row.get("commit_value_verdict", "")).strip(),
                "staged_manual_confidence_update": str(row.get("commit_value_confidence", "")).strip(),
                "staged_manual_decision_note": str(row.get("commit_value_note", "")).strip(),
                "commit_value_note_short": str(row.get("commit_value_note", "")).strip(),
                ...
                "authoritative_commit_allowed": str(row.get("authoritative_commit_allowed", "no")).strip(),
            }
        )
    summary = {
        "target_id": "AQP1",
        ...
        "next_required_step": "Use this card to confirm the exact AQP1 binder values to copy into manual_* fields. Keep all rows non-authoritative and stop if any evidence would reopen authoritative apply.",
    }
...
def _write_markdown(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "## Binder Rows",
        "",
        "| priority_rank | packet_step | update_sheet_row_ref | candidate_name | source_anchor_short | staged_manual_verdict | staged_manual_confidence_update | confirm_fields | stop_condition |",
        "| ---: | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for row in payload["rows"]:
        lines.append(
            ...
            f"`{row['confirm_fields']}` | `{row['stop_condition']}` |"
        )
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build an AQP1-only binder confirmation card from the commit packet and update sheet.")
