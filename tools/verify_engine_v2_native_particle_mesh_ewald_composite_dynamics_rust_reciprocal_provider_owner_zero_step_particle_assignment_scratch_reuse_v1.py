#!/usr/bin/env python3
"""Verify stateful Rust zero-step owner particle-assignment scratch reuse."""
...
SCHEMA_ID = (
    ...
    "rust_reciprocal_provider_owner_zero_step_particle_assignment_scratch_reuse_"
    "profile/1.0.0"
)
SOURCE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_particle_mesh_ewald_composite_dynamics_"
...
PREDECESSOR = {
    "pull_request": 469,
    "reviewed_head": "eacdbb34317a1991393d8cf87cd83b2b4ec70fe3",
    "merge_commit": "492580f21f4293457e46736167501c1d3f083d4e",
    "merge_tree": "b3fa4b675f4bffc3cd9311d8caed7ac5e553abc6",
    "profile_path": PREDECESSOR_PROFILE_RELATIVE_PATH.as_posix(),
    "profile_sha256": (
        "4f2c736dea22eceb1e513bfbefafe58ab7023091d24ff074200b4500dae2bd74"
    ),
    "source_manifest_path": PREDECESSOR_MANIFEST_RELATIVE_PATH.as_posix(),
    "source_manifest_sha256": (
        "edbe07b4dfae3b8d9c871e035a057942bdda505b98b80c2e5ffecfde3478770f"
    ),
    "source_manifest_entry_count": 345,
}
...
EXPECTED_PREDECESSOR_WORKFLOW_SHA256 = (
    "e91700010330d1278673215cf66b95cea26e9e90ae2610bb7d28ea84d842aa3d"
)
EXPECTED_PREDECESSOR_UNIT_SHA256 = (
    "1e7416175247fd1dea8fe98b8286531953cece2dfddeada592043454d988a62a"
)
...
EXPECTED_SUCCESSOR_IMPLEMENTATION_SHA256 = {
    ...
    PROVIDER_HEADER_RELATIVE_PATH: "f79b6ad90481b8eb63b912f9c837b5c0e0d00056860f4a881b9068be4448974c",
    COMPOSITE_TEST_RELATIVE_PATH: "85dcd3e89a014039c00ae843f30ccb506a82e439a4b2d173243eacb9f2a60acb",
    ...
    VENDOR_PROVIDER_HEADER_RELATIVE_PATH: "f79b6ad90481b8eb63b912f9c837b5c0e0d00056860f4a881b9068be4448974c",
    RUST_KERNEL_RELATIVE_PATH: "6ffa417c9738692bcd28931929a4174c5316ef6f17c8c44fdf9609b9ff988248",
}
...
def expected_predecessor_workflow() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_WORKFLOW_SHA256:
        fail("exact frozen PR 469 workflow digest drift")
    ...
    for predecessor_path, successor_path in evidence_wiring:
        ...
        if expected.count(anchor) != 2:
            fail("exact frozen PR 469 workflow trigger anchor drift: %s" % predecessor_path)
    ...
    old_region = source_region(
        expected,
        "      - name: Materialize exact PR 468 target and reviewed head\n",
        "\n\n  native-linux:\n",
        "exact frozen PR 469 immutable-evidence predecessor block",
    )
    new_region = """      - name: Materialize exact PR 469 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 492580f21f4293457e46736167501c1d3f083d4e^{tree})" = "b3fa4b675f4bffc3cd9311d8caed7ac5e553abc6"
          git merge-base --is-ancestor 492580f21f4293457e46736167501c1d3f083d4e HEAD
          git fetch --no-tags --depth=1 origin refs/pull/469/head
          test "$(git rev-parse FETCH_HEAD)" = "eacdbb34317a1991393d8cf87cd83b2b4ec70fe3"
          test "$(git rev-parse FETCH_HEAD^{tree})" = "b3fa4b675f4bffc3cd9311d8caed7ac5e553abc6"
      - name: Verify exact frozen PR 469 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=492580f21f4293457e46736167501c1d3f083d4e
          frozen_tree=b3fa4b675f4bffc3cd9311d8caed7ac5e553abc6
          ...
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_owner_zero_step_neutrality_sort_scratch_reuse_profile_v1.json >/dev/null
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_owner_zero_step_neutrality_sort_scratch_reuse_profile_v1_sources.json >/dev/null
          python3 -m pip install pytest==8.3.5
          python3 -m tools.verify_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_owner_zero_step_neutrality_sort_scratch_reuse_v1
          python3 -m pytest -q tests/unit/test_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_owner_zero_step_neutrality_sort_scratch_reuse_v1.py
          restore
          trap - EXIT"""
    return replace_once(
        expected,
        old_region,
        new_region,
        "exact frozen PR 469 immutable-evidence block",
    )
...
def expected_predecessor_unit() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_UNIT_SHA256:
        fail("exact frozen PR 469 unit digest drift")
    expected = frozen_raw.decode()
    expected = replace_once(
        expected,
        "from pathlib import Path\n",
        "from pathlib import Path\n\nimport pytest\n",
        "exact frozen PR 469 unit pytest import",
    )
    skip = """ROOT = Path(__file__).resolve().parents[2]
PME_RUST_RECIPROCAL_PROVIDER_OWNER_ZERO_STEP_PARTICLE_ASSIGNMENT_REUSE_EVIDENCE_PRESENT = (
    ROOT
...
pytestmark = pytest.mark.skipif(
    PME_RUST_RECIPROCAL_PROVIDER_OWNER_ZERO_STEP_PARTICLE_ASSIGNMENT_REUSE_EVIDENCE_PRESENT,
    reason=(
        "owner zero-step neutrality-sort scratch reuse evidence is verified from "
        "its exact frozen PR 469 object after zero-step particle-assignment "
        "scratch reuse evidence is present"
    ),
)"""
    return replace_once(
        expected,
        ...
        skip,
        "exact frozen PR 469 unit successor skip",
    )
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            ...
            "owner_zero_step_particle_assignment_scratch_reuse_current_sources_"
            "tests_evidence_pr469_target"
        ),
...
def build_profile(manifest_raw: bytes) -> dict:
    ...
    return {
        ...
        "implementation": {
            ...
            "stateful_rust_force_free_owner_particle_assignment_scratch_reused": True,
            "all_three_scratch_hidden_energy_entry_added": True,
            "predecessor_workspace_only_hidden_energy_entry_preserved": True,
            "predecessor_workspace_and_neutrality_hidden_energy_entry_preserved": True,
            "all_three_scratch_entry_accepts_no_force_descriptor": True,
            "all_three_scratch_mode_is_explicit": True,
            "all_three_scratch_mode_uses_force_storage_disabled": True,
            "all_three_scratch_mode_uses_shared_provider_preflight_and_leases": True,
            "three_descriptors_and_full_capacities_alias_preflight_before_lease": True,
            ...
            "three_scratch_drops_restore_empty_after_cold_unallocated_failure": True,
            "three_scratch_drops_restore_ready_after_success_error_or_panic": True,
            ...
            "later_workspace_failure_may_retain_ready_particle_assignment_scratch": True,
            ...
            "warm_capacity_sufficient_particle_assignment_reserve_elided": True,
            "call_local_particle_assignment_allocation_preserved": False,
            ...
            "particle_assignment_growth_oom_preserves_ready_raw_parts_and_payload": True,
            ...
            "independent_owner_particle_assignment_allocations_disjoint": True,
            "energy_forceful_all_three_scratch_interoperation_preserved": True,
            ...
            "particle_assignment_scratch_reuse_claimed": True,
        ...
        "validation": {
            ...
            "source_manifest_entry_count_exact": 351,
            "pull_request_trigger_path_count_exact": 168,
            "push_trigger_path_count_exact": 168,
            ...
            "rust_all_three_scratch_energy_mode_and_abi_exact": True,
...
def require_workflow_contract(root: Path = ROOT) -> None:
    ...
    if predecessor != expected_predecessor_workflow():
        fail("PR 469 predecessor workflow is not the exact frozen-object transformation")
    for workflow, expected_name in (
        ...
        if len(pull_paths) != 168 or len(set(pull_paths)) != 168 or push_paths != pull_paths:
            fail("workflow 168-path symmetric trigger contract drift: %s" % expected_name)
    ...
    for token in (
        ...
        if token not in predecessor:
            fail("PR 469 predecessor workflow freeze drift: %s" % token)
...
def require_predecessor_unit_freeze(root: Path = ROOT) -> None:
    ...
    if unit != expected_predecessor_unit():
        fail("PR 469 predecessor unit is not the exact frozen-object transformation")
    constants = {
        node.value
    ...
    for token in (
        ...
        if unit.count(token) != 1:
            fail("PR 469 predecessor unit freeze drift: %s" % token)
    if PROFILE_RELATIVE_PATH.as_posix() not in constants:
        fail("PR 469 predecessor unit successor-profile path drift")
    if not any("exact frozen PR 469 object" in value for value in constants):
        fail("PR 469 predecessor unit frozen-object reason drift")
    if "is_file()" not in unit:
        fail("PR 469 predecessor unit skip is not successor-profile scoped")
...
def require_exact_source_hashes(root: Path = ROOT) -> None:
    ...
    for relative in IMPLEMENTATION_DELTA_PATHS:
        ...
        if sha(predecessor_raw) != EXPECTED_PREDECESSOR_IMPLEMENTATION_SHA256[relative]:
            fail("frozen PR 469 implementation digest drift: %s" % relative)
...
def require_native_contract(root: Path = ROOT) -> None:
    ...
    for forbidden in ("out_forces", "force_output"):
        if forbidden in declaration:
            fail("private all-three-scratch declaration broadened: %s" % forbidden)
    for old_declaration in (
        ...
        if header.count(old_declaration) != 1:
            fail("predecessor hidden energy declaration drift: %s" % old_declaration)
    if header.count(
...
def require_docs_contract(root: Path = ROOT) -> None:
    ...
    for token in (
        ...
        "owner's reciprocal workspace, neutrality-sort scratch, and particle-assignment scratch",
        PRIVATE_SYMBOL,
        "EnergyWithWorkspaceAndNeutralitySortScratchAndParticleAssignmentScratch",
        "ForceStorageMode::Disabled",
        "whole backing capacity",
        "success-only",
        "allocation order remains neutrality sorting, particle assignment, then reciprocal workspace preparation",
        "Workspace, neutrality-sort, and particle-assignment payloads are derived scratch and are not transactional",
        "tracked neutrality, assignment, and workspace reserve requests",
        "Force x/y/z remain untouched",
        "do not establish a generally allocation-free provider",
