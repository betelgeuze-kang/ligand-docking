#!/usr/bin/env python3
"""Verify PME direct-local parent force-storage reuse in stateful dynamics."""
...
SCHEMA_ID = (
    ...
    "direct_parent_force_scratch_profile/1.0.0"
)
SOURCE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "direct_parent_force_scratch_sources/1.0.0"
)
PUBLIC_PROFILE_ID = (
...
EXPECTED_PREDECESSOR_WORKFLOW_SHA256 = (
    "203f02f9016c01946b44aad369aab7101347ab3b7dad8b23edcec9b72cb5d323"
)
    ...
    steps:
      - name: Check out exact evidence graph
        ...
        with:
          ...
          clean: true
          fetch-depth: 0
      - name: Materialize exact PR 453 architecture, PR 452 target, and PR 435 inherited Ewald evaluator
        shell: bash
        run: |
          set -euo pipefail
...
EXPECTED_NATIVE_JOB_BODY = """    runs-on: ubuntu-latest
    ...
    steps:
      ...
      - name: Synthetic release and sanitizer regressions
        run: |
          cmake -S . -B build/particle-mesh-ewald-direct-parent-force-scratch-release -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF
          cmake --build build/particle-mesh-ewald-direct-parent-force-scratch-release --target betelgeuze_engine_particle_mesh_ewald_composite_dynamics --parallel 2
          ctest --test-dir build/particle-mesh-ewald-direct-parent-force-scratch-release -R '^betelgeuze_engine_(particle_mesh_ewald_composite_dynamics|export_allowlist)$' --output-on-failure
          cmake -S . -B build/particle-mesh-ewald-direct-parent-force-scratch-sanitize -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF -DCMAKE_C_FLAGS='-fsanitize=address,undefined -fno-omit-frame-pointer' -DCMAKE_CXX_FLAGS='-fsanitize=address,undefined -fno-omit-frame-pointer'
          cmake --build build/particle-mesh-ewald-direct-parent-force-scratch-sanitize --target betelgeuze_engine_particle_mesh_ewald_composite_dynamics --parallel 2
          ASAN_OPTIONS=detect_leaks=1 UBSAN_OPTIONS=halt_on_error=1 ctest --test-dir build/particle-mesh-ewald-direct-parent-force-scratch-sanitize -R '^betelgeuze_engine_particle_mesh_ewald_composite_dynamics$' --output-on-failure
"""

EXPECTED_RUST_JOB_BODY = """    runs-on: ubuntu-latest
    timeout-minutes: 35
    env:
      CARGO_TARGET_DIR: ${{ github.workspace }}/build/particle-mesh-ewald-direct-parent-force-scratch-cargo
    steps:
...
EXPECTED_MACOS_JOB_BODY = """    runs-on: macos-15
    ...
    steps:
      ...
      - name: Exact export regression
        run: |
          cmake -S . -B build/particle-mesh-ewald-direct-parent-force-scratch-macos -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF
          cmake --build build/particle-mesh-ewald-direct-parent-force-scratch-macos --target betelgeuze_engine --parallel 2
          ctest --test-dir build/particle-mesh-ewald-direct-parent-force-scratch-macos -R '^betelgeuze_engine_export_allowlist$' --output-on-failure
"""
...
def expected_workflow_document() -> str:
    ...
    preamble = (
        ...
        "direct-parent-force-scratch-${{ github.ref }}\n"
...
def require_architecture_predecessor() -> dict:
    ...
    if architecture_profile.get("architecture_predecessor") != PREDECESSOR:
        fail("architecture predecessor no longer binds the PME target predecessor")
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            "particle_mesh_ewald_composite_dynamics_direct_parent_force_scratch_"
...
def build_profile(manifest_raw: bytes) -> dict:
    ...
    return {
        ...
        "profile_id": (
            ...
            "direct_parent_force_scratch_development_v1"
        ),
        "roadmap_issue": 434,
        ...
        "implementation": {
            "successful_stateful_forceful_direct_parent_aos_storage_reused": True,
            "steady_state_direct_parent_force_storage_reused": True,
            "direct_parent_force_scratch_is_derived_non_authoritative": True,
            "direct_parent_force_scratch_serialized_in_checkpoint": False,
            "direct_parent_force_scratch_bound_into_static_fingerprint": False,
            "reciprocal_parent_force_storage_reused": False,
        ...
        "validation": {
            ...
            "direct_parent_pointer_capacity_and_size_retention": True,
            "same_lane_reserved_peer_and_stateless_direct_parent_force_bit_identity": True,
...
def expected_frozen_predecessor_workflow(frozen: str) -> str:
    ...
    successor_pairs = (
        ...
        (Path("docs/engine_v2_native_direct_ewald_composite_dynamics_ewald_parent_force_scratch_v1.md"), DOC_RELATIVE_PATH),
        (Path("tools/verify_engine_v2_native_direct_ewald_composite_dynamics_ewald_parent_force_scratch_v1.py"), VERIFIER_RELATIVE_PATH),
    ...
    old_materialize = """      - name: Materialize exact PR 452 architecture, PR 451 target, and PR 435 inherited Ewald evaluator
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 8f371847d62c03efe99d1e3593c9c0473adcf968^{tree})" = "aa1ba05928e142f06dac11b31e323bb3e247bb17"
          ...
          test "$(git rev-parse FETCH_HEAD^{tree})" = "0530a50af2cceeff02341ccb6fab141fd8c43726"
"""
    new_materialize = """      - name: Materialize exact PR 453 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 35a8f0b0ba0e079bc2a1edee15d19ef2c2823f2a^{tree})" = "b22c5fd115a5c8e28856872df57127ecdd28d9b5"
...
"""
    new_verify = """      - name: Verify exact frozen PR 453 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=35a8f0b0ba0e079bc2a1edee15d19ef2c2823f2a
          frozen_tree=b22c5fd115a5c8e28856872df57127ecdd28d9b5
...
def expected_frozen_predecessor_unit(frozen: str) -> str:
    ...
    addition = """PME_DIRECT_PARENT_FORCE_SCRATCH_EVIDENCE_PRESENT = (
    ROOT
    / "config/engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "direct_parent_force_scratch_profile_v1.json"
).is_file()
pytestmark = pytest.mark.skipif(
    PME_DIRECT_PARENT_FORCE_SCRATCH_EVIDENCE_PRESENT,
    reason=(
        "Direct-Ewald Ewald-parent force-scratch evidence is verified from its "
        "exact frozen PR 453 object after PME direct-parent force-scratch "
        "evidence is present"
    ),
)
"""
...
def expected_composite_evaluator_header(frozen: str) -> str:
    source = replace_exact(
        ...
        "PME evaluator direct-parent include",
    )
    source = replace_exact(
        source,
    ...
    return replace_exact(
        ...
        "    cpu::Evaluation *stateful_force_output,\n",
        "PME evaluator direct-parent scratch parameter",
    )
...
def expected_composite_dynamics_header(frozen: str) -> str:
    ...
    return replace_exact(
        ...
        "PME dynamics direct-parent scratch owner",
    )
...
def expected_composite_dynamics_source(frozen: str) -> str:
    source = replace_exact(
        ...
        "}\n\n"
        "bool forcefield_storage_overlaps(\n",
        "PME dynamics direct-parent overlap helper",
    )
    source = replace_exact(
        source,
        ...
        "PME dynamics direct-parent owner overlap",
    )
    source = replace_count_exact(
        ...
        1,
        "PME dynamics provider direct-parent scratch",
    )
    source = replace_exact(
        source,
        ...
        "PME dynamics create direct-parent scratch",
    )
...
def expected_particle_mesh_ewald_composite_source(frozen: str) -> str:
    source = replace_exact(
        frozen,
        "    uint8_t *inout_rust_cpu_forcefield_validated,\n"
        "    cpu::Evaluation *stateful_force_output,\n",
        ...
        "    ewald::Evaluation *direct_parent_evaluation_scratch,\n"
        "    cpu::Evaluation *stateful_force_output,\n",
        "PME composite direct-parent scratch parameter",
    )
    source = replace_exact(
        source,
        ...
        "PME composite direct-parent scratch invariant",
    )
    source = replace_exact(
        source,
        ...
        "PME composite direct-parent scratch diagnostic",
    )
    old = """    bg_direct_ewald_model_v1 local_direct_model = direct_model;
...
"""
    new = """    bg_direct_ewald_model_v1 local_direct_model = direct_model;
    local_direct_model.reciprocal_max_indices = {{0, 0, 0}};
    ewald::Evaluation local_direct_evaluation;
    ewald::Evaluation *direct_evaluation = &local_direct_evaluation;
    const bool reuse_direct_parent_force_storage =
        stateful_scratch && compute_forces;
    if (reuse_direct_parent_force_storage) {
        direct_evaluation = direct_parent_evaluation_scratch;
    }
    ewald::Error direct_error;
    if (cpp_lane) {
        status = reuse_direct_parent_force_storage
            ? ewald::cpp_cpu::evaluate_reusing_force_storage(
                  system, local_direct_model, true, direct_evaluation,
                  &direct_error)
            : ewald::cpp_cpu::evaluate(
                  system, local_direct_model, compute_forces,
                  direct_evaluation, &direct_error);
    } else {
        status = reuse_direct_parent_force_storage
...
"""
    source = replace_exact(source, old, new, "PME direct-local parent dispatch")
    source = replace_exact(
        source,
        "    if (!direct_parent_is_valid(\n"
        "            direct_evaluation, direct_error, compute_forces, atom_count)) {\n",
        "    const ewald::Evaluation &direct_result = *direct_evaluation;\n"
        "    if (!direct_parent_is_valid(\n"
        "            direct_result, direct_error, compute_forces, atom_count)) {\n",
        "PME direct-local result reference",
    )
    source = replace_count_exact(
        source, "direct_evaluation.", "direct_result.", 15,
        "PME direct-local result uses",
    )
    return replace_exact(
        source,
        ...
        "            out_forces != nullptr, &evaluation, out_error);\n",
        "PME composite stateless direct-parent scratch",
    )


EXPECTED_DIRECT_PARENT_FORCE_SCRATCH_SHA256 = {
    "native/src/composite/particle_mesh_ewald_composite.cpp": "175beaf907faade5cfcffa30c3e8a165c5e07bbe14be9e35cf6335e04bf99210",
    "native/src/composite/particle_mesh_ewald_composite_dynamics.cpp": "337fcfcfe7b0a52f44a81cc268c0024eedd0c8a08cb4bb6b90fbf1a02ac52ae0",
    "native/src/composite/particle_mesh_ewald_composite_dynamics.hpp": "66fbc480e79273893d8c410d054d972958d44b58b2cca455dd2c7767f2fd8271",
    "native/src/composite/particle_mesh_ewald_composite_evaluator.hpp": "3dffca5d899575ebfd90e37cc4fb471e8bee2dc542852219db370065f43bb48b",
    "native/tests/particle_mesh_ewald_composite_dynamics.cpp": "86d8582c1bc4316b591d40a283b08b5baee9b5899757f7cf111cb89ef9cddf5c",
    "native/tests/particle_mesh_ewald_composite_dynamics_scratch.cpp": "3c14c4249ad1414da558fa1825e7d244600672ce494e45e1f9b9ca27b69cce4a",
    "native/tests/particle_mesh_ewald_composite_dynamics_scratch.hpp": "87febcd5dcc99c863419024415687c398ec8c786664eaa94ffa821f372b18671",
    "rust/betelgeuze-sys/vendor/native/src/composite/particle_mesh_ewald_composite.cpp": "175beaf907faade5cfcffa30c3e8a165c5e07bbe14be9e35cf6335e04bf99210",
    "rust/betelgeuze-sys/vendor/native/src/composite/particle_mesh_ewald_composite_dynamics.cpp": "337fcfcfe7b0a52f44a81cc268c0024eedd0c8a08cb4bb6b90fbf1a02ac52ae0",
    "rust/betelgeuze-sys/vendor/native/src/composite/particle_mesh_ewald_composite_dynamics.hpp": "66fbc480e79273893d8c410d054d972958d44b58b2cca455dd2c7767f2fd8271",
    "rust/betelgeuze-sys/vendor/native/src/composite/particle_mesh_ewald_composite_evaluator.hpp": "3dffca5d899575ebfd90e37cc4fb471e8bee2dc542852219db370065f43bb48b",
}


def require_direct_parent_force_scratch_contract(root: Path = ROOT) -> None:
    for relative, digest in EXPECTED_DIRECT_PARENT_FORCE_SCRATCH_SHA256.items():
        if sha((root / relative).read_bytes()) != digest:
            fail(f"exact PME direct-parent force scratch source drift: {relative}")
    pairs = (
    ...
    for relative in pairs:
        if (root / "native/src" / relative).read_bytes() != (root / "rust/betelgeuze-sys/vendor/native/src" / relative).read_bytes():
            fail(f"canonical/vendor PME direct-parent source drift: {relative}")
    merge = ARCHITECTURE_PREDECESSOR["merge_commit"]
    for relative, transform in (
        ("native/src/composite/particle_mesh_ewald_composite.cpp", expected_particle_mesh_ewald_composite_source),
        ("native/src/composite/particle_mesh_ewald_composite_dynamics.cpp", expected_composite_dynamics_source),
        ("native/src/composite/particle_mesh_ewald_composite_dynamics.hpp", expected_composite_dynamics_header),
        ("native/src/composite/particle_mesh_ewald_composite_evaluator.hpp", expected_composite_evaluator_header),
        ("rust/betelgeuze-sys/vendor/native/src/composite/particle_mesh_ewald_composite.cpp", expected_particle_mesh_ewald_composite_source),
        ("rust/betelgeuze-sys/vendor/native/src/composite/particle_mesh_ewald_composite_dynamics.cpp", expected_composite_dynamics_source),
        ("rust/betelgeuze-sys/vendor/native/src/composite/particle_mesh_ewald_composite_dynamics.hpp", expected_composite_dynamics_header),
        ("rust/betelgeuze-sys/vendor/native/src/composite/particle_mesh_ewald_composite_evaluator.hpp", expected_composite_evaluator_header),
    ):
        frozen = git("show", f"{merge}:{relative}").stdout.decode()
        if transform(frozen) != (root / relative).read_text():
            fail(f"frozen-input PME direct-parent transform drift: {relative}")
    joined = "\n".join(
        (root / path).read_text()
        for path in EXPECTED_DIRECT_PARENT_FORCE_SCRATCH_SHA256
    )
    for token in (
        "evaluate_reusing_force_storage(",
        "direct_parent_evaluation_scratch",
        "reuse_direct_parent_force_storage",
        "local_direct_model.reciprocal_max_indices = {{0, 0, 0}};",
        ...
        if token not in joined:
            fail(f"PME direct-parent force scratch contract token missing: {token}")
