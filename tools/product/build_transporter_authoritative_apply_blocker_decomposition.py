...
def build_payload(
    ...
    placeholder_driven_rows = int(apply_s.get("placeholder_driven_rows", 0) or 0)
    staged_non_authoritative_rows = int(apply_s.get("staged_non_authoritative_rows", 0) or 0)
    ready_for_apply_rows = int(apply_s.get("ready_for_apply_rows", 0) or 0)
    aqp1_keep_review_only = int(dashboard_s.get("aqp1_keep_review_only_count", 0) or 0)
    glut1_keep_review_only = int(dashboard_s.get("glut1_keep_review_only_count", 0) or 0)
    p0_open_count = int(readiness_s.get("p0_open_count", 0) or 0)
    aqp1_ready_seed_rows = int(aqp1_wb_s.get("ready_seed_row_count", 0) or 0)
    glut1_ready_seed_rows = int(glut1_wb_s.get("ready_seed_row_count", 0) or 0)

    raw_rows = [
        {
            "sort_group": 90 if placeholder_driven_rows == 0 else 10,
            "blocker_id": "placeholder_packet_rows",
            "blocker_status": "ready" if placeholder_driven_rows == 0 else "blocked",
            "scope": "family",
            "current_signal": (
            ...
            "next_action": (
                "placeholder-driven transporter rows are closed; keep focus on donor-policy and claim-safe binder promotion"
                if placeholder_driven_rows == 0
                else "replace placeholder-driven transporter workbook rows with target-specific reference/split/meta evidence before any authoritative apply discussion"
            ),
        },
        {
            "sort_group": 20,
            ...
            "scope": "target",
            "current_signal": (
                f"aqp1_keep_review_only={aqp1_keep_review_only}; "
                f"glut1_keep_review_only={glut1_keep_review_only}"
            ),
        ...
        },
        {
            ...
            "blocker_status": "blocked" if p0_open_count > 0 else "ready",
            "scope": "family",
            "current_signal": f"p0_open_count={p0_open_count}",
            "source_artifact": "runs/transporter_membrane_readiness_current.md",
            "next_action": (
                "P0 scaffold blockers are closed; keep donor policy controlled until binder promotion is claim-safe"
                if p0_open_count == 0
                else "burn down remaining AQP1/GLUT1 transporter P0 gaps before treating the family as runnable beyond draft/manual-review mode"
            ),
        },
        {
            "sort_group": 80,
        ...
        },
        {
            ...
            "blocker_status": "blocked" if (aqp1_ready_seed_rows + glut1_ready_seed_rows) == 0 else "ready",
            "scope": "target",
            "current_signal": (
                f"aqp1_ready_seed_rows={aqp1_ready_seed_rows}; "
                f"glut1_ready_seed_rows={glut1_ready_seed_rows}"
            ),
    ...
    rows = [
        {"blocker_rank": idx, **{key: value for key, value in row.items() if key != "sort_group"}}
        for idx, row in enumerate(sorted(raw_rows, key=lambda row: row["sort_group"]), start=1)
    ]
    hard_blockers = [row for row in rows if row["blocker_status"] == "blocked"]
    ...
    summary = {
        ...
        "authoritative_apply_ready": len(hard_blockers) == 0,
        "top_blocker_id": hard_blockers[0]["blocker_id"] if hard_blockers else "",
        "top_blocker_signal": hard_blockers[0]["current_signal"] if hard_blockers else "",
        "next_required_step": (
            hard_blockers[0]["next_action"]
            if hard_blockers
            else "Transporter authoritative-apply blockers are closed; keep scope widening behind the product scope contract."
        ),
    }
