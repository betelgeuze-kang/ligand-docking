...
def build_payload(
    ...
    checks = [
        ...
        },
        {
            ...
            "notes": (
                "GLUT1 target metadata is filled from the 4PYP structure-chain sequence; pocket centroid curation remains separate."
                if "TEMPLATE_SEQ_" not in str(glut1_target_meta.get("sequence", ""))
                else "GLUT1 target metadata still carries placeholder sequence/state context in the scaffold files."
            ),
        },
    ]
    return {
