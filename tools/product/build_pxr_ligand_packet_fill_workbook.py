...
PACKETS: Dict[str, Dict[str, str]] = {
    "core": {
        "reference": "config/ligand_binding_reference_blind_pxr_nr1i2_v1.csv",
        "split": "config/ligand_eval_splits_blind_pxr_nr1i2_v1.csv",
        "meta": "config/ligand_meta_blind_pxr_nr1i2_v1.csv",
    },
    "ood": {
        "reference": "config/ligand_binding_reference_blind_pxr_nr1i2_chembl50_v1.csv",
        "split": "config/ligand_eval_splits_blind_pxr_nr1i2_chembl50_v1.csv",
        "meta": "config/ligand_meta_blind_pxr_nr1i2_chembl50_v1.csv",
    },
}
...
def _next_action_for_row(packet_name: str, row: Dict[str, Any]) -> str:
    if row["fit_donor_carryover_candidate"]:
        return f"Decide whether this {packet_name} meta-only ligand belongs to the temporary fit-donor carryover set or should be dropped from the PXR ledger."
    if not row["in_reference"]:
        return f"Add {packet_name} reference row and provenance for ligand_id."
    if row["reference_placeholder"]:
        return f"Replace {packet_name} placeholder reference values with curated PXR evidence."
    if not row["in_meta"]:
    ...
    if row["target_mismatch"]:
        return f"Remove non-PXR target leakage from {packet_name} packet rows."
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a PXR ligand packet fill workbook.")
    parser.add_argument(
        "--template-json",
        default=DEFAULT_TEMPLATE_JSON,
        help="PXR family template JSON used to expose placeholder policy context.",
    )
    parser.add_argument(
        "--out-json",
        default="runs/pxr_ligand_packet_fill_workbook_current.json",
        help="JSON output path.",
    )
    parser.add_argument(
        "--out-csv",
        default="runs/pxr_ligand_packet_fill_workbook_current.csv",
        help="CSV workbook output path.",
    )
    parser.add_argument(
        "--out-md",
        default="runs/pxr_ligand_packet_fill_workbook_current.md",
