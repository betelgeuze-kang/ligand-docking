...
def _required_missing_fields(row: dict[str, Any]) -> str:
    ...
    if row["apply_split_row"] == "yes":
        required.append(("replacement_role", row["replacement_role"]))
...
def _seed_row(queue_row: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        "notes": queue_row.get(
            "notes",
            "Replace placeholder PXR packet slot with a synchronized reference/split/meta edit.",
        ),
    }
...
def build_payload(queue_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": {
            ...
            "next_required_step": "Fill replacement columns for one PXR packet_step at a time, then apply the row as a synchronized reference/split/meta triple edit.",
        },
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a PXR packet replacement workbook from the current fill queue.")
    parser.add_argument("--queue-json", default=DEFAULT_QUEUE_JSON)
    parser.add_argument("--out-json", default="runs/pxr_packet_replacement_workbook_current.json")
    parser.add_argument("--out-csv", default="runs/pxr_packet_replacement_workbook_current.csv")
    parser.add_argument("--out-md", default="runs/pxr_packet_replacement_workbook_current.md")
