...
def build_pr38_ci_runner_hygiene_remote_rerun_preflight(
    *,
    ...
    handoff_dirty_paths = (
        all_dirty_paths if expected_branch and current_branch == expected_branch else []
    )
    human_git_add_command = (
        _shell_join(["git", "add", *handoff_dirty_paths])
        if handoff_dirty_paths
    ...
    summary = {
        ...
        "human_owner_external_mutation_required": bool(
            handoff_dirty_paths or not expected_ref_published_for_dispatch
        ),
        "codex_should_not_run_handoff_commands": True,
        "human_commit_worktree_dir": worktree_dir,
        "human_commit_required_dirty_path_count": len(handoff_dirty_paths),
        "human_commit_required_dirty_paths": handoff_dirty_paths,
        "human_commit_dirty_paths_suppressed_wrong_branch": bool(
            all_dirty_paths and expected_branch and current_branch != expected_branch
        ),
