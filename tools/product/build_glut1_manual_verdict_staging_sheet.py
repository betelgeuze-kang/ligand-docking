...
CONFIRMATION_CARD_JSON = RUNS / "glut1_binder_confirmation_card_current.json"
UPDATE_SHEET_JSON = RUNS / "glut1_binder_verdict_update_sheet_current.json"
COMMIT_PACKET_JSON = RUNS / "glut1_manual_verdict_commit_packet_current.json"

OUT_JSON = RUNS / "glut1_manual_verdict_staging_sheet_current.json"
OUT_CSV = RUNS / "glut1_manual_verdict_staging_sheet_current.csv"
OUT_MD = RUNS / "glut1_manual_verdict_staging_sheet_current.md"
        ...
        rows.append(
            {
                ...
                "review_focus": update_row["caution"],
                ...
                "stop_condition": commit_row["promotion_blocker"],
                "stop_reason": "Keep GLUT1 second-wave rows non-authoritative and do not reopen transporter apply or donor policy from this staging sheet alone.",
...
def build_summary(card: dict, rows: list[dict]) -> dict:
    return {
        ...
        "next_required_step": "Use this staging sheet immediately before filling manual_* fields. Confirm the staged values and second-wave stop rule, then write into the update sheet by reviewer decision only.",
    }
...
def write_md(path: Path, summary: dict, rows: list[dict]) -> None:
    lines = [
        "# GLUT1 Manual Verdict Staging Sheet",
        "",
        ...
        "## Rule",
        "",
        "- Review the staged verdict, confidence, note, and second-wave stop rule here first. Keep all `manual_*` fields empty until the reviewer explicitly decides to fill them in the update sheet.",
        "",
        "## Staging Rows",
        "",
    ...
    lines.extend(
        [
            ...
            "- `runs/glut1_binder_verdict_update_sheet_current.md`",
