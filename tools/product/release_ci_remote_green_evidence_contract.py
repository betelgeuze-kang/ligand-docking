...
EVIDENCE_INPUTS: tuple[EvidenceInputSpec, ...] = (
    ...
    EvidenceInputSpec(
        ...
        collect_command=(
            f"gh api '{_workflow_runs_endpoint(event='schedule')}' --paginate > "
    ...
    EvidenceInputSpec(
        ...
        collect_command=(
            "gh api "
            f"repos/{REPO}/actions/runs/${{RELEASE_CI_FAILED_RUN_ID}}/artifacts --paginate > "
            "runs/release_ci_failed_run_artifacts_current.json"
        ),
        discovery_command=(
            f"RELEASE_CI_FAILED_RUN_ID=$(gh run list --repo {REPO} "
            f"--workflow={WORKFLOW_FILE} --status=failure --limit=1 --json databaseId "
            "-q '.[0].databaseId')"
        ),
    ...
    EvidenceInputSpec(
        ...
        collect_command=(
            f"gh api '{_workflow_runs_endpoint(event='push')}' --paginate > "
