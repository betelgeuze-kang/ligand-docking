...
DEFAULT_OUT_JSON = "runs/aqp1_next_verification_slice_current.json"
DEFAULT_OUT_CSV = "runs/aqp1_next_verification_slice_current.csv"
DEFAULT_OUT_MD = "runs/aqp1_next_verification_slice_current.md"
...
def build_payload(external_seed: dict[str, Any], manual_queue: dict[str, Any]) -> dict[str, Any]:
        ...
        prioritized_rows.append(
            {
                ...
                "notes": str(seed_row.get("caution", "")).strip(),
            }
        )
        rank += 1
    ...
    summary = {
        ...
        "next_required_step": "Review bacopaside II, AqB013, and AqB011 first, keep tetraethylammonium and acetazolamide caution-only, then finish the three AQP1 review-only negative slots without injecting proxy values.",
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the next verification slice for AQP1 transporter review.")
