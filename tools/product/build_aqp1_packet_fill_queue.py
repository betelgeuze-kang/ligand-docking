...
PACKET_FILES = {
    "reference_csv": "config/ligand_binding_reference_blind_aqp1_v1.csv",
    "eval_split_csv": "config/ligand_eval_splits_blind_aqp1_v1.csv",
    "ligand_meta_csv": "config/ligand_meta_blind_aqp1_v1.csv",
}
...
def _queue_row(ligand_id: str, ref: dict[str, str], split: dict[str, str], meta: dict[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": f"Replace placeholder AQP1 {binder_label} slot {slot_index} with a curated transporter ligand row before runnable promotion.",
    }
...
def build_payload(workbook_payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ref_rows = [
        row for row in _read_csv(_resolve(PACKET_FILES["reference_csv"]))
        if row.get("target", "").strip() == PRIMARY_TARGET
    ]
    split_rows = [
        row for row in _read_csv(_resolve(PACKET_FILES["eval_split_csv"]))
        if row.get("target", "").strip() == PRIMARY_TARGET
    ]
    ...
    workbook_by_step = {
        ...
        for row in (workbook_payload or {}).get("workbook_rows", []) or []
    ...
    for ligand_id, ref_row in sorted(ref_by_id.items()):
        ...
        if not _needs_queue_row(ligand_id, ref_row, split_row, meta_row):
            continue
        row = _queue_row(ligand_id, ref_row, split_row, meta_row)
        workbook_row = workbook_by_step.get(str(row["packet_step"]).strip(), {})
        for field in [
            "replacement_ligand_id",
            "replacement_reference_binding_kcal_mol",
            "replacement_source",
            ...
            value = str(workbook_row.get(field, "")).strip()
            if value:
                row[field] = value
        if str(workbook_row.get("row_ready_for_apply", "")).strip().lower() == "yes":
            row["curation_status"] = "ready_for_apply"
        elif any(
            str(workbook_row.get(field, "")).strip()
            for field in ["replacement_ligand_id", "replacement_source", "replacement_reference_binding_kcal_mol", "replacement_smiles", "replacement_scaffold"]
        ):
            row["curation_status"] = "staged_non_authoritative"
    ...
    payload = {
        ...
        "summary": {
            ...
            "next_required_step": "Replace every placeholder AQP1 slot with a curated transporter ligand_id/provenance/meta row before any runnable core or smoke packet attempt.",
        },
        "packet_summaries": [
            {
                "packet": "core",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build an AQP1 placeholder packet fill queue.")
    parser.add_argument("--workbook-json", default=DEFAULT_WORKBOOK_JSON)
    parser.add_argument("--out-json", default="runs/aqp1_packet_fill_queue_current.json")
    parser.add_argument("--out-csv", default="runs/aqp1_packet_fill_queue_current.csv")
    parser.add_argument("--out-md", default="runs/aqp1_packet_fill_queue_current.md")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    payload = build_payload(_load_json(_resolve(args.workbook_json)))
