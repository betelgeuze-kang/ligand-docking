...
def build_scores(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for row in rows:
        target = _text(row.get(args.target_col))
        ligand = _text(row.get(args.ligand_col))
    ...
    summary = {
        "packet_type": "public_benchmark_product_scores_from_htvs",
        "suite_id": _text(args.suite_id),
        "status": "public_benchmark_product_scores_ready" if out_rows else "blocked_public_benchmark_product_scores",
        ...
        "execution_failed_stage": _text(execution_summary.get("failed_stage")) if execution_summary else "",
        "score_col": score_col,
        "input_rows": len(rows),
        ...
        "claim_boundary": (
            "Public benchmark HTVS score export only; it converts local product-engine scores into a benchmark result CSV. "
            "It does not run docking, download datasets, submit predictions, or mutate external state."
        ),
        "next_required_step": "Build product benchmark provenance and a suite scorecard from the exported scores.",
    }
    ...
    out_md.write_text(
        "\n".join(
            [
                "# Public Benchmark Product Scores From HTVS",
                "",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export product HTVS scores into public benchmark result CSV format.")
    parser.add_argument("--suite-id", required=True)
    parser.add_argument("--htvs-scores-csv", required=True)
    ...
    parser.add_argument("--out-scores-csv", required=True)
    parser.add_argument("--out-json", default="runs/public_benchmark_product_scores_from_htvs_current.json")
    parser.add_argument("--out-md", default="runs/public_benchmark_product_scores_from_htvs_current.md")
