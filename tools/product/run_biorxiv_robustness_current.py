...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description="Run the seed-shift robustness rerun over the promoted bioRxiv validation stack and compare it against the accepted current run."
    )
    ap.add_argument("--tag", default=f"{dt.date.today().isoformat()}_biorxiv_robustness_v1")
    ap.add_argument("--sets", default="set3_operational_smoke,set1_core_blind,set2_expanded_ood")
    ap.add_argument("--set-spec-json", default="config/external_validation_biorxiv_robustness_sets_v1_seedshift1.json")
