...
METRIC_SOURCE_APPROVAL_TOKEN = "APPROVE_R9_STATISTICAL_SUPPORT_METRIC_SOURCE_PAYLOADS"
...
def _field_work_order_rows(
    *,
    ...
    metric_token = _text(metric_receipt.get("approval_token_required")) or METRIC_SOURCE_APPROVAL_TOKEN
...
def build_public_benchmark_external_receipts_audit(
    *,
    ...
    score_template_csv = _text(score_template_metrics.get("score_template_csv"))
    vina_gnina_score_template_row_count = _int(
        score_template_metrics.get("score_template_row_count")
    ) or _csv_row_count(score_template_csv, root=root)
    vina_gnina_approval_token = (
        _text(score_template_metrics.get("approval_token_required"))
        or _text(vina_gnina_work_order.get("approval_token_required"))
        or VINA_GNINA_APPROVAL_TOKEN
    )
    ...
    if not isinstance(vina_gnina_pending_field_counts, dict):
        vina_gnina_pending_field_counts = _csv_pending_field_counts(score_template_csv, root=root)
    ...
    vina_gnina_lane_ready = (
        _bool_true(score_template_metrics.get("score_template_validation_ready"))
        and _int(score_template_metrics.get("score_template_blocker_count")) == 0
        and vina_gnina_score_template_row_count > 0
    )
    metric_approval_token = _text(receipt.get("approval_token_required")) or METRIC_SOURCE_APPROVAL_TOKEN
    receipt_attach_lane_rows = [
        _receipt_attach_lane(
            lane_id="vina_gnina_same_input_scores",
            ready=bool(vina_gnina_lane_ready),
            source_artifact=(
                vina_gnina_score_template_receipt_json
                if score_template_receipt_present
                else vina_gnina_work_order_json
            ),
            operator_csv=score_template_csv,
            row_count=vina_gnina_score_template_row_count,
            pending_value_count=_int(score_template_metrics.get("score_value_pending_count")),
            pending_metadata_count=_int(score_template_metrics.get("operator_metadata_pending_count"))
            + _int(score_template_metrics.get("operator_placeholder_pending_count")),
            pending_license_count=_int(score_template_metrics.get("license_ok_pending_count")),
            pending_approval_token_count=_int(score_template_metrics.get("approval_token_pending_count")),
            approval_token_required=vina_gnina_approval_token,
        ...
        _receipt_attach_lane(
            ...
            source_artifact=receipt_json,
            operator_csv=receipt_csv,
            row_count=receipt_row_count,
            pending_value_count=_int(receipt.get("receipt_manual_field_pending_count")),
            pending_metadata_count=_int(receipt.get("receipt_manual_field_pending_count")),
            pending_license_count=_int(receipt.get("receipt_license_ok_pending_count")),
            pending_approval_token_count=_int(receipt.get("receipt_approval_token_pending_count")),
            approval_token_required=metric_approval_token,
    ...
    receipt_attach_blocked_lane_rows = [
        row for row in receipt_attach_lane_rows if row["operator_action_required"]
    ]
    field_work_order_rows = _field_work_order_rows(
        vina_gnina_source=score_template_metrics,
        ...
        metric_source_receipt_json=receipt_json,
        metric_source_receipt_csv=receipt_csv,
        root=root,
    )
    primary_field_work_order_row = field_work_order_rows[0] if field_work_order_rows else {}
    ...
    summary = {
        ...
        "receipt_attach_packet_ready": not receipt_attach_blocked_lane_rows,
        "receipt_attach_lane_row_count": len(receipt_attach_lane_rows),
        "receipt_attach_blocked_lane_count": len(receipt_attach_blocked_lane_rows),
        "receipt_attach_primary_blocker_id": (
            receipt_attach_blocked_lane_rows[0]["lane_id"]
            if receipt_attach_blocked_lane_rows
            else ""
        ),
        "receipt_attach_primary_blocker": (
            receipt_attach_blocked_lane_rows[0]["blocker"]
            if receipt_attach_blocked_lane_rows
            else ""
        ),
        "receipt_attach_primary_operator_csv": (
            receipt_attach_blocked_lane_rows[0]["operator_csv"]
            if receipt_attach_blocked_lane_rows
            else ""
        ),
        "receipt_attach_primary_source_artifact": (
            receipt_attach_blocked_lane_rows[0]["source_artifact"]
            if receipt_attach_blocked_lane_rows
            else ""
        ),
        ...
        "field_work_order_primary_lane_id": _text(primary_field_work_order_row.get("lane_id")),
        "field_work_order_primary_field_name": _text(
            primary_field_work_order_row.get("field_name")
        ),
        "field_work_order_primary_pending_row_count": _int(
            primary_field_work_order_row.get("pending_row_count")
        ),
        "field_work_order_primary_required_value": _text(
            primary_field_work_order_row.get("required_value")
        ),
        "field_work_order_primary_required_action": _text(
            primary_field_work_order_row.get("required_action")
        ),
        "field_work_order_primary_approval_token_required": _text(
            primary_field_work_order_row.get("approval_token_required")
        ),
        "field_work_order_primary_operator_csv": _text(
            primary_field_work_order_row.get("operator_csv")
        ),
        "field_work_order_primary_source_artifact": _text(
            primary_field_work_order_row.get("source_artifact")
        ),
...
def _render_md(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- receipt_attach_lane_row_count: `{summary['receipt_attach_lane_row_count']}`",
    ...
    lines.extend(
        [
            "",
            "## Receipt Attach Lanes",
            "",
            "| lane | status | rows | pending values | pending approvals | blocker |",
            "| --- | --- | ---: | ---: | ---: | --- |",
        ]
    )
    for row in payload.get("receipt_attach_lane_rows", []):
        lines.append(
