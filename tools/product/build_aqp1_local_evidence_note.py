...
DEFAULT_PROFILE_JSON = "config/ligand_htvs_blind_aqp1_v1.json"
DEFAULT_TARGET_CSV = "config/real_drug_targets_blind_aqp1_v1.csv"
DEFAULT_MANUAL_QUEUE_JSON = "runs/aqp1_manual_review_queue_current.json"
DEFAULT_P0_PLAN_JSON = "runs/aqp1_p0_packet_plan_current.json"
DEFAULT_OUT_JSON = "runs/aqp1_local_evidence_note_current.json"
DEFAULT_OUT_CSV = "runs/aqp1_local_evidence_note_current.csv"
DEFAULT_OUT_MD = "runs/aqp1_local_evidence_note_current.md"
...
def build_payload(
    ...
    p0_plan_payload: dict[str, Any],
) -> dict[str, Any]:
    aqp1_ref = [row for row in reference_rows if str(row.get("target", "")).strip() == TARGET_ID]
    aqp1_meta = [row for row in meta_rows if str(row.get("ligand_id", "")).strip().startswith("aqp1_placeholder_")]
    aqp1_split = [row for row in split_rows if str(row.get("target", "")).strip() == TARGET_ID]
    aqp1_target = next((row for row in target_rows if str(row.get("target", "")).strip() == TARGET_ID), {})
    placeholder_reference_count = sum(1 for row in aqp1_ref if "template_placeholder" in str(row.get("source", "")))
    placeholder_meta_count = sum(1 for row in aqp1_meta if "template_placeholder" in str(row.get("scaffold", "")))
    placeholder_split_count = sum(1 for row in aqp1_split if str(row.get("ligand_id", "")).strip().startswith("aqp1_placeholder_"))
    binder_rows = [row for row in aqp1_ref if str(row.get("is_binder", "")).strip() == "1"]
    nonbinder_rows = [row for row in aqp1_ref if str(row.get("is_binder", "")).strip() == "0"]
    ...
    p0_summary = dict(p0_plan_payload.get("summary", {}) or {})
    fit_targets = str(profile_payload.get("hard_decoy_fit_targets", "") or "")
    next_required_step = (
        "Keep all AQP1 rows draft/manual-review only. Replace placeholder ligand reference/meta/split rows with transporter-specific evidence and freeze a non-EGFR donor policy before any authoritative apply."
    )
    checks = [
        {
            ...
            "notes": "No transporter-specific AQP1 binder evidence is locally curated yet.",
        },
        {
            ...
            "notes": "Negative-like AQP1 slots stay review-only; do not inject proxy non-binder values.",
        },
        {
        ...
        },
        {
            ...
            "notes": "All six AQP1 ligand packet rows are still placeholders.",
        },
    ]
    return {
        "summary": {
            "target_id": TARGET_ID,
            "pdb_id": str(aqp1_target.get("pdb_id", "")).strip(),
            ...
            "aqp1_reference_row_count": len(aqp1_ref),
            "aqp1_meta_row_count": len(aqp1_meta),
            "aqp1_split_row_count": len(aqp1_split),
            ...
            "aqp1_p0_todo_count": int(p0_summary.get("todo_count", 0) or 0),
...
def _write_markdown(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- aqp1_reference_row_count: `{s['aqp1_reference_row_count']}`",
        f"- aqp1_meta_row_count: `{s['aqp1_meta_row_count']}`",
        f"- aqp1_split_row_count: `{s['aqp1_split_row_count']}`",
        ...
        f"- aqp1_p0_todo_count: `{s['aqp1_p0_todo_count']}`",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a local-evidence note explaining why AQP1 remains draft/manual-review only.")
    ...
    parser.add_argument("--p0-plan-json", default=DEFAULT_P0_PLAN_JSON)
