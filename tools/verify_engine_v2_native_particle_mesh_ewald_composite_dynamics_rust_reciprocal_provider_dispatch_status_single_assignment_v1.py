#!/usr/bin/env python3
"""Verify immutable single assignment of native PME provider dispatch status."""
...
SCHEMA_ID = (
    ...
    "rust_reciprocal_provider_dispatch_status_single_assignment_profile/1.0.0"
)
SOURCE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_dispatch_status_single_assignment_sources/1.0.0"
)
...
PREDECESSOR = {
    "pull_request": 478,
    "reviewed_head": "c61fd0637c25cbb09c762f6ed5dea70814bf7145",
    "merge_commit": "e02cb7721e50d35f0a8680cec12ac24801450bba",
    "merge_tree": "6369559489af14a8bfe604ab6af2cdc9b298e722",
    "profile_path": PREDECESSOR_PROFILE_RELATIVE_PATH.as_posix(),
    "profile_sha256": (
        "9c964f62946dd50e774ee24892ee2867a1c5d7c29deb63a21b8670275458593a"
    ),
    "source_manifest_path": PREDECESSOR_MANIFEST_RELATIVE_PATH.as_posix(),
    "source_manifest_sha256": (
        "f49e07a0b1d5950d84564ae579f0e82069f90046552c943689d379cf59fec9aa"
    ),
    "source_manifest_entry_count": 399,
}

PREDECESSOR_EVIDENCE_SHA256 = {
    PREDECESSOR_WORKFLOW_RELATIVE_PATH: (
        "39373b25f66fd19b2fc968b82dd0417ae04a9873913a60e28678cc4e764cb045"
    ),
    ...
    PREDECESSOR_DOC_RELATIVE_PATH: (
        "0067192ecdb8f2c76daff092b0abc4cd829886b00e768862c36f639c23dd8257"
    ),
    PREDECESSOR_UNIT_RELATIVE_PATH: (
        "e06223008d3a590fadb89c907aead687f53c1ae7c497c96ee62457c94727aabb"
    ),
    PREDECESSOR_VERIFIER_RELATIVE_PATH: (
        "21c2dc86925fb8e6ea05c06245a0cf91a6786286c8e1d54f9a8bc86fe97ab98c"
    ),
}
EXPECTED_PREDECESSOR_IMPLEMENTATION_SHA256 = (
    "2bd60c2a03a2c3520708eca21ce713ffbf8e4809565286991102a505aef18bc6"
)
...
OLD_DISPATCH = """    std::int32_t raw_status;
    if (compute_forces) {
...
NEW_DISPATCH = """    const std::int32_t raw_status = [&]() -> std::int32_t {
        if (compute_forces) {
            bg_rust_particle_mesh_reciprocal_force_output_v1 provider_forces{};
            active_provider_force_scratch.x.resize(atom_count);
            active_provider_force_scratch.y.resize(atom_count);
...
def require_predecessor() -> dict:
    ...
    if canonical_bytes(profile) != profile_raw or canonical_bytes(manifest) != manifest_raw:
        fail("PR 478 predecessor evidence is not canonical JSON")
    rows = manifest.get("files")
    if not isinstance(rows, list) or len(rows) != 399:
        fail("PR 478 predecessor manifest count drift")
    if [row.get("path") for row in rows] != sorted(
        {row.get("path") for row in rows}
    ):
        fail("PR 478 predecessor manifest paths are not sorted and unique")
    for path in (
        PREDECESSOR_PROFILE_RELATIVE_PATH,
        PREDECESSOR_MANIFEST_RELATIVE_PATH,
        PREDECESSOR_DOC_RELATIVE_PATH,
        ...
        if (ROOT / path).read_bytes() != frozen_bytes(path):
            fail("checked-out PR 478 predecessor evidence drift: %s" % path)
    ...
    if git("cat-file", "-e", "%s^{commit}" % reviewed, check=False).returncode == 0:
        ...
        if reviewed_tree != PREDECESSOR["merge_tree"]:
            fail("PR 478 reviewed-head tree drift")
    return manifest
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            ...
            "dispatch_status_single_assignment_current_sources_tests_evidence_"
            "pr478_target"
        ),
...
def build_profile(manifest_raw: bytes, root: Path = ROOT) -> dict:
    ...
    implementation.pop("scope_is_only_native_force_descriptor_branch_localization", None)
    implementation.update(
        {
            "scope_is_only_native_dispatch_status_single_assignment": True,
    ...
    validation.update(
        {
            ...
            "predecessor_adapter_exact_dispatch_status_single_assignment_transform": True,
...
def expected_predecessor_workflow() -> str:
    ...
    for predecessor_path, successor_path in zip(
        ...
        if expected.count(anchor) != 2:
            fail("exact PR 478 workflow trigger anchor drift: %s" % predecessor_path)
        expected = expected.replace(
            anchor, anchor + '      - "%s"\n' % successor_path.as_posix()
        )
    old_region = source_region(
        expected,
        "      - name: Materialize exact PR 477 target and reviewed head\n",
        "\n\n  native-linux:\n",
        "exact PR 478 immutable-evidence block",
    )
    new_region = """      - name: Materialize exact PR 478 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse e02cb7721e50d35f0a8680cec12ac24801450bba^{tree})" = "6369559489af14a8bfe604ab6af2cdc9b298e722"
          git merge-base --is-ancestor e02cb7721e50d35f0a8680cec12ac24801450bba HEAD
          git fetch --no-tags --depth=1 origin refs/pull/478/head
          test "$(git rev-parse FETCH_HEAD)" = "c61fd0637c25cbb09c762f6ed5dea70814bf7145"
          test "$(git rev-parse FETCH_HEAD^{tree})" = "6369559489af14a8bfe604ab6af2cdc9b298e722"
      - name: Verify exact frozen PR 478 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=e02cb7721e50d35f0a8680cec12ac24801450bba
          frozen_tree=6369559489af14a8bfe604ab6af2cdc9b298e722
          ...
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_force_descriptor_branch_localization_profile_v1.json >/dev/null
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_force_descriptor_branch_localization_profile_v1_sources.json >/dev/null
          python3 -m pip install pytest==8.3.5
          python3 -m tools.verify_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_force_descriptor_branch_localization_v1
          python3 -m pytest -q tests/unit/test_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_force_descriptor_branch_localization_v1.py
          restore
          trap - EXIT"""
    return replace_once(
        expected, old_region, new_region, "exact PR 478 predecessor workflow transform"
    )
...
def expected_predecessor_unit() -> str:
    ...
    expected = replace_once(
        ...
        "from pathlib import Path\n",
        "from pathlib import Path\n\nimport pytest\n",
        "exact PR 478 unit pytest import",
    )
...
pytestmark = pytest.mark.skipif(
    ...
    return replace_once(
        ...
        skip,
        "exact PR 478 unit successor skip",
    )


def expected_successor_workflow() -> str:
    old_hyphen = "rust-reciprocal-provider-force-descriptor-branch-localization"
    new_hyphen = "rust-reciprocal-provider-dispatch-status-single-assignment"
    old_underscore = "rust_reciprocal_provider_force_descriptor_branch_localization"
    new_underscore = "rust_reciprocal_provider_dispatch_status_single_assignment"
    expected = frozen_bytes(PREDECESSOR_WORKFLOW_RELATIVE_PATH).decode()
    ...
    replacements = (
        ("Materialize exact PR 477 target", "Materialize exact PR 478 target"),
        ("78a5ba2e0cc1cc6a8b71deca134ced017d6bbe51", PREDECESSOR["merge_commit"]),
        ("35073ef5ad542e1add4ed64fe0752a965b333b08", PREDECESSOR["merge_tree"]),
        ("refs/pull/477/head", "refs/pull/478/head"),
        ("1bff4017f9354e5f068c63475525220ddec99850", PREDECESSOR["reviewed_head"]),
    )
...
def require_workflow_contract(root: Path = ROOT) -> None:
    ...
    if predecessor != expected_predecessor_workflow():
        fail("PR 478 predecessor workflow is not the exact frozen-object transform")
    if successor != expected_successor_workflow():
        fail("successor workflow is not the exact PR 478-derived transform")
    expected_names = {
    ...
    for path, expected_name in expected_names.items():
        ...
        if len(pull_paths) != 222 or len(set(pull_paths)) != 222:
            fail("workflow 222-path unique pull-request trigger drift: %s" % path)
    ...
    for token in (
        ...
        if token not in predecessor:
            fail("PR 478 predecessor workflow freeze drift: %s" % token)
...
def require_predecessor_unit_freeze(root: Path = ROOT) -> None:
    ...
    if source != expected_predecessor_unit():
        fail("PR 478 predecessor unit is not the exact frozen-object transform")
    ...
    if not any("frozen PR 478 object" in value for value in constants):
        fail("PR 478 predecessor unit frozen-object reason drift")
    if source.count("pytest.mark.skipif(") != 1:
        fail("PR 478 predecessor unit skip drift")
...
def require_adapter_contract(root: Path = ROOT) -> None:
    ...
    if sha(frozen) != EXPECTED_PREDECESSOR_IMPLEMENTATION_SHA256:
        fail("exact PR 478 adapter digest drift")
    frozen_source = frozen.decode()
    expected = replace_once(
        frozen_source, OLD_DISPATCH, NEW_DISPATCH, "dispatch status single assignment"
    )
    for path in IMPLEMENTATION_DELTA_PATHS:
        ...
        if raw.decode() != expected:
            fail("adapter is not the exact PR 478 single-assignment transform: %s" % path)
    ...
    dispatch = source_region(
        ...
        "single-assignment dispatch",
    )
    if canonical.count("const std::int32_t raw_status =") != 1:
        fail("dispatch status const declaration count drift")
    if "std::int32_t raw_status;" in canonical:
        fail("uninitialized dispatch status declaration remains")
    if re.findall(r"(?m)^\s*raw_status\s*=", canonical):
        fail("dispatch status branch assignment remains")
    ...
    if sha(frozen_bytes(ADAPTER_TEST_RELATIVE_PATH)) != EXPECTED_PREDECESSOR_ADAPTER_TEST_SHA256:
        fail("exact PR 478 native adapter test digest drift")
    if sha(test_raw) != EXPECTED_PREDECESSOR_ADAPTER_TEST_SHA256:
        fail("native adapter test changed from exact PR 478 bytes")


def require_docs_contract(root: Path = ROOT) -> None:
    doc = (root / DOC_RELATIVE_PATH).read_text()
    required = (
        "exact PR 478 predecessor",
        "explicit `std::int32_t` return type",
        "immediately invoked",
        "`const std::int32_t raw_status`",
        "zero uninitialized `raw_status` declarations",
        "zero branch assignments to `raw_status`",
        "force descriptor remains local to the force branch",
        "common `provider_error` descriptor",
        "post-dispatch status normalization, validation, rollback, and commit remain exact PR 478 bytes",
        "four blockers",
