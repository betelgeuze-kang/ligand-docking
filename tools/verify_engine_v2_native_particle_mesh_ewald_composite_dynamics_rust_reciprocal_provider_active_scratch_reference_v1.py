#!/usr/bin/env python3
"""Verify native PME Rust-adapter active-scratch reference binding."""
...
SCHEMA_ID = (
    ...
    "rust_reciprocal_provider_active_scratch_reference_"
    "profile/1.0.0"
)
SOURCE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_particle_mesh_ewald_composite_dynamics_"
...
PREDECESSOR = {
    "pull_request": 475,
    "reviewed_head": "adde41de6ed90afa3de4b5640c5ea22c24772686",
    "merge_commit": "a843b3312c262ad8c64de2cfc70ce25ed319d2f2",
    "merge_tree": "dcba84c0d0d3a396904e8699689f6202cc4a0916",
    "profile_path": PREDECESSOR_PROFILE_RELATIVE_PATH.as_posix(),
    "profile_sha256": (
        "22a96b5edec400607a86c1063a82ffb1dfa6efe6e79762d33a1aef273623181e"
    ),
    "source_manifest_path": PREDECESSOR_MANIFEST_RELATIVE_PATH.as_posix(),
    "source_manifest_sha256": (
        "f4b073b7992d72c17a6ab81831971492fae3c7c2f31d32c4e83dd5a8e4c21881"
    ),
    "source_manifest_entry_count": 381,
}
...
EXPECTED_PREDECESSOR_WORKFLOW_SHA256 = (
    "799fc23578aac957e1e22d687d97dc782bba2a77fc37993efbbf9008c3975447"
)
EXPECTED_PREDECESSOR_UNIT_SHA256 = (
    "d7d8811c2688d73a93d0ef699253f1b5c491871c0655135e3c7abf746ef97b13"
)
...
def expected_predecessor_workflow() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_WORKFLOW_SHA256:
        fail("exact frozen PR 475 workflow digest drift")
    ...
    for predecessor_path, successor_path in evidence_wiring:
        ...
        if expected.count(anchor) != 2:
            fail("exact frozen PR 475 workflow trigger anchor drift: %s" % predecessor_path)
    ...
    old_region = source_region(
        expected,
        "      - name: Materialize exact PR 474 target and reviewed head\n",
        "\n\n  native-linux:\n",
        "exact frozen PR 475 immutable-evidence predecessor block",
    )
    new_region = """      - name: Materialize exact PR 475 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse a843b3312c262ad8c64de2cfc70ce25ed319d2f2^{tree})" = "dcba84c0d0d3a396904e8699689f6202cc4a0916"
          git merge-base --is-ancestor a843b3312c262ad8c64de2cfc70ce25ed319d2f2 HEAD
          git fetch --no-tags --depth=1 origin refs/pull/475/head
          test "$(git rev-parse FETCH_HEAD)" = "adde41de6ed90afa3de4b5640c5ea22c24772686"
          test "$(git rev-parse FETCH_HEAD^{tree})" = "dcba84c0d0d3a396904e8699689f6202cc4a0916"
      - name: Verify exact frozen PR 475 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=a843b3312c262ad8c64de2cfc70ce25ed319d2f2
          frozen_tree=dcba84c0d0d3a396904e8699689f6202cc4a0916
          ...
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_two_symbol_dispatch_consolidation_profile_v1.json >/dev/null
          python3 -m json.tool config/engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_two_symbol_dispatch_consolidation_profile_v1_sources.json >/dev/null
          python3 -m pip install pytest==8.3.5
          python3 -m tools.verify_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_two_symbol_dispatch_consolidation_v1
          python3 -m pytest -q tests/unit/test_engine_v2_native_particle_mesh_ewald_composite_dynamics_rust_reciprocal_provider_two_symbol_dispatch_consolidation_v1.py
          restore
          trap - EXIT"""
    return replace_once(
        expected,
        old_region,
        new_region,
        "exact frozen PR 475 immutable-evidence block",
    )
...
def expected_predecessor_unit() -> str:
    ...
    if sha(frozen_raw) != EXPECTED_PREDECESSOR_UNIT_SHA256:
        fail("exact frozen PR 475 unit digest drift")
    expected = frozen_raw.decode()
    expected = replace_once(
        expected,
        "from pathlib import Path\n",
        "from pathlib import Path\n\nimport pytest\n",
        "exact frozen PR 475 unit pytest import",
    )
...
pytestmark = pytest.mark.skipif(
    ...
    return replace_once(
        ...
        skip,
        "exact frozen PR 475 unit successor skip",
    )
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            ...
            "active_scratch_reference_current_sources_tests_evidence_"
            "pr475_target"
        ),
...
def build_profile(manifest_raw: bytes) -> dict:
    ...
    return {
        ...
        "implementation": {
            "scope_is_only_native_provider_active_scratch_reference_binding_and_member_syntax": True,
        ...
        "validation": {
            ...
            "source_manifest_entry_count_exact": 387,
            "pull_request_trigger_path_count_exact": 204,
            "push_trigger_path_count_exact": 204,
            ...
            "predecessor_adapter_exact_active_scratch_reference_transform": True,
            "active_scratch_reference_binding_source_exact": True,
            ...
            "provider_validation_and_commit_exact_after_member_syntax_normalization": True,
            "provider_force_scratch_destructor_exact_predecessor_bytes": True,
            "two_branch_two_symbol_adapter_dispatch_exact_after_member_syntax_normalization": True,
            "force_private_symbol_adapter_callsite_count_exact": 1,
            "energy_private_symbol_adapter_callsite_count_exact": 1,
...
def require_workflow_contract(root: Path = ROOT) -> None:
    ...
    if predecessor != expected_predecessor_workflow():
        fail("PR 475 predecessor workflow is not the exact frozen-object transformation")
    for workflow, expected_name in (
        ...
        if len(pull_paths) != 204 or len(set(pull_paths)) != 204 or push_paths != pull_paths:
            fail("workflow 204-path symmetric trigger contract drift: %s" % expected_name)
    ...
    release_region = source_region(
        successor,
        "          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-active-scratch-reference-release ",
        "          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-active-scratch-reference-sanitize ",
    ...
    for token in (
        ...
        if token not in predecessor:
            fail("PR 475 predecessor workflow freeze drift: %s" % token)
...
def require_predecessor_unit_freeze(root: Path = ROOT) -> None:
    ...
    if unit != expected_predecessor_unit():
        fail("PR 475 predecessor unit is not the exact frozen-object transformation")
    constants = {
        node.value
    ...
    for token in (
        ...
        if unit.count(token) != 1:
            fail("PR 475 predecessor unit freeze drift: %s" % token)
    if PROFILE_RELATIVE_PATH.as_posix() not in constants:
        fail("PR 475 predecessor unit successor-profile path drift")
    if not any("exact frozen PR 475 object" in value for value in constants):
        fail("PR 475 predecessor unit frozen-object reason drift")
    if "is_file()" not in unit:
        fail("PR 475 predecessor unit skip is not successor-profile scoped")
...
def require_exact_source_hashes(root: Path = ROOT) -> None:
    ...
    for relative in IMPLEMENTATION_DELTA_PATHS:
        ...
        if sha(predecessor_raw) != EXPECTED_PREDECESSOR_IMPLEMENTATION_SHA256[relative]:
            fail("frozen PR 475 implementation digest drift: %s" % relative)
...
""",
    ...
    ProviderForceScratch &active_provider_force_scratch =
        reuse_force_storage ? *provider_force_scratch
                            : local_provider_force_scratch.emplace();
""",
        "nullable active scratch pointer to non-null reference binding",
    )
    pointer_access = "active_provider_force_scratch->"
    reference_access = "active_provider_force_scratch."
    if expected_adapter.count(pointer_access) != 18:
        fail("frozen PR 475 active scratch pointer-access count drift")
    expected_adapter = expected_adapter.replace(pointer_access, reference_access)
    if expected_adapter.count(reference_access) != 18:
        fail("successor active scratch reference-access count drift")
    for relative in (ADAPTER_RELATIVE_PATH, VENDOR_ADAPTER_RELATIVE_PATH):
        if (root / relative).read_text() != expected_adapter:
            fail("adapter is not the exact PR 475 active-scratch reference transform: %s" % relative)
    ...
    if sha(frozen_adapter_test) != EXPECTED_PREDECESSOR_ADAPTER_TEST_SHA256:
        fail("frozen PR 475 native adapter-test digest drift")
    if (root / ADAPTER_TEST_RELATIVE_PATH).read_bytes() != frozen_adapter_test:
        fail("native adapter test changed from exact PR 475 bytes")
...
def require_native_contract(root: Path = ROOT) -> None:
    ...
    if rollback != frozen_rollback:
        fail("reusable Evaluation rollback guard changed from exact PR 475 bytes")
    ...
    if normalized_dispatch != frozen_dispatch:
        fail("two-symbol dispatch changed beyond active scratch member syntax")
    ...
    normalized_validation_and_commit = validation_and_commit.replace(
        "active_provider_force_scratch.",
        "active_provider_force_scratch->",
    )
    if normalized_validation_and_commit != frozen_validation_and_commit:
        fail("provider validation or external commit changed beyond active scratch member syntax")
    for token in (
    ...
    if destructor != frozen_destructor:
        fail("ProviderForceScratch destructor changed from exact PR 475 bytes")
...
def require_test_contract(root: Path = ROOT) -> None:
    ...
    if transactional_peer != frozen_transactional_peer:
        fail("raw-public transactional force-output peer block changed from PR 475")
    ...
    if stateless_predecessor_tests != frozen_stateless_predecessor_tests:
        fail("stateless lifecycle tests changed from exact PR 475 bytes")
...
def require_docs_contract(root: Path = ROOT) -> None:
    ...
    for token in (
        "changes only the native C++ adapter active `ProviderForceScratch` binding and its member-access syntax",
        "external `ProviderForceScratch *provider_force_scratch` parameter remains nullable at the function boundary",
        "existing reusable-owner null guard remains before the new reference binding",
        "exactly one non-null `ProviderForceScratch &active_provider_force_scratch`",
        "reusable calls select `*provider_force_scratch`, while stateless calls select `local_provider_force_scratch.emplace()`",
        "predecessor mutable nullable active pointer, its post-declaration reseat branch, and all active-pointer `->` member accesses are removed",
        "one active reference declaration and 18 active-reference `.` member accesses",
        "exact same transformation of the exact PR 475 predecessor",
        "replacing one pointer selection block and exactly 18 `active_provider_force_scratch->` tokens",
        "No other adapter byte may change",
        "two `compute_forces` branches, two provider symbols, five semantic route classes",
        "not claimed byte-identical because their member syntax changes from `->` to `.`",
        "equal the PR 475 regions exactly after normalizing only that member syntax",
        "native fake-provider adapter test is unchanged from exact PR 475 bytes",
        ...
        "predecessor workflow detaches the exact PR 475 merge object",
        "type-state and ownership hardening only",
        "No allocation-free, allocation-count, allocation-behavior, heap-allocation-elision, provider-allocation-elision, null-check-elision performance, reference-binding performance, stack-storage reduction, scratch-footprint reduction, object-size reduction, peak-memory reduction, performance, acceleration, scientific-equivalence, molecular, HIP, product, or operational claim",
