#!/usr/bin/env python3
"""Verify direct final-SoA force recording for stateful direct-Ewald dynamics."""
...
EXPECTED_TEST_SOURCE_SHA256 = (
    "387b429b7c1a77b54340af39e55a32a1b07dd239f3b65c1aac0e8ac9f6abec9c"
)
EXPECTED_PRODUCTION_SOURCE_SHA256 = {
    "direct_ewald.cpp":
        "79c60ff449553712b1cd9e5e6e7b40f21b9d2dff3ab64c555cc3aff60ab6308f",
    "direct_ewald_composite_dynamics.cpp":
        "d1580f6d47d5f21084cdb2ec723c3bb72ac67677142932c111faf90ce32d65f1",
    "evaluator.hpp":
        "a111814480c231a17a1cafd5c7db5906acd1fe78433b7fbd7cf6d19dff7a969c",
}
EXPECTED_PREDECESSOR_WORKFLOW_SHA256 = (
    "7a08374328007101ee15e26f07569f51e2b63440deb96c803254d455f88650d8"
)
EXPECTED_PREDECESSOR_UNIT_SHA256 = (
    "ace8ae27f08c1861951315914cee6fff22121eb7251cc2b69fc9e774b6b364ed"
)
    ...
    steps:
      - name: Check out exact evidence graph
        ...
        with:
          ...
          clean: true
          fetch-depth: 0
      - name: Materialize exact PR 450 architecture, PR 449 target, and PR 446 inherited final SoA
        shell: bash
        run: |
          set -euo pipefail
          ...
          git merge-base --is-ancestor 11ee408d89c44e70188af5133544ecebd604b182 75d3a4e2b7ba5b0f1dcf99007358f6f2c47c7330
          ...
          git merge-base --is-ancestor 29edcd1ea18e9fb64b9d416a0d05d87e0485be4b 11ee408d89c44e70188af5133544ecebd604b182
...
EXPECTED_NATIVE_JOB_BODY = """    runs-on: ubuntu-latest
    ...
    steps:
      ...
      - name: Synthetic release and sanitizer regressions
        run: |
          cmake -S . -B build/direct-ewald-combined-force-soa-release -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF
          ...
          cmake -S . -B build/direct-ewald-combined-force-soa-sanitize -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF -DCMAKE_C_FLAGS='-fsanitize=address,undefined -fno-omit-frame-pointer' -DCMAKE_CXX_FLAGS='-fsanitize=address,undefined -fno-omit-frame-pointer'
          cmake --build build/direct-ewald-combined-force-soa-sanitize --target betelgeuze_engine_direct_ewald_composite_dynamics --parallel 2
          ASAN_OPTIONS=detect_leaks=1 UBSAN_OPTIONS=halt_on_error=1 ctest --test-dir build/direct-ewald-combined-force-soa-sanitize -R '^betelgeuze_engine_direct_ewald_composite_dynamics$' --output-on-failure
"""

EXPECTED_RUST_JOB_BODY = """    runs-on: ubuntu-latest
    timeout-minutes: 35
    env:
      CARGO_TARGET_DIR: ${{ github.workspace }}/build/direct-ewald-combined-force-soa-cargo
    steps:
...
EXPECTED_MACOS_JOB_BODY = """    runs-on: macos-15
    ...
    steps:
      ...
      - name: Exact export regression
        run: |
          cmake -S . -B build/direct-ewald-combined-force-soa-macos -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF
          cmake --build build/direct-ewald-combined-force-soa-macos --target betelgeuze_engine --parallel 2
          ctest --test-dir build/direct-ewald-combined-force-soa-macos -R '^betelgeuze_engine_export_allowlist$' --output-on-failure
"""
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            "direct_ewald_composite_dynamics_stateful_combined_force_soa_"
...
def build_profile(manifest_raw: bytes) -> dict:
    ...
    return {
        ...
        "implementation": {
            ...
            "direct_ewald_failure_precedes_final_soa_write": True,
            ...
            "late_ewald_failure_last_successful_final_soa_bits_preserved": True,
        ...
        "validation": {
            ...
            "repeated_late_ewald_failure_authoritative_transactionality": True,
            "late_ewald_failure_last_successful_final_soa_bits": True,
...
def expected_frozen_predecessor_workflow(frozen: str) -> str:
    old_materialize = (
        "      - name: Materialize exact PR 449 architecture, PR 448 target, "
        "and PR 445 inherited force scratch\n"
    )
    ...
    replacement = """      - name: Materialize exact PR 450 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 75d3a4e2b7ba5b0f1dcf99007358f6f2c47c7330^{tree})" = "03ccd07339b71eafa435a9b2012d2ab6a863d4d9"
          ...
          test "$(git rev-parse FETCH_HEAD)" = "b0e26a8b2eea995a6038a484894808387486ff9e"
          test "$(git rev-parse FETCH_HEAD^{tree})" = "03ccd07339b71eafa435a9b2012d2ab6a863d4d9"
      - name: Verify exact frozen PR 450 evidence
        shell: bash
        run: |
          set -euo pipefail
...
def expected_frozen_predecessor_unit(frozen: str) -> str:
    ...
    addition = """DIRECT_EWALD_COMBINED_FORCE_SOA_EVIDENCE_PRESENT = (
    ROOT
    / "config/engine_v2_native_direct_ewald_composite_dynamics_"
    "combined_force_soa_profile_v1.json"
).is_file()
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        "particle-mesh Ewald composite dynamics short-parent force scratch "
        "evidence is verified from its exact frozen object after direct-Ewald "
        "combined-force SoA evidence is present"
    ),
)
"""
...
""",
        "validated direct final-SoA recording with stateless AoS fallback",
    )
...
def require_exact_regression_source(test: bytes) -> None:
    if sha(test) != EXPECTED_TEST_SOURCE_SHA256:
        fail("exact direct-Ewald dynamics regression source drift")
...
def require_combined_force_soa_contract(root: Path = ROOT) -> None:
    ...
    transforms = (
        ...
        ("direct_ewald_composite_dynamics.cpp", expected_dynamics_source),
    ...
    if observed_production != EXPECTED_PRODUCTION_SOURCE_SHA256:
        fail("exact direct-Ewald combined-force SoA production hashes drift")

    direct_source = (root / "native/src/composite/direct_ewald.cpp").read_text()
    direct_tokens = (
        "cpu::Evaluation *stateful_force_output,",
        "const bool requires_stateful_force_output =",
        "stateful_scratch && compute_forces;",
    ...
    if any(token not in direct_source for token in direct_tokens):
        fail("direct-Ewald final-SoA routing or validation drift")
    if direct_source.count("candidate.forces.resize(atom_count);") != 1:
        fail("stateless combined AoS fallback count drift")
    validation = direct_source.index(
        "const std::array<double, 9> parent_and_combined_forces{{"
    )
    final_write = direct_source.index(
    ...
    ewald_status = direct_source.index(
        "if (status != BG_STATUS_OK) {\n        return out_error->code",
        direct_source.index("ewald::Evaluation ewald_evaluation;"),
    )
    if ewald_status >= final_write:
        fail("direct-Ewald failure no longer precedes final SoA write")
    if direct_source.count("cpu::Evaluation *stateful_force_output") != 1:
    ...
    test_tokens = (
        ...
        "direct stateful SoA force bits differed from stateless AoS bits",
        "direct SoA force output differed from the peer bits",
        ...
        "late direct-Ewald failure overwrote the last successful final force bits",
    ...
    checkpoint_sources = (
        ...
        + (root / "native/src/composite/direct_ewald_composite_dynamics.hpp").read_text()
    )
