...
DEFAULT_RENDER_SUITE_JSON = "runs/dengue_ns2b_ns3_protease_render_suite_current.json"
DEFAULT_CONDITION_CARD_JSON = "runs/dengue_ns2b_ns3_protease_condition_card_current.json"
...
DEFAULT_OUT_MD = "runs/dengue_ns2b_ns3_protease_launch_packet_current.md"
...
def build_payload(
    ...
    rep_rows = [
        ...
        if str(row.get("target_id", "")).strip() == "Dengue NS2B-NS3 protease"
    ]
    nov_rows = [
        dict(row)
    ...
    return {
        "summary": {
            ...
            "partner_track_id": str(suite_s.get("partner_track_id", "IPK_dengue")).strip() or "IPK_dengue",
            "render_suite_status": str(suite_s.get("status", "")).strip(),
            "export_status": "dengue_ns2b_ns3_protease_ipk_dengue_export_ready",
            "required_artifact_count": len(rows),
            "primary_biochemical_arm": "recombinant dengue NS2B-NS3 protease arm with shallow-pocket-aware detergent sanity",
            "orthogonal_flaviviral_arm": str(condition_s.get("orthogonal_flaviviral_arm", "")).strip(),
            ...
            "execution_goal": "Open Dengue NS2B-NS3 protease only after Cathepsin K resolves and the shallow-pocket packet is fully filled.",
            "blocking_rule": "Do not start Dengue NS2B-NS3 protease live execution until Cathepsin K resolves and both repurposing and novelty lanes are filled.",
            "next_target_on_success": "DprE1",
            "next_target_on_hold": "DprE1",
            "headline": "Dengue NS2B-NS3 protease packet designed to separate real shallow-pocket protease engagement from sticky assay behavior.",
            "next_required_step": (
                "Keep Dengue NS2B-NS3 protease as the second Wave 2 slot and wait for Cathepsin K to resolve before launch."
                if launch_readiness == "ready_for_serialized_execution"
                else "Keep Dengue NS2B-NS3 protease queued behind Cathepsin K, but do not launch until the compound lanes are fully filled."
            ),
        },
        "rows": rows,
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Dengue NS2B-NS3 protease serialized launch packet for Wave 2.")
...
def main() -> None:
    ...
    write_artifact(DEFAULT_OUT_MD, "Dengue NS2B-NS3 Protease Launch Packet", payload)
