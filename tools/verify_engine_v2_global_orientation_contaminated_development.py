...
SCHEMA_ID = (
    "betelgeuze.engine_v2_global_orientation_"
    "contaminated_development_protocol/1.0.0"
)
CASE_IDS = (
...
SOURCE_RECEIPT_FIELDS = (
    "case_id", "source_case_member_receipt_sha256",
    "authenticated_input_receipt_sha256", "receptor_coordinate_sha256",
    "ligand_coordinate_sha256", "ligand_topology_sha256",
    "pocket_declaration_sha256", "preparation_policy_sha256",
)
ALLOWED_INPUTS = (
    "prepared_ligand_coordinates", "declared_pocket_center",
    "declared_pocket_normal", "bounded_receptor_surface_points",
    "frozen_global_orientation_config",
)
FORBIDDEN_INPUTS = (
    "native_pose", "reference_pose", "rmsd", "candidate_score",
    "prior_benchmark_outcome", "fresh_holdout_identity",
    "product_routing_state",
)
AUTHORITY_KEYS = (
    "historical_development_execution_authorized",
    "fresh_holdout_execution_authorized", "stage0_admission_authority",
    "profile_promotion_authority", "product_execution_authorized",
...
def _verify_generator_boundary() -> None:
    parameters = tuple(
        inspect.signature(generate_global_orientation_batch).parameters
    )
    _exact(
        parameters,
        (
    ...
    forbidden_tokens = (
        "native", "reference", "rmsd", "score", "benchmark", "fresh",
        "product",
    )
    if any(
...
def verify_protocol(protocol: Mapping[str, Any]) -> str:
    expected_top_keys = {
        "arm_contract", "authority", "cohort", "decision", "execution_gate",
        "information_boundary", "metrics", "protocol_role",
        "protocol_sha256", "schema_id", "shared_execution_contract",
    ...
    _exact(tuple(cohort.get("historical_case_ids", ())), CASE_IDS,
    ...
    _exact(
        cohort.get("previously_uncovered_case_count"), 7,
        name="previously-uncovered count",
    )

    sources = _mapping(protocol.get("source_bindings"),
    ...
    expected_sources = {
        ...
        "global_orientation_synthetic_contract_schema_id":
            "betelgeuze.engine_v2_global_orientation_synthetic_contract/1.1.0",
        "global_orientation_synthetic_contract_sha256":
            "b86059aca76425aacc7a2cc28bac98e9ce87f026b399c6a839e9560ff7332a44",
    ...
    _exact(
        dict(shared),
        {
            ...
            "scorer_terms_schema_id":
                "betelgeuze.engine_v2_scorer_v1_terms/1.1.0",
    ...
    experimental = _mapping(
        arms.get("experimental"), name="experimental arm"
    )
    _exact(
    ...
    concrete = GlobalOrientationConfig(
        ...
        translation_shell_radii=tuple(
            config.get("translation_shell_radii", ())
        ),
        translation_points_per_shell=config.get(
            "translation_points_per_shell"
        ),
        minimum_receptor_distance=config.get(
            "minimum_receptor_distance"
        ),
    )
    _exact(concrete.candidate_slot_count, 64,
           name="experimental denominator")
    _exact(experimental.get("candidate_slot_count"), 64,
           name="experimental candidate count")
    _exact(
        experimental.get("candidate_slot_formula"),
        "orientation_count*(1+translation_shell_count*"
        "translation_points_per_shell)",
    ...
    _exact(
        ...
        ("success", "proposal_failure", "validity_failure",
         "ranking_failure"),
    ...
    _exact(
        ...
        (
            "proposal_oracle_rmsd", "valid_proposal_oracle_rmsd",
            "ranked_top1_oracle_rmsd", "ranked_top5_oracle_rmsd",
            "selected_top1_rmsd", "selection_regret",
            "generated_candidate_count", "accepted_candidate_count",
            "rejected_candidate_count", "failure_class",
        ),
    ...
    _exact(decision.get("go_requires_all_invariants"), True,
           name="Go invariant requirement")
    _exact(decision.get("result_cannot_change_protocol"), True,
           name="result independence")
    _exact(
    ...
    _exact(
        ...
        "permit_separate_review_of_global_orientation_"
        "development_followup_only",
        name="Go effect",
    )
    _exact(
    ...
    if any(authority.get(key) is not False for key in AUTHORITY_KEYS):
        raise GlobalOrientationDevelopmentProtocolError(
            "all execution, product, promotion, and claim authority "
            "must remain false"
        )
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=(
            ...
            / "config/engine_v2_global_orientation_"
