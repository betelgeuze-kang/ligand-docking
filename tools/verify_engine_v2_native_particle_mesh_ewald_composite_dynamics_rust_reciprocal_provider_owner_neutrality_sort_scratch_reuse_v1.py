#!/usr/bin/env python3
"""Verify owner-private neutrality-sort scratch reuse in the stateful Rust force route."""
...
SCHEMA_ID = (
    ...
    "rust_reciprocal_provider_owner_neutrality_sort_scratch_reuse_profile/1.0.0"
)
SOURCE_SCHEMA_ID = (
    "betelgeuze.engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_owner_neutrality_sort_scratch_reuse_sources/1.0.0"
)
PUBLIC_PROFILE_ID = (
...
PREDECESSOR = {
    "pull_request": 464,
    "reviewed_head": "c3a212bff356675ae7a27bb3c54020b6436db6dd",
    "merge_commit": "5a629c5fffbfc0e2526de953dcdaaa4d946a5ee9",
    "merge_tree": "fc9c801052e3a87129696d51aa00068eb0bdc383",
    ...
    "source_manifest_entry_count": 314,
}
...
EXPECTED_PREDECESSOR_WORKFLOW_SHA256 = (
    "81fd32462e4ab75790186a151d77adc19f6630a8391af9f5965c48134020d5c4"
)
EXPECTED_PREDECESSOR_UNIT_SHA256 = (
    "f56d4a7971e05053fc7c9d6cb40bf3233af73e9ac0bb7bfe365455d380921b8b"
)
    ...
    steps:
      - name: Check out exact evidence graph
        ...
        with:
          ...
          clean: true
          fetch-depth: 0
      - name: Materialize exact PR 464 target, PR 453 architecture, PR 440 inherited evaluator, and PR 380 direct-output precedent
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 5a629c5fffbfc0e2526de953dcdaaa4d946a5ee9^{tree})" = "fc9c801052e3a87129696d51aa00068eb0bdc383"
          git merge-base --is-ancestor 5a629c5fffbfc0e2526de953dcdaaa4d946a5ee9 HEAD
          git fetch --no-tags --depth=1 origin refs/pull/464/head
          test "$(git rev-parse FETCH_HEAD)" = "c3a212bff356675ae7a27bb3c54020b6436db6dd"
          test "$(git rev-parse FETCH_HEAD^{tree})" = "fc9c801052e3a87129696d51aa00068eb0bdc383"
          test "$(git rev-parse 35a8f0b0ba0e079bc2a1edee15d19ef2c2823f2a^{tree})" = "b22c5fd115a5c8e28856872df57127ecdd28d9b5"
          git merge-base --is-ancestor 35a8f0b0ba0e079bc2a1edee15d19ef2c2823f2a 5a629c5fffbfc0e2526de953dcdaaa4d946a5ee9
          ...
          git merge-base --is-ancestor 6662f1b53829930a93de0f298b820d5a367cc3dc 5a629c5fffbfc0e2526de953dcdaaa4d946a5ee9
...
EXPECTED_NATIVE_JOB_BODY = """    runs-on: ubuntu-latest
    ...
    steps:
      ...
      - name: Synthetic release and sanitizer regressions
        run: |
          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse-release -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF
          cmake --build build/particle-mesh-ewald-rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse-release --target betelgeuze_engine_particle_mesh_reciprocal betelgeuze_engine_particle_mesh_ewald_composite_dynamics --parallel 2
          ctest --test-dir build/particle-mesh-ewald-rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse-release -R '^betelgeuze_engine_(particle_mesh_reciprocal|particle_mesh_ewald_composite_dynamics|export_allowlist)$' --output-on-failure
          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse-sanitize -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF -DCMAKE_C_FLAGS='-fsanitize=address,undefined -fno-omit-frame-pointer' -DCMAKE_CXX_FLAGS='-fsanitize=address,undefined -fno-omit-frame-pointer'
          cmake --build build/particle-mesh-ewald-rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse-sanitize --target betelgeuze_engine_particle_mesh_reciprocal betelgeuze_engine_particle_mesh_ewald_composite_dynamics --parallel 2
          ASAN_OPTIONS=detect_leaks=1 UBSAN_OPTIONS=halt_on_error=1 ctest --test-dir build/particle-mesh-ewald-rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse-sanitize -R '^betelgeuze_engine_(particle_mesh_reciprocal|particle_mesh_ewald_composite_dynamics)$' --output-on-failure
"""

EXPECTED_RUST_JOB_BODY = """    runs-on: ubuntu-latest
    timeout-minutes: 35
    env:
      CARGO_TARGET_DIR: ${{ github.workspace }}/build/particle-mesh-ewald-rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse-cargo
    steps:
...
EXPECTED_MACOS_JOB_BODY = """    runs-on: macos-15
    ...
    steps:
      ...
      - name: Exact export regression
        run: |
          cmake -S . -B build/particle-mesh-ewald-rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse-macos -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DBG_ENABLE_HIP=OFF -DBG_ENABLE_HIP_SAFE=OFF
          cmake --build build/particle-mesh-ewald-rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse-macos --target betelgeuze_engine --parallel 2
          ctest --test-dir build/particle-mesh-ewald-rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse-macos -R '^betelgeuze_engine_export_allowlist$' --output-on-failure
"""
...
def expected_workflow_document() -> str:
    ...
    preamble = (
        ...
        "rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse\n\n"
        "on:\n"
        "  pull_request:\n"
        "    paths:\n"
        f"{trigger_paths}"
        ...
        "rust-reciprocal-provider-owner-neutrality-sort-scratch-reuse-${{ github.ref }}\n"
...
def build_source_manifest(root: Path = ROOT) -> dict:
    ...
    return {
        ...
        "scope": (
            ...
            "owner_neutrality_sort_scratch_reuse_current_sources_tests_evidence_target_"
...
def build_profile(manifest_raw: bytes) -> dict:
    ...
    return {
        ...
        "profile_id": (
            ...
            "rust_reciprocal_provider_owner_neutrality_sort_scratch_reuse_development_v1"
        ),
        "roadmap_issue": 434,
...
def expected_frozen_predecessor_workflow(frozen: str) -> str:
    ...
    old_materialize = source_region(
        expected,
        "      - name: Materialize exact PR 463 target, PR 453 architecture, "
        "PR 440 inherited evaluator, and PR 380 direct-output precedent\n",
        "      - name: Verify bounded successor evidence\n",
        "frozen predecessor materialization",
    )
    new_materialize = """      - name: Materialize exact PR 464 evidence and reviewed head
        shell: bash
        run: |
          set -euo pipefail
          test "$(git rev-parse 5a629c5fffbfc0e2526de953dcdaaa4d946a5ee9^{tree})" = "fc9c801052e3a87129696d51aa00068eb0bdc383"
...
"""
    new_verify = """      - name: Verify exact frozen PR 464 evidence
        shell: bash
        run: |
          set -euo pipefail
          frozen=5a629c5fffbfc0e2526de953dcdaaa4d946a5ee9
          frozen_tree=fc9c801052e3a87129696d51aa00068eb0bdc383
...
def expected_frozen_predecessor_unit(frozen: str) -> str:
    ...
    addition = """PME_RUST_RECIPROCAL_PROVIDER_OWNER_NEUTRALITY_SORT_SCRATCH_REUSE_EVIDENCE_PRESENT = (
    ROOT
    / "config/engine_v2_native_particle_mesh_ewald_composite_dynamics_"
    "rust_reciprocal_provider_owner_neutrality_sort_scratch_reuse_profile_v1.json"
).is_file()
pytestmark = pytest.mark.skipif(
    PME_RUST_RECIPROCAL_PROVIDER_OWNER_NEUTRALITY_SORT_SCRATCH_REUSE_EVIDENCE_PRESENT,
    reason=(
        "PME Rust reciprocal-provider owner reciprocal-workspace reuse "
        "evidence is verified from its exact frozen PR 464 object after "
        "owner neutrality-sort scratch reuse evidence is present"
    ),
)
"""
...
def require_rust_reciprocal_provider_owner_neutrality_sort_scratch_reuse_contract(
    root: Path = ROOT,
) -> None:
    expected_production = {
    ...
    for relative, predecessor_digest in EXPECTED_PREDECESSOR_DELTA_SHA256.items():
        ...
        if sha(predecessor_raw) != predecessor_digest:
            fail(f"frozen PR 464 source digest drift: {relative}")
        successor_raw = (root / relative).read_bytes()
        if sha(successor_raw) != EXPECTED_SUCCESSOR_DELTA_SHA256[relative]:
            fail(f"owner neutrality-sort successor source digest drift: {relative}")
    ...
    frozen_compensated_sum = source_region(
        frozen_rust,
        "impl CompensatedSum {\n",
        "\nfn accurate_order_independent_sum(",
...
def require_contracts(root: Path = ROOT) -> None:
    ...
    require_rust_reciprocal_provider_owner_neutrality_sort_scratch_reuse_contract(root)
