    ...
    try:
        ...
        try:
            ...
            while observed <= 16 * 1024 * 1024:
                chunk = os.read(source_fd, min(1 << 20, 16 * 1024 * 1024 + 1 - observed))
                if not chunk:
                    break
                chunks.append(chunk)
                observed += len(chunk)
            ...
            raw = b"".join(chunks)
            if observed != before.st_size or _stable_identity(before) != _stable_identity(after):
...
def _discover_sources(root: Path, trees: tuple[Path, ...], suffixes: set[str]) -> set[str]:
...
def require_bound_source_tree(root: Path, manifest: dict[str, object]) -> dict[str, bytes]:
    rows = manifest["files"]
    ...
    rust_expected = {
        ...
        if any(path == tree.as_posix() or path.startswith(f"{tree.as_posix()}/") for tree in RUST_SOURCE_ROOTS)
        and Path(path).suffix == ".rs"
    }
    native_expected = {
        path
        for path in expected
        if any(path == tree.as_posix() or path.startswith(f"{tree.as_posix()}/") for tree in NATIVE_SOURCE_ROOTS)
    ...
    if _discover_sources(root, NATIVE_SOURCE_ROOTS, NATIVE_SOURCE_SUFFIXES) != native_expected:
...
def require_activation_source_contract(sources: dict[str, bytes]) -> None:
    ...
    if (
        ...
        or "pub(crate) fn run_native_fixed64_cpu_qualification_successor" not in qualification
...
def main() -> int:
    ...
    v5_profile_raw = read_bound_source_bytes(root, V5_PROFILE_RELATIVE_PATH)
