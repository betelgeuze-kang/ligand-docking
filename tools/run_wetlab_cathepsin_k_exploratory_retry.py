...
from tools import build_wetlab_cathepsin_k_exploratory_retry_lane as lane_mod
from tools import run_wetlab_broad_screen_primary_runner as primary_runner_mod
from tools.wetlab_target_render_utils import load_json, write_artifact

TARGET_ID = "Cathepsin K"
DEFAULT_LANE_JSON = "runs/wetlab_cathepsin_k_exploratory_retry_lane_current.json"
...
DEFAULT_OUT_MD = "runs/wetlab_cathepsin_k_exploratory_retry_runner_current.md"
...
def run(
    *,
    ...
    if refresh_lane:
        ...
        write_artifact(lane_json.replace(".json", ".md"), "Wet-Lab Cathepsin K Exploratory Retry Lane", lane_payload)
    else:
    ...
    if not bool(summary.get("ready_for_manual_retry", False)):
        raise SystemExit("Cathepsin K exploratory retry lane is not ready_for_manual_retry")
    if not selected_shard or not selected_kind:
        raise SystemExit("Cathepsin K exploratory retry lane is missing shard_id or selected_command_kind")
    ...
    payload = {
        "summary": {
            "status": "wetlab_cathepsin_k_exploratory_retry_runner_ready",
            "target_id": target_id,
            "shard_id": selected_shard,
    ...
    write_artifact(DEFAULT_OUT_MD, "Wet-Lab Cathepsin K Exploratory Retry Runner", payload)
    return payload


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch the Cathepsin K exploratory retry lane.")
