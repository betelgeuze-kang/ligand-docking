...
This queue starts after the current restricted `kinase`, `ion_channel`, and `gpcr` P0 delivery claim. It is planning-only, blocked, and does not widen the current delivery verdict. The first active queue item is GPCR scale-up recovery because the current 100k claim remains blocked. Within that lane, `claim_promotion_allowed=false` stays in force; the non-leaky GPCR +3 freeze, frozen-candidate scoreability, family-held-out scorecard, and leakage audit are green, but the latest r2 row-level evidence only recovers HTR2A/OPRM1 and leaves DRD2 at global rank `18923` / target rank `5315`. The v3 family-anchor CI-stability packet `runs/gpcr_residual_prototype_spec_family_anchor_ci_stability_v3.json` is diagnostic-only (`prototype_mode=shadow_only`, `scorer_apply_allowed=false`) and exists only to explain why the v2 shadow CI-low remains below `0.45`. The v4 acidic-anchor overcontact gate is implemented as fixed-reference, shadow-only, active-locked evidence, but its first score-only replay is reject evidence (`PR-AUC=0.008231735935435774`, `PR-AUC CI-low=0.0009935430341614215`, `top20=0.00`, gate activation `0/40000`). The refreshed `gpcr_core_family_anchor_rescore_v2` shadow replay is recorded in `runs/gpcr_family_anchor_v2_shadow_replay_eval_current.json` and improves the point estimate (`PR-AUC=0.5767474245351905`, `ranking_pr_auc_ci_low=0.21066694653866244`, top20 `0.25`, DRD2 `global_rank=8562` / `target_rank=2435`) but remains below the `0.45` CI-low threshold, so CI-low stability, not point PR-AUC, is the blocker. It is the current best guarded-comparison design signal, not claim evidence. The base pairwise diagnostic slice still has `decoys_above_positive_count=5314`, `pairwise_win_rate=0.46854685468546853`, and `ready_for_guarded_apply=false`; the refreshed shadow projection reduces DRD2 decoys-above-positive to `2434` and raises pairwise win-rate to `0.7565756575657565`, but guarded apply remains blocked by CI-low stability. The next step is claim-locked diagnostics/shadow/replay only, ordered fixed-reference-live feature redesign -> DRD2 pose/physics -> conserved anchor proxy -> prior gating -> target-internal pairwise diagnostic/guarded validation prep for the hard-decoy lane -> coverage expansion. Do not use full 100k reruns, threshold relaxation, target identity feature, or fake pass to move this lane forward; delivery/router/platform claim promotion remains forbidden.
...
- `fixed_family_reference`, `gpcr_core_decoy_intrusion_v1`, `gpcr_core_linear_rescore_v1`, `gpcr_core_mismatch_contact_rescore_v1`, and `gpcr_core_structure_support_rescore_v1` are comparison-only shadow/guarded/reject evidence; `claim_promotion_allowed=false` remains in force, and they do not promote the claim or router/platform wording. The paired shadow replay eval artifact is `runs/gpcr_family_anchor_v2_shadow_replay_eval_current.json`; keep it compare-only with `runs/gpcr_guarded_100k_rank_failure_diagnostics_current.json`.
- The next GPCR diagnostic order is claim-locked diagnostics/shadow/replay only: DRD2 pose/physics -> conserved anchor proxy -> prior gating -> target-internal pairwise diagnostic/guarded validation prep for the hard-decoy lane -> coverage expansion.
...
0. GPCR positive coverage expansion and scale-up recovery
   ...
   - The refreshed DRD2 diagnostic slice shows the positive is not missing the conserved acidic anchor: it contacts native `Asp114` at mean distance about `3.25 A`. The top DRD2 decoy cluster is even closer at about `2.48 A`, so the next scoring work should separate true ligand physics from decoy over-anchoring rather than simply rewarding anchor proximity.
   - `gpcr_core_family_anchor_rescore_v2` is now available as claim-locked shadow/replay only. It uses `binding_score_composite_v7_prior_active`, `gpcr_basic_amine_proxy`, conserved-anchor/prior gating, and anchor-chemistry mismatch pressure; local replay improves `PR-AUC=0.5186945103743427 -> 0.5767474245351905`, computes `ranking_pr_auc_ci_low=0.21066694653866244`, keeps top20 at `0.25`, and improves DRD2 `global_rank=18923 -> 8562`, but this is only the next guarded-comparison design step toward target-internal pairwise diagnostic/guarded validation prep for the hard-decoy lane and not claim evidence because CI-low remains below `0.45`. The base pairwise diagnostic slice still has `decoys_above_positive_count=5314`, `pairwise_win_rate=0.46854685468546853`, and `ready_for_guarded_apply=false`; the refreshed shadow projection improves DRD2 decoys-above-positive to `2434` and pairwise win-rate to `0.7565756575657565`.
   - The next scorer attempt should not relaunch v4 unchanged. Root cause from the reject replay is fixed-reference feature collapse/no activation, so the next implementation needs a fixed-reference-live gate that produces nonzero telemetry and beats v2 before any guarded apply.
   ...
   - Rank failure diagnostics source: `runs/gpcr_guarded_100k_rank_failure_diagnostics_current.json`; the packet remains diagnostic-only, and the next implementation is claim-locked diagnostics/shadow/replay in the order DRD2 pose/physics -> conserved anchor proxy -> prior gating -> target-internal pairwise diagnostic/guarded validation prep for the hard-decoy lane -> coverage expansion. Do not relaunch the same packet as claim evidence, and keep `claim_promotion_allowed=false` with threshold relaxation/fake pass forbidden.
