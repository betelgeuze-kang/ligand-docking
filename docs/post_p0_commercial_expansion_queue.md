...
- The next GPCR diagnostic order is claim-locked rework/shadow/replay only: DRD2 pose generation/backmapping repair -> hard-decoy construction -> complete v11 weak-base cationic frozen-row replay review -> all-basic synthetic-anchor overpromotion reduction or portable atom-anchor contract -> broader non-leaky positive coverage -> conserved anchor/prior gating review -> guarded validation prep. v8/v9 atom-window scorer patches are reject evidence and should not be relaunched unchanged; v10/v11 selected-slice green is not enough for router/platform claim.
...
0. GPCR positive coverage expansion and scale-up recovery
   ...
   - Rank failure diagnostics source: `runs/gpcr_guarded_100k_rank_failure_diagnostics_current.json`; the packet remains diagnostic-only, and the next implementation is DRD2 pose generation/backmapping repair, hard-decoy construction, and broader non-leaky positive coverage after the v8/v9 atom-window rejects. Do not relaunch the same packet as claim evidence, and keep `claim_promotion_allowed=false` with threshold relaxation/fake pass forbidden.
   - DRD2 repair sources: `runs/gpcr_drd2_pose_generation_repair_packet_current.json`, `runs/gpcr_drd2_pseudo_allatom_repair_current.json`, `runs/gpcr_atom_window_anchor_feature_cache_drd2_pseudo_allatom_repair_current.json`, `runs/gpcr_drd2_cationic_center_geometry_cache_current.json`, `runs/gpcr_drd2_hard_decoy_slice_packet_current.json`, `runs/gpcr_drd2_hard_decoy_penalty_envelope_current.json`, `runs/gpcr_cationic_pose_distortion_v10_shadow_replay_review_current.json`, `runs/gpcr_cationic_pose_distortion_frozen_cache_mode_review_current.json`, `runs/gpcr_cationic_weakbase_v11_shadow_replay_review_current.json`, and `runs/gpcr_cationic_weakbase_v11_frozen_shadow_replay_review_current.json`. Current selected-row repair is green as diagnostic materialization (`65/65` repaired, positive `14/14` heavy-atom coverage, `anchor_mode=positive_only`, positive atom-window mean `2.8214482267014858 A`, cationic-center mean `3.1999997921453787 A`). v10 selected-slice is green but frozen-mode review blocks portability; v11 selected-slice is green but complete true-base frozen replay remains blocked by all-basic synthetic-anchor overpromotion. Keep these diagnostic-only and fix the feature contract before any full guarded claim review.
