...
Legacy GPCR family/router diagnostics remain claim-locked comparison material. The tracked GPCR A1 independent repeat is now green for ranking parity, but it does not block or widen the current restricted local-delivery verdict, the tracked restricted 1M scale-up package, or broad GPCR/router/platform wording.

For the non-ADRB2 positive freeze workflow, `config/gpcr_non_adrb2_positive_candidates_v1.csv` is the curated candidate input schema. The intended handoff is candidate CSV -> `runs/gpcr_non_adrb2_positive_candidates_leakage_audit_current.json` -> `runs/gpcr_positive_coverage_freeze_packet_current.json` -> `runs/gpcr_frozen_candidate_profile_support_current/summary.json` -> `runs/gpcr_frozen_candidate_scoreability_current.json` -> guarded 100k rerun -> `runs/gpcr_family_heldout_scorecard_current.json` / `runs/gpcr_family_heldout_scorecard_guardrail_current.json` -> `runs/gpcr_guarded_100k_rerun_readiness_current.json`; the current freeze packet is `frozen=true` after three non-ADRB2 ChEMBL_36 positives and a pass leakage audit, profile support is `profile_ready=true`, scoreability is `pass=true`, and family-held-out is `pass=true`. The separate A1 repeat evidence is `runs/gpcr_rank_rescue_crossfit_repeat_r1_evidence_packet_current.json` with PR-AUC `0.8718530390764964`, PR CI-low `0.7611678630724843`, top20 `1.0`, and blockers `[]`. This closes the tracked ranking-parity evidence only; router/platform promotion remains locked. The current proxy hook is `runs/gpcr_guarded_100k_rank_failure_diagnostics_current.json` / `drd2_pose_physics_diagnostics`; the paired shadow replay eval is `runs/gpcr_family_anchor_v2_shadow_replay_eval_current.json`; keep the rest of the follow-up chain compare-only.
...
1. GPCR family/router diagnostics, claim-locked outside delivery
   - Keep broad `claim_safe=false` for router/platform wording even though the tracked A1 independent repeat now passes ranking parity. The guarded 100k frozen non-ADRB2 legacy rerun completed, and its diagnostic blocker remains CI-low/top20 scoring quality, not positive coverage or family-held-out. Do not use threshold relaxation, target identity feature, or fake pass to move broad GPCR-family wording forward, and do not generalize the v2 basic-amine feature donor into broad GPCR/basic-amine wording; the next broad-family work is external held-out expansion plus scorer/router deployment guardrails, with OPRM1 pose/anchor alignment evidence and HTR2A decoy support discrimination kept as regression diagnostics.
   ...
   - Family-held-out is green, and the tracked A1 repeat clears ranking parity, but router/platform claim remains forbidden until its own deployment and broad-family guardrails clear.
   - `fixed_family_reference`, `gpcr_core_decoy_intrusion_v1`, `gpcr_core_linear_rescore_v1`, `gpcr_core_mismatch_contact_rescore_v1`, and `gpcr_core_structure_support_rescore_v1` stay comparison-only shadow/guarded/reject evidence; do not promote them to a claim or router/platform win from these legacy lanes. Use `runs/gpcr_ci_low_recovery_packet_current.md` (`## Metric Table`, `## Rank And Bootstrap Diagnostics`, `## Recommended Next Actions`) as the operator packet for the legacy coverage check, and use `runs/external_validation_2026-05-03_family_balanced_frozen_r2_set1_core_blind_gpcr_core_full_p0_n100000_r1_stage5_ranking_summary.md` plus `..._stage5_ranking_rows.csv` for the legacy r2 rank evidence. `gpcr_core_family_anchor_rescore_v2` is the feature donor/baseline for the next DRD2 motif-aware diagnostic, not broad GPCR/basic-amine generalization.
   ...
   - `fixed_family_reference` comparison-only rerun also failed the 2026-05-02 full 100k gate (`PR-AUC=0.0328`, `PR-AUC CI low=0.0045`, `top20=0.05`), so it stays reject evidence and is not a new recovery lane.
     ...
     ```bash
     python3 tools/gpcr_replay/build_gpcr_100k_failure_analysis.py
     ...
     python3 tools/gpcr_replay/build_gpcr_drd2_hard_decoy_slice_packet.py
