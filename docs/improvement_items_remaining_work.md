...
- `runs/science_accuracy_frontier_current.json`은 GPCR claim-lock과 R9/OpenMM
  ...
  위한 다음 17개 target/pose 후보 목록은 좁혀졌고, 로컬 coordinate artifact까지
  metric materialization 입력으로 잡혔지만 아직 canonical intake/claim promotion은
  허용하지 않는다.
  ...
  포함하면 candidate 단계부터 `receptor_coordinate_artifact_present=true`로 잡는다.
  최신 queue/intake/validation은 public RCSB complex coordinate의 로컬 artifact를
  17/17로 잡아 17/17 pass로 닫았고, 남은 일은 이 좌표를 근거로 51개 metric source
  payload를 reviewed JSON으로 채우는 것이다.
  ...
  `receipt_method_pending_count=51`,
  `receipt_input_artifacts_reviewed_pending_count=51`,
  `receipt_input_artifact_sha256s_reviewed_pending_count=51`,
  `receipt_metric_source_artifact_reviewed_pending_count=51`,
  `receipt_payload_schema_reviewed_pending_count=51`,
  `receipt_license_ok_pending_count=51`,
  `receipt_operator_id_pending_count=51`,
  `receipt_reviewed_at_utc_pending_count=51`,
...
- `core/forcefield.py`/`core/topology.py`는 sequence-aware residue class
  ...
  빠지지 않게 한다. 최신 source-of-truth는 `row_count=264`, `pass_count=264`,
  `blocker_count=0`, `artifact_row_count=178`, `semantic_status_row_count=84`,
  `release_refresh_command_count=225`, `stale_artifact_count=0`,
  ...
  release decision, action board 4개 surface를 검증한다. source-of-truth final gate는 `row_count=264`,
  `pass_count=264`, `artifact_row_count=178`, `semantic_status_row_count=84`,
  `readme_row_count=2`, `release_refresh_command_count=225`를 exact-check해
  ...
  `goal_release_allowed=false`, `goal_release_blocker_count=1`으로 동기화되어
...
- `tools/run_product_release_current_refresh.py --execute`는 source-of-truth 순서에
  ...
  최신 계획 결과는
  `product_release_current_refresh_verified`, `command_count=203`, `executed_count=203`,
...
최신 coordinate fetch/staging plan은 `coordinate_fetch_required_row_count=0`,
`coordinate_fetch_primary_url_row_count=17`,
`coordinate_fetch_staging_destination_row_count=17`,
...
고정해, 좌표 입력은 로컬 artifact/validation으로 준비됐지만 apply 자체는 preview/무변이
상태임을 남긴다.
