...
- `core/forcefield.py`/`core/topology.py`는 sequence-aware residue class
  ...
  `commercial_readiness_handoff_bundle_artifact_reference_count=39`를 노출하고,
  ...
  `local_missing_artifact_reference_count=0`, `local_required_artifact_reference_count=35`,
  `artifact_reference_count=39`이다. 여기에 AQP1 첫 return bundle의
  ...
  실제 registry promotion과 operator field handoff를 분리한다. R9 engine-refinement
  claim evidence도
  ...
  빠지지 않게 한다. 최신 source-of-truth는 `row_count=103`, `pass_count=103`,
  `blocker_count=0`, `artifact_row_count=69`, `semantic_status_row_count=32`,
  `release_refresh_command_count=94`, `stale_artifact_count=0`,
  ...
  release decision, action board 4개 surface를 검증한다. source-of-truth final gate는 `row_count=103`,
  `pass_count=103`, `artifact_row_count=69`, `semantic_status_row_count=32`,
  `readme_row_count=2`, `release_refresh_command_count=94`를 exact-check해
  ...
  `engine_refinement_claim_evidence_operator_field_worksheet_semantic_ready` row로 고정되어
  placeholder evidence, 6/6 blocked rows, public benchmark work-order 8개 row,
  field-level pending count 108개, approval token requirement, first-blocked diagnostics가 source-of-truth에서 직접 검증된다. production AI
...
- `tools/run_product_release_current_refresh.py --execute`는 source-of-truth 순서에
  ...
  최신 실행 결과는
  `product_release_current_refresh_verified`, `command_count=94`, `executed_count=94`,
...
- release source-of-truth gate는 R4 preflight, R4 rollout smoke receipt artifact,
  ...
  receipt/priority packet/field worksheet, CAMEO official-result fetch preflight, R9 engine-refinement claim evidence priority packet/field worksheet,
  master gap closure rollup 포함 refresh 이후
  `product_release_source_of_truth_gate_ready`, `pass_count=103/103`,
  `blocker_count=0`, `stale_artifact_count=0`,
  `release_refresh_command_count=94`으로 재검증됐다.
