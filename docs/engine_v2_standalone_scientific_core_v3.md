...
This boundary is deliberately not yet the public `DockingPipeline` result.
Its policy keeps CLI, Python API, benchmark, and product-shadow activation
false so that consumer migration can be reviewed separately. It also grants no
external reservation, molecular execution, historical A/B, Fresh-128, public
benchmark, product mutation, customer-pose, Stage 0, HIP, or scientific-claim
authority. GitHub Actions and test doubles remain non-production actors.
