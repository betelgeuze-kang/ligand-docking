...
- if the rescue summary reports a top-k shortfall, including `top_k_effective=1` in the current attempt, or any `rescue_review_band_mismatch_count > 0`, keep the bundle blocked/internal-review; record the attempt as such, resolve candidate/band mismatch, and rerun rescue before using any downstream refresh as evidence
