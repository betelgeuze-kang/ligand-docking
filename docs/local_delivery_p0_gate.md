...
- `gpcr_core_100k` is `claim_safe=false/blocked`; `residual-v4 apply` failed the core lane (`PR-AUC=0.3888`, `top20=0.15`), `ChEMBL50` is pass only as bounded evidence, `linear C100` full rerun is reject negative evidence (`PR-AUC=0.2367`, `top20=0.05`), and `gpcr_core_decoy_intrusion_v1` stays shadow-first with claim/router promotion blocked
