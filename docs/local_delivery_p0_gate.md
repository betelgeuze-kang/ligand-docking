...
P0 starts here: the current local-delivery gate is blocked for the restricted `kinase,gpcr,ion_channel` scope. The latest `runs/local_delivery_verdict_gate_current.json` reports `summary.delivery_ready=false`, `verdict=blocked`, `p0_blocker_count=1`, `hard_blocker_count=1`, and `commercialization_queue_clear=false`; it is synchronized with `runs/local_engine_commercialization_queue_current.json`, which reports `queue_clear=false`, `blocked_count=1`, and `top_priority_id=nightly_reliability`. The preflight, requirements lock, environment manifest, canonical top-level nightly reentry, wetlab selected-allatom review chain, and wetlab execution readiness remain useful evidence, but delivery-ready wording is disallowed until the queue and verdict are green together. 운영자 주의: the current scale-up suite is green for the restricted delivery scope (`commercialization_ready_suite_count=3/3`, `pending_suite_ids=[]`) and the 1M pilot is `claim_safe_size_shift_speed_diagnostic`; equal-size speedpack A/B owns throughput wording, while 1M is scale/accuracy evidence. Older GPCR-family 100k lanes and the latest GPCR A1 independent repeat remain negative or comparison evidence, so no ADRB2 beta-blocker-like, GPCR-family, router, unattended, or broad commercial platform wording is allowed. The current delivery claim stays narrow and does not extend to transporter, CA2/PXR, or IDP broader-promotion wording. Transporter AQP1/GLUT1 evidence closure and CA2/PXR closure are post-P0 only; AQP1 stays first-wave, GLUT1 stays second-wave, and CA2/PXR remain prep-only until placeholder/provenance and `replacement_reference_binding_kcal_mol` are closed.

Relative to commercial tools, the current stack is roughly 70-75% of a local-delivery analysis service and 35-45% of a full commercial platform. The next improvement order is the current queue blocker, GPCR ligand-ranking parity, pose/structure refinement parity, CI/regression, broad benchmark, UX/report packaging, and the separate post-P0 commercial expansion queue in [`docs/post_p0_commercial_expansion_queue.md`](post_p0_commercial_expansion_queue.md). Do not widen the current verdict into transporter coverage, broad platform readiness, IDP broader promotion, or unattended decision-making.
...
C. The wetlab selected all-atom geometry, binding, and claim-attached review chain are green: the latest atomized parameterization/local-min overlay reports `parameterization_ready_count=7`, `protein_local_minimization_ready_count=7`, `validated_repair_count=7`, and `hard_block_count=0`; the selected-allatom burndown reports `wetlab_selected_allatom_pass=true` and no hard/semi-hard blockers. If `runs/wetlab_selected_allatom_repair_packet_current.md` exists, use it only as a future regression/retry reference, not as the current delivery verdict.
...
E. The commercialization queue is not clear for the current restricted local-delivery scope: `queue_clear=false`, `blocked_count=1`, `top_priority_id=nightly_reliability`. The transporter negative-evidence lane remains parked outside this verdict scope, and the post-P0 queue stays separate from the current delivery claim.
...
- the scale-up suite is currently delivery-scope claim-safe, but older GPCR-family 100k lanes remain negative/comparison evidence; keep the ADRB2 beta-blocker-like residual target-scoped and do not promote it to GPCR-family, router, unattended, or broad platform wording
