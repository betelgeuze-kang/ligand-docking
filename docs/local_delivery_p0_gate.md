...
P0 starts here: the current local-delivery gate is green for the restricted `kinase,gpcr,ion_channel` scope. The latest `runs/local_delivery_verdict_gate_current.json` reports `summary.delivery_ready=true`, `verdict=delivery_ready`, `p0_blocker_count=0`, `hard_blocker_count=0`, and `commercialization_queue_clear=true`; it is synchronized with `runs/local_engine_commercialization_queue_current.json`, which reports `queue_clear=true`, `blocked_count=0`, and `top_priority_status=keep_green`. The preflight, requirements lock, environment manifest, canonical top-level nightly reentry, wetlab selected-allatom review chain, and wetlab execution readiness are current supporting evidence. 운영자 주의: the current scale-up suite is green for the restricted delivery scope (`commercialization_ready_suite_count=3/3`, `pending_suite_ids=[]`) and the 1M pilot is `claim_safe_size_shift_speed_diagnostic`; equal-size speedpack A/B owns throughput wording, while 1M is scale/accuracy evidence. The GPCR A1 independent repeat now passes the tracked ranking-parity gate, but GPCR-family/router, unattended, and broad commercial platform wording remain separate locked claims. The current delivery claim stays narrow and does not extend to transporter, CA2/PXR, or IDP broader-promotion wording. Transporter AQP1/GLUT1 accounting closure and CA2/PXR review-only closure are post-P0 evidence surfaces only; direct target-specific binding evidence is still required before widening transporter wording.
...
E. The commercialization queue is clear for the current restricted local-delivery scope: `queue_clear=true`, `blocked_count=0`, `top_priority_id=nightly_reliability`, `top_priority_status=keep_green`. The transporter negative-evidence lane remains outside this verdict scope, and the post-P0 queue stays separate from the current delivery claim.
