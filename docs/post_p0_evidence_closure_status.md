...
This note is an operator map for the post-P0 lanes that stay outside the current local delivery claim. The restricted local P0 delivery state is green, but the GPCR scale-up regression guardrail is still the primary blocker for any commercial claim expansion. The current delivery claim remains limited to `kinase`, `ion_channel`, and `gpcr`. Do not use this note to widen delivery wording or to treat recovery-band signal as claim-safe or router/platform-ready; `claim_promotion_allowed=false` stays in force. The non-ADRB2 freeze workflow is no longer blocked on launch prerequisites: freeze, frozen-candidate scoreability, family-held-out, and leakage audit are green. Claim review remains blocked by CI-low stability, not point PR-AUC. The v3 family-anchor CI-stability packet `runs/gpcr_residual_prototype_spec_family_anchor_ci_stability_v3.json` is diagnostic-only (`prototype_mode=shadow_only`, `scorer_apply_allowed=false`) and exists only to explain why the v2 shadow CI-low remains below `0.45`. The v4 acidic-anchor overcontact gate is now implemented as fixed-reference, shadow-only, active-locked evidence, but its first score-only replay is reject evidence (`PR-AUC=0.008231735935435774`, `PR-AUC CI-low=0.0009935430341614215`, `top20=0.00`, gate activation `0/40000`). The latest r2 base evidence is `PR-AUC=0.5186945103743427`, `ranking_pr_auc_ci_low=0.1485815545422209`, `top20=0.25`, with DRD2 at global rank `18923` / target rank `5315`; the refreshed `gpcr_core_family_anchor_rescore_v2` shadow replay is recorded in `runs/gpcr_family_anchor_v2_shadow_replay_eval_current.json` and reaches `PR-AUC=0.5767474245351905`, `ranking_pr_auc_ci_low=0.21066694653866244`, `top20=0.25`, DRD2 `global_rank=8562` / `target_rank=2435`. That replay is not claim evidence; it is the current best comparison signal. The base pairwise diagnostic slice still has `decoys_above_positive_count=5314`, `pairwise_win_rate=0.46854685468546853`, and `ready_for_guarded_apply=false`; the refreshed shadow projection improves DRD2 decoys-above-positive to `2434` and pairwise win-rate to `0.7565756575657565`, but guarded apply remains blocked because CI-low is still below `0.45`.

## Claim Boundary

- `runs/post_p0_claim_blocker_rollup_current.json` is the current cross-lane blocker rollup. It keeps `claim_promotion_allowed=false` and orders the post-P0 work as GPCR CI-low/top20 scoring recovery first, then PDE translation quality, then transporter, CA2, PXR, and IDP boundaries. For the GPCR lane, the next step is claim-locked diagnostics/shadow/replay only, ordered DRD2 pose/physics -> conserved anchor proxy -> prior gating -> target-internal pairwise diagnostic/guarded validation prep for the hard-decoy lane -> coverage expansion; do not promote any of it into delivery/router/platform wording.
- GPCR recovery candidates (`gpcr_core_decoy_intrusion_v1`, `gpcr_core_linear_rescore_v1`, `gpcr_core_mismatch_contact_rescore_v1`, `gpcr_core_structure_support_rescore_v1`) stay comparison-only shadow/guarded-apply lanes. The `positive_count >= 9` frozen-packet launch prerequisite, family-held-out scorecard, and leakage audit are now green, but no claim promotion is allowed until the full 100k claim-review gate clears CI-low and top20 stability. Do not use full 100k reruns, threshold relaxation, target identity feature, or fake pass to force a pass.
...
- `runs/external_validation_2026-05-03_family_balanced_frozen_r2_set1_core_blind_gpcr_core_full_p0_n100000_r1_stage5_ranking_summary.md` and `..._stage5_ranking_rows.csv` are the latest r2 row-level artifacts. HTR2A and OPRM1 recover to target-rank 1 (`global_rank=2` and `global_rank=6`), while DRD2 is still buried at `global_rank=18923` / `target_rank=5315`. This is the next claim-locked diagnostics/shadow/replay hook, not claim evidence.
- `runs/gpcr_guarded_100k_rank_failure_diagnostics_current.json` remains diagnostic-only and should not be promoted into delivery/router/platform wording. Its `drd2_pose_physics_diagnostics` slice now includes atom-anchor evidence from DRD2 native `6CM4`: the DRD2 positive keeps the acidic `Asp114` anchor (`anchor_mean_distance_A ~= 3.25`), but the top DRD2 decoy cluster is even closer (`mean ~= 2.48`), so the next blocker is decoy over-anchoring / ligand-physics-prior separation rather than simple anchor absence. The accepted sequence is claim-locked diagnostics/shadow/replay -> guarded apply -> full 100k claim review, with the next GPCR order fixed as DRD2 pose/physics -> conserved anchor proxy -> prior gating -> target-internal pairwise diagnostic/guarded validation prep for the hard-decoy lane -> coverage expansion. The paired shadow replay summary lives at `runs/gpcr_family_anchor_v2_shadow_replay_eval_current.json`; keep both artifacts compare-only.
- `runs/gpcr_residual_prototype_spec_family_anchor_ci_stability_v3.json` is the diagnostic-only v3 packet for the blocker. Keep it frozen as `shadow_only` with `scorer_apply_allowed=false`; if a v4 scorer is not yet merged, treat it only as the next shadow/guarded-comparison direction, not completed evidence.
- `gpcr_core_family_anchor_rescore_v2` now exists as a claim-locked shadow/replay candidate only. It adds `gpcr_basic_amine_proxy`, conserved-anchor/prior gating, and anchor-chemistry mismatch pressure on top of `binding_score_composite_v7_prior_active`; local replay improves the r2 point estimate from `PR-AUC=0.5186945103743427` to `0.5767474245351905`, computes `ranking_pr_auc_ci_low=0.21066694653866244`, keeps top20 at `0.25`, and moves DRD2 from `global_rank=18923` to `8562`. This does not change `claim_promotion_allowed=false`; the base pairwise diagnostic remains `decoys_above_positive_count=5314`, `pairwise_win_rate=0.46854685468546853`, and `ready_for_guarded_apply=false`, while the refreshed shadow projection improves the DRD2 hard-decoy slice to `decoys_above_positive_count=2434` and `pairwise_win_rate=0.7565756575657565`. The next blocker is CI-low stability, not point PR-AUC. Source artifact: `runs/gpcr_family_anchor_v2_shadow_replay_eval_current.json`.
- `gpcr_core_acidic_anchor_overcontact_prior_gate_v4` is implemented and profiled as a fixed-family-reference, shadow-only, active-locked candidate, but its first score-only replay is reject evidence. Source artifacts: `runs/gpcr_residual_prototype_spec_acidic_anchor_overcontact_prior_gate_v4_shadow.json`, `runs/gpcr_scaleup_100k_acidic_anchor_overcontact_prior_gate_v4_candidate_current/`, `runs/gpcr_acidic_anchor_v4_shadow_replay_summary_current.json`, and `runs/gpcr_acidic_anchor_v4_shadow_replay_eval_current.json`. The replay loaded fixed reference stats and kept active score locked, but the overcontact gate activated on `0/40000` rows and metrics fell to `PR-AUC=0.008231735935435774`, `PR-AUC CI-low=0.0009935430341614215`, `top20=0.00`; keep this as negative evidence and do not relaunch it as claim evidence.
...
- `runs/gpcr_guarded_100k_rank_failure_diagnostics_current.md` is the operator packet for the current rank-quality blocker; it carries positive global/within-target ranks, top decoy intrusions, `post_v3_acidic_anchor_review`, and `next_action=shadow_replay_acidic_anchor_overcontact_prior_gate_v4` for the scoring-candidate work. The current v4 replay is reject evidence, so the next implementation must redesign the fixed-reference-live feature gate before any guarded apply.
