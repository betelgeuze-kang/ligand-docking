...
| Accuracy-parity claim against commercial/standard tools overall | 45-55% | 45-55 pts |
| Broad commercial drug-discovery platform | 40-50% | 50-60 pts |
...
| Syntekabio-like AI discovery/wetlab translation | `commercialization_readiness_current.json` has core commercial lane score 82.5 and all-category expansion 68.9. Ligand scale-up is now green for the tracked restricted suite: 3/3 commercialization-ready and no pending suites. T. cruzi PDE hard gate is green, but `wetlab_tcruzi_pde_translation_quality_packet_current.json` has `translation_quality_ready=false`, focus score 68.1, and blocker `binding_energy_proxy_too_weak_for_translation`. | Large-library screening, stable hit-to-lead prioritization, cross-target prospective evidence, integrated docking/MD refinement, assay/PoC continuity, and reproducible candidate packages. | Internal workflow discipline and scale evidence are stronger, but prospective translation and broader family coverage are still not comparable. Estimate 45-55% parity. |
