...
- C is hard-blocked: the latest rescue run is a real scoring pass (`execution_mode=pseudo_allatom_backmapping_scoring_executed`, `scoring_status=pass`) with score-CSV canonical lane sourcing, `top_k_effective=4`, `top_k_shortfall_count=4`, and `source_rescue_review_band_mismatch_count=0`; the latest review packet, `current_results_index`, `partnering_stack`, final campaign/dashboard, regenerated burndown, and verdict still report selected `mean_min_distance_A=2.756` versus `2.500` (`delta=0.256`). Because the selected metric remains above threshold, the hard blocker remains and the claim metric is still missing. The source-consistency chain is review packet -> `current_results_index` -> `partnering_stack` -> final/dashboard -> burndown -> verdict; any mismatch is a canonical freshness regression. The candidate JSON is only a ranking seed/fallback, so the lane must stay rooted in the same post-slice score CSV/review surface source; keep expensive lanes deferred until the metric clears.
...
12. Treat claim/equivalence inputs as post-gate readiness evidence only after the wetlab selected-allatom hard gate is green; before then, placeholder or missing data stays blocked/internal-review and cannot be counted as readiness evidence or used to justify a downstream refresh. No threshold relaxation, fake-pass, or claim-equivalence shortcut is allowed.
