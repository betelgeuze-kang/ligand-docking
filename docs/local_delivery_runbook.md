...
- C is hard-blocked: the latest rescue run is only a scoring pass (`scoring_status=pass`) with `top_k_effective=1`, `top_k_shortfall_count=7`, `source_rescue_review_band_mismatch_count=4`, and `actual_mean_min_distance_A=3.3751258858044944 > 2.500`; the latest review packet, `current_results_index`, `partnering_stack`, final campaign/dashboard, regenerated burndown, and verdict still report `mean_min_distance_A=3.375` versus `2.500` (`delta=0.875`). Because the candidate/band mismatch remains unresolved, the hard blocker remains and the claim metric is still missing. The source-consistency chain is review packet -> `current_results_index` -> `partnering_stack` -> final/dashboard -> burndown -> verdict; any mismatch is a canonical freshness regression. Resolve the candidate/band mismatch, regenerate the lane artifact, and rerun `strict_then_near_fill` before spending more compute; keep expensive lanes deferred until the metric clears.
