...
The preflight is a green, non-dry-run evidence refresh (`summary.overall_ok=true`, `verdict_gate_required_ok=true`); it is supporting-only evidence and not the delivery-ready verdict. The current `accuracy_gate` is green and the requirements/environment lock is resolved for the local-delivery required set (`installed=13/13`, `missing=0`, `loose_sources=0`, `optional_missing_count=7`, `requirements_lock_complete=true`, `environment_lock_complete=true`), but the conservative `verdict_gate` remains blocked (`summary.delivery_ready=false`). The verdict blocker code `wetlab_selected_allatom_not_green` now means binding/claim readiness is still not green, not that selected geometry failed: `mean_min_distance_A=0.672 <= 2.500` is green, while `binding_energy_proxy=0.113` versus `-0.050` and missing claim/equivalence evidence remain; `commercialization_queue_not_clear` stays downstream. The nightly stage6 summary is keep-green supporting evidence only and no longer a representative P0 blocker. Treat `runs/nightly_stage6_top_level_reentry_packet_current.md` together with `runs/nightly_stage6_top_level_reentry_profile_current.json` as the strict canonical reentry handoff; that pair preserves `smoke_then_full`, `KRAS_G12D,EGFR_KINASE,HIV1_PROTEASE`, `require_ood_eval=true`, and the unchanged `2.500` gate, and remains supporting-only evidence. The source-consistency chain is review packet -> `current_results_index` -> `partnering_stack` -> final/dashboard -> burndown -> verdict; any mismatch anywhere is a canonical freshness regression, not pass evidence. For P0-C, the canonical chain is `three_bead_slice` score CSV -> rescue review surface -> promoted top-4 packet -> allatom rescue lane -> rescue runner. The review band and numeric `mean_min_distance_A` must come from that same post-slice score source; `runs/wetlab_rescue_three_bead_candidates_current.json` is only a ranking seed/fallback for lane construction and never band authority. If a lane rebuild is needed after a mismatch, do not rerun `strict_then_near_fill` or refresh downstream artifacts until `rescue_review_band_mismatch_count=0`. Before bundle assembly, confirm `current_results_index.summary.partnering_stack_artifact_complete=true`, then inspect `runs/wetlab_partnering_stack_current.json` and `runs/wetlab_partnering_stack_current.md`; they are only acceptable when `runs/wetlab_partnering_stack_current.json.summary.status=wetlab_partnering_stack_ready`, `runs/wetlab_partnering_stack_current.json.artifact_completeness=full_partnering_stack`, and the selected-allatom metric/source/gates plus freshness/source provenance are present. If that flag is false/missing or the partnering stack is placeholder/minimal (`summary.status="ok"`), stop blocked/internal-review and record blocker `partnering_stack_placeholder_or_incomplete`. Do not use delivery-ready wording until the verdict gate reports `summary.delivery_ready=true`. Downstream execute evidence is supporting-only; it does not replace the green top-level nightly artifact. This is a local-test-first, results-only handoff model: run the smoke locally, keep the raw outputs unchanged, and only then send the resulting evidence bundle. Do not fake-pass the gate, relax thresholds, or manipulate results to coerce a pass. The partnering stack artifact must still root at `tcruzi_pde_allatom_review_packet.best_mean_min_distance_A`; a placeholder/minimal `summary.status="ok"` surface remains incomplete review input, not evidence. The latest allatom review packet, `current_results_index`, `partnering_stack`, final campaign/dashboard, regenerated burndown, and verdict now align on selected `mean_min_distance_A=0.672` versus `2.500` (`delta=-1.828`) from `tcruzi_pde_allatom_review_packet.best_mean_min_distance_A`. That closes the selected-geometry blocker, but the wetlab selected-allatom verdict remains blocked by `binding_energy_proxy=0.113` versus `-0.050` and missing claim/equivalence evidence. If the current preflight, requirements lock, environment manifest, or verdict-gate artifact is missing or stale, stop and create blocked/internal-review evidence first.
...
- C is partially green but still blocked: the latest rescue run is a real scoring pass (`execution_mode=pseudo_allatom_backmapping_scoring_executed`, `scoring_status=pass`) with score-CSV canonical lane sourcing, `top_k_effective=4`, `top_k_shortfall_count=4`, and `source_rescue_review_band_mismatch_count=0`; the latest review packet, `current_results_index`, `partnering_stack`, final campaign/dashboard, regenerated burndown, and verdict now report selected `mean_min_distance_A=0.672` versus `2.500` (`delta=-1.828`). Because the selected geometry metric is below threshold, the geometry blocker is closed. The wetlab selected-allatom verdict still remains blocked by commercial binding support (`binding_energy_proxy=0.113` versus `-0.050`) and missing claim/equivalence evidence. The source-consistency chain is review packet -> `current_results_index` -> `partnering_stack` -> final/dashboard -> burndown -> verdict; any mismatch is a canonical freshness regression. The candidate JSON is only a ranking seed/fallback, so the lane must stay rooted in the same post-slice score CSV/review surface source; keep expensive lanes deferred until the remaining commercial hard blockers clear.
- The preflight evidence is green, but the verdict gate remains blocked with `summary.delivery_ready=false`; the next action is `recompute_binding_energy_proxy` / `strengthen_binding_energy_proxy`, followed by claim/equivalence evidence only after the remaining hard blockers clear. The commercialization queue is downstream of wetlab selected-allatom, so it cannot clear independently. If `runs/wetlab_selected_allatom_repair_packet_current.md` exists, use it as a fallback execution reference only when geometry regresses or source-chain mismatch returns. Do not issue delivery-ready wording.
...
12. Treat claim/equivalence inputs as post-hard-blocker readiness evidence only after selected geometry and binding support are green; before then, placeholder or missing data stays blocked/internal-review and cannot be counted as readiness evidence or used to justify a downstream refresh. No threshold relaxation, fake-pass, or claim-equivalence shortcut is allowed.
