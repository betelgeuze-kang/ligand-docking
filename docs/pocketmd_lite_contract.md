...
(stage2 dominates wall-clock). PocketMD Lite refines only the **top-k** (family
lanes or `rank_pct <= top_k_threshold_pct`, default 5%) and grades each refined
candidate into an uncertainty band.

## Selection

`is_refine_selected(family, rank_pct, top_k_threshold_pct)`: true for the
`gpcr` / `kinase` / `ion_channel` lanes, or when `rank_pct <= threshold`
(default `0.05`). Non-selected candidates are `coarse_only` (kept from cheap
screening, not a refinement claim). A caller may force `selected_for_refine`.
...
`hbond_persistence`, `contact_persistence`, `clash_count`.

## Batch KPIs

`build_pocketmd_lite_report(candidates)` rolls up `band_counts`, `refined_count`,
`coarse_only_count`, and two KPIs:
