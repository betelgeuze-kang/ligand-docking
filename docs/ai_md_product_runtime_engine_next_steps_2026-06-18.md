...
- 같은 gate는 local ROCm clean-container 증거(`runs/product_image_smoke_preflight_current.json`)와 repo self-hosted runner inventory(`runs/github_self_hosted_runner_inventory_current.json`)를 함께 읽는다. 비용 없는 경로의 남은 차단은 GitHub-hosted billing limit 증액이 아니라 repo self-hosted Linux runner(`self-hosted, linux`)와 ROCm runner(`self-hosted, linux, rocm`)를 등록/온라인화한 뒤 `product-api-worker.yml`과 `product-image-smoke.yml` `rocm-runtime` mode를 재실행하는 것이다.
- `tools/product/build_github_self_hosted_runner_host_preflight.py`는 local Docker/ROCm/product receipt와 read-only GitHub runner inventory를 결합해 `runs/github_self_hosted_runner_host_preflight_current.json`에 기록한다. 현재 기대 상태는 local host ready + repo runner registration required이며, operator runbook은 `docs/ai/github_self_hosted_runner_setup_2026-06-21.md`에 둔다.
...
- Product evidence bundle validator는 `term_result_contract_rows`가 정확히 `directional_hbond`, `hydrophobic_contact`, `legacy_lj` 3개 term set과 row count를 만족하지 않으면 `kpi_force_term_result_contract_terms_invalid`로 fail-closed한다.
- PM-facing Product KPI도 `force_term_result_contract_term_set_ready`, term count, observed terms, expected terms를 노출하며 raw Product KPI와 일치하지 않으면 bundle validation이 fail-closed한다.
- GitHub-hosted Actions billing/spending-limit blocker가 있을 때 비용 없는 경로는 spending limit 증액이 아니라 self-hosted runner다. `product-api-worker`와 `product-image-smoke` build smoke는 기본 runner를 self-hosted Linux로 두고, `product-image-smoke`의 `rocm-runtime`은 self-hosted ROCm runner에서만 실행한다. Product CI runtime gate와 AI-MD Product/PM KPI는 이 billing-free self-hosted path, rerun command, `self_hosted_runner_inventory_present`, `self_hosted_linux_runner_online`, `self_hosted_rocm_runner_online`을 노출한다. 2026-06-21 KST GitHub API read 기준 repo runner inventory는 `total_count=0`이므로 release CI rerun 전 runner 등록/온라인화가 선행되어야 한다.
- GitHub self-hosted runner 등록 준비 상태는 `runs/github_self_hosted_runner_host_preflight_current.json`로 별도 기록한다. 이 artifact는 registration token을 요청하지 않고, runner service를 시작하지 않고, workflow를 dispatch하지 않는다.
