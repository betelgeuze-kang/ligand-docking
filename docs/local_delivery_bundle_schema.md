...
Current status snapshot: the current `accuracy_gate` is `pass=true` with failed metrics `0`, and the Morton presort neighbor/self-pair parity smoke records `total_pair_outliers=0`; the requirements/environment lock is green for the local-delivery required set (`installed=13/13`, `missing=0`, `loose_sources=0`, `requirements_lock_complete=true`, `environment_lock_complete=true`, `optional_missing_count=7`). A is closed, the preflight evidence is green, and B has green downstream execute evidence plus a keep-green canonical top-level nightly summary, so B is no longer a representative P0 blocker. `runs/nightly_stage6_top_level_reentry_packet_current.md` together with `runs/nightly_stage6_top_level_reentry_profile_current.json` is the strict handoff for the next canonical rerun if needed: the pair preserves `smoke_then_full`, `KRAS_G12D,EGFR_KINASE,HIV1_PROTEASE`, `require_ood_eval=true`, and the unchanged `2.500` gate while carrying the override CSV as supporting-only provenance. C is hard-blocked because `mean_min_distance_A=3.375` is above `2.500`, `delta=0.875`, the claim metric is missing, and the hard block / missing metric counts are `2/1`; start from `recompute_mean_min_distance_A` and keep expensive lanes deferred. The verdict gate remains blocked with `summary.delivery_ready=false` because `wetlab_selected_allatom_not_green` remains and `commercialization_queue_not_clear` is downstream of wetlab closure.
...
- Internal-review only; not delivery-ready for commercial handoff because `summary.delivery_ready=false` and the remaining P0 blockers are `commercialization_queue_not_clear` and `wetlab_selected_allatom_not_green`; wetlab selected-allatom is hard-blocked at `mean_min_distance_A=3.375` with `delta=0.875` and the claim metric missing, so start from `recompute_mean_min_distance_A` and keep expensive lanes deferred until the metric clears. The nightly stage6 summary is keep-green and no longer a representative P0 blocker, and the current preflight evidence, accuracy gate, and requirements/environment lock are green, but that does not override the wetlab/queue blockers.
