...
- The minimization trajectory-comparison prerequisite is implemented and receipt-verifiable. It binds all 14 rows, exact step alignment, predefined coordinate/energy thresholds, all failure dispositions, and three checkpoint/restart digest-equality cases through runner, writer, and result review. The refrozen v2.1 protocol adds internal half-tolerance projection-convergence headroom without changing the declared acceptance threshold; the local non-production check is now 14/14, including both fixed-Born rows. This remains implementation evidence, not a production receipt or independent human disposition.
- A frozen single-host OpenMM external result-review contract now freshly reverifies both Engine result-review chains and both complete OpenMM receipts, checks exact 27/59 Engine component/total/force outputs plus all fourteen retained operational traces, and signs the host/CPU/session/custody/runtime/source/dependency/seed/nonce projection with a role-separated Ed25519 reviewer. The verifier requires canonical transport, freshness, and explicit revocation/supersession inputs. No reviewer key, attestation, production receipt, authenticated custody, second-host equality, or final human S0 approval is provisioned, so all promotion flags remain false.
- A frozen two-host S0 bundle verifier now freshly reverifies exactly two complete host inputs, rejects reused host/CPU/session/custody/receipt/environment/authorization/review identities, requires exact commit/source/dependency/OpenMM-runtime/seed and energy-force/minimization physics-projection equality, and verifies a time-bounded, revocable/supersedable Ed25519 approval by a final human reviewer separated from every nested role. After both hosts verify, it can emit a secret-free detached signing request and exact canonical approval bytes for an external/HSM signer; the installable CLI accepts no private key, verifies the returned signature using only a public key, and attaches it without overwriting files. Attachment still requires full raw-evidence re-verification for acceptance. A verified runtime bundle opens only frozen synthetic S0 acceptance and S1 admission. No host evidence, trust key, authenticated external custody, or final approval is provisioned, so the repository's static S0 and S1 decisions remain false and every chemistry/fitting/benchmark/product claim remains closed.
...
- A fixed-policy N/F/Q witness-quorum verifier now binds an exact ordered roster with distinct caller-pinned declared witness/operator/fault-domain identifiers, `2Q-N>F`, one target-independent exact-anchor fork scope, and Q exact-lineage Ed25519 votes. It applies validity and post-certificate denial checks to all N members. The verified fact is conditional and anchor-scoped only: the verifier does not observe the fault bound, enforce exclusive voting, reconcile independent journals, or exclude a hidden sibling certificate. Realm-wide non-equivocation, global latest, epoch continuity, and all promotion facts stay false; no policy, keys, certificate, journals, or post-quorum status is provisioned.
- A verifier-only adjacent epoch-transition continuity contract now freshly re-verifies the previous witness-quorum proof, requires exact ordinal adjacency, carries the terminal state root unchanged into a derived sequence-zero genesis checkpoint, and requires disjoint previous/next Ed25519 quorums over one exact transition statement. It proves only the supplied transition; separately signed siblings can each verify, so locking, journal agreement, unique successor, realm-wide non-equivocation, and all promotion facts remain false. No transition proof, next policy, keys, votes, or post-transition status is provisioned.
- Next required evidence is externally enforced witness locking and independent journal comparison, environment/later custody, and an actually provisioned adjacent transition plus registry/authenticated-receipt/later-head/quorum/current-status proof chain; then execute and provision the 27/59 plus 14-case production receipts on two distinct CPU hosts, obtain exact physics equality and the signed OpenMM host reviews, and obtain the final independent human S0 approval through the frozen bundle contract.
