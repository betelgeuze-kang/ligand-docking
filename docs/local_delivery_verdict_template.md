...
Current status snapshot: the current `accuracy_gate` is `pass=true` with failed metrics `0`, and the Morton presort neighbor/self-pair parity smoke records `total_pair_outliers=0`; the requirements/environment lock is green for the local-delivery required set (`installed=13/13`, `missing=0`, `loose_sources=0`, `requirements_lock_complete=true`, `environment_lock_complete=true`, `optional_missing_count=7`). The canonical top-level nightly reentry pass is green, wetlab selected-allatom is pass at `mean_min_distance_A=2.120 <= 2.500`, the wetlab execution-readiness queue is clear, and `runs/local_delivery_verdict_gate_current.json` reports `summary.delivery_ready=true`, `p0_blocker_count=0`, `hard_blocker_count=0`, and `commercialization_queue_clear=true`. The next packaging step is `python3 tools/build_local_delivery_bundle.py`, followed immediately by `python3 tools/validate_local_delivery_bundle.py --bundle-dir <bundle_dir>`, and the verdict must stay within the restricted local-delivery scope.
The bundle is green end-to-end: `runs/local_delivery_verdict_gate_current.json` reports `summary.delivery_ready=true`, the source-consistency chain is aligned on `mean_min_distance_A=2.120` and `binding_energy_proxy=-0.146`, and the commercialization queue is clear. If `runs/wetlab_selected_allatom_repair_packet_current.md` exists, it can still serve as a next-execution reference for future regressions, but it does not define the current delivery-ready state.

The next packaging path is `python3 tools/build_local_delivery_bundle.py` -> `python3 tools/validate_local_delivery_bundle.py --bundle-dir <bundle_dir>`, and any future drift should fall back to internal-review wording only if those checks stop passing.
