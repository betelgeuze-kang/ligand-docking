# HIST_COMPLEX_07_TCRUZI_PDE_BINDINGDB_PDEB1_007_BDB50397079 Lane Decision

- status: `strict_blind_replacement_or_authority_required`
- benchmark: `hist_seed_07_tcruzi_pde_bindingdb_pdeb1_007_bdb50397079`
- scope: `complex`
- source chronology: `operator_authoritative_chronology_evidence_required`
- strict blind eligible: `False`
- retrospective calibration review allowed: `False`
- competitive proof allowed: `False`
- identity intake allowed: `False`
- sidechain-native benchmark allowed: `False`
- operator decision required: `True`
- blockers: `native_authority_not_pass,authoritative_native_date_missing,prediction_date_candidate_missing`
- next action: attach authoritative native/source chronology or replace with a strict blind eligible seed row

## Claim Boundary

Local CASP17 historical seed lane decision packet only. It prevents post-native or authority-incomplete seed rows from being promoted into strict blind competitive proof. Retrospective rows may remain useful for calibration or engineering review only after separate no-template/no-leak evidence. The packet does not clear provenance, mutate manifest/operator CSVs, compute official CASP metrics, or submit to CASP.
