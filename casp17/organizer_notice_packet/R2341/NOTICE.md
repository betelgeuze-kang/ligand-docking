# R2341 Organizer Notice

## Guardrails

- model_pool_policy: `external_rerank_accuracy_estimation_pool`
- internal_prediction_policy: `do_not_mark_as_internal_prediction`
- submission_policy: `rule_check_required_before_any_human_submission_use`
- large_download_policy: `tarballs_not_downloaded_by_notice_packet`
- massivefold_generation_scope: `all_human_rna_and_hybrid_targets_plus_protein_targets`

## Notices

| notice_id | type | status | action |
| - | - | - | - |
| `organizer_notice_003` | `massivefold_first_rna_set_available` | `massivefold_external_model_set_available` | `track_as_external_candidate_pool_for_rerank_and_accuracy_estimation` |

## MassiveFold Model Sets

| model_set | bundle | size_bytes | url |
| - | - | - | - |
| `R2341` | `cif_bundle` | `667779936` | `ftp://files.plbs.fr:21211/CASP17-CAPRI/R2341_all_cifs_MassiveFold.tar.gz` |
