"""Sparse recurrent temporal state without sequence self-attention."""
...
from .sparse_graph import ComplexityMetadata, SparseNeighborGraph, coerce_sparse_graph, segment_sum
...
TEMPORAL_COMPLEXITY = ComplexityMetadata(
    ...
    assumptions=("E <= K*N with fixed K", "hidden width is fixed"),
    prohibited_dense_operations=("T-by-T sequence mixing", "N-by-N spatial mixing"),
    claim_scope="temporal sparse-state propagation only",
)
...
class TemporalStateGNN(nn.Module):
    ...
    def __init__(self, input_features: int, hidden_features: int = 48, max_neighbors: int = 64):
        ...
        self.input_embedding = nn.Sequential(nn.Linear(self.input_features, self.hidden_features), nn.SiLU())
    ...
    def initial_state(self, node_features: torch.Tensor) -> torch.Tensor:
        ...
        return node_features.new_zeros((node_features.shape[0], node_features.shape[1], self.hidden_features))
    ...
    def forward(
        ...
        prior = previous_state.reshape(graph.node_count, self.hidden_features)
        if graph.edge_count:
            edge_input = torch.cat((prior[graph.src], prior[graph.dst], embedded[graph.src]), dim=-1)
            messages = self.message(edge_input)
            aggregate = segment_sum(messages, graph.dst, graph.node_count)
            degree = segment_sum(messages.new_ones((graph.edge_count, 1)), graph.dst, graph.node_count)
            aggregate = aggregate / degree.clamp_min(1.0).sqrt()
        else:
            aggregate = torch.zeros_like(prior)
        next_state = self.recurrent(torch.cat((embedded, aggregate), dim=-1), prior)
        return self.output_norm(next_state).reshape(batch_size, atom_count, self.hidden_features)
    ...
    def rollout(
        ...
        *,
        ...
        dynamic = isinstance(neighbors, Sequence) and not isinstance(neighbors, (tuple, SparseNeighborGraph))
        if dynamic and len(neighbors) != int(sequence.shape[0]):
        ...
        for step in range(int(sequence.shape[0])):
            if detach_interval is not None and step > 0 and step % int(detach_interval) == 0:
                ...
                state = state.detach()
            graph = neighbors[step] if dynamic else neighbors
        ...
        diagnostics.update({
            ...
            "retained_state_memory": (
                "O(T*N*C) because every state is returned" if return_history else
                ("O(W*N*C) autograd state with truncated window W" if detach_interval is not None else "O(T*N*C) autograd state without truncation")
            ),
        })
        return TemporalRollout(
            states=torch.stack(outputs, dim=0) if outputs is not None else None,
            final_state=state,
