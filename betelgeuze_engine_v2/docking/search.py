"""Bounded docking search with failure, score, and pose-validity row preservation."""
...
class DockingSearchResult:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "search_space_fingerprint_sha256": self.search_space_fingerprint_sha256,
            "numeric_policy_sha256": self.numeric_policy_sha256,
            "rng_stream_initial_state_sha256": (
                self.rng_stream_initial_state_sha256
            ),
...
def _pose_distance(
    ...
    *,
    ...
    if metric == "symmetry_aware_direct_rmsd":
        return symmetry_aware_rmsd(
            first.coordinates,
            second.coordinates,
            permutations=symmetry_permutations,
...
def run_bounded_docking_search(
    ...
    *,
    ...
    authenticated_problem_input = (
        problem if isinstance(problem, DockingProblemInput) else None
    )
    if authenticated_problem_input is not None:
        authenticated_problem_input.assert_integrity()
        if (
            authenticated_problem_input.search_space.fingerprint_sha256
            != search_space.fingerprint_sha256
        ):
            raise DockingSearchError(
                "authenticated docking input is cross-wired to another search space"
            )
        problem_identity = authenticated_problem_input.identity
        problem_input_fingerprint = (
            authenticated_problem_input.input_fingerprint_sha256
        )
    else:
    ...
    if (
        diversity_metric
        in {"symmetry_aware_kabsch_rmsd", "symmetry_aware_direct_rmsd"}
    ...
    elif authenticated_problem_input is None:
        blockers.append("docking_problem_input_not_authenticated")
    ...
    return DockingSearchResult(
        ...
        search_fingerprint_sha256=_search_fingerprint(
            ...
            problem_input_fingerprint=problem_input_fingerprint,
            score_descriptor=descriptor,
