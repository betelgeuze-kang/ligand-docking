"""Bounded lexical and block parser for a supported single-block CIF subset.

This module handles only data names, scalar values, and loops. It deliberately
stops at that structural layer: dictionary validation, ``_atom_site``
semantics, molecular identity, topology, and scientific interpretation are out
of scope.

Unlike ``shlex``, supported CIF quoting has no backslash escapes, quoted ``.``
and ``?`` are ordinary strings, control words are special only when unquoted,
and a semicolon in column one opens a multiline text value. Preserving those
lexical distinctions prevents silent structural loss without granting any
semantic-ingest, preparation, simulation, or validation claim.
"""
...
def _physical_line_separator_count(text: str, *, end: int | None = None) -> int:
...
def tokenize_cif(text: str) -> tuple[CifToken, ...]:
    """Tokenize the supported CIF subset with quote and source locations."""

    if type(text) is not str:
        raise TypeError("CIF text must be a string")
    line_separator_count = _physical_line_separator_count(text)
    ...
    for offset, character in enumerate(text):
        ...
        if character not in {"\t", "\n", "\r"} and not 0x20 <= codepoint <= 0x7E:
            line_number = _physical_line_separator_count(text, end=offset) + 1
            raise CifSyntaxError(
                "invalid_character",
                f"character U+{codepoint:04X} is outside the supported CIF character set",
    ...
    for line_index, line in enumerate(lines):
        if len(line) > _MAX_CIF_LINE_LENGTH:
            raise CifSyntaxError(
                "line_too_long",
                f"CIF input lines may contain at most {_MAX_CIF_LINE_LENGTH} characters",
        ...
        while cursor < len(line):
            ...
            if value[0] in {"$", "[", "]"}:
                raise CifSyntaxError(
                    ...
                    f"unquoted value may not begin with {value[0]!r} in this CIF subset",
                    line_number=line_number,
                )
            append_token(
                CifToken(
...
def parse_cif_block(text: str) -> CifBlock:
    ...
    while cursor < len(tokens):
        ...
        if _is_control(token, "stop_"):
            raise CifSyntaxError(
                "unsupported_stop",
                "stop_ is unsupported by this single-level loop subset",
                line_number=token.line_number,
            )
