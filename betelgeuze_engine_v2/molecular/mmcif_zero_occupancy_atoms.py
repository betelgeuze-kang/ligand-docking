"""Opt-in mmCIF source-reported zero-occupancy atom envelope.
...
zero-occupancy metadata, and emits the selected declarations deterministically.

The result proves only that selected ordered source declarations survive a
semantic round trip and point at atom-site rows whose numeric occupancy is
exactly zero.  It does not establish that an atom is actually missing, assign
an occupancy population, validate refinement, or prepare the system.
"""
...
MMCIF_ZERO_OCCUPANCY_ATOM_ENVELOPE_VERSION = "1.0.0"
MMCIF_ZERO_OCCUPANCY_ATOM_PARSER_VERSION = "1.0.0"
MMCIF_ZERO_OCCUPANCY_ATOM_WRITER_VERSION = "1.0.0"
MMCIF_ZERO_OCCUPANCY_ATOM_PARSER_NAME = (
    "betelgeuze_engine_v2.molecular.mmcif_zero_occupancy_atoms"
)
MMCIF_ZERO_OCCUPANCY_ATOM_PROFILE_ID = (
    "strict_mmcif_source_reported_zero_occupancy_atom_envelope/1.0.0"
)
...
MMCIF_ZERO_OCCUPANCY_ATOM_PROJECTION_SCHEMA_ID = (
    "betelgeuze.mmcif_zero_occupancy_atom_projection/1.0.0"
)
MMCIF_ZERO_OCCUPANCY_ATOM_SOURCE_BINDING_SCHEMA_ID = (
    "betelgeuze.mmcif_zero_occupancy_atom_source_binding/1.0.0"
)
MMCIF_ZERO_OCCUPANCY_ATOM_RECORD_STATE_SCHEMA_ID = (
    "betelgeuze.mmcif_zero_occupancy_atom_record_state/1.0.0"
)
MMCIF_ZERO_OCCUPANCY_ATOM_WRITE_RECEIPT_SCHEMA_ID = (
    "betelgeuze.mmcif_zero_occupancy_atom_write_receipt/1.0.0"
)
MMCIF_ZERO_OCCUPANCY_ATOM_ROUND_TRIP_REPORT_SCHEMA_ID = (
    "betelgeuze.mmcif_zero_occupancy_atom_round_trip_report/1.0.0"
)

MAX_MMCIF_ZERO_OCCUPANCY_ATOM_INPUT_BYTES = 64 * 1024 * 1024
...
MAX_MMCIF_ZERO_OCCUPANCY_ATOM_ROWS = 40_000 // 14
MAX_MMCIF_ZERO_OCCUPANCY_ATOM_SOURCE_ID_BYTES = 4_096
...
_RESIDUE_ZERO_OCCUPANCY_CATEGORY = "_pdbx_unobs_or_zero_occ_residues"
_POLYMER_CATEGORIES = frozenset(
    {
        "_entity",
        "_struct_asym",
        "_entity_poly_seq",
        _ZERO_OCCUPANCY_ATOM_CATEGORY,
        "_atom_site",
    }
)
...
class MmcifZeroOccupancyAtomError(ValueError):
    ...
    def __init__(
        ...
        super().__init__(
            f"mmcif_zero_occupancy_atom:{self.code}{suffix}: {self.detail}"
        )
...
def _load_canonical_json_document(
    ...
    *,
    ...
    if type(payload) is not bytes:
        raise MmcifZeroOccupancyAtomError(
...
def _validate_stored_token(value: Any, *, allow_missing: bool = False) -> str:
    if (
        ...
        raise TypeError(
            "zero-occupancy atom identity values must be bounded ASCII tokens"
        )
    return value


@dataclass(frozen=True, slots=True, init=False, repr=False)
class MmcifZeroOccupancyAtomRow:
    source_id: int
    auth_asym_id: str
    auth_comp_id: str
    auth_seq_id: str
    ...
    def __init__(
        ...
        *,
        ...
        if _factory_token is not _FACTORY_TOKEN:
            raise TypeError("MmcifZeroOccupancyAtomRow is factory-only")
        for name, value in (
            ("source_id", source_id),
    ...
    def __repr__(self) -> str:
        return "MmcifZeroOccupancyAtomRow(<source-reported-identity>)"


def _row_document(row: MmcifZeroOccupancyAtomRow, *, ordinal: int) -> dict[str, Any]:
    return {
        "ordinal": ordinal,
...
def _projection_document(rows: tuple[MmcifZeroOccupancyAtomRow, ...]) -> dict[str, Any]:
    return {
        "schema_id": MMCIF_ZERO_OCCUPANCY_ATOM_PROJECTION_SCHEMA_ID,
        ...
        "semantics": MMCIF_ZERO_OCCUPANCY_ATOM_PROJECTION_SCOPE,
        "source_declaration_kind": "zero_occupancy_atom",
...
def _projection_sha256(rows: tuple[MmcifZeroOccupancyAtomRow, ...]) -> str:
...
def _base_missingness_metadata_document(base_ingest: Any) -> dict[str, Any]:
    try:
        ...
        document = {
            "schema_id": ("betelgeuze.mmcif_zero_occupancy_atom_base_metadata/1.0.0"),
...
def _emit_selected_categories(block: CifBlock, *, drop: frozenset[str]) -> bytes:
    ...
    if len(payload) > MAX_MMCIF_ZERO_OCCUPANCY_ATOM_INPUT_BYTES:
        raise MmcifZeroOccupancyAtomError(
...
class _ParsedEnvelope:
    ...
    rows: tuple[MmcifZeroOccupancyAtomRow, ...]


def _parse_components(data: bytes, *, source_id: str) -> _ParsedEnvelope:
    if type(data) is not bytes:
        raise TypeError("mmCIF zero-occupancy atom input must be bytes")
    ...
    if len(encoded_source_id) > MAX_MMCIF_ZERO_OCCUPANCY_ATOM_SOURCE_ID_BYTES:
        raise MmcifZeroOccupancyAtomError(
            "source_id_too_large", "source_id exceeds the fixed 4096-byte UTF-8 cap"
        )
    if not data:
        raise MmcifZeroOccupancyAtomError("empty_input", "input is empty")
    if len(data) > MAX_MMCIF_ZERO_OCCUPANCY_ATOM_INPUT_BYTES:
        raise MmcifZeroOccupancyAtomError(
    ...
    if _RESIDUE_ZERO_OCCUPANCY_CATEGORY in categories:
        raise MmcifZeroOccupancyAtomError(
            "mixed_residue_zero_occupancy_unsupported",
            "v1 accepts one atom declaration loop and no residue declaration loop",
        )
    ...
    declaration_loop = _loop_for(block, _ZERO_OCCUPANCY_ATOM_CATEGORY)
    ...
    _require_headers(declaration_loop, MMCIF_ZERO_OCCUPANCY_ATOM_HEADERS)
    _require_headers(atom_loop, _COMMON_CORE21_ATOM_SITE_HEADERS)
    if not declaration_loop.rows:
        raise MmcifZeroOccupancyAtomError(
            "empty_zero_occupancy_atom_loop",
            "the selected loop must contain at least one row",
        )
    if len(declaration_loop.rows) > MAX_MMCIF_ZERO_OCCUPANCY_ATOM_ROWS:
        raise MmcifZeroOccupancyAtomError(
            "too_many_zero_occupancy_atom_rows", "row count exceeds the fixed cap"
        )
    ...
    coordinate_residues: set[tuple[int, str, int, str, str]] = set()
    coordinate_atom_occupancies: dict[
        tuple[int, str, int, str, str, str, str], list[CifToken]
    ] = {}
    for atom_tokens in atom_loop.rows:
        ...
        atom_key = (
            *residue_key,
            _bounded_bare_token(atom_tokens[3]),
            semantic_marker(atom_tokens[4]),
        )
        coordinate_residues.add(residue_key)
        coordinate_atom_occupancies.setdefault(atom_key, []).append(atom_tokens[13])
    ...
    parsed_rows: list[MmcifZeroOccupancyAtomRow] = []
    for tokens in declaration_loop.rows:
        ...
        if tokens[3].value != "1":
            raise MmcifZeroOccupancyAtomError(
                "unsupported_zero_occupancy_atom_model", "v1 accepts only exact model 1"
            )
        ...
        if entity_id is None:
            raise MmcifZeroOccupancyAtomError(
                ...
                "zero-occupancy label asym does not join struct-asym",
            )
        ...
        if (entity_id, label_seq, label_comp) not in membership:
            raise MmcifZeroOccupancyAtomError(
                ...
                "selected declaration does not join polymer sequence membership",
            )
        ...
        if residue_key not in coordinate_residues:
            raise MmcifZeroOccupancyAtomError(
                ...
                "selected atom declarations require their parent residue in atom_site",
            )
        ...
        matching_occupancies = coordinate_atom_occupancies.get(semantic_key)
        if not matching_occupancies:
            raise MmcifZeroOccupancyAtomError(
                "zero_occupancy_atom_atom_absent",
                "selected zero-occupancy atom identity is absent from atom_site",
            )
        statuses = tuple(occupancy_status(token) for token in matching_occupancies)
        if "nonzero" in statuses:
            raise MmcifZeroOccupancyAtomError(
                "zero_occupancy_atom_occupancy_nonzero",
                "a matching atom_site row has nonzero occupancy",
            )
        if any(status != "zero" for status in statuses):
            raise MmcifZeroOccupancyAtomError(
                "zero_occupancy_atom_occupancy_unavailable",
                "every matching atom_site occupancy must be an exact numeric zero",
            )
        parsed_rows.append(
    ...
    carrier_source = _emit_selected_categories(
        block, drop=frozenset({_ZERO_OCCUPANCY_ATOM_CATEGORY})
    )
    ...
    if (
        ...
        or base_metadata["residue_row_count"] != 0
        or base_metadata["atom_row_count"] != len(parsed_rows)
        or base_metadata["unobserved_residue_claim_count"] != 0
        or base_metadata["unobserved_atom_claim_count"] != 0
        or base_metadata["zero_occupancy_residue_row_count"] != 0
        or base_metadata["zero_occupancy_atom_row_count"] != len(parsed_rows)
        ...
        or "source_reports_zero_occupancy_atoms" not in coverage.blockers
...
def _record_state_document(
    *,
    ...
    return {
        "schema_id": MMCIF_ZERO_OCCUPANCY_ATOM_RECORD_STATE_SCHEMA_ID,
        "envelope_version": MMCIF_ZERO_OCCUPANCY_ATOM_ENVELOPE_VERSION,
        "parser_version": MMCIF_ZERO_OCCUPANCY_ATOM_PARSER_VERSION,
        ...
        "zero_occupancy_atom_projection_sha256": projection_sha256,
        "base_missingness_metadata_sha256": base_missingness_metadata_sha256,
        "declaration_semantics": MMCIF_ZERO_OCCUPANCY_ATOM_PROJECTION_SCOPE,
    }


@dataclass(frozen=True, slots=True, init=False, repr=False)
class MmcifZeroOccupancyAtomIngestResult:
    full_source_sha256: str
    carrier_source_sha256: str
    canonical_carrier_source_sha256: str
    source_id_sha256: str
    zero_occupancy_atom_projection_sha256: str
    ...
    nonpoly_record_state_sha256: str | None
    zero_occupancy_atom_rows: tuple[MmcifZeroOccupancyAtomRow, ...]
    _full_source: bytes
    _source_id: str
    ...
    def __init__(
        ...
        if _factory_token is not _FACTORY_TOKEN:
            raise TypeError("MmcifZeroOccupancyAtomIngestResult is factory-only")
        ...
        binding_sha = _sha256_document(
            {
                "schema_id": MMCIF_ZERO_OCCUPANCY_ATOM_SOURCE_BINDING_SCHEMA_ID,
                "profile_id": MMCIF_ZERO_OCCUPANCY_ATOM_PROFILE_ID,
        ...
        values = {
            ...
            "canonical_carrier_source_sha256": canonical_carrier_sha,
            "source_id_sha256": source_id_sha,
            "zero_occupancy_atom_projection_sha256": projection_sha,
            "record_state_sha256": record_sha,
            ...
            "nonpoly_record_state_sha256": nonpoly_state,
            "zero_occupancy_atom_rows": components.rows,
    ...
    def __repr__(self) -> str:
        return "MmcifZeroOccupancyAtomIngestResult(<source-bound-evidence>)"
...
def _ingest_evidence_document(
    ingest: MmcifZeroOccupancyAtomIngestResult,
) -> dict[str, Any]:
    document: dict[str, Any] = {
        "envelope_version": MMCIF_ZERO_OCCUPANCY_ATOM_ENVELOPE_VERSION,
        "parser_version": MMCIF_ZERO_OCCUPANCY_ATOM_PARSER_VERSION,
        ...
        "zero_occupancy_atom_row_count": len(ingest.zero_occupancy_atom_rows),
        "source_reported_missing_residue_claim_count": 0,
        "source_reported_missing_atom_claim_count": 0,
        "source_reported_zero_occupancy_atom_declaration_count": len(
            ingest.zero_occupancy_atom_rows
        ),
        "source_reported_zero_occupancy_atom_declarations_preserved": True,
        "zero_occupancy_atom_projection_sha256": ingest.zero_occupancy_atom_projection_sha256,
...
def _same_ingest(
    left: MmcifZeroOccupancyAtomIngestResult, right: MmcifZeroOccupancyAtomIngestResult
) -> bool:
    scalar_fields = (
        "full_source_sha256",
        ...
        "source_id_sha256",
        "zero_occupancy_atom_projection_sha256",
        "record_state_sha256",
    ...
    return (
        ...
        and left.zero_occupancy_atom_rows == right.zero_occupancy_atom_rows
...
def _validate_exact_rows(
    rows: Any,
) -> tuple[MmcifZeroOccupancyAtomRow, ...]:
    if type(rows) is not tuple:
        raise TypeError("zero-occupancy atom rows must be an exact tuple")
    for row in rows:
        if type(row) is not MmcifZeroOccupancyAtomRow:
            raise TypeError("zero-occupancy atom rows must contain exact row objects")
        row.__post_init__()
    return rows
...
def _validate_fresh_ingest(
    ingest: MmcifZeroOccupancyAtomIngestResult,
) -> tuple[MmcifZeroOccupancyAtomIngestResult, _ParsedEnvelope]:
    if type(ingest) is not MmcifZeroOccupancyAtomIngestResult:
        raise TypeError("ingest must be a MmcifZeroOccupancyAtomIngestResult")
    try:
        _validate_exact_rows(ingest.zero_occupancy_atom_rows)
    ...
    except (TypeError, ValueError, OverflowError, MmcifZeroOccupancyAtomError):
        raise MmcifZeroOccupancyAtomError(
...
def _ingest_from_components(
    ...
    return MmcifZeroOccupancyAtomIngestResult(components, _factory_token=_FACTORY_TOKEN)


def parse_mmcif_zero_occupancy_atoms(
    data: bytes, *, source_id: str = ""
) -> MmcifZeroOccupancyAtomIngestResult:
    return _ingest_from_components(_parse_components(data, source_id=source_id))


def mmcif_zero_occupancy_atom_projection_sha256(
    value: MmcifZeroOccupancyAtomIngestResult,
) -> str:
    fresh, _ = _validate_fresh_ingest(value)
    return fresh.zero_occupancy_atom_projection_sha256


def mmcif_zero_occupancy_atom_record_state_sha256(
    value: MmcifZeroOccupancyAtomIngestResult,
) -> str:
    fresh, _ = _validate_fresh_ingest(value)
...
def _emit_zero_occupancy_atom_loop(
    rows: tuple[MmcifZeroOccupancyAtomRow, ...],
) -> bytes:
    lines = ["loop_", *MMCIF_ZERO_OCCUPANCY_ATOM_HEADERS]
    for row in rows:
        tokens = (
            str(row.source_id),
            "Y",
...
def _compose_output(
    ...
    *,
    ...
    for category in _CANONICAL_ORDER:
        if category == _ZERO_OCCUPANCY_ATOM_CATEGORY:
            parts.append(
                _emit_zero_occupancy_atom_loop(ingest.zero_occupancy_atom_rows)
            )
...
def _receipt_document(
    ingest: MmcifZeroOccupancyAtomIngestResult, payload: bytes
) -> dict[str, Any]:
    document: dict[str, Any] = {
        "schema_id": MMCIF_ZERO_OCCUPANCY_ATOM_WRITE_RECEIPT_SCHEMA_ID,
        "envelope_version": MMCIF_ZERO_OCCUPANCY_ATOM_ENVELOPE_VERSION,
        "writer_version": MMCIF_ZERO_OCCUPANCY_ATOM_WRITER_VERSION,
        "profile_id": MMCIF_ZERO_OCCUPANCY_ATOM_PROFILE_ID,
...
@dataclass(frozen=True, slots=True, init=False, repr=False)
class MmcifZeroOccupancyAtomWriteReceipt:
    _ingest: MmcifZeroOccupancyAtomIngestResult
    _payload: bytes
    ...
    def __init__(
        ...
        *,
        ...
        if _factory_token is not _FACTORY_TOKEN:
            raise TypeError("MmcifZeroOccupancyAtomWriteReceipt is factory-only")
    ...
    def __repr__(self) -> str:
        return "MmcifZeroOccupancyAtomWriteReceipt(<source-bound-evidence>)"
    ...
    def to_dict(self) -> dict[str, Any]:
        if (
            type(self._ingest) is not MmcifZeroOccupancyAtomIngestResult
...
@dataclass(frozen=True, slots=True, init=False, repr=False)
class MmcifZeroOccupancyAtomWriteResult:
    payload: bytes
    receipt: MmcifZeroOccupancyAtomWriteReceipt
    ...
    def __init__(
        ...
        *,
        ...
        if _factory_token is not _FACTORY_TOKEN:
            raise TypeError("MmcifZeroOccupancyAtomWriteResult is factory-only")
    ...
    def __repr__(self) -> str:
        return "MmcifZeroOccupancyAtomWriteResult(<canonical-payload>)"

    def to_dict(self) -> dict[str, Any]:
        if (
            type(self.payload) is not bytes
            or type(self.receipt) is not MmcifZeroOccupancyAtomWriteReceipt
...
def emit_mmcif_zero_occupancy_atoms(
    ingest: MmcifZeroOccupancyAtomIngestResult,
) -> MmcifZeroOccupancyAtomWriteResult:
    ...
    return MmcifZeroOccupancyAtomWriteResult(
        payload,
        receipt,
...
def serialize_mmcif_zero_occupancy_atoms(
    ingest: MmcifZeroOccupancyAtomIngestResult,
) -> bytes:
    return emit_mmcif_zero_occupancy_atoms(ingest).payload


def _report_document(
    source: MmcifZeroOccupancyAtomIngestResult,
    reparsed: MmcifZeroOccupancyAtomIngestResult,
    write_result: MmcifZeroOccupancyAtomWriteResult,
    reemitted_write_result: MmcifZeroOccupancyAtomWriteResult,
    *,
    ...
    projection_equal = (
        source.zero_occupancy_atom_projection_sha256
        == reparsed.zero_occupancy_atom_projection_sha256
    )
    polymer_equal = (
    ...
    document: dict[str, Any] = {
        "schema_id": MMCIF_ZERO_OCCUPANCY_ATOM_ROUND_TRIP_REPORT_SCHEMA_ID,
        "envelope_version": MMCIF_ZERO_OCCUPANCY_ATOM_ENVELOPE_VERSION,
        "profile_id": MMCIF_ZERO_OCCUPANCY_ATOM_PROFILE_ID,
        ...
        "source_projection_sha256": source.zero_occupancy_atom_projection_sha256,
        "reparsed_projection_sha256": reparsed.zero_occupancy_atom_projection_sha256,
        ...
        "zero_occupancy_atom_projection_sha256_equal": projection_equal,
...
@dataclass(frozen=True, slots=True, init=False, repr=False)
class MmcifZeroOccupancyAtomRoundTripReport:
    _source: MmcifZeroOccupancyAtomIngestResult
    _reparsed: MmcifZeroOccupancyAtomIngestResult
    _write_result: MmcifZeroOccupancyAtomWriteResult
    _reemitted_write_result: MmcifZeroOccupancyAtomWriteResult
    _second_emission_byte_stable: bool
    _document_bytes: bytes

    def __init__(
        self,
        source: MmcifZeroOccupancyAtomIngestResult | None = None,
        reparsed: MmcifZeroOccupancyAtomIngestResult | None = None,
        write_result: MmcifZeroOccupancyAtomWriteResult | None = None,
        reemitted_write_result: MmcifZeroOccupancyAtomWriteResult | None = None,
        ...
        *,
        ...
        if _factory_token is not _FACTORY_TOKEN:
            raise TypeError("MmcifZeroOccupancyAtomRoundTripReport is factory-only")
        if (
            source is None
            or reparsed is None
    ...
    def __repr__(self) -> str:
        return "MmcifZeroOccupancyAtomRoundTripReport(<semantic-evidence>)"
    ...
    @property
    def zero_occupancy_atom_projection_sha256_equal(self) -> bool:
        return bool(self.to_dict()["zero_occupancy_atom_projection_sha256_equal"])
    ...
    def to_dict(self) -> dict[str, Any]:
        if (
            type(self._source) is not MmcifZeroOccupancyAtomIngestResult
            or type(self._reparsed) is not MmcifZeroOccupancyAtomIngestResult
            or type(self._write_result) is not MmcifZeroOccupancyAtomWriteResult
            or type(self._reemitted_write_result)
            is not MmcifZeroOccupancyAtomWriteResult
...
def _raw_ingest_binding_document(
    ...
    rows = _validate_exact_rows(ingest.zero_occupancy_atom_rows)
    return {
        "object_id": id(ingest),
...
def _raw_round_trip_chain_document(
    source: MmcifZeroOccupancyAtomIngestResult,
    write: MmcifZeroOccupancyAtomWriteResult,
    reparsed: MmcifZeroOccupancyAtomIngestResult,
    reemitted: MmcifZeroOccupancyAtomWriteResult,
    report: MmcifZeroOccupancyAtomRoundTripReport,
) -> dict[str, Any]:
    return {
...
@dataclass(frozen=True, slots=True, init=False, repr=False)
class MmcifZeroOccupancyAtomRoundTripResult:
    _source_ingest: MmcifZeroOccupancyAtomIngestResult
    _write_result: MmcifZeroOccupancyAtomWriteResult
    _reparsed_ingest: MmcifZeroOccupancyAtomIngestResult
    _reemitted_write_result: MmcifZeroOccupancyAtomWriteResult
    _report: MmcifZeroOccupancyAtomRoundTripReport
    _chain_binding_bytes: bytes

    def __init__(
        self,
        source_ingest: MmcifZeroOccupancyAtomIngestResult,
        write_result: MmcifZeroOccupancyAtomWriteResult,
        reparsed_ingest: MmcifZeroOccupancyAtomIngestResult,
        reemitted_write_result: MmcifZeroOccupancyAtomWriteResult,
        report: MmcifZeroOccupancyAtomRoundTripReport,
        *,
        ...
        if _factory_token is not _FACTORY_TOKEN:
            raise TypeError("MmcifZeroOccupancyAtomRoundTripResult is factory-only")
    ...
    def __repr__(self) -> str:
        return "MmcifZeroOccupancyAtomRoundTripResult(<bound-artifacts>)"

    def _validate_prevalidated_factory_artifacts(self) -> None:
        if (
            type(self._source_ingest) is not MmcifZeroOccupancyAtomIngestResult
            or type(self._reparsed_ingest) is not MmcifZeroOccupancyAtomIngestResult
            or type(self._write_result) is not MmcifZeroOccupancyAtomWriteResult
            or type(self._reemitted_write_result)
            is not MmcifZeroOccupancyAtomWriteResult
            or type(self._report) is not MmcifZeroOccupancyAtomRoundTripReport
        ):
            raise MmcifZeroOccupancyAtomError(
    ...
    @property
    def source_ingest(self) -> MmcifZeroOccupancyAtomIngestResult:
        self._validate_access_binding()
        return self._source_ingest

    @property
    def write_result(self) -> MmcifZeroOccupancyAtomWriteResult:
        self._validate_access_binding()
        return self._write_result

    @property
    def reparsed_ingest(self) -> MmcifZeroOccupancyAtomIngestResult:
        self._validate_access_binding()
        return self._reparsed_ingest

    @property
    def reemitted_write_result(self) -> MmcifZeroOccupancyAtomWriteResult:
        self._validate_access_binding()
        return self._reemitted_write_result

    @property
    def report(self) -> MmcifZeroOccupancyAtomRoundTripReport:
        self._validate_access_binding()
        return self._report
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "report": report,
            "source_reported_zero_occupancy_atom_declarations_preserved": report[
                "source_reported_zero_occupancy_atom_declarations_preserved"
            ],
...
def round_trip_mmcif_zero_occupancy_atoms_source(
    data: bytes, *, source_id: str = ""
) -> MmcifZeroOccupancyAtomRoundTripResult:
    source = parse_mmcif_zero_occupancy_atoms(data, source_id=source_id)
    write_result = emit_mmcif_zero_occupancy_atoms(source)
    reparsed = parse_mmcif_zero_occupancy_atoms(
        write_result.payload, source_id=source_id
    )
    second = emit_mmcif_zero_occupancy_atoms(reparsed)
    ...
    return MmcifZeroOccupancyAtomRoundTripResult(
        source,
        write_result,
        reparsed,
        second,
...
__all__ = [
    ...
    "mmcif_zero_occupancy_atom_projection_sha256",
    "mmcif_zero_occupancy_atom_record_state_sha256",
