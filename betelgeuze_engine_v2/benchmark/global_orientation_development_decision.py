    ...
    def __post_init__(self) -> None:
        ...
        if preparation_failure:
            ...
            if (
                ...
                or self.experimental_candidate_count
                != CANDIDATE_DENOMINATOR_PER_ARM
            ):
                raise GlobalOrientationDevelopmentDecisionError(
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "baseline_preparation_succeeded":
                self.baseline_preparation_succeeded,
            "experimental_preparation_succeeded":
                self.experimental_preparation_succeeded,
            "baseline_candidate_count": self.baseline_candidate_count,
            "experimental_candidate_count": self.experimental_candidate_count,
            "baseline_valid_proposal_oracle_rmsd":
                self.baseline_valid_proposal_oracle_rmsd,
            "experimental_valid_proposal_oracle_rmsd":
                self.experimental_valid_proposal_oracle_rmsd,
            "baseline_selected_top1_rmsd": self.baseline_selected_top1_rmsd,
            "experimental_selected_top1_rmsd":
                self.experimental_selected_top1_rmsd,
            "baseline_selected_top1_valid":
                self.baseline_selected_top1_valid,
            "experimental_selected_top1_valid":
                self.experimental_selected_top1_valid,
            "source_geometry_evidence_complete":
                self.source_geometry_evidence_complete,
            "observation_evidence_complete":
                self.observation_evidence_complete,
        }
...
class GlobalOrientationDevelopmentDecision:
    ...
    def _projection(self) -> dict[str, object]:
        return {
            ...
            "new_valid_proposal_recovery_case_ids":
                list(self.new_valid_proposal_recovery_case_ids),
            ...
            "baseline_recovered_regression_case_ids":
                list(self.baseline_recovered_regression_case_ids),
            "observation_receipt_sha256s":
                list(self.observation_receipt_sha256s),
...
def evaluate_global_orientation_development(
    ...
    if any(
        ...
        or (
            ...
            and row.baseline_candidate_count
            != CANDIDATE_DENOMINATOR_PER_ARM
        )
        for row in rows
    ):
    ...
    new_recoveries = tuple(
        ...
        and row.experimental_valid_proposal_oracle_rmsd
    ...
    baseline_regressions = tuple(
        ...
        and (
            ...
            or row.experimental_selected_top1_rmsd
            > RMSD_THRESHOLD_ANGSTROM
        )
    )
    ...
    if experimental_invalid <= baseline_invalid:
        go_criteria.append("no_increase_in_invalid_selected_top1_count")

    verdict = (
        "go"
        if not hard_no_go
        and len(go_criteria) == 2
        else "no_go"
    )
