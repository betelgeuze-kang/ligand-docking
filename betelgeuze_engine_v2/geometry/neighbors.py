...
NEIGHBOR_SCHEMA_VERSION = "compact_radius_graph/2.1.0"
...
class NeighborBuildDiagnostics:
    ...
    @property
    def capacity_contract_satisfied(self) -> bool:
        return bool(not self.overflow and not self.nxn_allocation_observed)

    @property
    def claim_safe(self) -> bool:
        """Deprecated compatibility alias; this is not a scientific claim gate."""

        return self.capacity_contract_satisfied
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "capacity_contract_satisfied": self.capacity_contract_satisfied,
...
class CompactNeighborList:
    ...
    minimum-image ``r_i - r_j`` vectors. ``image_shifts`` are integer lattice
    multiples subtracted from raw displacements and make periodic geometry
    explicit for later differentiable energy consumers.
    """
    ...
    def __post_init__(self) -> None:
        ...
        expected_vector_shape = (*self.indices.shape, 3)
        if self.displacements.shape != expected_vector_shape:
            raise ValueError("displacements must have shape [B, N, K, 3]")
        if self.image_shifts.shape != expected_vector_shape:
            raise ValueError("image_shifts must have shape [B, N, K, 3]")
...
def _cell_parameters(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(
            "compact cell-list v2 currently requires an orthorhombic unit cell"
        ) from exc
    lengths = [
        float(value)
        for value in lengths_t.detach().to(dtype=torch.float64, device="cpu").tolist()
    ]
...
def build_compact_radius_graph(
    ...
    *,
    ...
    """Build a compact radius graph with conditional expected O(B*N) work.

    A Python/CPU cell map selects discrete neighbors. Geometry is gathered from
    the original tensor in O(B*N*K), preserving coordinate gradients. Cell and
    row capacities are hard gates; overflow never returns a truncated graph.
    """
    ...
    for batch_index in range(batch_size):
        ...
        for source_index in range(atom_count):
            ...
            for key in _neighbor_cell_keys(
                keys[source_index],
                periodic=periodic,
                grid_dims=grid_dims,
            ):
    ...
    batch_offsets = (
        torch.arange(batch_size, device=coordinates.device, dtype=torch.long)
        .view(batch_size, 1, 1)
        * atom_count
    )
    ...
    raw_displacements = coordinates.unsqueeze(2) - target_coordinates
    image_shifts = torch.zeros_like(raw_displacements, dtype=torch.long)
    ...
    if lengths is not None and any(periodic):
        ...
        rounded = torch.round(raw_displacements / box).detach().to(dtype=torch.long)
        image_shifts = torch.where(
            periodic_mask,
            rounded,
            torch.zeros_like(rounded),
        )
        displacements = raw_displacements - box * image_shifts.to(dtype=coordinates.dtype)
    else:
