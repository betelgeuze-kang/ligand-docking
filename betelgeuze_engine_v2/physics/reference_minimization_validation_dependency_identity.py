...
REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_IDENTITY_SCHEMA_ID = (
    "betelgeuze.engine_v2_reference_minimization_validation_dependency_identity/1.1.0"
)
REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_MANIFEST_SCHEMA_ID = (
    "betelgeuze.engine_v2_reference_minimization_validation_dependency_manifest/1.0.0"
)
...
REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_MAX_ENTRIES = 60_000
REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_MAX_PATH_BYTES = 4_096
REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_RECORD_MAX_BYTES = 16 * 1024**2
REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_RECORD_MAX_LINE_BYTES = 8_192
REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_TOTAL_MAX_FILES = 60_000
...
class _ScanBudget:
    ...
    def start_entry(self) -> None:
        ...
        if (
            self.entries
            > REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_TOTAL_MAX_ENTRIES
        ):
            raise ReferenceMinimizationValidationDependencyIdentityError(
    ...
    def start_file(self, size: int) -> None:
        ...
        if (
            ...
            or size < 0
            or size
            > REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_TOTAL_MAX_BYTES - self.bytes
        ):
            raise ReferenceMinimizationValidationDependencyIdentityError(
                "dependency file exceeds its pre-read aggregate byte bound"
            )
        self.files += 1
        if self.files > REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_TOTAL_MAX_FILES:
            raise ReferenceMinimizationValidationDependencyIdentityError(
    ...
    def add_bytes(self, count: int) -> None:
        self.checkpoint()
        self.bytes += count
        if self.bytes > REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_TOTAL_MAX_BYTES:
            raise ReferenceMinimizationValidationDependencyIdentityError(
            ...
            with os.scandir(directory) as stream:
                ...
                for entry in stream:
                    ...
                    if (
                        not relative
                        or len(path_bytes)
                        > REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_MAX_PATH_BYTES
                    ):
                        raise ReferenceMinimizationValidationDependencyIdentityError(
...
def _require_manifest_file_rows(rows: object) -> tuple[int, int]:
    if (
        not isinstance(rows, list)
        or not rows
        or len(rows) > REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_MAX_FILES
    ):
        raise ReferenceMinimizationValidationDependencyIdentityError(
    ...
    for row in rows:
        ...
        if (
            ...
            or size < 0
            or size > REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_MAX_BYTES
        ):
            raise ReferenceMinimizationValidationDependencyIdentityError(
                "dependency manifest file row is invalid"
            )
        previous_path = path
        total_bytes += size
        if total_bytes > REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_MAX_BYTES:
            raise ReferenceMinimizationValidationDependencyIdentityError(
    ...
    *,
    ...
    for path, relative, file_stat in _bounded_tree_entries(
        ...
        digest, size = _hash_regular_file(
            ...
            budget=budget,
            maximum_bytes=(
                REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_MAX_BYTES - total_bytes
            ),
        )
        total_bytes += size
...
def _read_distribution_record_rows(
    ...
    *,
    ...
    def append_record_row(raw_line: bytes) -> None:
        ...
        budget.start_entry()
        if len(rows) >= REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_MAX_FILES:
            raise ReferenceMinimizationValidationDependencyIdentityError(
    ...
    try:
        ...
        if (
            ...
            <= REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_RECORD_MAX_BYTES
        ):
            raise ReferenceMinimizationValidationDependencyIdentityError(
...
def _dependency_manifest_document(
    ...
    if (
        ...
        or len(artifacts)
        != len(REFERENCE_MINIMIZATION_VALIDATION_REQUIRED_DEPENDENCY_ARTIFACT_IDS)
    ...
    for artifact in artifacts:
        ...
        if (
            file_count > REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_TOTAL_MAX_FILES
            or total_bytes
            > REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_TOTAL_MAX_BYTES
        ):
            raise ReferenceMinimizationValidationDependencyIdentityError(
                "dependency manifest exceeds its aggregate bounds"
            )
    projection = {
        "schema_id": REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_MANIFEST_SCHEMA_ID,
        "identity_schema_id": (
            REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_IDENTITY_SCHEMA_ID
        ),
        "artifacts": artifacts,
...
def require_reference_minimization_validation_dependency_manifest_document(
    value: object,
) -> dict[str, object]:
...
def observed_reference_minimization_validation_dependency_manifest_document(
    ...
    *,
    ...
    if deadline is None:
        deadline = (
            time.monotonic()
            + REFERENCE_MINIMIZATION_VALIDATION_DEPENDENCY_PREFLIGHT_MAX_WALL_SECONDS
        )
...
def observed_reference_minimization_validation_dependency_artifact_sha256_rows(
    ...
    *,
    ...
    manifest = observed_reference_minimization_validation_dependency_manifest_document(
        dependency_roots,
        deadline=deadline,
    )
