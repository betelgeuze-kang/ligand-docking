...
_CURRENT_BLOCKERS = (
    ...
    "production_nonce_reservation_root_missing",
...
class ReferenceMinimizationValidationRunStartError(ValueError):
...
def reference_minimization_validation_artifact_output_root_identity_sha256(
...
def _require_reference_minimization_validation_root_outside_checkout(
    root: str | os.PathLike[str],
    *,
    name: str,
) -> Path:
...
def build_signed_reference_minimization_validation_network_isolation_attestation(
    *,
    authorization_receipt_sha256: str,
    authorization_nonce_sha256: str,
    authorization_operator_identity_sha256: str,
...
class ReferenceMinimizationValidationNetworkIsolationVerification:
...
def verify_signed_reference_minimization_validation_network_isolation_attestation(
    source: str | bytes | Mapping[str, Any],
    *,
    trusted_operator_keys: Mapping[str, MinimizationAuthorizationOperatorTrustAnchor],
    checked_at: datetime,
    expected_authorization: ReferenceMinimizationValidationAuthorizationVerification,
    ...
    return ReferenceMinimizationValidationNetworkIsolationVerification(
...
def reference_minimization_validation_bootstrap_path() -> str:
    """Return the reserved absolute path for the future bounded bootstrap.

    The run-start primitive freezes the secret-free command now, but the
    bootstrap and bounded runner remain a later contract stage.  Consequently
...
def _read_logical_runner_argv() -> tuple[str, ...]:
    ...
    if (
        len(decoded) != len(REFERENCE_MINIMIZATION_VALIDATION_LOGICAL_RUNNER_ARGV)
        or not _PYTHON_EXECUTABLE_RE.fullmatch(Path(decoded[0]).name)
        or decoded[1:-1] != REFERENCE_MINIMIZATION_VALIDATION_LOGICAL_RUNNER_ARGV[1:-1]
        or not bootstrap_matches
    ):
        raise ReferenceMinimizationValidationRunStartError(
            "process argv is not the frozen secret-free validation runner command"
        )
    return REFERENCE_MINIMIZATION_VALIDATION_LOGICAL_RUNNER_ARGV
...
def _contract_projection() -> dict[str, Any]:
    return {
        "schema_id": REFERENCE_MINIMIZATION_VALIDATION_RUN_START_CONTRACT_SCHEMA_ID,
...
def require_reference_minimization_validation_run_start_contract_document(
...
@dataclass(frozen=True, slots=True)
class ReferenceMinimizationValidationExecutionEnvironmentReceipt:
    receipt_sha256: str
...
def _assert_reservation_matches_authorization(
    reservation: ReferenceMinimizationValidationNonceReservation,
...
def _build_environment_receipt(
    *,
    ...
    provisional = ReferenceMinimizationValidationExecutionEnvironmentReceipt(
        receipt_sha256="0" * 64,
        **values,
    )
    return ReferenceMinimizationValidationExecutionEnvironmentReceipt(
...
def create_reference_minimization_validation_execution_environment_receipt(
...
def read_reference_minimization_validation_execution_environment_receipt(
    artifact_output_root: str | os.PathLike[str],
    authorization_nonce_sha256: str,
) -> ReferenceMinimizationValidationExecutionEnvironmentReceipt:
...
def require_reference_minimization_validation_execution_environment_receipt_for_runner(
    artifact_output_root: str | os.PathLike[str],
    authorization_nonce_sha256: str,
    *,
    ...
    receipt = read_reference_minimization_validation_execution_environment_receipt(
        artifact_output_root,
        authorization_nonce_sha256,
    )
...
def reference_minimization_validation_run_start_contract_decision() -> dict[str, Any]:
    contract = reference_minimization_validation_run_start_contract_document()
    return {
