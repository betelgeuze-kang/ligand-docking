...
never drops a failed case, variant, or metric.  Receipt creation is not an
...
from . import reference_validation_runner as _reference_validation_runner
...
class ReferenceValidationResultWriterError(RuntimeError):
    """Result finalization, persistence, or verification failed closed."""


class ReferenceValidationResultReceiptAlreadyExistsError(
...
def _parse_utc(value: object, *, name: str) -> datetime:
    if not isinstance(value, str):
        raise ReferenceValidationResultWriterError(f"{name} must be UTC text")
    try:
...
def _contract_projection() -> dict[str, Any]:
    return {
        "schema_id": REFERENCE_VALIDATION_RESULT_WRITER_CONTRACT_SCHEMA_ID,
        ...
        "persistence": {
            ...
            "maximum_receipt_bytes": REFERENCE_VALIDATION_RESULT_RECEIPT_MAX_BYTES,
        ...
        "verification": {
            ...
            "exact_worker_request_transport_reparsed": True,
            "worker_request_cross_checked_against_outer_run_identity": True,
            "worker_request_environment_cross_checked_against_persisted_receipt_and_live_process": True,
            "worker_child_pid_cross_checked_against_native_snapshots": True,
        ...
        "coverage": {
            ...
            "incomplete_worker_partial_transcript_independently_replayable": False,
            "incomplete_worker_accepted_payload_count": 0,
...
def reference_validation_result_writer_contract_document() -> dict[str, Any]:
...
def require_reference_validation_result_writer_contract_document(
    ...
    expected = reference_validation_result_writer_contract_document()
...
def _require_worker_execution_receipt_binding(
    ...
    *,
    environment: ReferenceValidationExecutionEnvironmentReceipt | None = None,
) -> None:
    ...
    manifest_provenance = run_document["manifest_worker_execution_provenance"]
    case_provenance = run_document["case_worker_execution_provenance"]
    try:
        ...
        if environment is not None:
            if (
                ...
                or environment.environment_fingerprint_sha256
                != observation.environment_fingerprint_sha256
                or environment.application_seed != observation.seed
            ):
                raise ReferenceValidationResultWriterError(
                    "result receipt worker environment is cross-wired"
                )
            dependency_roots = (
                _reference_validation_runner._require_isolated_python_bootstrap_runtime(
            ...
            worker_environment = _reference_validation_runner._case_worker_environment(
                environment.environment_variable_rows,
                dependency_python_path=dependency_python_path,
            )
            trusted_runtime = {
                "expected_python_hash_seed": environment.python_hash_seed,
                "expected_worker_environment": worker_environment,
                "expected_dependency_roots": dependency_roots,
                "expected_python_hash_probe_sha256": (
                    _reference_validation_runner._python_hash_probe_sha256()
                ),
            }
        require_reference_validation_worker_execution_provenance_document(
            manifest_provenance,
            ...
            expected_authorization_nonce_sha256=(
                observation.authorization_nonce_sha256
            ),
    ...
    except (ReferenceValidationRunnerError, AttributeError) as exc:
        raise ReferenceValidationResultWriterError(
            "result receipt worker execution provenance is invalid"
        ) from exc
...
class ReferenceValidationResultReceipt:
    receipt_sha256: str
...
def _result_projection(
    observation: ReferenceValidationRunObservation,
    environment: ReferenceValidationExecutionEnvironmentReceipt,
    *,
    ...
    _require_worker_execution_receipt_binding(
        observation,
    ...
    return {
        ...
        "authorization_receipt_sha256": observation.authorization_receipt_sha256,
...
def _persist_result_receipt(
    ...
    if len(encoded) > REFERENCE_VALIDATION_RESULT_RECEIPT_MAX_BYTES:
        raise ReferenceValidationResultWriterError(
    ...
    try:
        try:
            ...
            raise ReferenceValidationResultReceiptAlreadyExistsError(
...
def write_reference_validation_result_receipt(
    artifact_output_root: str | os.PathLike[str],
    authorization_nonce_sha256: str,
    observation: ReferenceValidationRunObservation,
    *,
    ...
    if not isinstance(observation, ReferenceValidationRunObservation):
        raise ReferenceValidationResultWriterError(
    ...
    checked_at = _utc_now()
    try:
        review = verify_signed_reference_validation_review_attestation(
            review_attestation,
    ...
    expected_authorization_rows = {
        "receipt_sha256": observation.authorization_receipt_sha256,
    ...
    expected_environment_rows = {
        "receipt_sha256": observation.execution_environment_receipt_sha256,
    ...
    try:
        start_record = read_reference_validation_runner_start_record(
            artifact_output_root,
            nonce,
    ...
    try:
        try:
            ...
            while True:
                ...
                chunks.append(chunk)
                total += len(chunk)
                if total > REFERENCE_VALIDATION_RESULT_RECEIPT_MAX_BYTES:
                    raise ReferenceValidationResultWriterError(
                        "result receipt exceeds the size limit"
                    )
...
def read_reference_validation_result_receipt(
    artifact_output_root: str | os.PathLike[str],
    authorization_nonce_sha256: str,
) -> ReferenceValidationResultReceipt:
...
def verify_reference_validation_result_receipt(
    ...
    *,
    ...
    receipt = read_reference_validation_result_receipt(
        artifact_output_root,
        authorization_nonce_sha256,
    )
...
def reference_validation_result_writer_contract_decision() -> dict[str, Any]:
    contract = reference_validation_result_writer_contract_document()
    return {
