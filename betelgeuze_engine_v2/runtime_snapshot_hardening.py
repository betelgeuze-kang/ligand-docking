...
def _snapshot_document() -> dict[str, object]:
    ...
    if (
        ...
        or len(state) != 6
        or state[4] != manifest_sha256
        or state[5] != getattr(finder, "finder_identity_sha256", None)
    ):
        raise VerifiedSourceRuntimeHardeningError(
    ...
    projection: dict[str, object] = {
        ...
        "finder_identity_sha256": state[5],
        "resource_audit_sha256": audit_sha256,
        "repository_root": state[1],
        "bootstrap_path": state[0],
        "dependency_roots": list(state[2]),
        "frozen_sys_path": list(state[3]),
