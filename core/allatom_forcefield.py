...
def infer_torsions(bonds: list[tuple[int, int]]) -> list[tuple[int, int, int, int]]:
    ...
    for j, k in bonds:
        for i in neighbors.get(j, []):
            if i == k:
                continue
            for fourth in neighbors.get(k, []):
                if fourth == j or fourth == i:
                    continue
                torsion = (i, j, k, fourth)
                reverse = (fourth, k, j, i)
...
def dihedral_energy(
    ...
    *,
    ...
    pts = np.asarray(coords, dtype=np.float64)
    total = 0.0
    for i, j, k, fourth in torsions:
        phi = _dihedral_angle(pts[i], pts[j], pts[k], pts[fourth])
