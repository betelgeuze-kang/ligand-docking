...
def softcore_lj_at_lambda(
    ...
    *,
    ...
    """Scale the interaction toward a decoupled endpoint with a soft core."""
    r = np.maximum(
        np.asarray(dist_a, dtype=np.float64),
        0.5 + float(alpha) * (1.0 - float(lam)),
    )
...
def _ligand_protein_cross_energy(
    ...
    *,
    ...
    prot_elements, _ = _normalized_elements(
        protein_elements,
        int(prot.shape[0]),
        default=protein_element,
    )
    lig_elements, _ = _normalized_elements(
        ligand_elements,
        int(lig.shape[0]),
        default=ligand_element,
    )
    total = 0.0
    for i, prot_element in enumerate(prot_elements):
        ...
        for j, lig_element in enumerate(lig_elements):
            ...
            total += float(
                softcore_lj_at_lambda(
                    np.asarray([d[i, j]], dtype=np.float64),
                    sigma,
                    epsilon,
                    lam=float(lam),
                )[0]
...
def _element_metadata(
    ...
    fallback_used = bool(
        protein_element_fallback_used or ligand_element_fallback_used
    )
    return {
...
def fep_lambda_energies(
    ...
    *,
    ...
    """Evaluate static lambda-point energies for a fixed coordinate pair."""
    protein_atom_count = int(np.asarray(protein_xyz).shape[0])
    ligand_atom_count = int(np.asarray(ligand_xyz).shape[0])
    protein_element_list, protein_fallback = _normalized_elements(
        protein_elements,
        protein_atom_count,
    )
    ligand_element_list, ligand_fallback = _normalized_elements(
        ligand_elements,
        ligand_atom_count,
    )
    element_meta = _element_metadata(
        protein_fallback,
        ligand_fallback,
        protein_atom_count,
        ligand_atom_count,
...
def bar_free_energy(
    ...
    *,
    ...
    Static lambda energies are intentionally rejected. The estimator requires
    separate forward and reverse ensemble samples.
    """
...
def static_alchemical_endpoint_proxy(
    ...
    *,
    ...
    """Return a fixed-coordinate endpoint difference with blocked claims."""
    ...
    return {
        ...
        "protein_element_fallback_used": bool(
            first.get("protein_element_fallback_used", True)
        ),
        "ligand_element_fallback_used": bool(
            first.get("ligand_element_fallback_used", True)
        ),
