...
impl Sha256 {
    ...
    pub(crate) fn update(&mut self, mut input: &[u8]) {
        self.length_bytes = self
            ...
            .expect("bounded canonical identity length fits u64");
        ...
        while input.len() >= 64 {
            let block: &[u8; 64] = input[..64].try_into().expect("slice has one SHA block");
            self.compress(block);
            input = &input[64..];
        }
    ...
    pub(crate) fn finalize(mut self) -> [u8; 32] {
        let bit_length = self
            .length_bytes
            .checked_mul(8)
            .expect("bounded canonical identity bit length fits u64");
...
mod tests {
    ...
    #[test]
    fn matches_fips_vectors_across_chunk_boundaries() {
