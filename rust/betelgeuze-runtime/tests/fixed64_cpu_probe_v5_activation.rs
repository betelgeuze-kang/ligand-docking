...
use betelgeuze_runtime::{
    fixed64_cpu_v5_live_activation_admitted, FIXED64_CPU_V5_LIVE_ACTIVATION_ADMITTED,
};
...
fn native_fixed64_cpu_probe_is_blocked_before_measurement() {
    ...
    let activation_constant = std::hint::black_box(FIXED64_CPU_V5_LIVE_ACTIVATION_ADMITTED);
    assert!(!activation_constant);
    assert!(!fixed64_cpu_v5_live_activation_admitted());

    let output = Command::new(env!("CARGO_BIN_EXE_betelgeuze-fixed64-cpu-probe-v5"))
        .output()
    ...
    assert_eq!(
        ...
        "fixed64 CPU probe v5 failed closed: reviewed live activation is absent\n"
    );
}
