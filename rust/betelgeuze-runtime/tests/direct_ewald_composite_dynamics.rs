...
use runtime::{
    AtomNonbonded, Backend, Context, ContextOptions, DirectEwaldCompositeSimulation,
    DirectEwaldErrorCode, DirectEwaldModel, DirectEwaldPairExclusion, DirectEwaldPairScale,
    DirectEwaldParameters, DirectEwaldSettings, DistanceConstraints, ErrorCode, ForceField,
    ForceFieldInput, HarmonicAngle, HarmonicBond, Integrator, NonbondedSettings, OrthorhombicCell,
    PairExclusion, PairScale, ParticleSnapshot, ParticleSoa, PeriodicTorsion, PositionSoa,
...
fn profile_deep_ownership_and_velocity_verlet_boundary_are_explicit() {
    assert_eq!(
        runtime::direct_ewald_composite_dynamics_profile_id().unwrap(),
        "betelgeuze.native_direct_ewald_composite_dynamics/1.0.0"
    );
    assert!(std::mem::needs_drop::<DirectEwaldCompositeSimulation>());

    // This owner outlives every constructor input, exercising the ABI's deep
    // copy boundary before Drop performs its one native destroy.
    let simulation = neutral_simulation(options(0.1)).unwrap();
    assert_eq!(simulation.len(), 4);
    assert!(!simulation.is_empty());
    assert_eq!(simulation.snapshot().unwrap().len(), 4);
    drop(simulation);

    let error = match neutral_simulation(SimulationOptions {
...
#[test]
fn exact_neutral_cpu_lanes_restart_and_continue_bit_exactly() {
    for backend in [Backend::CppCpuReference, Backend::RustCpu] {
        let context = context(backend);
        let mut uninterrupted = neutral_simulation(options(0.1)).unwrap();
        let mut restarted = neutral_simulation(options(0.1)).unwrap();

        let initial_snapshot = uninterrupted.snapshot().unwrap();
        let initial_checkpoint = uninterrupted.checkpoint().unwrap();
        assert!(initial_checkpoint.starts_with(b"BGDEC001"));
        let zero = context
            .integrate_direct_ewald_composite(&mut uninterrupted, 0)
            .unwrap();
        ...
        context
            .integrate_direct_ewald_composite(&mut uninterrupted, 3)
            .unwrap();
        ...
        let uninterrupted_report = context
            .integrate_direct_ewald_composite(&mut uninterrupted, 5)
            .unwrap();
        let restarted_report = context
            .integrate_direct_ewald_composite(&mut restarted, 5)
            .unwrap();
...
fn mismatched_and_corrupt_checkpoints_preserve_every_dynamic_byte() {
    ...
    let mut source = neutral_simulation(options(0.1)).unwrap();
    context
        .integrate_direct_ewald_composite(&mut source, 4)
        .unwrap();
    let checkpoint = source.checkpoint().unwrap();

    let mut target = neutral_simulation(options(0.2)).unwrap();
...
fn nonneutral_creation_preserves_the_typed_direct_ewald_error() {
    let error = match simulation(&NONNEUTRAL_CHARGES, options(0.1)) {
...
fn neutral_simulation(
    ...
    simulation(&NEUTRAL_CHARGES, options)
}
...
fn simulation(
    ...
    let model = model();
    DirectEwaldCompositeSimulation::new(
        &system,
        &forcefield,
        &model,
