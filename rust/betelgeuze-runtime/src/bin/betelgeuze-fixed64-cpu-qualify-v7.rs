...
fn validated_path_text(path: &Path) -> &str {
    path.to_str()
        .expect("v7 validates all returned paths as UTF-8 before consuming the attempt")
}

fn fail(message: &str) -> ExitCode {
    eprintln!("native fixed64 CPU qualification v7 failed closed: {message}");
    ExitCode::from(1)
}
...
fn main() -> ExitCode {
    ...
    if arguments.len() == 2 && arguments[1] == OsStr::new("--verify-activation") {
        let status = match verify_native_fixed64_cpu_v7_activation() {
            Ok(value) => value,
    ...
    if arguments.len() == 2 && arguments[1] == OsStr::new("--preflight") {
        let preflight = match preflight_native_fixed64_cpu_v7() {
            Ok(value) => value,
            Err(error) => return fail(error.message()),
        };
        println!(
    ...
    if arguments.len() == 3 && arguments[1] == OsStr::new("--run-output") {
        let result = match run_native_fixed64_cpu_qualification_v7(Path::new(&arguments[2])) {
            Ok(value) => value,
            Err(error) => return fail(error.message()),
        };
        println!(
    ...
    eprintln!(
        "usage: betelgeuze-fixed64-cpu-qualify-v7 \
