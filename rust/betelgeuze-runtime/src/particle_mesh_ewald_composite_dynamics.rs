//! Safe ownership for deterministic short-range + particle-mesh Ewald dynamics.
...
use crate::{
    checked_count, copy_native_channel, invalid, Backend, Context, DistanceConstraints,
    DynamicsReport, ErrorCode, ForceField, Integrator, ParticleSnapshot, PositionSoaOwned,
    SimulationOptions, System, VelocitySoaOwned,
};

const PROFILE_ID: &str = "betelgeuze.native_particle_mesh_ewald_composite_dynamics/1.0.0";
const CHECKPOINT_MAGIC: &[u8; 8] = b"BGPME001";
const CHECKPOINT_HEADER_SIZE: usize = 104;
const BOLTZMANN_KCAL_PER_MOL_KELVIN: f64 = 0.001_987_204_258_640_831_6;

/// A deep-owned particle-mesh Ewald composite Velocity-Verlet simulation.
///
/// Creation copies the system, force field, both Ewald models, constraints,
/// and options. Native calls require external synchronization, so the owner
/// is deliberately neither `Send` nor `Sync` and destroys its handle once.
///
/// ```compile_fail
/// use betelgeuze_runtime::ParticleMeshEwaldCompositeSimulation;
/// fn require_send<T: Send>() {}
/// require_send::<ParticleMeshEwaldCompositeSimulation>();
/// ```
///
/// ```compile_fail
/// use betelgeuze_runtime::ParticleMeshEwaldCompositeSimulation;
/// fn require_sync<T: Sync>() {}
/// require_sync::<ParticleMeshEwaldCompositeSimulation>();
/// ```
///
/// ```compile_fail
/// use betelgeuze_runtime::ParticleMeshEwaldCompositeSimulation;
/// fn require_clone<T: Clone>() {}
/// require_clone::<ParticleMeshEwaldCompositeSimulation>();
/// ```
pub struct ParticleMeshEwaldCompositeSimulation {
    handle: NonNull<sys::bg_particle_mesh_ewald_composite_simulation_v1>,
    particle_count: usize,
    constraint_count: usize,
    _not_send_or_sync: PhantomData<Rc<()>>,
}

impl ParticleMeshEwaldCompositeSimulation {
    /// Deep-copy all immutable inputs into a new Velocity-Verlet owner.
    pub fn new(
        system: &System,
        ...
        ensure_particle_mesh_ewald_composite_dynamics_abi_compatibility()?;
        ...
        if direct_model.len()? != particle_count {
            return Err(local_invalid(
        ...
        {
            return Err(local_invalid(
                "particle-mesh reciprocal model atom count must match the system",
            ));
        }
        ...
        // SAFETY: All handles and descriptors remain live through the call;
        // successful native creation deep-copies their semantic contents.
        let status = unsafe {
            sys::bg_particle_mesh_ewald_composite_simulation_v1_create(
                system.raw_handle(),
        ...
        if status != sys::BG_STATUS_OK {
            // Reclaim an abnormal non-null failure handle exactly once.
            drop(simulation);
            return Err(error_from_call(status, &raw_error));
        }
        let simulation = simulation.ok_or_else(|| {
            abi_error("particle-mesh composite simulation creation succeeded with a null handle")
        })?;
    ...
    pub fn snapshot(&self) -> DirectEwaldResult<ParticleSnapshot> {
        ...
        // per channel and self remains borrowed during every copy.
        unsafe {
            Ok(ParticleSnapshot {
    ...
    fn particle_view(&self) -> DirectEwaldResult<sys::bg_particle_soa_view> {
        ...
        // SAFETY: The owner remains live and borrowed through validation/copy.
        plain_status(unsafe {
            sys::bg_particle_mesh_ewald_composite_simulation_v1_get_particles(
                self.handle.as_ptr(),
                &mut view,
            )
        })?;
            ...
            || view.reserved != [0; 4]
        {
            return Err(abi_error(
                "native particle-mesh composite simulation returned an invalid particle view descriptor",
            ));
        }
        Ok(view)
    }
    ...
    pub fn absolute_step(&self) -> DirectEwaldResult<u64> {
        ...
        plain_status(unsafe {
            sys::bg_particle_mesh_ewald_composite_simulation_v1_get_absolute_step(
                self.handle.as_ptr(),
                &mut step,
            )
        })?;
    ...
    /// Serialize the canonical, integrity-protected `BGPME001` restart image.
    pub fn checkpoint(&self) -> DirectEwaldResult<Vec<u8>> {
        let mut required = 0_u64;
        // SAFETY: The owned handle is live and required is writable.
        plain_status(unsafe {
            sys::bg_particle_mesh_ewald_composite_simulation_v1_checkpoint_size(
                self.handle.as_ptr(),
                &mut required,
            )
        })?;
        let length = usize::try_from(required).map_err(|_| DirectEwaldError {
            ...
            code: None,
            detail: "native particle-mesh composite checkpoint size exceeds usize".to_owned(),
        })?;
        if length < CHECKPOINT_HEADER_SIZE {
            return Err(abi_error(
                "native particle-mesh composite checkpoint size is smaller than its frozen header",
            ));
        }
        if length > isize::MAX as usize {
            return Err(DirectEwaldError {
                ...
                code: None,
                detail:
                    "native particle-mesh composite checkpoint exceeds addressable Rust capacity"
                        .to_owned(),
            });
        }
        ...
        bytes
            ...
            .map_err(|error| DirectEwaldError {
                ...
                code: None,
                detail: format!("cannot allocate particle-mesh composite checkpoint: {error}"),
            })?;
        bytes.resize(length, 0);
        let mut written = 0_u64;
        // SAFETY: bytes exposes exactly required writable bytes.
        plain_status(unsafe {
            sys::bg_particle_mesh_ewald_composite_simulation_v1_checkpoint_write(
                self.handle.as_ptr(),
        ...
        if written != required {
            return Err(abi_error(
                "native particle-mesh composite checkpoint writer returned an inconsistent byte count",
            ));
        }
        if bytes[..CHECKPOINT_MAGIC.len()] != CHECKPOINT_MAGIC[..] {
            return Err(abi_error(
                "native particle-mesh composite checkpoint writer returned an invalid magic",
            ));
        }
        Ok(bytes)
    }

    /// Transactionally restore state from a canonical `BGPME001` image.
    pub fn load_checkpoint(&mut self, checkpoint: &[u8]) -> DirectEwaldResult<()> {
        if checkpoint.len() < CHECKPOINT_HEADER_SIZE || !checkpoint.starts_with(CHECKPOINT_MAGIC) {
            return Err(local_invalid(
                "particle-mesh composite checkpoint must contain a complete BGPME001 header",
            ));
        }
        let length = checked_count(checkpoint.len()).map_err(DirectEwaldError::from)?;
        // SAFETY: checkpoint exposes length readable bytes; native load is
        // transactional and commits only after complete validation.
        plain_status(unsafe {
            sys::bg_particle_mesh_ewald_composite_simulation_v1_checkpoint_load(
                self.handle.as_ptr(),
...
impl Drop for ParticleMeshEwaldCompositeSimulation {
    fn drop(&mut self) {
        // SAFETY: This non-clone owner holds one non-null handle and destroys
        // it exactly once, including handles wrapped during failed creation.
        unsafe {
            sys::bg_particle_mesh_ewald_composite_simulation_v1_destroy(self.handle.as_ptr())
        };
    }
}

impl Context {
    /// Integrate the particle-mesh composite owner on an explicit CPU lane.
    pub fn integrate_particle_mesh_ewald_composite(
        &self,
        simulation: &mut ParticleMeshEwaldCompositeSimulation,
        step_count: u64,
    ) -> DirectEwaldResult<DynamicsReport> {
        self.require_particle_mesh_ewald_composite_dynamics_backend()?;
        ensure_particle_mesh_ewald_composite_dynamics_abi_compatibility()?;
        ...
        let expected_absolute_step =
            initial_step
                ...
                .ok_or_else(|| DirectEwaldError {
                    status: ErrorCode::CapacityOverflow,
                    code: None,
                    detail: "absolute particle-mesh composite dynamics step would overflow uint64"
                        .to_owned(),
                })?;
        let expected_degrees_of_freedom = simulation
            ...
            .ok_or_else(|| {
                abi_error(
                    "safe particle-mesh composite simulation contains an invalid degree-of-freedom count",
                )
            })?;
        ...
        let mut raw_error = initialized_error()?;
        // SAFETY: Both handles are live; mutable owner borrow excludes safe
        // concurrent access and both initialized outputs are writable.
        let status = unsafe {
            sys::bg_context_integrate_particle_mesh_ewald_composite_v1(
                self.raw_handle(),
    ...
    fn require_particle_mesh_ewald_composite_dynamics_backend(&self) -> DirectEwaldResult<()> {
        // The immutable request is authoritative: AUTO/HIP is rejected before
        // any resolved-backend, ABI, or simulation access.
        let requested = self.requested_backend();
        require_particle_mesh_ewald_composite_dynamics_backend(requested)?;
        ...
        require_particle_mesh_ewald_composite_dynamics_backend(resolved)?;
        Ok(())
    }
}

/// Query and validate the frozen native particle-mesh dynamics profile.
pub fn particle_mesh_ewald_composite_dynamics_profile_id() -> DirectEwaldResult<String> {
    ensure_particle_mesh_ewald_composite_dynamics_abi_compatibility()?;
    // SAFETY: The ABI returns a process-lifetime NUL-terminated string.
    unsafe {
        let pointer = sys::bg_particle_mesh_ewald_composite_dynamics_v1_profile_id();
        if pointer.is_null() {
            return Err(abi_error(
                "native particle-mesh composite-dynamics profile id is null",
            ));
        }
        let value = CStr::from_ptr(pointer).to_str().map_err(|_| {
            abi_error("native particle-mesh composite-dynamics profile id is not UTF-8")
        })?;
        if value != PROFILE_ID {
            return Err(abi_error(format!(
                "native particle-mesh composite-dynamics profile id {value:?} does not match {PROFILE_ID:?}"
            )));
        }
        Ok(value.to_owned())
    }
...
fn ensure_particle_mesh_ewald_composite_dynamics_abi_compatibility() -> DirectEwaldResult<()> {
    ...
    let observed_composite = unsafe { sys::bg_particle_mesh_ewald_composite_abi_version() };
    if observed_composite != sys::BG_PARTICLE_MESH_EWALD_COMPOSITE_ABI_VERSION {
        return Err(abi_error(format!(
            "native particle-mesh composite ABI version {observed_composite} does not match required version {}",
            sys::BG_PARTICLE_MESH_EWALD_COMPOSITE_ABI_VERSION
        )));
    }
    // SAFETY: This version query also takes no pointers.
    let observed_dynamics = unsafe { sys::bg_particle_mesh_ewald_composite_dynamics_abi_version() };
    if observed_dynamics != sys::BG_PARTICLE_MESH_EWALD_COMPOSITE_DYNAMICS_ABI_VERSION {
        return Err(abi_error(format!(
            "native particle-mesh composite-dynamics ABI version {observed_dynamics} does not match required version {}",
            sys::BG_PARTICLE_MESH_EWALD_COMPOSITE_DYNAMICS_ABI_VERSION
        )));
    }
    Ok(())
}

fn require_particle_mesh_ewald_composite_dynamics_backend(
    backend: Backend,
) -> DirectEwaldResult<()> {
    match backend {
        ...
        Backend::Auto | Backend::HipFast | Backend::HipSafe => Err(DirectEwaldError {
            ...
            code: None,
            detail: format!(
                "particle-mesh Ewald composite dynamics requires an explicitly requested C++ or Rust CPU backend; {backend:?} cannot fall back"
            ),
        }),
    }
}
...
fn validate_integrator(integrator: Integrator) -> DirectEwaldResult<()> {
    if integrator == Integrator::VelocityVerlet {
        ...
        Err(local_invalid(
            "particle-mesh Ewald composite dynamics supports only Velocity-Verlet",
        ))
    }
}
...
fn dynamics_report_from_raw(
    ...
    {
        return Err(abi_error(
            "native particle-mesh composite integrator returned an invalid report descriptor",
        ));
    }
    ...
    {
        return Err(abi_error(
            "native particle-mesh composite integrator returned a semantically invalid report",
        ));
    }
    Ok(DynamicsReport {
...
mod tests {
    ...
    #[test]
    fn unsupported_lanes_fail_closed_before_native_access() {
        for backend in [Backend::Auto, Backend::HipFast, Backend::HipSafe] {
            let error =
                require_particle_mesh_ewald_composite_dynamics_backend(backend).unwrap_err();
