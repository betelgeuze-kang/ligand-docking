...
constexpr std::array<uint8_t, 8> kCompositeMagic = {
    'B', 'G', 'D', 'E', 'C', '0', '0', '1'};
...
bg_status reject_vector_overlap(
    ...
    if (storage.size() >
        ...
        return fail(
            ...
            "composite checkpoint owner storage size is not representable");
    }
    ...
    if (!make_byte_range(
        ...
        return fail(
            BG_STATUS_INTERNAL_ERROR,
            "composite checkpoint owner storage range is not representable");
    }
...
bg_status reject_semantic_storage_overlap(
    ...
    if (!make_byte_range(&owner, sizeof(owner), &owner_range) ||
        ...
        return fail(
            BG_STATUS_INTERNAL_ERROR,
            "composite checkpoint owner range is not representable");
    }
    if (ranges_overlap(candidate, owner_range) ||
        ranges_overlap(candidate, simulation_range)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite checkpoint bytes must not overlap the owner");
    }
    ...
    for (const std::vector<double> *channel : system_channels) {
        const bg_status status = reject_vector_overlap(
            *channel,
            candidate,
            "composite checkpoint bytes must not overlap owned particle channels");
    ...
    for (const std::vector<double> *channel : forcefield_double_channels) {
        const bg_status status = reject_vector_overlap(
            *channel,
            candidate,
            "composite checkpoint bytes must not overlap owned force-field storage");
        if (status != BG_STATUS_OK) {
            return status;
        }
    }
    ...
    status = reject_vector_overlap(
        ...
        candidate,
        "composite checkpoint bytes must not overlap owned constraint storage");
    if (status != BG_STATUS_OK) {
        return status;
    }
    return reject_vector_overlap(
        owner.model.pair_rules,
        candidate,
        "composite checkpoint bytes must not overlap owned direct-Ewald model storage");
}
...
bg_status validate_output_aliases(
    ...
    if (!pointer_is_aligned(written_size)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite checkpoint written-size output must be naturally aligned");
    }
    ...
    if (!make_byte_range(buffer, size, &buffer_range) ||
        ...
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite checkpoint output range is not representable");
    }
    if (ranges_overlap(buffer_range, written_range)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite checkpoint outputs must not overlap");
    }
...
bg_status validate_scalar_output_alias(
    ...
    if (!pointer_is_aligned(output)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite checkpoint size output must be naturally aligned");
    }
    ByteRange output_range;
    if (!make_byte_range(output, sizeof(*output), &output_range)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite checkpoint size output range is not representable");
    }
...
bg_status validate_input_alias(
    ...
    if (!make_byte_range(buffer, size, &buffer_range)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite checkpoint input range is not representable");
    }
...
bg_direct_ewald_composite_simulation_v1_checkpoint_size(
    ...
    return guarded_status([&]() -> bg_status {
        if (simulation == nullptr || required_size == nullptr) {
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite simulation and checkpoint size output must not be null");
        }
...
bg_direct_ewald_composite_simulation_v1_checkpoint_write(
    ...
    return guarded_status([&]() -> bg_status {
        if (simulation == nullptr || written_size == nullptr) {
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite simulation and checkpoint written-size output must not be null");
        }
        bg_status status = validate_owner_invariant(*simulation);
        if (status != BG_STATUS_OK) {
            return status;
        ...
        if (buffer_capacity < required_u64) {
            return fail(
                ...
                "composite checkpoint output buffer is too small");
        }
        if (buffer == nullptr) {
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite checkpoint output buffer must not be null");
        }
        if constexpr (sizeof(std::size_t) < sizeof(uint64_t)) {
            if (required_u64 >
                ...
                return fail(
                    ...
                    "composite checkpoint size exceeds native address space");
            }
        }
        ...
        std::copy(
            kCompositeMagic.begin(), kCompositeMagic.end(), bytes.begin());
        std::copy(
...
bg_direct_ewald_composite_simulation_v1_checkpoint_load(
    ...
    return guarded_status([&]() -> bg_status {
        if (simulation == nullptr || buffer == nullptr) {
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite simulation and checkpoint input buffer must not be null");
        }
        if constexpr (sizeof(std::size_t) < sizeof(uint64_t)) {
            if (buffer_size >
                static_cast<uint64_t>(
                ...
                return fail(
                    BG_STATUS_CAPACITY_OVERFLOW,
                    "composite checkpoint input size exceeds native address space");
            }
        }
        ...
        if (size < kHeaderSize) {
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite checkpoint input is shorter than its canonical header");
        }
        ...
        if (!constant_time_equal(
            ...
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite checkpoint integrity digest does not match its bytes");
        }
        if (!std::equal(
                kCompositeMagic.begin(), kCompositeMagic.end(),
                bytes.begin()) ||
            ...
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite checkpoint header is not a supported canonical format");
        }
        ...
        if (load_u64(bytes.data() + 24U) != atom_count) {
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite checkpoint particle count does not match the simulation");
        }
        ...
        if (expected_size != buffer_size) {
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite checkpoint payload size is not canonical for its particle count");
        }
        if (!constant_time_equal(
            ...
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite checkpoint static evaluator fingerprint does not match");
        }
