#include "direct_ewald_composite_dynamics.hpp"
...
bool create_input_storage_overlaps(
    ...
    if ((system != nullptr &&
        ...
        (model != nullptr &&
         (fixed_storage_overlaps(model, sizeof(*model), output) ||
          model_storage_overlaps(*model, output))) ||
        (options != nullptr &&
         fixed_storage_overlaps(options, sizeof(*options), output))) {
        return true;
    }
    return constraints != nullptr &&
           (fixed_storage_overlaps(
                constraints, sizeof(*constraints), output) ||
...
bg_status validate_create_outputs(
    ...
    if (out_error == nullptr || !pointer_is_aligned(out_error)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite dynamics typed-error output must be non-null and naturally aligned");
    }
    ...
    if (out_simulation != nullptr && !pointer_is_aligned(out_simulation)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite dynamics owner output must be naturally aligned");
    }
    ...
    if (!make_byte_range(out_error, sizeof(*out_error), &error_range)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite dynamics output byte range is not representable");
    }
    if (create_input_storage_overlaps(
            system, forcefield, model, constraints, options, error_range)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite dynamics typed-error output must not overlap borrowed inputs");
    }
    if (out_simulation != nullptr) {
        ...
        if (!make_byte_range(
            ...
            create_input_storage_overlaps(
                system, forcefield, model, constraints, options,
                owner_range)) {
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite dynamics owner output must not overlap another output or borrowed input");
        }
    }
    return BG_STATUS_OK;
}
...
bg_status validate_integrate_outputs(
    ...
    if (out_report != nullptr && !pointer_is_aligned(out_report)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite dynamics report output must be naturally aligned");
    }
    ...
    if ((context != nullptr && fixed_storage_overlaps(
        ...
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite dynamics typed-error output must not overlap an input owner");
    }
    if (out_report != nullptr) {
        ...
        if (!make_byte_range(out_report, sizeof(*out_report), &report_range) ||
            ...
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite dynamics report output must not overlap another output or input owner");
        }
    }
    return BG_STATUS_OK;
}
...
struct ProviderState final {
    bg_direct_ewald_composite_simulation_v1 *owner = nullptr;
    ewald::Error *typed_error = nullptr;
};
...
bg_status evaluate_composite_provider(
    ...
    if (state == nullptr || out_evaluation == nullptr) {
        return fail(
            ...
            "composite dynamics force-provider state or output is null");
    }
    ...
    if (provider->owner == nullptr || provider->typed_error == nullptr ||
        ...
        return fail(
            BG_STATUS_INTERNAL_ERROR,
            "composite dynamics force-provider owner is inconsistent");
    }

    Evaluation combined;
    const bg_status status = evaluate_prevalidated(
        context,
        system,
...
std::array<uint8_t, 32> compute_static_fingerprint(
    ...
    const bg_direct_ewald_model_v1 &model = owner.model;
    ...
    for (const int32_t maximum : model.reciprocal_max_indices) {
        betelgeuze::native::dynamics::hash_u32(
            &hash, static_cast<uint32_t>(maximum));
    }
...
bg_status validate_owner_invariant(
    ...
    if (owner.simulation == nullptr) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite dynamics owner contains no Engine simulation");
    }
    const bg_simulation &simulation = *owner.simulation;
    if (simulation.integrator != BG_INTEGRATOR_VELOCITY_VERLET) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite dynamics owner must use Velocity Verlet");
    }
    bg_status status = validate_static_compatibility(
        simulation.system, simulation.forcefield, owner.model);
    if (status != BG_STATUS_OK) {
        return status;
    }
    if (simulation.static_fingerprint !=
        ...
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite dynamics legacy static fingerprint is inconsistent");
    }
    if (owner.static_fingerprint != compute_static_fingerprint(owner)) {
        return fail(
            BG_STATUS_INVALID_ARGUMENT,
            "composite dynamics static fingerprint is inconsistent");
    }
    return BG_STATUS_OK;
}
...
bg_direct_ewald_composite_dynamics_abi_version(void) BG_NOEXCEPT {
    return BG_DIRECT_EWALD_COMPOSITE_DYNAMICS_ABI_VERSION;
}

extern "C" BG_API uint32_t BG_CALL
bg_direct_ewald_composite_dynamics_abi_version_major(void) BG_NOEXCEPT {
    return BG_DIRECT_EWALD_COMPOSITE_DYNAMICS_ABI_VERSION_MAJOR;
}

extern "C" BG_API uint32_t BG_CALL
bg_direct_ewald_composite_dynamics_abi_version_minor(void) BG_NOEXCEPT {
    return BG_DIRECT_EWALD_COMPOSITE_DYNAMICS_ABI_VERSION_MINOR;
}
...
bg_direct_ewald_composite_simulation_v1_create(
    ...
    return guarded_status([&]() -> bg_status {
        ...
        if (system == nullptr || forcefield == nullptr || model == nullptr ||
            options == nullptr || out_simulation == nullptr) {
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "system, forcefield, model, options, and owner output must not be null");
        }
        ...
        if (legacy->integrator != BG_INTEGRATOR_VELOCITY_VERLET) {
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "composite dynamics supports only Velocity Verlet");
        }
        auto candidate =
        ...
        status = validate_static_compatibility(
...
bg_direct_ewald_composite_simulation_v1_get_absolute_step(
    ...
    return guarded_status([&]() -> bg_status {
        ...
        if (!make_byte_range(
            ...
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "absolute_step output must not overlap composite dynamics owner storage");
        }
...
bg_context_integrate_direct_ewald_composite_v1(
    ...
    return guarded_status([&]() -> bg_status {
        ...
        if (context->unit_system != legacy->system.unit_system) {
            return fail(
                BG_STATUS_INVALID_ARGUMENT,
                "context and composite dynamics simulation units must match");
        }
        ...
        if (typed_error.code != BG_DIRECT_EWALD_ERROR_NONE) {
            return fail(
                BG_STATUS_INTERNAL_ERROR,
                "successful composite dynamics integration retained a typed error");
        }
        rollback.commit();
        *out_report = report;
        return BG_STATUS_OK;
