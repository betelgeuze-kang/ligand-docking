...
#[test]
fn particle_mesh_ewald_composite_dynamics_identity_and_null_failures_are_transactional() {
    // SAFETY: Identity functions take no pointers. Every descriptor points to
    // initialized writable storage, and null handles deliberately exercise
    // invalid-argument paths without native dereferences.
    unsafe {
        assert_eq!(
        ...
        assert_eq!(
            owned_string(bg_particle_mesh_ewald_composite_dynamics_abi_version_string()),
            "1.0.0"
        );
        assert_eq!(
            owned_string(bg_particle_mesh_ewald_composite_dynamics_v1_profile_id()),
            "betelgeuze.native_particle_mesh_ewald_composite_dynamics/1.0.0"
        );

        bg_particle_mesh_ewald_composite_simulation_v1_destroy(ptr::null_mut());
        ...
        view.particle_count = 77;
        assert_eq!(
            bg_particle_mesh_ewald_composite_simulation_v1_get_particles(ptr::null(), &mut view,),
            BG_STATUS_INVALID_ARGUMENT
        );
        assert_eq!(view.particle_count, 77);
